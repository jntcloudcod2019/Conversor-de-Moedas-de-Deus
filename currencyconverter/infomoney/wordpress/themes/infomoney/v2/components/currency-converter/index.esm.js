function sh(m) {
  return m && m.__esModule && Object.prototype.hasOwnProperty.call(m, "default") ? m.default : m;
}
var o4 = { exports: {} }, S2 = {};
var y6;
function dh() {
  if (y6) return S2;
  y6 = 1;
  var m = /* @__PURE__ */ Symbol.for("react.transitional.element"), i = /* @__PURE__ */ Symbol.for("react.fragment");
  function u(f, E, O) {
    var G = null;
    if (O !== void 0 && (G = "" + O), E.key !== void 0 && (G = "" + E.key), "key" in E) {
      O = {};
      for (var Q in E)
        Q !== "key" && (O[Q] = E[Q]);
    } else O = E;
    return E = O.ref, {
      $$typeof: m,
      type: f,
      key: G,
      ref: E !== void 0 ? E : null,
      props: O
    };
  }
  return S2.Fragment = i, S2.jsx = u, S2.jsxs = u, S2;
}
var F6;
function oh() {
  return F6 || (F6 = 1, o4.exports = dh()), o4.exports;
}
var F = oh(), E4 = { exports: {} }, X = {}, D6;
function Eh() {
  if (D6) return X;
  D6 = 1;
  var m = { env: {} };
  var i = /* @__PURE__ */ Symbol.for("react.transitional.element"), u = /* @__PURE__ */ Symbol.for("react.portal"), f = /* @__PURE__ */ Symbol.for("react.fragment"), E = /* @__PURE__ */ Symbol.for("react.strict_mode"), O = /* @__PURE__ */ Symbol.for("react.profiler"), G = /* @__PURE__ */ Symbol.for("react.consumer"), Q = /* @__PURE__ */ Symbol.for("react.context"), T = /* @__PURE__ */ Symbol.for("react.forward_ref"), H = /* @__PURE__ */ Symbol.for("react.suspense"), R = /* @__PURE__ */ Symbol.for("react.memo"), k = /* @__PURE__ */ Symbol.for("react.lazy"), Y = /* @__PURE__ */ Symbol.for("react.activity"), je = Symbol.iterator;
  function Ve(p) {
    return p === null || typeof p != "object" ? null : (p = je && p[je] || p["@@iterator"], typeof p == "function" ? p : null);
  }
  var W = {
    isMounted: function() {
      return !1;
    },
    enqueueForceUpdate: function() {
    },
    enqueueReplaceState: function() {
    },
    enqueueSetState: function() {
    }
  }, Fe = Object.assign, ne = {};
  function oe(p, B, V) {
    this.props = p, this.context = B, this.refs = ne, this.updater = V || W;
  }
  oe.prototype.isReactComponent = {}, oe.prototype.setState = function(p, B) {
    if (typeof p != "object" && typeof p != "function" && p != null)
      throw Error(
        "takes an object of state variables to update or a function which returns an object of state variables."
      );
    this.updater.enqueueSetState(this, p, B, "setState");
  }, oe.prototype.forceUpdate = function(p) {
    this.updater.enqueueForceUpdate(this, p, "forceUpdate");
  };
  function f1() {
  }
  f1.prototype = oe.prototype;
  function Be(p, B, V) {
    this.props = p, this.context = B, this.refs = ne, this.updater = V || W;
  }
  var $e = Be.prototype = new f1();
  $e.constructor = Be, Fe($e, oe.prototype), $e.isPureReactComponent = !0;
  var ve = Array.isArray;
  function be() {
  }
  var ce = { H: null, A: null, T: null, S: null }, Ue = Object.prototype.hasOwnProperty;
  function Pe(p, B, V) {
    var _ = V.ref;
    return {
      $$typeof: i,
      type: p,
      key: B,
      ref: _ !== void 0 ? _ : null,
      props: V
    };
  }
  function h1(p, B) {
    return Pe(p.type, B, p.props);
  }
  function Ge(p) {
    return typeof p == "object" && p !== null && p.$$typeof === i;
  }
  function m1(p) {
    var B = { "=": "=0", ":": "=2" };
    return "$" + p.replace(/[=:]/g, function(V) {
      return B[V];
    });
  }
  var G1 = /\/+/g;
  function v1(p, B) {
    return typeof p == "object" && p !== null && p.key != null ? m1("" + p.key) : B.toString(36);
  }
  function C(p) {
    switch (p.status) {
      case "fulfilled":
        return p.value;
      case "rejected":
        throw p.reason;
      default:
        switch (typeof p.status == "string" ? p.then(be, be) : (p.status = "pending", p.then(
          function(B) {
            p.status === "pending" && (p.status = "fulfilled", p.value = B);
          },
          function(B) {
            p.status === "pending" && (p.status = "rejected", p.reason = B);
          }
        )), p.status) {
          case "fulfilled":
            return p.value;
          case "rejected":
            throw p.reason;
        }
    }
    throw p;
  }
  function b(p, B, V, _, J) {
    var $ = typeof p;
    ($ === "undefined" || $ === "boolean") && (p = null);
    var ie = !1;
    if (p === null) ie = !0;
    else
      switch ($) {
        case "bigint":
        case "string":
        case "number":
          ie = !0;
          break;
        case "object":
          switch (p.$$typeof) {
            case i:
            case u:
              ie = !0;
              break;
            case k:
              return ie = p._init, b(
                ie(p._payload),
                B,
                V,
                _,
                J
              );
          }
      }
    if (ie)
      return J = J(p), ie = _ === "" ? "." + v1(p, 0) : _, ve(J) ? (V = "", ie != null && (V = ie.replace(G1, "$&/") + "/"), b(J, B, V, "", function(Xe) {
        return Xe;
      })) : J != null && (Ge(J) && (J = h1(
        J,
        V + (J.key == null || p && p.key === J.key ? "" : ("" + J.key).replace(
          G1,
          "$&/"
        ) + "/") + ie
      )), B.push(J)), 1;
    ie = 0;
    var ue = _ === "" ? "." : _ + ":";
    if (ve(p))
      for (var he = 0; he < p.length; he++)
        _ = p[he], $ = ue + v1(_, he), ie += b(
          _,
          B,
          V,
          $,
          J
        );
    else if (he = Ve(p), typeof he == "function")
      for (p = he.call(p), he = 0; !(_ = p.next()).done; )
        _ = _.value, $ = ue + v1(_, he++), ie += b(
          _,
          B,
          V,
          $,
          J
        );
    else if ($ === "object") {
      if (typeof p.then == "function")
        return b(
          C(p),
          B,
          V,
          _,
          J
        );
      throw B = String(p), Error(
        "Objects are not valid as a React child (found: " + (B === "[object Object]" ? "object with keys {" + Object.keys(p).join(", ") + "}" : B) + "). If you meant to render a collection of children, use an array instead."
      );
    }
    return ie;
  }
  function L(p, B, V) {
    if (p == null) return p;
    var _ = [], J = 0;
    return b(p, _, "", "", function($) {
      return B.call(V, $, J++);
    }), _;
  }
  function I(p) {
    if (p._status === -1) {
      var B = p._result;
      B = B(), B.then(
        function(V) {
          (p._status === 0 || p._status === -1) && (p._status = 1, p._result = V);
        },
        function(V) {
          (p._status === 0 || p._status === -1) && (p._status = 2, p._result = V);
        }
      ), p._status === -1 && (p._status = 0, p._result = B);
    }
    if (p._status === 1) return p._result.default;
    throw p._result;
  }
  var ge = typeof reportError == "function" ? reportError : function(p) {
    if (typeof window == "object" && typeof window.ErrorEvent == "function") {
      var B = new window.ErrorEvent("error", {
        bubbles: !0,
        cancelable: !0,
        message: typeof p == "object" && p !== null && typeof p.message == "string" ? String(p.message) : String(p),
        error: p
      });
      if (!window.dispatchEvent(B)) return;
    } else if (typeof m == "object" && typeof m.emit == "function") {
      m.emit("uncaughtException", p);
      return;
    }
    console.log(p);
  }, e1 = {
    map: L,
    forEach: function(p, B, V) {
      L(
        p,
        function() {
          B.apply(this, arguments);
        },
        V
      );
    },
    count: function(p) {
      var B = 0;
      return L(p, function() {
        B++;
      }), B;
    },
    toArray: function(p) {
      return L(p, function(B) {
        return B;
      }) || [];
    },
    only: function(p) {
      if (!Ge(p))
        throw Error(
          "React.Children.only expected to receive a single React element child."
        );
      return p;
    }
  };
  return X.Activity = Y, X.Children = e1, X.Component = oe, X.Fragment = f, X.Profiler = O, X.PureComponent = Be, X.StrictMode = E, X.Suspense = H, X.__CLIENT_INTERNALS_DO_NOT_USE_OR_WARN_USERS_THEY_CANNOT_UPGRADE = ce, X.__COMPILER_RUNTIME = {
    __proto__: null,
    c: function(p) {
      return ce.H.useMemoCache(p);
    }
  }, X.cache = function(p) {
    return function() {
      return p.apply(null, arguments);
    };
  }, X.cacheSignal = function() {
    return null;
  }, X.cloneElement = function(p, B, V) {
    if (p == null)
      throw Error(
        "The argument must be a React element, but you passed " + p + "."
      );
    var _ = Fe({}, p.props), J = p.key;
    if (B != null)
      for ($ in B.key !== void 0 && (J = "" + B.key), B)
        !Ue.call(B, $) || $ === "key" || $ === "__self" || $ === "__source" || $ === "ref" && B.ref === void 0 || (_[$] = B[$]);
    var $ = arguments.length - 2;
    if ($ === 1) _.children = V;
    else if (1 < $) {
      for (var ie = Array($), ue = 0; ue < $; ue++)
        ie[ue] = arguments[ue + 2];
      _.children = ie;
    }
    return Pe(p.type, J, _);
  }, X.createContext = function(p) {
    return p = {
      $$typeof: Q,
      _currentValue: p,
      _currentValue2: p,
      _threadCount: 0,
      Provider: null,
      Consumer: null
    }, p.Provider = p, p.Consumer = {
      $$typeof: G,
      _context: p
    }, p;
  }, X.createElement = function(p, B, V) {
    var _, J = {}, $ = null;
    if (B != null)
      for (_ in B.key !== void 0 && ($ = "" + B.key), B)
        Ue.call(B, _) && _ !== "key" && _ !== "__self" && _ !== "__source" && (J[_] = B[_]);
    var ie = arguments.length - 2;
    if (ie === 1) J.children = V;
    else if (1 < ie) {
      for (var ue = Array(ie), he = 0; he < ie; he++)
        ue[he] = arguments[he + 2];
      J.children = ue;
    }
    if (p && p.defaultProps)
      for (_ in ie = p.defaultProps, ie)
        J[_] === void 0 && (J[_] = ie[_]);
    return Pe(p, $, J);
  }, X.createRef = function() {
    return { current: null };
  }, X.forwardRef = function(p) {
    return { $$typeof: T, render: p };
  }, X.isValidElement = Ge, X.lazy = function(p) {
    return {
      $$typeof: k,
      _payload: { _status: -1, _result: p },
      _init: I
    };
  }, X.memo = function(p, B) {
    return {
      $$typeof: R,
      type: p,
      compare: B === void 0 ? null : B
    };
  }, X.startTransition = function(p) {
    var B = ce.T, V = {};
    ce.T = V;
    try {
      var _ = p(), J = ce.S;
      J !== null && J(V, _), typeof _ == "object" && _ !== null && typeof _.then == "function" && _.then(be, ge);
    } catch ($) {
      ge($);
    } finally {
      B !== null && V.types !== null && (B.types = V.types), ce.T = B;
    }
  }, X.unstable_useCacheRefresh = function() {
    return ce.H.useCacheRefresh();
  }, X.use = function(p) {
    return ce.H.use(p);
  }, X.useActionState = function(p, B, V) {
    return ce.H.useActionState(p, B, V);
  }, X.useCallback = function(p, B) {
    return ce.H.useCallback(p, B);
  }, X.useContext = function(p) {
    return ce.H.useContext(p);
  }, X.useDebugValue = function() {
  }, X.useDeferredValue = function(p, B) {
    return ce.H.useDeferredValue(p, B);
  }, X.useEffect = function(p, B) {
    return ce.H.useEffect(p, B);
  }, X.useEffectEvent = function(p) {
    return ce.H.useEffectEvent(p);
  }, X.useId = function() {
    return ce.H.useId();
  }, X.useImperativeHandle = function(p, B, V) {
    return ce.H.useImperativeHandle(p, B, V);
  }, X.useInsertionEffect = function(p, B) {
    return ce.H.useInsertionEffect(p, B);
  }, X.useLayoutEffect = function(p, B) {
    return ce.H.useLayoutEffect(p, B);
  }, X.useMemo = function(p, B) {
    return ce.H.useMemo(p, B);
  }, X.useOptimistic = function(p, B) {
    return ce.H.useOptimistic(p, B);
  }, X.useReducer = function(p, B, V) {
    return ce.H.useReducer(p, B, V);
  }, X.useRef = function(p) {
    return ce.H.useRef(p);
  }, X.useState = function(p) {
    return ce.H.useState(p);
  }, X.useSyncExternalStore = function(p, B, V) {
    return ce.H.useSyncExternalStore(
      p,
      B,
      V
    );
  }, X.useTransition = function() {
    return ce.H.useTransition();
  }, X.version = "19.2.3", X;
}
var S6;
function w4() {
  return S6 || (S6 = 1, E4.exports = Eh()), E4.exports;
}
var ye = w4();
const a = /* @__PURE__ */ sh(ye), j6 = () => {
  const m = /* @__PURE__ */ new Date(), i = String(m.getDate()).padStart(2, "0"), u = String(m.getMonth() + 1).padStart(2, "0"), f = m.getFullYear(), E = String(m.getHours()).padStart(2, "0"), O = String(m.getMinutes()).padStart(2, "0");
  return `${i}/${u}/${f} ${E}:${O}`;
};
var z4 = { exports: {} }, Ie = {};
var A6;
function zh() {
  if (A6) return Ie;
  A6 = 1;
  var m = w4();
  function i(T) {
    var H = "https://react.dev/errors/" + T;
    if (1 < arguments.length) {
      H += "?args[]=" + encodeURIComponent(arguments[1]);
      for (var R = 2; R < arguments.length; R++)
        H += "&args[]=" + encodeURIComponent(arguments[R]);
    }
    return "Minified React error #" + T + "; visit " + H + " for the full message or use the non-minified dev environment for full errors and additional helpful warnings.";
  }
  function u() {
  }
  var f = {
    d: {
      f: u,
      r: function() {
        throw Error(i(522));
      },
      D: u,
      C: u,
      L: u,
      m: u,
      X: u,
      S: u,
      M: u
    },
    p: 0,
    findDOMNode: null
  }, E = /* @__PURE__ */ Symbol.for("react.portal");
  function O(T, H, R) {
    var k = 3 < arguments.length && arguments[3] !== void 0 ? arguments[3] : null;
    return {
      $$typeof: E,
      key: k == null ? null : "" + k,
      children: T,
      containerInfo: H,
      implementation: R
    };
  }
  var G = m.__CLIENT_INTERNALS_DO_NOT_USE_OR_WARN_USERS_THEY_CANNOT_UPGRADE;
  function Q(T, H) {
    if (T === "font") return "";
    if (typeof H == "string")
      return H === "use-credentials" ? H : "";
  }
  return Ie.__DOM_INTERNALS_DO_NOT_USE_OR_WARN_USERS_THEY_CANNOT_UPGRADE = f, Ie.createPortal = function(T, H) {
    var R = 2 < arguments.length && arguments[2] !== void 0 ? arguments[2] : null;
    if (!H || H.nodeType !== 1 && H.nodeType !== 9 && H.nodeType !== 11)
      throw Error(i(299));
    return O(T, H, null, R);
  }, Ie.flushSync = function(T) {
    var H = G.T, R = f.p;
    try {
      if (G.T = null, f.p = 2, T) return T();
    } finally {
      G.T = H, f.p = R, f.d.f();
    }
  }, Ie.preconnect = function(T, H) {
    typeof T == "string" && (H ? (H = H.crossOrigin, H = typeof H == "string" ? H === "use-credentials" ? H : "" : void 0) : H = null, f.d.C(T, H));
  }, Ie.prefetchDNS = function(T) {
    typeof T == "string" && f.d.D(T);
  }, Ie.preinit = function(T, H) {
    if (typeof T == "string" && H && typeof H.as == "string") {
      var R = H.as, k = Q(R, H.crossOrigin), Y = typeof H.integrity == "string" ? H.integrity : void 0, je = typeof H.fetchPriority == "string" ? H.fetchPriority : void 0;
      R === "style" ? f.d.S(
        T,
        typeof H.precedence == "string" ? H.precedence : void 0,
        {
          crossOrigin: k,
          integrity: Y,
          fetchPriority: je
        }
      ) : R === "script" && f.d.X(T, {
        crossOrigin: k,
        integrity: Y,
        fetchPriority: je,
        nonce: typeof H.nonce == "string" ? H.nonce : void 0
      });
    }
  }, Ie.preinitModule = function(T, H) {
    if (typeof T == "string")
      if (typeof H == "object" && H !== null) {
        if (H.as == null || H.as === "script") {
          var R = Q(
            H.as,
            H.crossOrigin
          );
          f.d.M(T, {
            crossOrigin: R,
            integrity: typeof H.integrity == "string" ? H.integrity : void 0,
            nonce: typeof H.nonce == "string" ? H.nonce : void 0
          });
        }
      } else H == null && f.d.M(T);
  }, Ie.preload = function(T, H) {
    if (typeof T == "string" && typeof H == "object" && H !== null && typeof H.as == "string") {
      var R = H.as, k = Q(R, H.crossOrigin);
      f.d.L(T, R, {
        crossOrigin: k,
        integrity: typeof H.integrity == "string" ? H.integrity : void 0,
        nonce: typeof H.nonce == "string" ? H.nonce : void 0,
        type: typeof H.type == "string" ? H.type : void 0,
        fetchPriority: typeof H.fetchPriority == "string" ? H.fetchPriority : void 0,
        referrerPolicy: typeof H.referrerPolicy == "string" ? H.referrerPolicy : void 0,
        imageSrcSet: typeof H.imageSrcSet == "string" ? H.imageSrcSet : void 0,
        imageSizes: typeof H.imageSizes == "string" ? H.imageSizes : void 0,
        media: typeof H.media == "string" ? H.media : void 0
      });
    }
  }, Ie.preloadModule = function(T, H) {
    if (typeof T == "string")
      if (H) {
        var R = Q(H.as, H.crossOrigin);
        f.d.m(T, {
          as: typeof H.as == "string" && H.as !== "script" ? H.as : void 0,
          crossOrigin: R,
          integrity: typeof H.integrity == "string" ? H.integrity : void 0
        });
      } else f.d.m(T);
  }, Ie.requestFormReset = function(T) {
    f.d.r(T);
  }, Ie.unstable_batchedUpdates = function(T, H) {
    return T(H);
  }, Ie.useFormState = function(T, H, R) {
    return G.H.useFormState(T, H, R);
  }, Ie.useFormStatus = function() {
    return G.H.useHostTransitionStatus();
  }, Ie.version = "19.2.3", Ie;
}
var B6;
function G6() {
  if (B6) return z4.exports;
  B6 = 1;
  function m() {
    if (!(typeof __REACT_DEVTOOLS_GLOBAL_HOOK__ > "u" || typeof __REACT_DEVTOOLS_GLOBAL_HOOK__.checkDCE != "function"))
      try {
        __REACT_DEVTOOLS_GLOBAL_HOOK__.checkDCE(m);
      } catch (i) {
        console.log(i);
      }
  }
  return m(), z4.exports = zh(), z4.exports;
}
var ph = G6(), gh = ["title"], Mh = ["title"], wh = ["title"], xh = ["title"], Hh = ["title"], yh = ["title"], Fh = ["title"], Dh = ["title"], Sh = ["title"], Ah = ["title"], Bh = ["title"], bh = ["title"], Th = ["title"], Lh = ["title"], Vh = ["title"], Ch = ["title"], Oh = ["title"], Nh = ["title"], _h = ["title"], Uh = ["title"], Rh = ["title"], jh = ["title"], Gh = ["title"], Yh = ["title"], qh = ["title"], Qh = ["title"], Xh = ["title"], Zh = ["title"], Kh = ["title"], Jh = ["title"], kh = ["title"], Wh = ["title"], $h = ["title"], Ih = ["title"], Ph = ["title"], em = ["title"], tm = ["title"], lm = ["title"], am = ["title"], nm = ["title"], cm = ["title"], im = ["title"], um = ["title"], rm = ["title"], fm = ["title"], hm = ["title"], mm = ["title"], vm = ["title"], sm = ["title"], dm = ["title"], om = ["title"], Em = ["title"], zm = ["title"], pm = ["title"], gm = ["title"], Mm = ["title"], wm = ["title"], xm = ["title"], Hm = ["title"], ym = ["title"], Fm = ["title"], Dm = ["title"], Sm = ["title"], Am = ["title"], Bm = ["title"], bm = ["title"], Tm = ["title"], Lm = ["title"], Vm = ["title"], Cm = ["title"], Om = ["title"], Nm = ["title"], _m = ["title"], Um = ["title"], Rm = ["title"], jm = ["title"], Gm = ["title"], Ym = ["title"], qm = ["title"], Qm = ["title"], Xm = ["title"], Zm = ["title"], Km = ["title"], Jm = ["title"], km = ["title"], Wm = ["title"], $m = ["title"], Im = ["title"], Pm = ["title"], ev = ["title"], tv = ["title"], lv = ["title"], av = ["title"], nv = ["title"], cv = ["title"], iv = ["title"], uv = ["title"], rv = ["title"], fv = ["title"], hv = ["title"], mv = ["title"], vv = ["title"], sv = ["title"], dv = ["title"], ov = ["title"], Ev = ["title"], zv = ["title"], pv = ["title"], gv = ["title"], Mv = ["title"], wv = ["title"], xv = ["title"], Hv = ["title"], yv = ["title"], Fv = ["title"], Dv = ["title"], Sv = ["title"], Av = ["title"], Bv = ["title"], bv = ["title"], Tv = ["title"], Lv = ["title"], Vv = ["title"], Cv = ["title"], Ov = ["title"], Nv = ["title"], _v = ["title"], Uv = ["title"], Rv = ["title"], jv = ["title"], Gv = ["title"], Yv = ["title"], qv = ["title"], Qv = ["title"], Xv = ["title"], Zv = ["title"], Kv = ["title"], Jv = ["title"], kv = ["title"], Wv = ["title"], $v = ["title"], Iv = ["title"], Pv = ["title"], es = ["title"], ts = ["title"], ls = ["title"], as = ["title"], ns = ["title"], cs = ["title"], is = ["title"], us = ["title"], rs = ["title"], fs = ["title"], hs = ["title"], ms = ["title"], vs = ["title"], ss = ["title"], ds = ["title"], os = ["title"], Es = ["title"], zs = ["title"], ps = ["title"], gs = ["title"], Ms = ["title"], ws = ["title"], xs = ["title"], Hs = ["title"], ys = ["title"], Fs = ["title"], Ds = ["title"], Ss = ["title"], As = ["title"], Bs = ["title"], bs = ["title"], Ts = ["title"], Ls = ["title"], Vs = ["title"], Cs = ["title"], Os = ["title"], Ns = ["title"], _s = ["title"], Us = ["title"], Rs = ["title"], js = ["title"], Gs = ["title"], Ys = ["title"], qs = ["title"], Qs = ["title"], Xs = ["title"], Zs = ["title"], Ks = ["title"], Js = ["title"], ks = ["title"], Ws = ["title"], $s = ["title"], Is = ["title"], Ps = ["title"], ed = ["title"], td = ["title"], ld = ["title"], ad = ["title"], nd = ["title"], cd = ["title"], id = ["title"], ud = ["title"], rd = ["title"], fd = ["title"], hd = ["title"], md = ["title"], vd = ["title"], sd = ["title"], dd = ["title"], od = ["title"], Ed = ["title"], zd = ["title"], pd = ["title"], gd = ["title"], Md = ["title"], wd = ["title"], xd = ["title"], Hd = ["title"], yd = ["title"], Fd = ["title"], Dd = ["title"], Sd = ["title"], Ad = ["title"], Bd = ["title"], bd = ["title"], Td = ["title"], Ld = ["title"], Vd = ["title"], Cd = ["title"], Od = ["title"], Nd = ["title"], _d = ["title"], Ud = ["title"], Rd = ["title"], jd = ["title"], Gd = ["title"], Yd = ["title"], qd = ["title"], Qd = ["title"], Xd = ["title"], Zd = ["title"], Kd = ["title"], Jd = ["title"], kd = ["title"], Wd = ["title"], $d = ["title"], Id = ["title"], Pd = ["title"], eo = ["title"], to = ["title"], lo = ["title"], ao = ["title"], no = ["title"], co = ["title"], io = ["title"], uo = ["title"], ro = ["title"], fo = ["title"], ho = ["title"], mo = ["title"], vo = ["title"], so = ["title"];
function s() {
  return s = Object.assign || function(m) {
    for (var i = 1; i < arguments.length; i++) {
      var u = arguments[i];
      for (var f in u)
        Object.prototype.hasOwnProperty.call(u, f) && (m[f] = u[f]);
    }
    return m;
  }, s.apply(this, arguments);
}
function d(m, i) {
  if (m == null) return {};
  var u = oo(m, i), f, E;
  if (Object.getOwnPropertySymbols) {
    var O = Object.getOwnPropertySymbols(m);
    for (E = 0; E < O.length; E++)
      f = O[E], !(i.indexOf(f) >= 0) && Object.prototype.propertyIsEnumerable.call(m, f) && (u[f] = m[f]);
  }
  return u;
}
function oo(m, i) {
  if (m == null) return {};
  var u = {}, f = Object.keys(m), E, O;
  for (O = 0; O < f.length; O++)
    E = f[O], !(i.indexOf(E) >= 0) && (u[E] = m[E]);
  return u;
}
var Y6 = function(i) {
  var u = i.title, f = d(i, gh);
  return /* @__PURE__ */ a.createElement("svg", s({
    xmlns: "http://www.w3.org/2000/svg",
    viewBox: "0 0 513 342"
  }, f), u && /* @__PURE__ */ a.createElement("title", null, u), /* @__PURE__ */ a.createElement("path", {
    fill: "#0052B4",
    d: "M0 0h513v342H0z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#FFF",
    d: "M440.1 181.1c-.1 39.2-6.4 81.4-57.4 101.5-51.1-20.1-57.3-62.3-57.4-101.5h114.8z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#29DBFF",
    d: "M439.6 197.7c-2.8 34.9-12.4 67.4-57 85-44.4-17.6-54.5-51.2-56.9-84.9"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#FFF",
    d: "M437.8 214.1c-3.2 24.3-16.7 53.5-55.1 68.6-38.4-15.1-50.5-42.5-55.1-68.4"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#29DBFF",
    d: "M434.2 230.3c-5.7 17.7-19.3 39.4-51.3 52.8-32-12.6-45.2-33.8-51.4-53"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#FFF",
    d: "M426.7 246.9c-6.5 11.3-17.7 25.4-44 35.9-27.5-11.5-37.4-25.3-44-36.1"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#29DBFF",
    d: "M412.4 265.1c-8.1 7.2-12 11.2-29.6 17.9-20.1-7.9-22.6-11.6-29.2-17.5"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#5CC85C",
    d: "M383.3 231.6c-.2-.2-27.9 35.7-27.9 35.7-1.8-1.3-10-9.5-13.3-15l41.3-50.1 40.2 49.7c-3.9 6.5-11.4 13.6-13.2 15M382.6 85.3l-26.5 45h53z"
  }), /* @__PURE__ */ a.createElement("ellipse", {
    transform: "rotate(-82.3 291.431 225.686)",
    fill: "#F7A226",
    cx: 291.4,
    cy: 225.7,
    rx: 48.7,
    ry: 15.7
  }), /* @__PURE__ */ a.createElement("ellipse", {
    transform: "rotate(-68.099 303.087 164.67)",
    fill: "#DDC7AB",
    cx: 303.1,
    cy: 164.7,
    rx: 11.7,
    ry: 7.2
  }), /* @__PURE__ */ a.createElement("ellipse", {
    transform: "rotate(-81.738 291.697 271.697)",
    fill: "#DDC7AB",
    cx: 291.7,
    cy: 271.7,
    rx: 11.4,
    ry: 3.3
  }), /* @__PURE__ */ a.createElement("ellipse", {
    transform: "matrix(.9986 -.05353 .05353 .9986 -13.992 16.424)",
    fill: "#DDC7AB",
    cx: 299.6,
    cy: 269.4,
    rx: 3.3,
    ry: 11
  }), /* @__PURE__ */ a.createElement("ellipse", {
    transform: "rotate(-21.518 314.54 201.06)",
    fill: "#DDC7AB",
    cx: 314.5,
    cy: 201.1,
    rx: 4.1,
    ry: 13.7
  }), /* @__PURE__ */ a.createElement("ellipse", {
    transform: "rotate(-21.518 317.511 178.077)",
    fill: "#DDC7AB",
    cx: 317.5,
    cy: 178.1,
    rx: 13.7,
    ry: 4.1
  }), /* @__PURE__ */ a.createElement("ellipse", {
    transform: "rotate(-7.7 473.781 225.857)",
    fill: "#F7A226",
    cx: 473.6,
    cy: 225.9,
    rx: 15.7,
    ry: 48.7
  }), /* @__PURE__ */ a.createElement("ellipse", {
    transform: "rotate(-21.901 462.057 164.866)",
    fill: "#DDC7AB",
    cx: 462,
    cy: 164.9,
    rx: 7.2,
    ry: 11.7
  }), /* @__PURE__ */ a.createElement("ellipse", {
    transform: "rotate(-8.27 473.27 271.908)",
    fill: "#DDC7AB",
    cx: 473.4,
    cy: 271.9,
    rx: 3.3,
    ry: 11.4
  }), /* @__PURE__ */ a.createElement("ellipse", {
    transform: "matrix(.05353 -.9986 .9986 .05353 171.34 719.998)",
    fill: "#DDC7AB",
    cx: 465.5,
    cy: 269.6,
    rx: 11,
    ry: 3.3
  }), /* @__PURE__ */ a.createElement("ellipse", {
    transform: "rotate(-68.482 450.552 201.247)",
    fill: "#DDC7AB",
    cx: 450.6,
    cy: 201.2,
    rx: 13.7,
    ry: 4.1
  }), /* @__PURE__ */ a.createElement("ellipse", {
    transform: "rotate(-68.482 447.58 178.265)",
    fill: "#DDC7AB",
    cx: 447.6,
    cy: 178.3,
    rx: 4.1,
    ry: 13.7
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#B0C6CC",
    d: "M373.3 130.3 356.1 155l17.8 26.1H396l4.7-25.4-5.4-25.4z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#FFF",
    d: "M256 0v117.4h-46.1l46.1 30.7v22.6h-22.6L160 121.8v48.9H96v-48.9l-73.4 48.9H0v-22.6l46.1-30.7H0v-64h46.1L0 22.7V0h22.6L96 48.9V0h64v48.9L233.4 0z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#D80027",
    d: "M144 0h-32v69.4H0v32h112v69.3h32v-69.3h112v-32H144z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#2E52B2",
    d: "M256 22.7v30.7h-46.1z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#D80027",
    d: "M0 170.7v-11l62.5-42.3h22.6L7.3 170.7zM7.3.1l77.8 53.2H62.5L0 11.1V.1zM256 .1v11l-62.5 42.3h-22.6L248.7.1zm-7.3 170.6-77.8-53.3h22.6l62.5 42.3v11z"
  }));
}, q6 = function(i) {
  var u = i.title, f = d(i, Mh);
  return /* @__PURE__ */ a.createElement("svg", s({
    xmlns: "http://www.w3.org/2000/svg",
    viewBox: "0 0 513 342"
  }, f), u && /* @__PURE__ */ a.createElement("title", null, u), /* @__PURE__ */ a.createElement("path", {
    fill: "#FFDA44",
    d: "M0 0h513v342H0z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#0052B4",
    d: "M0 0h171v342H0z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#D80027",
    d: "M342 0h171v342H342zM198.9 113.6H256v64.8h-57.1z"
  }), /* @__PURE__ */ a.createElement("path", {
    d: "M267.6 113.6v64.7m16.6-64.7v64.7m16.6-64.7v64.7",
    stroke: "#D80027",
    strokeWidth: 7
  }), /* @__PURE__ */ a.createElement("path", {
    d: "M247.4 178.4v64.7m-13.3-64.7v64.7m-13.3-64.6v53.9m-13.7-53.9v47.1",
    stroke: "#D80027",
    strokeWidth: 5
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#FFDA44",
    d: "m199.9 146 27.6-32.4 28.5 32.3-28.1 32.4z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#BC8B00",
    d: "M182.2 95.9v92.2c0 34.3 27.3 54.2 48.6 64.5-.8 1.4 25.1 8.3 25.1 8.3s25.9-6.9 25.1-8.3c21.4-10.3 48.6-30.1 48.6-64.5V95.9H182.2zm129.9 92.2c0 16.9-10 29.4-32.8 43.9-8.6 5.5-17.5 9-23.3 11-5.8-2-14.7-5.5-23.3-11-22.8-14.5-32.8-28-32.8-43.9v-74.5h112.2v74.5z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#D80027",
    d: "M264.4 188.9h29.5v8.6h-29.5zm0 17.6h29.5v8.6h-29.5z"
  }));
}, Q6 = function(i) {
  var u = i.title, f = d(i, wh);
  return /* @__PURE__ */ a.createElement("svg", s({
    xmlns: "http://www.w3.org/2000/svg",
    viewBox: "0 0 513 342"
  }, f), u && /* @__PURE__ */ a.createElement("title", null, u), /* @__PURE__ */ a.createElement("path", {
    fill: "#FFF",
    d: "M0 0h513v342H0z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#009e49",
    d: "M0 0h513v114H0z"
  }), /* @__PURE__ */ a.createElement("path", {
    d: "M0 228h513v114H0z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#ce1126",
    d: "M0 0h171v342H0z"
  }));
}, X6 = function(i) {
  var u = i.title, f = d(i, xh);
  return /* @__PURE__ */ a.createElement("svg", s({
    xmlns: "http://www.w3.org/2000/svg",
    viewBox: "0 0 513 342"
  }, f), u && /* @__PURE__ */ a.createElement("title", null, u), /* @__PURE__ */ a.createElement("path", {
    fill: "#DB3E00",
    d: "M0 0h513v342H0z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#479900",
    d: "M331 0h182v342H331z"
  }), /* @__PURE__ */ a.createElement("path", {
    d: "M0 0h181.8v342H0z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#FFF",
    d: "M256 126.7c-19.4 0-35.2 15.8-35.2 35.2v52.8h70.4v-52.8c0-19.5-15.8-35.2-35.2-35.2z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#FFF",
    d: "M256 84.3c-47.7 0-86.4 38.7-86.4 86.4S208.3 257 256 257s86.4-38.7 86.4-86.4-38.7-86.3-86.4-86.3zm0 157.7c-39.4 0-71.4-32-71.4-71.4 0-39.4 32-71.4 71.4-71.4 39.4 0 71.4 32 71.4 71.4 0 39.5-32 71.4-71.4 71.4z"
  }));
}, Z6 = function(i) {
  var u = i.title, f = d(i, Hh);
  return /* @__PURE__ */ a.createElement("svg", s({
    xmlns: "http://www.w3.org/2000/svg",
    viewBox: "0 0 513 342"
  }, f), u && /* @__PURE__ */ a.createElement("title", null, u), /* @__PURE__ */ a.createElement("path", {
    d: "M0 0h513v342H0z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#0072c6",
    d: "M88.8 136.5c-2.2 12.9-3.4 26.2-3.4 39.8 0 13.6 1.2 27 3.4 39.8L256 228.3l167.2-12.1c2.2-12.9 3.4-26.2 3.4-39.8s-1.2-27-3.4-39.8"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#FFF",
    d: "M423.2 219H88.8c15.8 69.8 84.7 122.3 167.2 122.3S407.4 288.8 423.2 219z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#FFDA44",
    d: "M365.9 136.5H146.1l44.9-21.1-23.9-43.5 48.8 9.4L222 32l34 36.2L290 32l6.1 49.3 48.8-9.4-23.9 43.5z"
  }), /* @__PURE__ */ a.createElement("g", {
    fill: "#ce1126"
  }, /* @__PURE__ */ a.createElement("path", {
    d: "M256.5 342 0 0v342z"
  }), /* @__PURE__ */ a.createElement("path", {
    d: "M513 342V0L256 342z"
  })));
}, K6 = function(i) {
  var u = i.title, f = d(i, yh);
  return /* @__PURE__ */ a.createElement("svg", s({
    xmlns: "http://www.w3.org/2000/svg",
    viewBox: "0 0 513 342"
  }, f), u && /* @__PURE__ */ a.createElement("title", null, u), /* @__PURE__ */ a.createElement("path", {
    fill: "#00318b",
    d: "M0 0h513v342H0z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#FFF",
    d: "M454.8 265.38c7.94-10.93 13.24-24.27 13.24-40.42V104.89a59.754 59.754 0 0 1-36.02 12.01c-19.64 0-37.07-9.43-48.03-24.01-10.95 14.58-28.39 24.01-48.03 24.01-13.52 0-25.99-4.47-36.02-12v120.06c0 16.16 5.3 29.5 13.24 40.42H454.8z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#8DCCFF",
    d: "M310.23 260.98C332.65 296.96 384 309 384 309s51.35-12.04 73.77-48.02H310.23z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#D87B00",
    d: "M396.66 172.21c.32 1.42 8.72 10.17 17.56 20.15 1.39 1.57-8.74 12.63-6.91 12.59 4.75-.12 19.27-17.26 19.34 6.65.05 15.94-30 27.51-30 27.51h17.47l.65 14.96s4.97-13.34 7.59-16.57c8.1-9.97 20.18-30.07 34.59-27.1 14.41 2.97-13.66-13.92-13.66-13.92s-5.2-15.19-16.63-16.97c-10.06-1.57-14.29-2.51-26.64-7.3-1.68-.65-3.88-2.32-3.36 0z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#D87B00",
    d: "M359.69 198.69c1.07-.99 4.46-12.63 8.69-25.28.67-1.99 15.31 1.27 14.35-.3-2.47-4.06-24.58-8.08-3.9-20.08 13.79-8 38.82 12.27 38.82 12.27l-8.72-15.14 12.64-8.03s-14.05 2.35-18.14 1.69c-12.68-2.04-36.13-2.48-40.75-16.45-4.62-13.97-5.25 18.79-5.25 18.79s-10.56 12.08-6.4 22.88c3.66 9.5 4.96 13.64 6.97 26.73.29 1.79-.06 4.53 1.69 2.92z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#D87B00",
    d: "M395.67 219.87c-1.39-.43-13.16 2.49-26.22 5.18-2.06.42-6.59-13.88-7.47-12.26-2.27 4.17 5.36 25.31-15.41 13.45-13.84-7.91-8.89-39.73-8.89-39.73l-8.71 15.14-13.29-6.9s9.09 10.96 10.57 14.84c4.6 11.99 16 32.49 6.24 43.49-9.76 11.01 18.88-4.9 18.88-4.9s15.76 3.07 23-5.96c6.38-7.94 9.3-11.14 19.61-19.45 1.43-1.12 3.97-2.19 1.69-2.9z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#FFF",
    d: "M256.5 0h-23.1L160 48.9V0H96v48.9L22.6 0H0v22.7l46.1 30.7H0v64h46.1L0 148.1V171h22.6L96 121.8V171h64v-49.2l73.4 49.2h23.1v-22.9l-46.6-30.7h46.6v-64h-46.6l46.6-30.7z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#D80027",
    d: "M144 0h-32v69.4H0v32h112V171h32v-69.6h112.5v-32H144z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#D80027",
    d: "M0 171v-11.3l62.5-42.3h22.6L7.3 171zM7.3.1l77.8 53.2H62.5L0 11.1V.1zm249.2 0v11l-63 42.3h-22.6L248.7.1zM248.7 171l-77.8-53.6h22.6l63 42.3V171z"
  }));
}, J6 = function(i) {
  var u = i.title, f = d(i, Fh);
  return /* @__PURE__ */ a.createElement("svg", s({
    xmlns: "http://www.w3.org/2000/svg",
    viewBox: "0 0 90 60"
  }, f), u && /* @__PURE__ */ a.createElement("title", null, u), /* @__PURE__ */ a.createElement("path", {
    fill: "#ED2024",
    d: "M0 0h90v60H0z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#212121",
    d: "M32.5 12.2v16l7.1 2.5-5 4.9 2.4 3 5.3-5.5 1.2 2.3-3 4.9 3.6 5.6-1.8 2.6 2.8 3.6 2.7-3.7-1.5-2.6 3-5.8-2.7-4.6 1.2-2.3 5.2 5.5 2.5-2.9-5.2-5.1 7.2-2.6V12.2l-5.2 1.9-.1 3.8-3.3.3v-2.6l1.7-2.3 5.6-2.2-2.3-.4 1.4-1.2.9.4-.8-1.4L54 9l-1-1.1L47.5 9l1.4 1.1-3.8 4.9-3.8-4.8 1.3-1.1-5-1.2L36.2 9l-1.5-.4-.8 1.4.9-.6 1.6 1.1-2.3.5 5.5 2 1.6 2.3v2.9l-3.3-.3v-3.6z"
  }), /* @__PURE__ */ a.createElement("path", {
    d: "M26.2 25h6.4v2.5h-6.4zm0-4.1h6.4v2.7h-6.4zm0-4.2h6.4v2.6h-6.4zm0-4h6.4v2.5h-6.4zM57.4 25h6.4v2.5h-6.4zm0-4.1h6.4v2.7h-6.4zm0-4.2h6.4v2.6h-6.4zm0-4h6.4v2.5h-6.4zM53.4 36.1h6.4V38h-6.4zm-23.2 0h6.4V38h-6.4z",
    fill: "#212121"
  }));
}, k6 = function(i) {
  var u = i.title, f = d(i, Dh);
  return /* @__PURE__ */ a.createElement("svg", s({
    xmlns: "http://www.w3.org/2000/svg",
    viewBox: "0 0 513 342"
  }, f), u && /* @__PURE__ */ a.createElement("title", null, u), /* @__PURE__ */ a.createElement("path", {
    fill: "#0052B4",
    d: "M0 0h513v342H0z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#D80027",
    d: "M0 0h513v114H0z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#FF9811",
    d: "M0 228h513v114H0z"
  }));
}, W6 = function(i) {
  var u = i.title, f = d(i, Sh);
  return /* @__PURE__ */ a.createElement("svg", s({
    xmlns: "http://www.w3.org/2000/svg",
    viewBox: "0 0 513 342"
  }, f), u && /* @__PURE__ */ a.createElement("title", null, u), /* @__PURE__ */ a.createElement("path", {
    d: "M0 171h513v171H0z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#D80027",
    d: "M0 0h513v171H0z"
  }), /* @__PURE__ */ a.createElement("g", {
    fill: "#FFDA44"
  }, /* @__PURE__ */ a.createElement("path", {
    d: "m332.7 135.6 17.5-8.6-7.4-19-18.7 6.2zm-45.6-56 5.5-18.8-19.1-7.2-8 18zm29.4 23.2 12.2-15.3-14.9-13.9-14.2 13.6zm19.4 68.9 19.5-.8v-21.3l-22.9.2zm-12.6 38.1 18.8 5.5 7.3-19-18-8zm-29 32.4 14.9 12.7 14.4-14.4-13.1-14.7zm-47-147.7 9.8 19.9 21.9 3.3-15.9 15.5 3.6 21.9-19.6-10.4-19.7 10.2 3.8-21.9-15.8-15.5 22-3.1z"
  }), /* @__PURE__ */ a.createElement("path", {
    d: "m250.8 61.3-2.7 18.3c43.1 3.2 77 39.2 77 83.1 0 46-37.3 82.9-83.4 83.4-29.2.3-51.3-14.8-67-33.7l-13.9 12.3c15 19 40.8 39.9 80.9 39.9 56.3 0 101.9-45.6 101.9-101.9.1-53.2-40.7-96.9-92.8-101.4z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#000",
    d: "m291.9 223.4-11.8 14s51.9 38.9 53.6 40.4c1.7 1.5 5.2 2 9.1-2.7 3.7-4.5 2.8-8.1.9-9.7-2-1.5-51.8-42-51.8-42z"
  }), /* @__PURE__ */ a.createElement("path", {
    d: "M206.1 157.2c-7.7 10.3-7.5 23.1 2.8 30.9 0 0 135.2 101.5 136.9 103 1.7 1.5 5.2 2 9.1-2.7 3.7-4.5 2.8-8.1.9-9.7-1.9-1.5-149.7-121.5-149.7-121.5zm47.7 99.1 7.3 18.2 19.5-6-4.8-19.1zm-40.4-3.7-2 19.5 20.1 3.6 4.5-19.1zm-35.7-21.2L167.4 248l16.4 12.1 12.5-15.2z"
  })));
}, $6 = function(i) {
  var u = i.title, f = d(i, Ah);
  return /* @__PURE__ */ a.createElement("svg", s({
    xmlns: "http://www.w3.org/2000/svg",
    viewBox: "0 0 744 496"
  }, f), u && /* @__PURE__ */ a.createElement("title", null, u), /* @__PURE__ */ a.createElement("path", {
    fill: "#3A7DCE",
    d: "M0 0h744v496H0V0z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#fff",
    d: "m120 125 90 63 54-14 23-81 61-36 92 14 105 55 6 90 34 12v92l-65 115-78 24-72-17 18-30-8-32-10 9-162-25-49-85 20-46-49-62z"
  }));
}, I6 = function(i) {
  var u = i.title, f = d(i, Bh);
  return /* @__PURE__ */ a.createElement("svg", s({
    xmlns: "http://www.w3.org/2000/svg",
    viewBox: "0 0 513 342"
  }, f), u && /* @__PURE__ */ a.createElement("title", null, u), /* @__PURE__ */ a.createElement("path", {
    fill: "#FFF",
    d: "M0 0h512v342H0z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#338AF3",
    d: "M0 0h512v114H0zm0 228h512v114H0z"
  }), /* @__PURE__ */ a.createElement("circle", {
    fill: "#FFDA44",
    stroke: "#d6ab00",
    strokeWidth: 5,
    cx: 256.5,
    cy: 171,
    r: 40
  }));
}, P6 = function(i) {
  var u = i.title, f = d(i, bh);
  return /* @__PURE__ */ a.createElement("svg", s({
    xmlns: "http://www.w3.org/2000/svg",
    viewBox: "0 0 513 342"
  }, f), u && /* @__PURE__ */ a.createElement("title", null, u), /* @__PURE__ */ a.createElement("path", {
    fill: "#10338c",
    d: "M0 0h513v342H0z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#D80027",
    d: "M513 33 95.3 171 513 310.76V342L0 171 513 0z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#FFF",
    d: "M513 287.18v24.58L81.72 171 513 30v24.16"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#A2001D",
    d: "m476.98 140.21-21.89 10.68-3.18-15.32 31.19-29.77s-9.42-40.65-13.75-44.98l-112.32 55.82-6.84 36.76-31.9 28.59-.4 34.2 34.29-22.76 67.23-2.66-1.51 38.11h22.23l11.9-44.64 31.55-24.61-6.6-19.42z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#EFC100",
    stroke: "#231F20",
    strokeMiterlimit: 10,
    d: "m317.89 238.41-22.24-11.11 22.24-11.11h144.46v22.22z"
  }));
}, e7 = function(i) {
  var u = i.title, f = d(i, Th);
  return /* @__PURE__ */ a.createElement("svg", s({
    xmlns: "http://www.w3.org/2000/svg",
    viewBox: "0 0 513 342"
  }, f), u && /* @__PURE__ */ a.createElement("title", null, u), /* @__PURE__ */ a.createElement("path", {
    fill: "#FFF",
    d: "M0 114h513v114H0z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#D80027",
    d: "M0 0h513v114H0zm0 228h513v114H0z"
  }));
}, t7 = function(i) {
  var u = i.title, f = d(i, Lh);
  return /* @__PURE__ */ a.createElement("svg", s({
    xmlns: "http://www.w3.org/2000/svg",
    viewBox: "0 0 513 342"
  }, f), u && /* @__PURE__ */ a.createElement("title", null, u), /* @__PURE__ */ a.createElement("path", {
    fill: "#10338c",
    d: "M0 0h513v342H0z"
  }), /* @__PURE__ */ a.createElement("g", {
    fill: "#FFF"
  }, /* @__PURE__ */ a.createElement("path", {
    d: "M222.2 170.7c.3-.3.5-.6.8-.9-.2.3-.5.6-.8.9zM188 212.6l11 22.9 24.7-5.7-11 22.8 19.9 15.8-24.8 5.6.1 25.4-19.9-15.9-19.8 15.9.1-25.4-24.8-5.6 19.9-15.8-11.1-22.8 24.8 5.7zm197.9 28.5 5.2 10.9 11.8-2.7-5.3 10.9 9.5 7.5-11.8 2.6v12.2l-9.4-7.6-9.5 7.6.1-12.2-11.8-2.6 9.5-7.5-5.3-10.9 11.8 2.7zm-48.6-116 5.2 10.9 11.8-2.7-5.3 10.9 9.5 7.5-11.8 2.7v12.1l-9.4-7.6-9.5 7.6.1-12.1-11.9-2.7 9.5-7.5-5.3-10.9L332 136zm48.6-66.2 5.2 10.9 11.8-2.7-5.3 10.9 9.5 7.5-11.8 2.7v12.1l-9.4-7.6-9.5 7.6.1-12.1-11.8-2.7 9.5-7.5-5.3-10.9 11.8 2.7zm42.5 49.7 5.2 10.9 11.8-2.7-5.3 10.9 9.5 7.5-11.8 2.6V150l-9.4-7.6-9.5 7.6v-12.2l-11.8-2.6 9.5-7.5-5.3-10.9 11.8 2.7zM398 166.5l4.1 12.7h13.3l-10.8 7.8 4.2 12.7-10.8-7.9-10.8 7.9 4.1-12.7-10.7-7.8h13.3z"
  }), /* @__PURE__ */ a.createElement("path", {
    d: "M254.8 0v30.6l-45.1 25.1h45.1V115h-59.1l59.1 32.8v22.9h-26.7l-73.5-40.9v40.9H99v-48.6l-87.4 48.6H-1.2v-30.6L44 115H-1.2V55.7h59.1L-1.2 22.8V0h26.7L99 40.8V0h55.6v48.6L242.1 0z"
  })), /* @__PURE__ */ a.createElement("path", {
    fill: "#D80027",
    d: "M142.8 0h-32v69.3h-112v32h112v69.4h32v-69.4h112v-32h-112z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#0052B4",
    d: "m154.6 115 100.2 55.7v-15.8L183 115z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#FFF",
    d: "m154.6 115 100.2 55.7v-15.8L183 115z"
  }), /* @__PURE__ */ a.createElement("path", {
    d: "m154.6 115 100.2 55.7v-15.8L183 115zm-83.9 0-71.9 39.9v15.8L99 115z",
    fill: "#D80027"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#0052B4",
    d: "M99 55.7-1.2 0v15.7l71.9 40z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#FFF",
    d: "M99 55.7-1.2 0v15.7l71.9 40z"
  }), /* @__PURE__ */ a.createElement("path", {
    d: "M99 55.7-1.2 0v15.7l71.9 40zm84 0 71.8-40V0L154.6 55.7z",
    fill: "#D80027"
  }));
}, l7 = function(i) {
  var u = i.title, f = d(i, Vh);
  return /* @__PURE__ */ a.createElement("svg", s({
    xmlns: "http://www.w3.org/2000/svg",
    viewBox: "0 0 363 242"
  }, f), u && /* @__PURE__ */ a.createElement("title", null, u), /* @__PURE__ */ a.createElement("path", {
    fill: "#338AF3",
    d: "M0 0h363v242H0z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#FFF",
    d: "M57 96.9 14.7 78.2 57 59.6l18.6-42.2 18.6 42.2 42.3 18.6-42.3 18.7-18.6 42.2z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#f30028",
    d: "m75.6 40.6 11.5 26.1 26.1 11.5-26.1 11.5-11.5 26.1-11.5-26.1L38 78.2l26.1-11.5z"
  }), /* @__PURE__ */ a.createElement("path", {
    d: "M0 152.2h363v15.7H0zm0 31.4h363v15.7H0z",
    fill: "#FFDA44"
  }));
}, a7 = function(i) {
  var u = i.title, f = d(i, Ch);
  return /* @__PURE__ */ a.createElement("svg", s({
    xmlns: "http://www.w3.org/2000/svg",
    viewBox: "0 0 513 342"
  }, f), u && /* @__PURE__ */ a.createElement("title", null, u), /* @__PURE__ */ a.createElement("path", {
    fill: "#0052B4",
    d: "M0 0h513v342H0z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#FFDA44",
    d: "M513 210.9H202.2V342h-79.8V210.9H0v-79.8h122.4V0h79.8v131.1H513v61.2z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#D80027",
    d: "M513 149.7v42.6H183.7V342H141V192.3H0v-42.6h141V0h42.7v149.7z"
  }));
}, n7 = function(i) {
  var u = i.title, f = d(i, Oh);
  return /* @__PURE__ */ a.createElement("svg", s({
    xmlns: "http://www.w3.org/2000/svg",
    viewBox: "0 0 513 342"
  }, f), u && /* @__PURE__ */ a.createElement("title", null, u), /* @__PURE__ */ a.createElement("path", {
    fill: "#ef3340",
    d: "M0 0h513v342H0z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#00b5e2",
    d: "M0 0h513v114H0z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#509e2f",
    d: "M0 228h513v114H0z"
  }), /* @__PURE__ */ a.createElement("g", {
    fill: "#FFF"
  }, /* @__PURE__ */ a.createElement("path", {
    d: "M265.6 212.6c-23 0-41.6-18.6-41.6-41.6s18.6-41.6 41.6-41.6c7.2 0 13.9 1.8 19.8 5-9.2-9-21.9-14.6-35.8-14.6-28.3 0-51.2 22.9-51.2 51.2s22.9 51.2 51.2 51.2c13.9 0 26.6-5.6 35.8-14.6-5.9 3.2-12.6 5-19.8 5z"
  }), /* @__PURE__ */ a.createElement("path", {
    d: "m297.6 142.2 5.5 15.5 14.9-7.1-7.1 14.9 15.5 5.5-15.5 5.5 7.1 14.9-14.9-7.1-5.5 15.5-5.5-15.5-14.9 7.1 7.1-14.9-15.5-5.5 15.5-5.5-7.1-14.9 14.9 7.1z"
  })));
}, c7 = function(i) {
  var u = i.title, f = d(i, Nh);
  return /* @__PURE__ */ a.createElement("svg", s({
    xmlns: "http://www.w3.org/2000/svg",
    viewBox: "0 0 513 342"
  }, f), u && /* @__PURE__ */ a.createElement("title", null, u), /* @__PURE__ */ a.createElement("path", {
    fill: "#10338C",
    d: "M0 0h513.1v342H0z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#F6C500",
    d: "M99.9 0h342v342"
  }), /* @__PURE__ */ a.createElement("g", {
    fill: "#FFF"
  }, /* @__PURE__ */ a.createElement("path", {
    d: "M19.9 21.1 40.3 6.2 60.9 21 54 0H26.7z"
  }), /* @__PURE__ */ a.createElement("path", {
    d: "M92.3 25.5 84.4 1.3l-7.8 24.2H51.3l20.5 14.8-7.8 24 20.4-14.8L105 64.3l-7.9-24 20.5-14.9z"
  }), /* @__PURE__ */ a.createElement("path", {
    d: "m136.3 69.2-7.8-24-7.8 24H95.5l20.4 14.9-7.8 24 20.4-14.8 20.6 14.8-7.8-24 20.4-14.9z"
  }), /* @__PURE__ */ a.createElement("path", {
    d: "m179.4 112.3-7.8-24.1-7.8 24.1h-25.3l20.5 14.9-7.8 24 20.4-14.8 20.6 14.8-7.9-24 20.5-15z"
  }), /* @__PURE__ */ a.createElement("path", {
    d: "m222.5 155.3-7.8-24-7.8 24h-25.4l20.6 15-7.8 24 20.4-14.9 20.4 14.9-7.8-24 20.6-15z"
  }), /* @__PURE__ */ a.createElement("path", {
    d: "m265.6 198.4-7.8-24-7.8 24h-25.4l20.6 14.9-7.8 24.1 20.4-14.9 20.4 14.9-7.8-24.2 20.5-14.8z"
  }), /* @__PURE__ */ a.createElement("path", {
    d: "m308.7 241.5-7.9-24-7.8 24h-25.3l20.5 14.8-7.9 24.2 20.6-14.9 20.4 14.8-7.8-24.1 20.5-14.8z"
  }), /* @__PURE__ */ a.createElement("path", {
    d: "m351.7 284.6-7.8-24.2-7.8 24.2h-25.3l20.4 14.8-7.8 24.1 20.5-14.9 20.5 14.9-7.8-24.1 20.4-14.8z"
  }), /* @__PURE__ */ a.createElement("path", {
    d: "m387 303.5-7.9 24.1-25.3.1 19.8 14.3h26.6l19.9-14.4h-25.3z"
  })));
}, i7 = function(i) {
  var u = i.title, f = d(i, _h);
  return /* @__PURE__ */ a.createElement("svg", s({
    xmlns: "http://www.w3.org/2000/svg",
    viewBox: "0 0 513 342"
  }, f), u && /* @__PURE__ */ a.createElement("title", null, u), /* @__PURE__ */ a.createElement("path", {
    fill: "#ffc726",
    d: "M0 0h513v342H0z"
  }), /* @__PURE__ */ a.createElement("path", {
    d: "M0 0h171v342H0zm342 0h171v342H342z",
    fill: "#00267f"
  }), /* @__PURE__ */ a.createElement("path", {
    d: "m325.74 101.02-31.97 12.4c-.68 1.35-5.79 7.54-8.18 53.06h-17.05v-60.42L256 78.68l-12.54 27v60.8H226.4c-2.39-45.53-7.8-52.48-8.47-53.84l-31.68-11.63c.15.31 15.4 31.34 15.4 78.01v12.54h41.81v71.07h25.08v-71.07h41.81v-12.54c0-24.13 4.17-44.02 7.68-56.46 3.82-13.57 7.7-21.49 7.74-21.57l-.03.03z"
  }));
}, u7 = function(i) {
  var u = i.title, f = d(i, Uh);
  return /* @__PURE__ */ a.createElement("svg", s({
    xmlns: "http://www.w3.org/2000/svg",
    viewBox: "0 85.333 513 342"
  }, f), u && /* @__PURE__ */ a.createElement("title", null, u), /* @__PURE__ */ a.createElement("path", {
    fill: "#2d6e41",
    d: "M0 85.331h513v342H0z"
  }), /* @__PURE__ */ a.createElement("circle", {
    fill: "#F40B32",
    cx: 218.902,
    cy: 256.5,
    r: 115
  }));
}, r7 = function(i) {
  var u = i.title, f = d(i, Rh);
  return /* @__PURE__ */ a.createElement("svg", s({
    xmlns: "http://www.w3.org/2000/svg",
    viewBox: "0 0 513 342"
  }, f), u && /* @__PURE__ */ a.createElement("title", null, u), /* @__PURE__ */ a.createElement("path", {
    fill: "#fdda25",
    d: "M0 0h513v342H0z"
  }), /* @__PURE__ */ a.createElement("path", {
    d: "M0 0h171v342H0z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#ef3340",
    d: "M342 0h171v342H342z"
  }));
}, f7 = function(i) {
  var u = i.title, f = d(i, jh);
  return /* @__PURE__ */ a.createElement("svg", s({
    xmlns: "http://www.w3.org/2000/svg",
    viewBox: "0 0 513 342"
  }, f), u && /* @__PURE__ */ a.createElement("title", null, u), /* @__PURE__ */ a.createElement("path", {
    fill: "#3d944f",
    d: "M0 0h513v342H0z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#ef2b2d",
    d: "M0 0h513v171H0z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#FFDA44",
    d: "m256 102.6 16.9 52h54.7l-44.2 32.2 16.8 52-44.2-32.1-44.2 32.1 16.8-52-44.2-32.2h54.7z"
  }));
}, h7 = function(i) {
  var u = i.title, f = d(i, Gh);
  return /* @__PURE__ */ a.createElement("svg", s({
    xmlns: "http://www.w3.org/2000/svg",
    viewBox: "0 0 513 342"
  }, f), u && /* @__PURE__ */ a.createElement("title", null, u), /* @__PURE__ */ a.createElement("path", {
    fill: "#00966e",
    d: "M0 0h513v342H0z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#FFF",
    d: "M0 0h513v114H0z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#d62612",
    d: "M0 228h513v114H0z"
  }));
}, m7 = function(i) {
  var u = i.title, f = d(i, Yh);
  return /* @__PURE__ */ a.createElement("svg", s({
    xmlns: "http://www.w3.org/2000/svg",
    viewBox: "0 0 513 342"
  }, f), u && /* @__PURE__ */ a.createElement("title", null, u), /* @__PURE__ */ a.createElement("path", {
    fill: "#D80027",
    d: "M0 0h513v342H0z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#FFF",
    d: "m222.8 34.3-85.2 34.2 85.2 34.1-85.2 34.1 85.2 34.1-85.2 34.1 85.2 34.1-85.2 34.1 85.2 34.1-85.2 34.8H0V0h137.6z"
  }));
}, v7 = function(i) {
  var u = i.title, f = d(i, qh);
  return /* @__PURE__ */ a.createElement("svg", s({
    xmlns: "http://www.w3.org/2000/svg",
    viewBox: "0 0 513 342"
  }, f), u && /* @__PURE__ */ a.createElement("title", null, u), /* @__PURE__ */ a.createElement("path", {
    fill: "#D80027",
    d: "M0 0h513v342H0z"
  }), /* @__PURE__ */ a.createElement("path", {
    d: "M215.9 170.7 0 314.6V26.8zM513 26.8v287.8L297.1 170.7z",
    fill: "#47a644"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#0052B4",
    d: "M513 26.8 296.1 170.7 513 314.6V342h-41.1L256 197.4 40.1 342H0v-27.4l215.9-143.9L0 26.8V0h40.1L256 143.9 471.9 0H513z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#FFF",
    d: "M513 26.8 297.1 170.7 513 314.6V342h-40L256 197.4 39 342H0v-27.4l215.9-143.9L0 26.8V0h40.1L256 143.9 471.9 0H513z"
  }), /* @__PURE__ */ a.createElement("circle", {
    fill: "#FFF",
    cx: 251.6,
    cy: 170.7,
    r: 100.2
  }), /* @__PURE__ */ a.createElement("path", {
    d: "m251.4 103.6 7.4 12.9h14.8l-7.4 12.8 7.4 12.8h-14.8l-7.4 12.9-7.4-12.9h-14.8l7.4-12.8-7.4-12.8H244zm38.8 66.7 7.4 12.9h14.8L305 196l7.4 12.8h-14.8l-7.4 12.9-7.4-12.9h-14.9l7.4-12.8-7.4-12.8h14.9zm-77.2 0 7.4 12.9h14.9l-7.4 12.8 7.4 12.8h-14.9l-7.4 12.9-7.4-12.9h-14.8l7.4-12.8-7.4-12.8h14.8z",
    fill: "#D80027",
    stroke: "#47a644",
    strokeWidth: 3
  }));
}, s7 = function(i) {
  var u = i.title, f = d(i, Qh);
  return /* @__PURE__ */ a.createElement("svg", s({
    xmlns: "http://www.w3.org/2000/svg",
    viewBox: "0 85.333 513 342"
  }, f), u && /* @__PURE__ */ a.createElement("title", null, u), /* @__PURE__ */ a.createElement("path", {
    fill: "#008751",
    d: "M0 85.333h513v342H0z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#fcd116",
    d: "M196.666 85.333H513v171H196.666z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#e8112d",
    d: "M196.666 256H513v171H196.666z"
  }));
}, d7 = function(i) {
  var u = i.title, f = d(i, Xh);
  return /* @__PURE__ */ a.createElement("svg", s({
    xmlns: "http://www.w3.org/2000/svg",
    viewBox: "0 0 513 342"
  }, f), u && /* @__PURE__ */ a.createElement("title", null, u), /* @__PURE__ */ a.createElement("path", {
    fill: "#FFF",
    d: "M0 0h513v342H0z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#E6BC00",
    d: "M157.31 199.76s-9.04 43.93-19.31 40.39c-10.62-3.54-17.53-18.76-17.53-18.76l-14.52 21.24s38.18 8.48 41.01 5.47c1.22-1.14 8.21-15.1 10.34-26.05v-22.29zm-56.13 60.57s-2.48 20.18-1.06 22.66c1.24 2.3 39.48 11.33 39.48 11.33s-24.96 16.46-29.74 20.54L91.8 284.94l-.71-24.61h10.09zm11.33-165.69s39.65 41.6 40.36 45.5c.71 3.72-4.25 3.19-4.25 3.19s-44.79-37.53-49.92-41.07c-5.31-3.55 13.81-7.62 13.81-7.62z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#ADADAD",
    d: "M99.41 104.73s16.99-.71 16.64-11.51c-.35-10.62-11.15-14.87-19.12-12.39-7.97 2.3-23.01 13.45-13.63 35.05 9.38 21.42 23.37 29.39 26.2 47.8 0 0-5.46 9.86-19.12.35-16.9-11.77-6.2-38.24-10.44-44.96-4.25-7.08-40.95-38.92-40.95-38.92s-30.24 39.37-18 74.92c16.95 49.21 51.74 31.98 64.61 36.76 0 0-42.13 61.96-49.57 67.09 0 0 24.08 8.32 47.8-18.76 0 0 5.84 9.74 7.08 20.18h10.27c4.78-26.2 51.51-64.26 49.92-82.32-1.77-21.25-49.21-49.39-51.69-73.29z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#E6BC00",
    d: "M359.17 201.27s8.96 42.43 19.23 38.89c10.62-3.54 17.53-18.76 17.53-18.76l14.52 21.24s-37 8.48-39.83 5.47c-1.21-1.14-9.32-13.79-11.52-24.66l.07-22.18zm56.05 59.06s2.48 20.18 1.06 22.66c-1.24 2.3-39.48 11.33-39.48 11.33s24.96 16.46 29.74 20.54l18.06-29.92.71-24.61h-10.09zM403.89 94.64s-39.65 41.6-40.36 45.5c-.71 3.72 4.25 3.19 4.25 3.19s44.79-37.53 49.92-41.07c5.14-3.55-13.81-7.62-13.81-7.62z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#125ECC",
    d: "M157.31 87.06H359.1v145.46c0 16.06-9.69 21.39-21.91 31.85l-45.92 39.41c-18.25 15.62-47.91 15.57-66.07 0l-46.01-39.43c-12.08-10.35-21.88-15.71-21.88-31.82V87.06z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#FFE600",
    d: "M200.55 147.71c-7.96 0-14.41-6.45-14.41-14.41s6.45-14.41 14.41-14.41 14.41 6.45 14.41 14.41-6.45 14.41-14.41 14.41zm57.65-3c-7.96 0-14.41-6.45-14.41-14.41s6.45-14.41 14.41-14.41 14.41 6.45 14.41 14.41-6.45 14.41-14.41 14.41zm57.65 3c-7.96 0-14.41-6.45-14.41-14.41s6.45-14.41 14.41-14.41 14.41 6.45 14.41 14.41-6.45 14.41-14.41 14.41zM214.96 274.43c-7.96 0-14.41-6.45-14.41-14.41s6.45-14.41 14.41-14.41 14.41 6.45 14.41 14.41c.01 7.96-6.45 14.41-14.41 14.41zm43.24 14.42c-7.96 0-14.41-6.45-14.41-14.41s6.45-14.41 14.41-14.41 14.41 6.45 14.41 14.41c0 7.95-6.45 14.41-14.41 14.41zm43.24-14.42c-7.96 0-14.41-6.45-14.41-14.41s6.45-14.41 14.41-14.41 14.41 6.45 14.41 14.41-6.45 14.41-14.41 14.41z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#D60537",
    d: "M157.31 175.36H359.1v57.65H157.31v-57.65z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#EACE00",
    d: "M142.9 43.82s57.65-14.41 115.31-14.41 115.31 14.41 115.31 14.41l-14.41 57.65s-50.45-14.41-100.89-14.41-100.89 14.41-100.89 14.41L142.9 43.82z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#7A6920",
    d: "M258.2 72.65c-7.96 0-14.41-6.45-14.41-14.41s6.45-14.41 14.41-14.41 14.41 6.45 14.41 14.41-6.45 14.41-14.41 14.41zm57.65 5c-7.96 0-14.41-6.45-14.41-14.41s6.45-14.41 14.41-14.41 14.41 6.45 14.41 14.41-6.45 14.41-14.41 14.41zm-115.3 0c-7.96 0-14.41-6.45-14.41-14.41s6.45-14.41 14.41-14.41 14.41 6.45 14.41 14.41-6.45 14.41-14.41 14.41z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#ADADAD",
    d: "M365.08 178.02c-1.59 18.06 45.14 56.12 49.92 82.32h10.27c1.24-10.44 7.08-20.18 7.08-20.18 23.72 27.09 47.8 18.76 47.8 18.76-7.44-5.13-49.57-67.09-49.57-67.09 12.88-4.78 47.67 12.45 64.61-36.76 12.24-35.55-18-74.92-18-74.92s-36.7 31.84-40.95 38.92c-4.25 6.73 6.46 33.2-10.44 44.96-13.66 9.51-19.12-.35-19.12-.35 2.83-18.41 16.82-26.38 26.2-47.8 9.38-21.6-5.66-32.75-13.63-35.05-7.97-2.48-18.76 1.77-19.12 12.39-.35 10.8 16.64 11.51 16.64 11.51-2.48 23.9-49.92 52.04-51.69 73.29z"
  }), /* @__PURE__ */ a.createElement("circle", {
    fill: "#FFF",
    cx: 258.2,
    cy: 204.19,
    r: 16.93
  }));
}, o7 = function(i) {
  var u = i.title, f = d(i, Zh);
  return /* @__PURE__ */ a.createElement("svg", s({
    xmlns: "http://www.w3.org/2000/svg",
    viewBox: "0 0 513 342"
  }, f), u && /* @__PURE__ */ a.createElement("title", null, u), /* @__PURE__ */ a.createElement("path", {
    fill: "#c8102e",
    d: "M0 0h513v342H0z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#012169",
    d: "M0 .1h256.5v171H0z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#FFF",
    d: "M256 0h-22.6L160 48.9V0H96v48.9L22.6 0H0v22.7l46.1 30.7H0v64h46.1L0 148.1v22.6h22.6L96 121.8v48.9h64v-48.9l73.4 48.9H256v-22.6l-46.1-30.7H256v-64h-46.1L256 22.7z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#c8102e",
    d: "M144 0h-32v69.4H0v32h112v69.3h32v-69.3h112v-32H144z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#c8102e",
    d: "M0 170.7v-11l62.5-42.3h22.6L7.3 170.7zM7.3.1l77.8 53.2H62.5L0 11.1V.1zM256 .1v11l-62.5 42.3h-22.6L248.7.1zm-7.3 170.6-77.8-53.3h22.6l62.5 42.3v11z"
  }), /* @__PURE__ */ a.createElement("g", {
    transform: "translate(13 5)"
  }, /* @__PURE__ */ a.createElement("path", {
    fill: "#fff",
    d: "M303.9 94.94v99.69c0 59.81 79.75 99.69 79.75 99.69s79.75-39.88 79.75-99.69V94.94H303.9z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#2F8F22",
    d: "M436.37 254.44H330.93c23.13 25.08 52.72 39.88 52.72 39.88s29.59-14.8 52.72-39.88z"
  }), /* @__PURE__ */ a.createElement("circle", {
    fill: "#65B5D2",
    cx: 383.65,
    cy: 214.56,
    r: 39.88
  }), /* @__PURE__ */ a.createElement("circle", {
    fill: "#c8102e",
    cx: 343.77,
    cy: 194.63,
    r: 19.94
  }), /* @__PURE__ */ a.createElement("circle", {
    fill: "#c8102e",
    cx: 423.52,
    cy: 194.63,
    r: 19.94
  }), /* @__PURE__ */ a.createElement("circle", {
    fill: "#c8102e",
    cx: 383.65,
    cy: 154.75,
    r: 19.94
  })));
}, E7 = function(i) {
  var u = i.title, f = d(i, Kh);
  return /* @__PURE__ */ a.createElement("svg", s({
    xmlns: "http://www.w3.org/2000/svg",
    viewBox: "0 0 513 342"
  }, f), u && /* @__PURE__ */ a.createElement("title", null, u), /* @__PURE__ */ a.createElement("path", {
    fill: "#FFDA44",
    d: "M0 0h513v342H0z"
  }), /* @__PURE__ */ a.createElement("path", {
    d: "m0 193.74 513 127.19V214.26L0 97.08z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#FFF",
    d: "M513 234.26v-86.67L0 20.41v86.67"
  }), /* @__PURE__ */ a.createElement("g", {
    fill: "#D80027",
    stroke: "#231F20",
    strokeWidth: 3,
    strokeMiterlimit: 10
  }, /* @__PURE__ */ a.createElement("path", {
    d: "M306.4 134.01a50.28 50.28 0 0 1 6.07 24.02c0 21.55-13.51 39.94-32.52 47.19v-59.51c5.7-1.19 10-6.37 10-12.59 6.9 0 12.5-5.75 12.5-12.85h-.42c6.9 0 12.91-5.75 12.91-12.85h-34.99V76.58l-14.99-27.71-15 27.71v30.84h-34.99c0 7.1 6.01 12.85 12.91 12.85h-.42c0 7.1 5.6 12.85 12.5 12.85 0 6.22 4.29 11.4 9.99 12.59v61.37c-22.09-5.39-38.48-25.3-38.48-49.05 0-8.7 2.2-16.88 6.07-24.02-10.96 11.21-17.72 26.53-17.72 43.44 0 34.32 27.82 62.15 62.15 62.15s62.15-27.83 62.15-62.15c0-16.91-6.77-32.24-17.72-43.44z"
  }), /* @__PURE__ */ a.createElement("path", {
    d: "M198.46 226.81s20.69 27.71 66.5 27.71 66.5-27.71 66.5-27.71l13.47 28.37s-21.03 27.71-79.97 27.71-79.97-27.71-79.97-27.71l13.47-28.37zm-69.22-93.63h20.88l24.93 27.26v73.45h-26.28v-68.37zm270.76 0h-20.88l-24.93 27.26v73.45h26.27v-68.37z"
  })));
}, z7 = function(i) {
  var u = i.title, f = d(i, Jh);
  return /* @__PURE__ */ a.createElement("svg", s({
    xmlns: "http://www.w3.org/2000/svg",
    viewBox: "0 0 513 342"
  }, f), u && /* @__PURE__ */ a.createElement("title", null, u), /* @__PURE__ */ a.createElement("path", {
    fill: "#d52b1e",
    d: "M0 0h513v114H0z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#f9e300",
    d: "M0 114h513v114H0z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#007934",
    d: "M0 228h513v114H0z"
  }));
}, p7 = function(i) {
  var u = i.title, f = d(i, kh);
  return /* @__PURE__ */ a.createElement("svg", s({
    xmlns: "http://www.w3.org/2000/svg",
    viewBox: "0 0 513 342"
  }, f), u && /* @__PURE__ */ a.createElement("title", null, u), /* @__PURE__ */ a.createElement("path", {
    fill: "#FFF",
    d: "M0 0h513v342H0z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#012a87",
    d: "M0 342h513V0z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#f9d90f",
    d: "M250.4 0H0v166.9z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#dc171d",
    d: "m140.4 118.3 12.2 21.2h24.5l-12.3 21.1 12.3 21.2h-24.5L140.4 203l-12.2-21.2h-24.5l12.3-21.2-12.3-21.1h24.5z"
  }), /* @__PURE__ */ a.createElement("circle", {
    fill: "none",
    stroke: "#000",
    strokeWidth: 9,
    strokeMiterlimit: 10,
    cx: 140.4,
    cy: 160.6,
    r: 57.7
  }));
}, g7 = function(i) {
  var u = i.title, f = d(i, Wh);
  return /* @__PURE__ */ a.createElement("svg", s({
    xmlns: "http://www.w3.org/2000/svg",
    viewBox: "0 0 513 342"
  }, f), u && /* @__PURE__ */ a.createElement("title", null, u), /* @__PURE__ */ a.createElement("path", {
    fill: "#DC171D",
    d: "M512 342H0V.7h512V342z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#012A87",
    d: "M512 342H0V171.3h512V342z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#FFF",
    d: "M256 342 0 171.3 256 .7l256 170.7L256 342z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#F9D90F",
    d: "m199.2 249.5 21.7-66.8-56.8-41.2h70.5L256 74.7l21.7 66.8h70.2l-56.8 41.2 21.7 66.8-56.8-41.3z"
  }));
}, M7 = function(i) {
  var u = i.title, f = d(i, $h);
  return /* @__PURE__ */ a.createElement("svg", s({
    xmlns: "http://www.w3.org/2000/svg",
    width: 900,
    height: 600
  }, f), u && /* @__PURE__ */ a.createElement("title", null, u), /* @__PURE__ */ a.createElement("path", {
    d: "M0 0h900v600H0z",
    fill: "#dc171d"
  }), /* @__PURE__ */ a.createElement("path", {
    d: "M450 168.787 215.691 300 450 431.213 684.309 300z",
    fill: "#fff"
  }), /* @__PURE__ */ a.createElement("path", {
    d: "M281.477 336.852V376h69.961l290.727-52.4V376h-93.668z",
    fill: "#dc171f"
  }), /* @__PURE__ */ a.createElement("path", {
    d: "M188 288H20V20h418v128zm524 0h168V20H462v128zm-524 24H20v268h418V452zm524 0h168v268H462V452z",
    fill: "#012a87"
  }), /* @__PURE__ */ a.createElement("path", {
    d: "m450 199.5 8.068 24.827h26.105l-21.12 15.346 8.067 24.827L450 249.157 428.88 264.5l8.066-24.827-21.119-15.345 26.105-.001z",
    fill: "#f9d90f"
  }), /* @__PURE__ */ a.createElement("path", {
    d: "M281.477 336.852c2.712-8.848 8.614-12.77 16.594-10.995 4.94-18.73 27.739-22.07 36.786-12.857l22.273 22.37c4.837 2.251 12.207 3.095 17.326 3.095 6.33.1 5.621 3.005 10.91 3.106 14.096.27 51.143.458 57.857 0 2.75-.187 64.134-67.857 64.134-67.857 5.12-6.31 18.453-8.333 25.714-3.214 6.786 6.429 13.929 7.143 18.929 2.857 4.405-6.547 14.392-7.309 20.714-3.571l69.45 53.815L548.497 376H351.438l-69.96-39.148z",
    fill: "#008830"
  }));
}, w7 = function(i) {
  var u = i.title, f = d(i, Ih);
  return /* @__PURE__ */ a.createElement("svg", s({
    xmlns: "http://www.w3.org/2000/svg",
    viewBox: "0 0 513 342"
  }, f), u && /* @__PURE__ */ a.createElement("title", null, u), /* @__PURE__ */ a.createElement("path", {
    fill: "#FFF",
    d: "M0 0h513v342H0z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#012a87",
    d: "M0 342h513V0z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#f9d90f",
    d: "M250.4 0H0v166.9z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#dc171d",
    d: "m140.4 118.3 12.2 21.2h24.5l-12.3 21.1 12.3 21.2h-24.5L140.4 203l-12.2-21.2h-24.5l12.3-21.2-12.3-21.1h24.5z"
  }), /* @__PURE__ */ a.createElement("circle", {
    fill: "none",
    stroke: "#000",
    strokeWidth: 9,
    strokeMiterlimit: 10,
    cx: 140.4,
    cy: 160.6,
    r: 57.7
  }));
}, x7 = function(i) {
  var u = i.title, f = d(i, Ph);
  return /* @__PURE__ */ a.createElement("svg", s({
    xmlns: "http://www.w3.org/2000/svg",
    viewBox: "0 0 513 342"
  }, f), u && /* @__PURE__ */ a.createElement("title", null, u), /* @__PURE__ */ a.createElement("path", {
    fill: "#009b3a",
    d: "M0 0h513v342H0z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#fedf00",
    d: "m256.5 19.3 204.9 151.4L256.5 322 50.6 170.7z"
  }), /* @__PURE__ */ a.createElement("circle", {
    fill: "#FFF",
    cx: 256.5,
    cy: 171,
    r: 80.4
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#002776",
    d: "M215.9 165.7c-13.9 0-27.4 2.1-40.1 6 .6 43.9 36.3 79.3 80.3 79.3 27.2 0 51.3-13.6 65.8-34.3-24.9-31-63.2-51-106-51zm119 20.3c.9-5 1.5-10.1 1.5-15.4 0-44.4-36-80.4-80.4-80.4-33.1 0-61.5 20.1-73.9 48.6 10.9-2.2 22.1-3.4 33.6-3.4 46.8.1 89 19.5 119.2 50.6z"
  }));
}, H7 = function(i) {
  var u = i.title, f = d(i, em);
  return /* @__PURE__ */ a.createElement("svg", s({
    xmlns: "http://www.w3.org/2000/svg",
    viewBox: "0 0 513 342"
  }, f), u && /* @__PURE__ */ a.createElement("title", null, u), /* @__PURE__ */ a.createElement("path", {
    fill: "#ffc72c",
    d: "M0 0h513v342H0z"
  }), /* @__PURE__ */ a.createElement("path", {
    d: "M0 0h513v114H0zm0 228h513v114H0z",
    fill: "#00778b"
  }), /* @__PURE__ */ a.createElement("path", {
    d: "M256 171 0 342V0z"
  }));
}, y7 = function(i) {
  var u = i.title, f = d(i, tm);
  return /* @__PURE__ */ a.createElement("svg", s({
    xmlns: "http://www.w3.org/2000/svg",
    viewBox: "0 0 513 342"
  }, f), u && /* @__PURE__ */ a.createElement("title", null, u), /* @__PURE__ */ a.createElement("path", {
    fill: "#FF7415",
    d: "M0 0h513v342H0z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#FFDA44",
    d: "M513 0H0v342"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "none",
    stroke: "#FFF",
    strokeWidth: 42,
    strokeMiterlimit: 10,
    d: "M128.7 255.5s35 54 67.3 32.4c56.9-37.9-68.9-108.6-2.9-152.6 58.3-38.8 76.6 103.5 137.6 62.8 59-39.3-64.7-111.4-9.2-148.4 33.4-22.2 67.1 32.6 67.1 32.6"
  }));
}, F7 = function(i) {
  var u = i.title, f = d(i, lm);
  return /* @__PURE__ */ a.createElement("svg", s({
    xmlns: "http://www.w3.org/2000/svg",
    viewBox: "0 33.333 1100 733.333"
  }, f), u && /* @__PURE__ */ a.createElement("title", null, u), /* @__PURE__ */ a.createElement("path", {
    fill: "#ef2b2d",
    d: "M0 0h1100v800H0z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#fff",
    d: "M300 0h200v800H300z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#fff",
    d: "M0 300h1100v200H0z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#002868",
    d: "M350 0h100v800H350z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#002868",
    d: "M0 350h1100v100H0z"
  }));
}, D7 = function(i) {
  var u = i.title, f = d(i, am);
  return /* @__PURE__ */ a.createElement("svg", s({
    xmlns: "http://www.w3.org/2000/svg",
    viewBox: "0 0 513 342"
  }, f), u && /* @__PURE__ */ a.createElement("title", null, u), /* @__PURE__ */ a.createElement("path", {
    fill: "#FFF",
    d: "M0 0h513v342H0z"
  }), /* @__PURE__ */ a.createElement("path", {
    d: "M0 238h513v104H0zM0 0h513v104H0z",
    fill: "#6da9d2"
  }), /* @__PURE__ */ a.createElement("path", {
    d: "M0 125.5h513v89.656H0z"
  }));
}, S7 = function(i) {
  var u = i.title, f = d(i, nm);
  return /* @__PURE__ */ a.createElement("svg", s({
    xmlns: "http://www.w3.org/2000/svg",
    viewBox: "0 0 513 342"
  }, f), u && /* @__PURE__ */ a.createElement("title", null, u), /* @__PURE__ */ a.createElement("path", {
    fill: "#007C30",
    d: "M0 0h513v342H0z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#CE1720",
    d: "M0 230h513V0H0"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#FFF",
    d: "M100 230V0H0v342h513-413z"
  }), /* @__PURE__ */ a.createElement("path", {
    d: "M28 159.6 8.6 128.2 28 97.4l19.3 30.8zm44.6 0-19.3-31.4 19.3-30.8L92 128.2zM28 241.2 8.6 209.8 28 179l19.3 30.8zm44.6 0-19.3-31.4L72.6 179 92 209.8z",
    fill: "#CE1720"
  }), /* @__PURE__ */ a.createElement("path", {
    d: "m28 73.8-16.1-26L28 22.2l16 25.6zm44.6 0-16-26 16-25.6 16.1 25.6zM28 318l-16.1-26.1L28 266.3l16 25.6zm44.6 0-16-26.1 16-25.6 16.1 25.6z",
    fill: "none",
    stroke: "#CE1720",
    strokeWidth: 7
  }));
}, A7 = function(i) {
  var u = i.title, f = d(i, cm);
  return /* @__PURE__ */ a.createElement("svg", s({
    xmlns: "http://www.w3.org/2000/svg",
    viewBox: "0 0 513 342"
  }, f), u && /* @__PURE__ */ a.createElement("title", null, u), /* @__PURE__ */ a.createElement("path", {
    fill: "#003e87",
    d: "M0 0h513v342H0z"
  }), /* @__PURE__ */ a.createElement("circle", {
    fill: "#FFF",
    cx: 260.9,
    cy: 170.9,
    r: 118.9
  }), /* @__PURE__ */ a.createElement("circle", {
    fill: "none",
    stroke: "#6DA544",
    strokeWidth: 18,
    strokeMiterlimit: 10,
    cx: 261.9,
    cy: 173.1,
    r: 94.5
  }), /* @__PURE__ */ a.createElement("g", {
    stroke: "#000"
  }, /* @__PURE__ */ a.createElement("path", {
    fill: "#003e87",
    d: "m261.9 151.5-50.6 23.4v20c0 11.8 6.1 22.8 16.2 28.9L262 239l34.5-15.2c10-6.2 16.2-17.1 16.2-28.9v-20l-50.8-23.4z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#FFDA44",
    d: "M211.3 128.1h101.3v46.7H211.3z"
  })), /* @__PURE__ */ a.createElement("path", {
    d: "M0 0h513v35H0zm0 306h513v35H0z",
    fill: "#ce1127"
  }));
}, B7 = function(i) {
  var u = i.title, f = d(i, im);
  return /* @__PURE__ */ a.createElement("svg", s({
    xmlns: "http://www.w3.org/2000/svg",
    viewBox: "0 0 513 342"
  }, f), u && /* @__PURE__ */ a.createElement("title", null, u), /* @__PURE__ */ a.createElement("path", {
    fill: "#FFF",
    d: "M0 0h513v342H0z"
  }), /* @__PURE__ */ a.createElement("path", {
    d: "M0 0h142v342H0zm371 0h142v342H371zm-64.5 206 50.4-25.2-25.2-12.6V143l-50.4 25.2 25.2-50.4h-25.2L256.1 80l-25.2 37.8h-25.2l25.2 50.4-50.4-25.2v25.2l-25.2 12.6 50.4 25.2-12.6 25.2h50.4V269h25.2v-37.8h50.4z",
    fill: "red"
  }));
}, b7 = function(i) {
  var u = i.title, f = d(i, um);
  return /* @__PURE__ */ a.createElement("svg", s({
    xmlns: "http://www.w3.org/2000/svg",
    viewBox: "0 0 513 342"
  }, f), u && /* @__PURE__ */ a.createElement("title", null, u), /* @__PURE__ */ a.createElement("path", {
    fill: "green",
    d: "M0 0h513v342H0z"
  }), /* @__PURE__ */ a.createElement("path", {
    d: "m422.7 252.4 6.2 12.8 13.8-3.2-6.2 12.8 11.2 8.8-13.9 3.2V301l-11.1-8.9-11.1 8.9v-14.2l-13.8-3.2 11.1-8.8-6.2-12.8 13.9 3.2zm-46.6-136.3 6.2 12.9 13.8-3.2-6.2 12.8 11.2 8.8-13.9 3.1v14.3l-11.1-8.9-11.1 8.9.1-14.3-13.9-3.1 11.1-8.8-6.2-12.8L370 129zm37-77.8 6.2 12.8 13.8-3.2-6.2 12.8 11.2 8.9-13.9 3.1v14.2L413.1 78 402 86.9l.1-14.2-13.9-3.1 11.1-8.9-6.2-12.8 13.9 3.2zm51.8 58.4 6.2 12.8 13.9-3.2-6.3 12.8 11.2 8.9-13.9 3.1.1 14.2-11.2-8.9-11.1 8.9.1-14.2L440 128l11.2-8.9-6.3-12.8 13.9 3.2zm-28 68.1 4.9 14.8h15.6l-12.6 9.2 4.8 14.9-12.7-9.2-12.6 9.2 4.8-14.9-12.6-9.2h15.6zM306.8 254.7c-49.2 0-89.1-39.9-89.1-89.1s39.9-89.1 89.1-89.1c15.3 0 29.8 3.9 42.4 10.7C329.4 67.9 302.3 56 272.5 56c-60.5 0-109.6 49.1-109.6 109.6S212 275.3 272.5 275.3c29.8 0 56.9-11.9 76.6-31.3-12.6 6.8-27 10.7-42.3 10.7zM140.4 59.5C129.7 41 109.7 28.6 86.8 28.6S44 41 33.3 59.5h107.1zm.2.4-53.8 53.8-53.7-53.8C28 68.9 25 79.3 25 90.5c0 34.2 27.7 61.9 61.9 61.9s61.9-27.7 61.9-61.9c-.1-11.2-3.1-21.6-8.2-30.6z",
    fill: "#FFDA44"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#A2001D",
    d: "M71.4 98.2v52.2c4.9 1.3 10.1 1.9 15.5 1.9s10.5-.7 15.5-1.9V98.2h-31z"
  }));
}, T7 = function(i) {
  var u = i.title, f = d(i, rm);
  return /* @__PURE__ */ a.createElement("svg", s({
    xmlns: "http://www.w3.org/2000/svg",
    viewBox: "0 0 513 342"
  }, f), u && /* @__PURE__ */ a.createElement("title", null, u), /* @__PURE__ */ a.createElement("path", {
    fill: "#338AF3",
    d: "M0 0h513v342H0z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#FFDA44",
    d: "M513 66.9V0H411.7L0 274.4V342h100.3z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#D80027",
    d: "M513 0v40.1L60.2 342H0v-40.8L451.8 0z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#FFDA44",
    d: "m93.6 31.2 16.3 50.4H163l-42.9 31.2 16.4 50.5-42.9-31.2-43 31.2L67 112.8 24.1 81.6h53.1z"
  }));
}, L7 = function(i) {
  var u = i.title, f = d(i, fm);
  return /* @__PURE__ */ a.createElement("svg", s({
    xmlns: "http://www.w3.org/2000/svg",
    viewBox: "0 0 513 342"
  }, f), u && /* @__PURE__ */ a.createElement("title", null, u), /* @__PURE__ */ a.createElement("path", {
    fill: "#d21034",
    d: "M0 0h513v342H0z"
  }), /* @__PURE__ */ a.createElement("path", {
    d: "M0 171h211.5v85.3H0zm300.5 0H512v85.3H300.5z",
    fill: "#3a9927"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#003082",
    d: "M0 0v85.3h211.5V0h89v85.3H513V0z"
  }), /* @__PURE__ */ a.createElement("path", {
    d: "M300.5 85.3H512v85.3H300.5zM0 85.3h211.5v85.3H0z",
    fill: "#FFF"
  }), /* @__PURE__ */ a.createElement("path", {
    d: "M300.5 342h-89v-86H0v86h513v-86H300.5zM105.7 8.6l8.6 26.2h27.5L119.5 51l8.6 26.2L105.7 61 83.4 77.2 91.9 51 69.6 34.8h27.6z",
    fill: "#ffce00"
  }));
}, V7 = function(i) {
  var u = i.title, f = d(i, hm);
  return /* @__PURE__ */ a.createElement("svg", s({
    xmlns: "http://www.w3.org/2000/svg",
    viewBox: "0 85.333 513 342"
  }, f), u && /* @__PURE__ */ a.createElement("title", null, u), /* @__PURE__ */ a.createElement("path", {
    fill: "#da1a35",
    d: "M0 85.331h513v342H0z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#009543",
    d: "M443.726 85.331 102.4 426.657H0V85.331z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#fbde4a",
    d: "M500.124 85.331 158.798 426.657H11.876L353.202 85.331z"
  }));
}, C7 = function(i) {
  var u = i.title, f = d(i, mm);
  return /* @__PURE__ */ a.createElement("svg", s({
    xmlns: "http://www.w3.org/2000/svg",
    viewBox: "0 85.333 513 342"
  }, f), u && /* @__PURE__ */ a.createElement("title", null, u), /* @__PURE__ */ a.createElement("path", {
    fill: "red",
    d: "M0 85.337h513v342H0z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#FFF",
    d: "M356.174 222.609h-66.783v-66.783h-66.782v66.783h-66.783v66.782h66.783v66.783h66.782v-66.783h66.783z"
  }));
}, O7 = function(i) {
  var u = i.title, f = d(i, vm);
  return /* @__PURE__ */ a.createElement("svg", s({
    xmlns: "http://www.w3.org/2000/svg",
    viewBox: "0 0 513 342"
  }, f), u && /* @__PURE__ */ a.createElement("title", null, u), /* @__PURE__ */ a.createElement("path", {
    fill: "#FFF",
    d: "M0 0h513v342H0z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#009e60",
    d: "M342 0h171v342H342z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#f77f00",
    d: "M0 0h171v342H0z"
  }));
}, N7 = function(i) {
  var u = i.title, f = d(i, sm);
  return /* @__PURE__ */ a.createElement("svg", s({
    xmlns: "http://www.w3.org/2000/svg",
    viewBox: "0 85.333 513 342"
  }, f), u && /* @__PURE__ */ a.createElement("title", null, u), /* @__PURE__ */ a.createElement("path", {
    fill: "#01237a",
    d: "M0 85.332h513v342H0z"
  }), /* @__PURE__ */ a.createElement("path", {
    d: "m384 156.705 5.283 16.254h17.089l-13.826 10.045 5.281 16.254L384 189.211l-13.825 10.047 5.28-16.254-13.825-10.045h17.089zm-70.209 29.081 15.228 7.758 12.084-12.083-2.673 16.879 15.227 7.759-16.879 2.673-2.674 16.88-7.76-15.227-16.878 2.673 12.086-12.084zm-29.081 70.209 16.254-5.281.001-17.089 10.044 13.825 16.254-5.282-10.046 13.827 10.046 13.826-16.254-5.28-10.044 13.825v-17.09zm29.081 70.209 7.759-15.229-12.084-12.084 16.881 2.674 7.757-15.227 2.674 16.879 16.879 2.672-15.227 7.759 2.673 16.882-12.084-12.087zM384 355.284l-5.281-16.253h-17.091l13.827-10.045-5.28-16.254L384 322.776l13.827-10.044-5.281 16.254 13.826 10.045h-17.089zm70.209-29.08-15.229-7.758-12.083 12.084 2.673-16.882-15.227-7.756 16.879-2.675 2.675-16.879 7.756 15.227 16.881-2.674-12.086 12.085zm29.081-70.209-16.254 5.281v17.09l-10.045-13.826-16.254 5.281 10.046-13.826-10.046-13.827 16.254 5.282 10.045-13.825v17.089zm-29.081-70.207-7.757 15.226 12.082 12.084-16.881-2.673-7.756 15.227-2.675-16.88-16.879-2.675 15.227-7.757-2.673-16.878 12.084 12.082zM0 186.665v16h46.069L0 233.377v7.539l57.377-38.252H80L0 255.998h112v-69.334H0zm96 69.331H22.628L96 207.083v48.913zm80-117.331 80-53.334H144v69.334h112v-16h-46.069L256 107.951v-7.539l-57.377 38.251H176v.002zm-16-53.332h73.372L160 134.246V85.333zm-16 170.665h112l-80-53.334h22.623L256 240.917v-7.539l-46.069-30.713H256v-16H144v69.333zm16-48.915 73.372 48.913H160v-48.913zM112 85.331H0l80 53.334H57.377L0 100.413v7.539l46.069 30.712H0v16h112V85.331zm-16 48.915L22.628 85.333H96v48.913z",
    fill: "#FFF"
  }), /* @__PURE__ */ a.createElement("g", {
    fill: "#D80027"
  }, /* @__PURE__ */ a.createElement("path", {
    d: "M144 85.331h-32v69.334H0v32h112v69.333h32v-69.333h112v-32H144z"
  }), /* @__PURE__ */ a.createElement("path", {
    d: "M80 138.665 0 85.331v15.082l57.377 38.252zm96 0h22.623L256 100.413V85.331zm-118.623 64L0 240.917v15.081l80-53.333zm118.623 0 80 53.333v-15.081l-57.377-38.252z"
  })));
}, _7 = function(i) {
  var u = i.title, f = d(i, dm);
  return /* @__PURE__ */ a.createElement("svg", s({
    xmlns: "http://www.w3.org/2000/svg",
    viewBox: "0 0 513 342"
  }, f), u && /* @__PURE__ */ a.createElement("title", null, u), /* @__PURE__ */ a.createElement("path", {
    fill: "#D80027",
    d: "M0 0h513v342H0z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#FFF",
    d: "M196 0h317v171H196z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#0037A1",
    d: "M0 0h196v171H0z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#FFF",
    d: "M98 24.5 113.1 71H162l-39.6 28.7 15.2 46.5L98 117.5l-39.6 28.7 15.2-46.5L34 71h48.9z"
  }));
}, U7 = function(i) {
  var u = i.title, f = d(i, om);
  return /* @__PURE__ */ a.createElement("svg", s({
    xmlns: "http://www.w3.org/2000/svg",
    viewBox: "0 0 513 342"
  }, f), u && /* @__PURE__ */ a.createElement("title", null, u), /* @__PURE__ */ a.createElement("path", {
    fill: "#ce1126",
    d: "M0 0h513v342H0z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#007a5e",
    d: "M0 0h171v342H0z"
  }), /* @__PURE__ */ a.createElement("path", {
    d: "M342 0h171v342H342zm-86 102.2 17.2 53H329L283.9 188l17.2 53-45.1-32.7-45.1 32.7 17.2-53-45.1-32.8h55.8z",
    fill: "#fcd116"
  }));
}, R7 = function(i) {
  var u = i.title, f = d(i, Em);
  return /* @__PURE__ */ a.createElement("svg", s({
    xmlns: "http://www.w3.org/2000/svg",
    viewBox: "0 0 513 342"
  }, f), u && /* @__PURE__ */ a.createElement("title", null, u), /* @__PURE__ */ a.createElement("path", {
    fill: "#D80027",
    d: "M0 0h513v342H0z"
  }), /* @__PURE__ */ a.createElement("path", {
    d: "m226.8 239.2-9.7-15.6-17.9 4.4 11.9-14.1-9.7-15.6 17.1 6.9 11.8-14.1-1.3 18.4 17.1 6.9-17.9 4.4zM290.6 82l-10.1 15.4 11.6 14.3-17.7-4.8-10.1 15.5-1-18.4-17.7-4.8 17.2-6.6-1-18.4 11.6 14.3zm-54.4-56.6-2 18.3 16.8 7.6-18 3.8-2 18.3-9.2-16-17.9 3.8 12.3-13.7-9.2-15.9 16.8 7.5zm56.6 136.4-14.9 10.9 5.8 17.5-14.9-10.8-14.9 11 5.6-17.6-14.9-10.7 18.4-.1 5.6-17.6 5.8 17.5zM115 46.3l17.3 53.5h56.2l-45.4 32.9 17.3 53.5-45.4-33-45.5 33 17.4-53.5-45.5-32.9h56.3z",
    fill: "#FFDA44"
  }));
}, j7 = function(i) {
  var u = i.title, f = d(i, zm);
  return /* @__PURE__ */ a.createElement("svg", s({
    xmlns: "http://www.w3.org/2000/svg",
    viewBox: "0 0 513 342"
  }, f), u && /* @__PURE__ */ a.createElement("title", null, u), /* @__PURE__ */ a.createElement("path", {
    fill: "#FFDA44",
    d: "M0 0h513v342H0z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#D80027",
    d: "M0 256.5h513V342H0z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#0052B4",
    d: "M0 171h513v85.5H0z"
  }));
}, G7 = function(i) {
  var u = i.title, f = d(i, pm);
  return /* @__PURE__ */ a.createElement("svg", s({
    xmlns: "http://www.w3.org/2000/svg",
    viewBox: "0 0 513 342"
  }, f), u && /* @__PURE__ */ a.createElement("title", null, u), /* @__PURE__ */ a.createElement("path", {
    fill: "#FFF",
    d: "M0 0h513v342H0z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#D80027",
    d: "M0 114h513v114H0z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#0052B4",
    d: "M0 285h513v57H0zM0 0h513v57H0z"
  }));
}, Y7 = function(i) {
  var u = i.title, f = d(i, gm);
  return /* @__PURE__ */ a.createElement("svg", s({
    xmlns: "http://www.w3.org/2000/svg",
    viewBox: "0 0 513 342"
  }, f), u && /* @__PURE__ */ a.createElement("title", null, u), /* @__PURE__ */ a.createElement("path", {
    fill: "#FF9811",
    d: "M0-40.8v422.9-211.4z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#FFF",
    d: "M0 0h513v342H0z"
  }), /* @__PURE__ */ a.createElement("path", {
    d: "M0 0h513v68.3H0zm0 136.5h513v68.3H0zm0 136.6h513v68.3H0z",
    fill: "#0052B4"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#D80027",
    d: "M256 170.7 0 342V0z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#FFF",
    d: "m86.5 111.4 12.7 39.2h41.3l-33.4 24.2 12.8 39.3-33.4-24.2-33.4 24.2 12.8-39.3-33.4-24.2h41.2z"
  }));
}, q7 = function(i) {
  var u = i.title, f = d(i, Mm);
  return /* @__PURE__ */ a.createElement("svg", s({
    xmlns: "http://www.w3.org/2000/svg",
    viewBox: "0 0 513 342"
  }, f), u && /* @__PURE__ */ a.createElement("title", null, u), /* @__PURE__ */ a.createElement("path", {
    fill: "#003893",
    d: "M0 0h513v342H0z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#cf2027",
    d: "M0 182.2h513v41.4H0z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#FFF",
    d: "M0 140.8h513v41.4H0zm0 82.8h513V265H0z"
  }), /* @__PURE__ */ a.createElement("path", {
    d: "m150.4 70 6.9 21.1h22.2l-18 13.1 6.9 21.1-18-13.1-17.9 13.1 6.8-21.1-17.9-13.1h22.2zm0 209.7 6.9 21.1h22.2l-18 13 6.9 21.2-18-13.1-17.9 13.1 6.8-21.2-17.9-13h22.2zm-97.6-70.9 6.9 21.1h22.2l-18 13 6.9 21.2-18-13.1-17.9 13.1 6.8-21.2-17.9-13H46zM90.1 91l6.9 21.1h22.2l-18 13 6.9 21.2-18-13.1-17.9 13.1 6.8-21.2-18-13h22.3zm-66.3 71H46l6.8-21.1 6.9 21.1h22.2l-18 13.1 6.9 21.1-18-13-17.9 13 6.8-21.1zm48.4 148.9 6.8-21.1-18-13h22.3l6.8-21.2 6.9 21.2h22.2l-18 13 6.9 21.1-18-13zM248 208.8l-6.9 21.1h-22.2l18 13-6.9 21.2 18-13.1 18 13.1-6.9-21.2 18-13h-22.2zM210.7 91l-6.8 21.1h-22.2l17.9 13-6.8 21.2 17.9-13.1 18 13.1-6.9-21.2 18-13h-22.2zm66.4 71h-22.2l-6.9-21.1-6.9 21.1h-22.2l18 13.1-6.9 21.1 18-13 18 13-6.9-21.1zm-48.4 148.9-6.9-21.1 18-13h-22.2l-6.9-21.2-6.8 21.2h-22.2l17.9 13-6.8 21.1 17.9-13z",
    fill: "#f7d116",
    stroke: "#000"
  }));
}, Q7 = function(i) {
  var u = i.title, f = d(i, wm);
  return /* @__PURE__ */ a.createElement("svg", s({
    xmlns: "http://www.w3.org/2000/svg",
    viewBox: "0 0 513 342"
  }, f), u && /* @__PURE__ */ a.createElement("title", null, u), /* @__PURE__ */ a.createElement("path", {
    fill: "#002b7f",
    d: "M0 0h513v342H0z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#f9e814",
    d: "M0 233.5h513v51H0z"
  }), /* @__PURE__ */ a.createElement("path", {
    d: "m168.7 86.5 12.9 39.8h41.8l-33.8 24.5 12.9 39.7-33.8-24.5-33.8 24.5 12.9-39.7-33.8-24.5h41.8zm-83.3-54 7.8 23.9h25L97.9 71.1l7.8 23.8-20.3-14.7-20.3 14.7 7.8-23.8-20.3-14.7h25.1z",
    fill: "#FFF"
  }));
}, X7 = function(i) {
  var u = i.title, f = d(i, xm);
  return /* @__PURE__ */ a.createElement("svg", s({
    xmlns: "http://www.w3.org/2000/svg",
    viewBox: "0 0 513 342"
  }, f), u && /* @__PURE__ */ a.createElement("title", null, u), /* @__PURE__ */ a.createElement("path", {
    fill: "#0021ad",
    d: "M0 0h513v342H0z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#1c8a42",
    d: "M0 0h513v342"
  }), /* @__PURE__ */ a.createElement("path", {
    d: "m77.6 272 5.7 11.9 12.8-3-5.7 11.9 10.3 8.1-12.8 2.9V317l-10.3-8.3-10.2 8.3v-13.2l-12.8-2.9 10.3-8.1-5.8-11.9 12.9 3zm-37.1-98.9 5.7 11.9 12.8-3-5.7 11.8 10.3 8.2-12.9 2.9.1 13.2-10.3-8.3-10.3 8.3.1-13.2-12.9-2.9 10.4-8.2L22 182l12.8 3zm37.1-80.9 5.7 11.8 12.8-2.9-5.7 11.8 10.3 8.2-12.8 2.9v13.1l-10.3-8.2-10.2 8.2V124l-12.8-2.9 10.3-8.2-5.8-11.8L72 104zm46.1 62.9 5.7 11.9 12.8-3-5.7 11.9 10.3 8.1-12.8 2.9v13.2l-10.3-8.2-10.2 8.2v-13.2l-12.8-2.9 10.3-8.1-5.7-11.9 12.8 3zm-32.9 54 4.4 13.7h14.5L98 231.3l4.5 13.7-11.7-8.4-11.7 8.4 4.5-13.7-11.7-8.5h14.4z",
    fill: "#FFF"
  }), /* @__PURE__ */ a.createElement("circle", {
    fill: "#ffc639",
    cx: 267.1,
    cy: 170.7,
    r: 74.5
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#1c8a42",
    d: "M267.1 220.3h24.8s10.8-19 0-37.2l24.8-24.8-12.4-24.8h-12.4s-6.2 18.6-31 18.6-31-18.6-31-18.6h-12.4l12.4 24.8-12.4 24.8 12.4 12.4s12.4-24.8 37.2-12.4c0 0 10.5 15.5 0 37.2z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#ffc639",
    d: "M464.4 92.2c.6-2.9-.2-17.6-.2-20.7 0-21.3-13.9-39.4-33.2-45.7 5.9 12 9.2 25.4 9.2 39.7 0 4.8-.4 9.5-1.1 14.1-2.9-4.7-6.6-8.9-11.2-12.6-17.1-13.6-40.6-14-57.9-2.5 13.4 2.9 26.3 8.9 37.7 18 9 7.1 16.2 16.8 21.7 26.1 0 0-17.8 10.9-31 15.1s-42.3 7.9-42.3 7.9c72 12 132-36 132-36-6.5-13.4-15.8-4-23.7-3.4z"
  }));
}, Z7 = function(i) {
  var u = i.title, f = d(i, Hm);
  return /* @__PURE__ */ a.createElement("svg", s({
    xmlns: "http://www.w3.org/2000/svg",
    viewBox: "0 0 513 342"
  }, f), u && /* @__PURE__ */ a.createElement("title", null, u), /* @__PURE__ */ a.createElement("path", {
    fill: "#FFF",
    d: "M0 0h513v342H0z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#DB7D00",
    d: "M141.7 154.7s.2 67.1 74.7 65.3l4.5 13.9h8.9s-7.4-41.1 60.1-41.5c0 0 0-27.6 27.6-27.6H359s-66-51.8 58.9-118l1.8-13.1s-129.9 71-198.9 57.2c0 0 10.7 42.5-10.8 42.5-10.8 0-9.7-8.1-32.3-8.1-18.7 0-17.3 19.7-26.3 19.5-8.9-.2-18.8-12.3-19.6-10.2-.7 2.1 9.9 20.1 9.9 20.1z"
  }), /* @__PURE__ */ a.createElement("g", {
    fill: "#006651"
  }, /* @__PURE__ */ a.createElement("path", {
    d: "M237.2 308.1c6.9-5 13-6.6 22.4-8.3s19.4-4.4 24.6-5.8-17.7 6.6-23.5 8.3c-5.8 1.6-23.5 5.8-23.5 5.8zm37.9-14.7c-1.9-11.9 2.8-24.3 13.5-29.3 2.5 8.6-5.2 23.2-13.5 29.3zm18.2-6.2c-5.8-9.8 4-22.6 11.1-28.8 3.3 6-2.5 23.7-11.1 28.8zm16.9-7.6c-6.2-8.4 1.1-23.2 8.8-29 3.1 8.2.1 23.2-8.8 29zm16.9-10.6c-5.6-8-1.7-20.4 6.3-28.4 5.8 6.6.9 21-6.3 28.4zm13.5-10.7c-4.7-7.5 1.1-25.4 8.6-30.4 3.3 6.6.8 25.4-8.6 30.4zm10.8-2.8c-1.4-10.8 17.4-22.7 25.2-22.4-.9 8.9-8.9 18.6-25.2 22.4zm-10.5 12.2c8.8-9.1 26-9.1 32.1-7.2-1.7 5.3-21.9 16.9-32.1 7.2z"
  }), /* @__PURE__ */ a.createElement("path", {
    d: "M328.7 276.8c12.4-3.3 20.5-6.1 27.9 1.7-5.2 6.6-25.4 4.7-27.9-1.7zm-17.7 8c11.9-6.4 26.3 3 28.5 8.6-13.3 5.5-28.7-7.2-28.5-8.6zm-16.3 9.2c10.8-4.1 23.2 1.4 28.2 7.5-5.8 2.7-21 5.7-28.2-7.5zm-14.9 4.7c12.4-1.4 24.4 8 27 13.4-15.9 1.5-22-3.2-27-13.4zm-4 9.4c-6.9-5-13-6.6-22.4-8.3-9.4-1.7-19.4-4.4-24.6-5.8-5.3-1.4 17.7 6.6 23.5 8.3 5.8 1.6 23.5 5.8 23.5 5.8zm-37.9-14.7c1.9-11.9-2.8-24.3-13.5-29.3-2.5 8.6 5.2 23.2 13.5 29.3zm-18.2-6.2c5.8-9.8-4-22.6-11.1-28.8-3.3 6 2.5 23.7 11.1 28.8zm-16.9-7.6c6.2-8.4-1.1-23.2-8.8-29-3.1 8.2-.1 23.2 8.8 29zM185.9 269c5.6-8 1.7-20.4-6.3-28.4-5.8 6.6-.9 21 6.3 28.4zm-13.5-10.7c4.7-7.5-1.1-25.4-8.6-30.4-3.3 6.6-.8 25.4 8.6 30.4zm-10.8-2.8c1.4-10.8-17.4-22.7-25.2-22.4.9 8.9 8.9 18.6 25.2 22.4zm10.5 12.2c-8.8-9.1-26-9.1-32.1-7.2 1.7 5.3 21.9 16.9 32.1 7.2z"
  }), /* @__PURE__ */ a.createElement("path", {
    d: "M184.3 276.8c-12.4-3.3-20.5-6.1-27.9 1.7 5.2 6.6 25.4 4.7 27.9-1.7zm17.7 8c-11.9-6.4-26.3 3-28.5 8.6 13.3 5.5 28.7-7.2 28.5-8.6zm16.3 9.2c-10.8-4.1-23.2 1.4-28.2 7.5 5.8 2.7 21 5.7 28.2-7.5zm14.9 4.7c-12.4-1.4-24.4 8-27 13.4 15.9 1.5 22-3.2 27-13.4z"
  })));
}, K7 = function(i) {
  var u = i.title, f = d(i, ym);
  return /* @__PURE__ */ a.createElement("svg", s({
    xmlns: "http://www.w3.org/2000/svg",
    viewBox: "0 0 513 342"
  }, f), u && /* @__PURE__ */ a.createElement("title", null, u), /* @__PURE__ */ a.createElement("path", {
    fill: "#11457e",
    d: "M0 0h513v342H0z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#d7141a",
    d: "M513 171v171H0l256.5-171z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#FFF",
    d: "M513 0v171H256.5L0 0z"
  }));
}, J7 = function(i) {
  var u = i.title, f = d(i, Fm);
  return /* @__PURE__ */ a.createElement("svg", s({
    xmlns: "http://www.w3.org/2000/svg",
    viewBox: "0 0 513 342"
  }, f), u && /* @__PURE__ */ a.createElement("title", null, u), /* @__PURE__ */ a.createElement("path", {
    fill: "#D80027",
    d: "M0 0h513v342H0z"
  }), /* @__PURE__ */ a.createElement("path", {
    d: "M0 0h513v114H0z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#FFDA44",
    d: "M0 228h513v114H0z"
  }));
}, k7 = function(i) {
  var u = i.title, f = d(i, Dm);
  return /* @__PURE__ */ a.createElement("svg", s({
    xmlns: "http://www.w3.org/2000/svg",
    viewBox: "0 0 513 342"
  }, f), u && /* @__PURE__ */ a.createElement("title", null, u), /* @__PURE__ */ a.createElement("path", {
    fill: "#12ad2b",
    d: "M0 0h513v342H0z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#6ab2e7",
    d: "M513 0v166.7L0 170.8V0z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#FFF",
    d: "M256 170.7 0 342V0z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#d7141a",
    d: "m89.8 92.5 17 52.4H162l-44.6 32.5 17 52.4-44.6-32.4-44.6 32.4 17-52.4-44.6-32.5h55.2z"
  }));
}, W7 = function(i) {
  var u = i.title, f = d(i, Sm);
  return /* @__PURE__ */ a.createElement("svg", s({
    xmlns: "http://www.w3.org/2000/svg",
    viewBox: "0 0 513 342"
  }, f), u && /* @__PURE__ */ a.createElement("title", null, u), /* @__PURE__ */ a.createElement("path", {
    fill: "#c60c30",
    d: "M0 0h513v342H0z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#FFF",
    d: "M190 0h-60v140H0v60h130v142h60V200h323v-60H190z"
  }));
}, $7 = function(i) {
  var u = i.title, f = d(i, Am);
  return /* @__PURE__ */ a.createElement("svg", s({
    xmlns: "http://www.w3.org/2000/svg",
    viewBox: "0 0 513 342"
  }, f), u && /* @__PURE__ */ a.createElement("title", null, u), /* @__PURE__ */ a.createElement("path", {
    fill: "#496E2D",
    d: "M0 0h513v342H0z"
  }), /* @__PURE__ */ a.createElement("path", {
    d: "M0 110.7v120h513v-120z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#FFDA44",
    d: "M0 110.7h513v40H0z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#FFF",
    d: "M0 190.7h513v40H0z"
  }), /* @__PURE__ */ a.createElement("path", {
    d: "M196 0h120v342H196z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#FFDA44",
    d: "M196 0h40v342h-40z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#FFF",
    d: "M274.7 0h40v342h-40z"
  }), /* @__PURE__ */ a.createElement("circle", {
    fill: "#D80027",
    cx: 256,
    cy: 170.7,
    r: 123.1
  }), /* @__PURE__ */ a.createElement("path", {
    d: "m256 58.6 4.6 14.2h15l-12.1 8.9 4.6 14.2-12.1-8.8-12.1 8.8 4.6-14.2-12.1-8.9h15zM190.1 80l12.2 8.8 12.1-8.8-4.6 14.3 12.1 8.8h-15l-4.6 14.2-4.7-14.2h-15l12.1-8.8zm-40.7 56h15l4.6-14.2 4.7 14.2h15l-12.2 8.8 4.7 14.3-12.2-8.8-12.1 8.8 4.6-14.3zm0 69.3 12.1-8.8-4.6-14.3L169 191l12.2-8.8-4.7 14.3 12.2 8.8h-15l-4.7 14.3-4.6-14.3zm40.7 56.1 4.6-14.3-12.1-8.8h15l4.7-14.3 4.6 14.3h15l-12.1 8.8 4.6 14.3-12.1-8.9zm65.9 21.4-4.6-14.3h-15l12.1-8.8-4.6-14.3 12.1 8.8 12.1-8.8-4.6 14.3 12.1 8.8h-15zm65.9-21.4-12.2-8.9-12.1 8.9 4.6-14.3-12.1-8.8h15l4.6-14.3 4.7 14.3h15l-12.1 8.8zm40.7-56.1h-15l-4.6 14.3-4.7-14.3h-15l12.2-8.8-4.7-14.3L343 191l12.1-8.8-4.6 14.3zm0-69.3-12.1 8.8 4.6 14.3-12.1-8.8-12.2 8.8 4.7-14.3-12.2-8.8h15l4.7-14.2 4.6 14.2zm-40.7-56-4.6 14.3 12.1 8.8h-15l-4.7 14.2-4.6-14.2h-15l12.1-8.8-4.6-14.3 12.1 8.8zM279.3 168.7c-11-21.1-14.5-25.1-14.5-25.1s.4-9.7.4-15.6c0-8.8-7.4-15.8-16.5-15.8-8.6 0-15.7 2.9-16.5 11-4.2.9-8.6 4.1-8.6 10.7 0 4.8 1 7.3 5.2 9.3 2.1-4.6 4.3-4.8 9.3-6.4.8.6 1.7 3 2.6 3.4l.3 1s-13.3 6.6-13.3 30.9c0 29.5 22 45.4 22 45.4l-1.8.3-1.9 7.1h22v-7.2l11 17.5c.3-.3 10.2-47.6.3-66.5z",
    fill: "#496E2D"
  }));
}, I7 = function(i) {
  var u = i.title, f = d(i, Bm);
  return /* @__PURE__ */ a.createElement("svg", s({
    xmlns: "http://www.w3.org/2000/svg",
    viewBox: "0 0 513 342"
  }, f), u && /* @__PURE__ */ a.createElement("title", null, u), /* @__PURE__ */ a.createElement("path", {
    fill: "#FFF",
    d: "M0 0h513v342H0z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#D80027",
    d: "M0 216h211v126H0z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#0052B4",
    d: "M0 0h211v126H0z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#D80027",
    d: "M302 0h211v126H302z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#0052B4",
    d: "M302 216h211v126H302z"
  }), /* @__PURE__ */ a.createElement("g", {
    stroke: "#FFF",
    strokeWidth: 5,
    strokeMiterlimit: 10
  }, /* @__PURE__ */ a.createElement("path", {
    fill: "#0052B4",
    d: "M256 130h-49.9v49.4s19.5 6 49.9 6V130z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#D80027",
    d: "M206.1 179.4v6c0 27.5 22.3 49.9 49.9 49.9v-49.9c-30.4 0-49.9-6-49.9-6z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#0052B4",
    d: "M256 235.3c27.5 0 49.9-22.3 49.9-49.9v-6s-19.5 6-49.9 6v49.9z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#D80027",
    d: "M256 130v55.4c30.4 0 49.9-6 49.9-6V130H256z"
  })));
}, P7 = function(i) {
  var u = i.title, f = d(i, bm);
  return /* @__PURE__ */ a.createElement("svg", s({
    xmlns: "http://www.w3.org/2000/svg",
    viewBox: "0 0 513 342"
  }, f), u && /* @__PURE__ */ a.createElement("title", null, u), /* @__PURE__ */ a.createElement("path", {
    fill: "#41662E",
    d: "M0 0h513v342H0z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#FFF",
    d: "M256.5 0H513v342H256.5z"
  }), /* @__PURE__ */ a.createElement("g", {
    fill: "#D80027"
  }, /* @__PURE__ */ a.createElement("path", {
    d: "m341.5 105.3-29.4 40.4-47.5-15.4 29.4 40.4-29.4 40.4 47.5-15.4 29.4 40.4v-50l47.5-15.5-47.5-15.4z"
  }), /* @__PURE__ */ a.createElement("path", {
    d: "M309.9 276.7c-58.5 0-106-47.5-106-106s47.5-106 106-106c18.3 0 35.4 4.6 50.4 12.7-23.5-23-55.7-37.2-91.2-37.2-72 0-130.4 58.4-130.4 130.4S197.1 301 269.1 301c35.5 0 67.7-14.2 91.2-37.2-14.9 8.2-32.1 12.9-50.4 12.9z"
  })));
}, ei = function(i) {
  var u = i.title, f = d(i, Tm);
  return /* @__PURE__ */ a.createElement("svg", s({
    xmlns: "http://www.w3.org/2000/svg",
    viewBox: "0 0 513 342"
  }, f), u && /* @__PURE__ */ a.createElement("title", null, u), /* @__PURE__ */ a.createElement("path", {
    fill: "#FFDA44",
    d: "M0 0h513v342H0z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#0052B4",
    d: "M0 170.7h513V256H0z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#D80027",
    d: "M0 256h513v85.3H0z"
  }), /* @__PURE__ */ a.createElement("circle", {
    fill: "#FFDA44",
    cx: 256,
    cy: 171,
    r: 60
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#4C1F00",
    d: "M369.2 79.9s-27.2-13.8-33.9-16c-6.7-2.2-49.1-13.2-49.1-7 0 10.3-13.5 12-15.5 12s-2-5.3-14.8-5.3-13.7 4.8-15.8 4.8c-2.2 0-14.5-1-14.5-11.5 0-5.2-42.9 4-49.1 7-6.1 3.1-33.9 16-33.9 16s45.8 2.7 53.9 6.2 43.8 16.5 43.8 16.5l-2.8 13.3h37.8l-3.8-13.3s35.1-12.7 43.8-16.5 53.9-6.2 53.9-6.2z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#57BA17",
    d: "M217.7 171.7c0 21.1 17.2 38.3 38.3 38.3 21.1 0 38.3-17.2 38.3-38.3v-11.5h-76.5v11.5z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#338AF3",
    d: "M256 110.5c-21.1 0-38.3 17.2-38.3 38.3v11.5h76.5v-11.5c.1-21.1-17.1-38.3-38.2-38.3z"
  }));
}, ti = function(i) {
  var u = i.title, f = d(i, Lm);
  return /* @__PURE__ */ a.createElement("svg", s({
    xmlns: "http://www.w3.org/2000/svg",
    viewBox: "0 0 513 342"
  }, f), u && /* @__PURE__ */ a.createElement("title", null, u), /* @__PURE__ */ a.createElement("path", {
    fill: "#111",
    d: "M0 0h513v342H0z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#368FD8",
    d: "M0 0h513v114H0z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#FFF",
    d: "M0 228h513v114H0z"
  }));
}, li = function(i) {
  var u = i.title, f = d(i, Vm);
  return /* @__PURE__ */ a.createElement("svg", s({
    xmlns: "http://www.w3.org/2000/svg",
    viewBox: "0 0 513 342"
  }, f), u && /* @__PURE__ */ a.createElement("title", null, u), /* @__PURE__ */ a.createElement("path", {
    fill: "#FFF",
    d: "M0 0h513v342H0z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#D80027",
    d: "M0 0h513v114H0z"
  }), /* @__PURE__ */ a.createElement("path", {
    d: "M0 228h513v114H0z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#C09300",
    d: "M220.3 204.4s0-58.4 4.5-64.7c3.1-4.3 16.8 5.2 22.7 4.5 0 0 4.2-7.5 4.5-12 .3-4.6-1.1-7.6-4.9-6.2 0 0-1.2-2.1.5-3.3 1.6-1.2 5.6.1 5.6.1s-.5-1 1.6-.9c2.9.2 7.2 1.4 7.4 5.6.2 3.1.3 7.7.4 8.7.7 6.8 2.7 8.7 2.7 8.7s18.4-9.2 22-5.2c3.3 3.8 4.5 64.7 4.5 64.7l-18.1-16.8 12.1 29.5s-14.4 2.4-28.9 2.4c-14.5 0-31.1-4.2-31.1-4.2l13.8-28.2-19.3 17.3z"
  }));
}, ai = function(i) {
  var u = i.title, f = d(i, Cm);
  return /* @__PURE__ */ a.createElement("svg", s({
    xmlns: "http://www.w3.org/2000/svg",
    viewBox: "0 0 513 342"
  }, f), u && /* @__PURE__ */ a.createElement("title", null, u), /* @__PURE__ */ a.createElement("path", {
    fill: "#FFF",
    d: "M0 0h513v342H0z"
  }), /* @__PURE__ */ a.createElement("path", {
    d: "M0 0h513v114H0z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#428823",
    d: "M0 228h513v114H0z"
  }), /* @__PURE__ */ a.createElement("g", {
    fill: "#D80027"
  }, /* @__PURE__ */ a.createElement("path", {
    d: "M256 171 0 342V0zm53.1 0c0-22.9 13.1-42.1 34.6-46.8-3.3-.7-6.7-1.1-10.3-1.1-26.4 0-47.9 21.4-47.9 47.9s21.4 47.9 47.9 47.9c3.5 0 7-.4 10.3-1.1-21.5-5.1-34.6-24.2-34.6-46.8z"
  }), /* @__PURE__ */ a.createElement("path", {
    d: "m365 129.2 10.3 31.7h33.3l-27 19.6 10.3 31.7-26.9-19.6-27 19.6 10.3-31.7-27-19.6h33.4z"
  })));
}, ni = function(i) {
  var u = i.title, f = d(i, Om);
  return /* @__PURE__ */ a.createElement("svg", s({
    xmlns: "http://www.w3.org/2000/svg",
    viewBox: "0 0 513 342"
  }, f), u && /* @__PURE__ */ a.createElement("title", null, u), /* @__PURE__ */ a.createElement("path", {
    fill: "#338AF3",
    d: "M0 0h513v342H0z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#56AF35",
    d: "M0 0h513v171H0z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#D80027",
    d: "m0 342 513-171L0 0v342z"
  }), /* @__PURE__ */ a.createElement("g", {
    fill: "#ffc945"
  }, /* @__PURE__ */ a.createElement("path", {
    d: "M134.7 231.5c33.6 0 60.8-27.2 60.8-60.8s-27.2-60.8-60.8-60.8-60.8 27.2-60.8 60.8 27.2 60.8 60.8 60.8zm0 24.3c-47 0-85.2-38.1-85.2-85.2s38.1-85.2 85.2-85.2 85.2 38.1 85.2 85.2-38.2 85.2-85.2 85.2z"
  }), /* @__PURE__ */ a.createElement("circle", {
    cx: 119.5,
    cy: 148.3,
    r: 17.5
  }), /* @__PURE__ */ a.createElement("circle", {
    cx: 148.9,
    cy: 158.5,
    r: 17.5
  }), /* @__PURE__ */ a.createElement("circle", {
    cx: 134.7,
    cy: 135.2,
    r: 17.5
  }), /* @__PURE__ */ a.createElement("circle", {
    cx: 119.5,
    cy: 172.7,
    r: 17.5
  }), /* @__PURE__ */ a.createElement("circle", {
    cx: 149.9,
    cy: 182.8,
    r: 17.5
  }), /* @__PURE__ */ a.createElement("circle", {
    cx: 122.5,
    cy: 198,
    r: 17.5
  }), /* @__PURE__ */ a.createElement("circle", {
    cx: 145.9,
    cy: 205.2,
    r: 17.5
  })));
}, ci = function(i) {
  var u = i.title, f = d(i, Nm);
  return /* @__PURE__ */ a.createElement("svg", s({
    xmlns: "http://www.w3.org/2000/svg",
    width: 810,
    height: 540
  }, f), u && /* @__PURE__ */ a.createElement("title", null, u), /* @__PURE__ */ a.createElement("path", {
    fill: "#FCDD09",
    d: "M0 0h810v540H0z"
  }), /* @__PURE__ */ a.createElement("path", {
    stroke: "#DA121A",
    strokeWidth: 60,
    d: "M0 90h810m0 120H0m0 120h810m0 120H0"
  }));
}, ii = function(i) {
  var u = i.title, f = d(i, _m);
  return /* @__PURE__ */ a.createElement("svg", s({
    xmlns: "http://www.w3.org/2000/svg",
    viewBox: "0 0 22.5 15"
  }, f), u && /* @__PURE__ */ a.createElement("title", null, u), /* @__PURE__ */ a.createElement("path", {
    fill: "#FFF",
    d: "M0 0h22.5v15H0V0z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#D03433",
    d: "M0 0h22.5v4H0V0zm0 11h22.5v4H0v-4z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#FBCA46",
    d: "M0 4h22.5v7H0V4z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#FFF",
    d: "M7.8 7h1v.5h-1V7z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#A41517",
    d: "M7.2 8.5c0 .3.3.5.6.5s.6-.2.6-.5L8.5 7H7.1l.1 1.5zM6.6 7c0-.3.2-.5.4-.5h1.5c.3 0 .5.2.5.4V7l-.1 1.5c-.1.6-.5 1-1.1 1-.6 0-1-.4-1.1-1L6.6 7z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#A41517",
    d: "M6.8 7.5h2V8h-.5l-.5 1-.5-1h-.5v-.5zM5.3 6h1v3.5h-1V6zm4 0h1v3.5h-1V6zm-2.5-.5c0-.3.2-.5.5-.5h1c.3 0 .5.2.5.5v.2c0 .2-.1.3-.3.3H7c-.1 0-.2-.1-.2-.2v-.3z"
  }));
}, ui = function(i) {
  var u = i.title, f = d(i, Um);
  return /* @__PURE__ */ a.createElement("svg", s({
    xmlns: "http://www.w3.org/2000/svg",
    viewBox: "0 0 22.5 15"
  }, f), u && /* @__PURE__ */ a.createElement("title", null, u), /* @__PURE__ */ a.createElement("path", {
    fill: "#20AA46",
    d: "M0 0h22.5v5H0V0z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#E92F3B",
    d: "M0 10h22.5v5H0v-5z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#FADF50",
    d: "M0 5h22.5v5H0V5z"
  }), /* @__PURE__ */ a.createElement("circle", {
    fill: "#205CCA",
    cx: 11.3,
    cy: 7.5,
    r: 5.2
  }), /* @__PURE__ */ a.createElement("g", {
    stroke: "#FFDB3D",
    fill: "none"
  }, /* @__PURE__ */ a.createElement("path", {
    d: "m11.3 8.8-2.1 1.5.8-2.4-2-1.5h2.5l.8-2.3.8 2.3h2.6l-2.1 1.5.8 2.4-2.1-1.5zm-1-2.4h1.9M9.9 7.8l1.3 1m.8-2.6.7 1.8M10 7.9l.6-1.8M11 9l1.6-1.1",
    strokeWidth: 0.5
  }), /* @__PURE__ */ a.createElement("path", {
    d: "m8.7 3.9 1.1 1.6m4-1.7-1 1.6m-1.5 4.7V12M8.9 8.3 7.1 9m8.4 0-1.8-.7",
    strokeWidth: 0.25
  })));
}, ri = function(i) {
  var u = i.title, f = d(i, Rm);
  return /* @__PURE__ */ a.createElement("svg", s({
    xmlns: "http://www.w3.org/2000/svg",
    viewBox: "0 0 810 540"
  }, f), u && /* @__PURE__ */ a.createElement("title", null, u), /* @__PURE__ */ a.createElement("path", {
    fill: "#039",
    d: "M0 0h810v540H0z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#FC0",
    d: "m404.951 59.865 6.735 20.718h21.894l-17.633 12.812 6.638 20.74-17.634-12.812-17.634 12.812 6.736-20.729-17.633-12.812h21.796zm-90 24 6.735 20.718h21.894l-17.633 12.812 6.638 20.74-17.634-12.812-17.634 12.812 6.736-20.729-17.633-12.812h21.796zm-66 66 6.735 20.718h21.894l-17.633 12.812 6.638 20.74-17.634-12.812-17.634 12.812 6.736-20.729-17.633-12.812h21.796zm-24 89.999 6.735 20.719h21.894l-17.633 12.813 6.638 20.74-17.634-12.813-17.634 12.813 6.736-20.731-17.633-12.811h21.796zm24 90.001 6.735 20.719h21.894l-17.633 12.811 6.638 20.74-17.634-12.811-17.634 12.811 6.736-20.729-17.633-12.812h21.796zm66 66 6.735 20.719h21.894l-17.633 12.811 6.638 20.74-17.634-12.811-17.634 12.811 6.736-20.729-17.633-12.812h21.796zm180-312 6.735 20.718h21.894l-17.633 12.812 6.637 20.74-17.633-12.812-17.635 12.812 6.737-20.729-17.633-12.812h21.795zm66 66 6.735 20.718h21.894l-17.633 12.812 6.637 20.74-17.633-12.812-17.635 12.812 6.737-20.729-17.633-12.812h21.795zm24 90 6.735 20.718h21.894l-17.633 12.813 6.638 20.74-17.634-12.813-17.634 12.813 6.736-20.731-17.633-12.811h21.796zm-24 90 6.735 20.719h21.894l-17.633 12.811 6.638 20.74-17.634-12.811-17.634 12.811 6.736-20.729-17.633-12.812h21.796zm-156 90 6.735 20.719h21.894l-17.633 12.811 6.638 20.74-17.634-12.811-17.634 12.811 6.736-20.729-17.633-12.812h21.796zm90-24.001 6.735 20.719h21.894l-17.633 12.812 6.639 20.74-17.635-12.812-17.633 12.812 6.735-20.73-17.633-12.811h21.797z"
  }));
}, fi = function(i) {
  var u = i.title, f = d(i, jm);
  return /* @__PURE__ */ a.createElement("svg", s({
    xmlns: "http://www.w3.org/2000/svg",
    viewBox: "0 0 513 342"
  }, f), u && /* @__PURE__ */ a.createElement("title", null, u), /* @__PURE__ */ a.createElement("path", {
    fill: "#FFF",
    d: "M0 0h513v342H0z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#2E52B2",
    d: "M513 129.3V212H203.7v130H121V212H0v-82.7h121V0h82.7v129.3z"
  }));
}, hi = function(i) {
  var u = i.title, f = d(i, Gm);
  return /* @__PURE__ */ a.createElement("svg", s({
    xmlns: "http://www.w3.org/2000/svg",
    viewBox: "0 0 513 342"
  }, f), u && /* @__PURE__ */ a.createElement("title", null, u), /* @__PURE__ */ a.createElement("path", {
    fill: "#62B5E5",
    d: "M0 0h513v342H0z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#F3F3F3",
    d: "M307.1 127.1v92c0 61.6 80.5 80.5 80.5 80.5s80.4-19 80.4-80.6v-92l-80.5-23-80.4 23.1z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#D80027",
    d: "M468 132.8V98.3H307.1v34.5h69v69h-69v23h69V296c6.9 2.5 11.5 3.5 11.5 3.5s4.6-1.1 11.5-3.5v-71.2h69v-23h-69v-69H468z"
  }), /* @__PURE__ */ a.createElement("g", {
    fill: "#FFF"
  }, /* @__PURE__ */ a.createElement("path", {
    d: "M0 0h256.5v170.6H0V0z"
  }), /* @__PURE__ */ a.createElement("path", {
    d: "m155.9 115 100.6 55.6v-15.8L184.6 115h-28.7zm-55.7-59.3L0 0v15.8l71.8 39.9h28.4z"
  })), /* @__PURE__ */ a.createElement("path", {
    d: "m196.9 115 59.6 32.9V115h-59.6zm-41 0 100.6 55.6v-15.8L184.6 115h-28.7zm73.4 55.6-73.5-40.9v40.9h73.5zM45.2 115 0 140.1V115h45.2zm55 7.1v48.6H12.8l87.4-48.6zM59.1 55.7 0 22.8v32.8s59.1 0 59.1.1zm41.1 0L0 0v15.8l71.8 39.9h28.4zM26.7 0l73.5 40.8V0H26.7zm184.2 55.7 45.6-25.1v25.1h-45.6zm-55-7.2V0h87.4l-87.4 48.5z",
    fill: "#0052B4"
  }), /* @__PURE__ */ a.createElement("g", {
    fill: "#D80027"
  }, /* @__PURE__ */ a.createElement("path", {
    d: "M144.3-.2h-32.1v69.5H0v32.1h112.2v69.5h32.1v-69.5h112.2V69.3H144.3V-.2z"
  }), /* @__PURE__ */ a.createElement("path", {
    d: "m155.9 115 100.6 55.6v-15.8L184.6 115h-28.7zm-84.1 0L0 154.9v15.8L100.2 115H71.8zm28.4-59.3L0 0v15.8l71.8 39.9h28.4zm83.9 0 72.4-39.9V0L155.9 55.7h28.2z"
  })));
}, mi = function(i) {
  var u = i.title, f = d(i, Ym);
  return /* @__PURE__ */ a.createElement("svg", s({
    xmlns: "http://www.w3.org/2000/svg",
    viewBox: "0 0 513 342"
  }, f), u && /* @__PURE__ */ a.createElement("title", null, u), /* @__PURE__ */ a.createElement("path", {
    fill: "#0052B4",
    d: "M0 0h513v342H0z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#FFF",
    d: "M256 0v30.6l-45.2 25.1H256V115h-59.1l59.1 32.8v22.9h-26.7l-73.5-40.9v40.9h-55.6v-48.6l-87.5 48.6H0v-30.6L45.2 115H0V55.7h59.1L0 22.8V0h26.7l73.5 40.8V0h55.6v48.6L243.3 0z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#D80027",
    d: "M144 0h-32v69.3H0v32h112v69.4h32v-69.4h112v-32H144z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#0052B4",
    d: "M155.8 115 256 170.7v-15.8L184.2 115z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#FFF",
    d: "M155.8 115 256 170.7v-15.8L184.2 115z"
  }), /* @__PURE__ */ a.createElement("path", {
    d: "M155.8 115 256 170.7v-15.8L184.2 115zm-84 0L0 154.9v15.8L100.2 115z",
    fill: "#D80027"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#0052B4",
    d: "M100.2 55.6 0 0v15.7l71.8 39.9z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#FFF",
    d: "M100.2 55.6 0 0v15.7l71.8 39.9z"
  }), /* @__PURE__ */ a.createElement("path", {
    d: "M100.2 55.6 0 0v15.7l71.8 39.9zm84 0L256 15.7V0L155.8 55.6z",
    fill: "#D80027"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#1F8BDE",
    d: "M312.2 194.2v-90c0-8.5 6.5-15.3 15-15.3h120.5c8.3 0 15 6.8 15 15.3v90c0 75.2-75.2 105.3-75.2 105.3s-75.3-30.1-75.3-105.3z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#FFF",
    d: "M312.2 194.2v-90c0-8.5 6.5-15.3 15-15.3h120.5c8.3 0 15 6.8 15 15.3v90c0 75.2-75.2 105.3-75.2 105.3s-75.3-30.1-75.3-105.3zm79.6 86.3c7.9-4.5 15.9-10 23.2-16.7 20.5-18.5 32.6-41.4 32.6-69.7v-90c0-.3-120.4-.3-120.4-.3v90.3c0 28.3 12.1 51.2 32.6 69.7 8.3 7.5 17.6 13.9 27.6 19 1.4-.6 2.8-1.4 4.4-2.3z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#187536",
    d: "M347.1 193.4c-2.6-7.9 1.7-14.3 10.1-14.3h60.5c8.2 0 12.8 6.3 10.1 14.3l-5.5 16.5c-2.6 7.9-8.9 9.2-15.8 4 0 0 2.6-4.8-19.1-4.8-21.7 0-19.1 4.8-19.1 4.8-6.1 5.7-13.1 4-15.8-4 .1.1-5.4-16.5-5.4-16.5z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#FFF",
    d: "M357.4 179.1c8.1 1.6 17.8-15 30.1-15 13.1 0 21.8 17.1 30.1 15 7.4-1.8 15-22.2 15-30.1 0-16.6-20.2-30.1-45.1-30.1-24.9 0-45.1 13.5-45.1 30.1-.1 8.5 6.6 28.5 15 30.1zm-1.4 91.6c6.2.4 12.5-1.8 17.2-6.5l9-9c2.9-2.9 7.5-2.9 10.4 0l9 9c4.7 4.7 11 6.9 17.1 6.5 5.3-.3 32.5-33.5 25.7-32.6-4.6.6-9.1 2.8-12.7 6.3l-.1.1-9 9c-2.9 2.9-7.5 2.9-10.4 0l-9-9c-8.7-8.8-22.9-8.8-31.7-.1l-.1.1-9 9c-2.9 2.9-7.5 2.9-10.4 0l-9-9c-3.5-3.5-7.8-5.6-12.3-6.3-6.7-1.1 20.1 32.2 25.3 32.5zM322.1 210c2.9-2.9 7.5-2.9 10.4 0l9 9c8.8 8.7 22.9 8.7 31.7 0l9-9c2.9-2.9 7.5-2.9 10.4 0l9 9c8.8 8.7 22.9 8.7 31.7 0l9-9c2.9-2.9 7.5-2.9 10.4 0 0 0 5.5-14.8 2.5-15.9-7.9-2.9-17.2-1.2-23.5 5.2l-.1.1-9 9c-2.9 2.9-7.5 2.9-10.4 0l-9-9c-8.7-8.8-22.9-8.8-31.7-.1l-.1.1-9 9c-2.9 2.9-7.5 2.9-10.4 0l-9-9c-6.2-6.2-15.1-8-22.8-5.5-3.1 1.1 1.9 16.1 1.9 16.1z"
  }));
}, vi = function(i) {
  var u = i.title, f = d(i, qm);
  return /* @__PURE__ */ a.createElement("svg", s({
    xmlns: "http://www.w3.org/2000/svg",
    viewBox: "0 0 513 342"
  }, f), u && /* @__PURE__ */ a.createElement("title", null, u), /* @__PURE__ */ a.createElement("path", {
    fill: "#338AF3",
    d: "M0 0h513v342H0z"
  }), /* @__PURE__ */ a.createElement("path", {
    d: "m256 37.9 10.1 31.2H299l-26.6 19.3 10.2 31.2-26.6-19.3-26.6 19.3 10.2-31.2L213 69.1h32.9zM123.2 170.7l31.2-10.2v-32.8l19.3 26.6 31.2-10.2-19.3 26.6 19.3 26.5-31.2-10.1-19.3 26.5v-32.8zM256 303.5l-10.1-31.2H213l26.6-19.3-10.2-31.3L256 241l26.6-19.3-10.2 31.3 26.6 19.3h-32.9zm132.8-132.8-31.2 10.1v32.8l-19.3-26.5-31.2 10.1 19.3-26.5-19.3-26.6 31.2 10.2 19.3-26.6v32.8z",
    fill: "#FFF"
  }));
}, si = function(i) {
  var u = i.title, f = d(i, Qm);
  return /* @__PURE__ */ a.createElement("svg", s({
    xmlns: "http://www.w3.org/2000/svg",
    viewBox: "0 0 513 342"
  }, f), u && /* @__PURE__ */ a.createElement("title", null, u), /* @__PURE__ */ a.createElement("path", {
    fill: "#FFF",
    d: "M0 0h513v342H0z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#0F61A5",
    d: "M513 214.5H206.2V342h-87.8V214.5H0v-87.7h118.4V0h87.8v126.8H513V192z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#E50E3D",
    d: "M513 149.3V192H183.7v150H141V192H0v-42.7h141V0h42.7v149.3z"
  }));
}, di = function(i) {
  var u = i.title, f = d(i, Xm);
  return /* @__PURE__ */ a.createElement("svg", s({
    xmlns: "http://www.w3.org/2000/svg",
    viewBox: "0 0 513 342"
  }, f), u && /* @__PURE__ */ a.createElement("title", null, u), /* @__PURE__ */ a.createElement("path", {
    fill: "#FFF",
    d: "M0 0h513v342H0z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#00318A",
    d: "M0 0h171v342H0z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#D80027",
    d: "M342 0h171v342H342z"
  }));
}, oi = function(i) {
  var u = i.title, f = d(i, Zm);
  return /* @__PURE__ */ a.createElement("svg", s({
    xmlns: "http://www.w3.org/2000/svg",
    viewBox: "0 0 513 342"
  }, f), u && /* @__PURE__ */ a.createElement("title", null, u), /* @__PURE__ */ a.createElement("path", {
    fill: "#FFDA44",
    d: "M0 0h513v342H0z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#6DA544",
    d: "M0 0h513v114H0z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#0052B4",
    d: "M0 228h513v114H0z"
  }));
}, Ei = function(i) {
  var u = i.title, f = d(i, Km);
  return /* @__PURE__ */ a.createElement("svg", s({
    xmlns: "http://www.w3.org/2000/svg",
    viewBox: "0 0 513 342"
  }, f), u && /* @__PURE__ */ a.createElement("title", null, u), /* @__PURE__ */ a.createElement("path", {
    fill: "#FFF",
    d: "M0 0h513v342H0z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#D80027",
    d: "M0 136h513v70H0z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#D80027",
    d: "M221.5 0h70v342h-70z"
  }));
}, zi = function(i) {
  var u = i.title, f = d(i, Jm);
  return /* @__PURE__ */ a.createElement("svg", s({
    xmlns: "http://www.w3.org/2000/svg",
    viewBox: "0 0 513 342"
  }, f), u && /* @__PURE__ */ a.createElement("title", null, u), /* @__PURE__ */ a.createElement("path", {
    fill: "#FFF",
    d: "M0 0h513v342H0z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#D80027",
    d: "M0 136h513v70H0z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#D80027",
    d: "M221.5 0h70v342h-70z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#FFF",
    stroke: "#231F20",
    strokeMiterlimit: 10,
    d: "m256.5 87.8-24 41.4h-48.8l24.7 42.1-24.7 42.1h48.8l24 42.1 24-42.1h48.8l-24.7-42.1 24.7-42.1h-48.8l-24-41.4z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#D80027",
    d: "M258.6 130.6c-3.3 0-3.3 3.3-3.3 3.3V160h-2v-22s0-4-3.3-4c-4 0-4 4-4 4v23.4l-2 .7v-16.7s0-4-3.3-4c-4 0-4 4-4 4V174c0 6.7 3.3 13.4 8 17.4v14h24V194c6-3.3 10.7-9.4 12-16.7 0-4 1.3-7.3 2.7-10.7l2.7-9.4s.7-3.3-2.7-4.7-4.7 2-4.7 2l-5.3 12.7h-2v-31.4s0-3.3-3.3-3.3c-4 0-4 3.3-4 3.3v24h-1.3v-26.1c-.2.2-.2-3.1-4.2-3.1z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#FFDA44",
    stroke: "#231F20",
    strokeMiterlimit: 10,
    d: "M249.2 25.7V33h-7.3v14.7h7.3v14c-4-4-9.4-6-14.7-6-12.2-.1-22.1 9.6-22.3 21.8-.1 6.1 2.4 12 6.9 16.3v13.4h74.8V93.8c4-4.7 6.7-10 6.7-16-.2-12.2-10.3-21.9-22.4-21.6-5.3.1-10.4 2.1-14.3 5.6v-14h7.3V33.1h-7.3v-7.3l-14.7-.1z"
  }));
}, pi = function(i) {
  var u = i.title, f = d(i, km);
  return /* @__PURE__ */ a.createElement("svg", s({
    xmlns: "http://www.w3.org/2000/svg",
    viewBox: "0 0 513 342"
  }, f), u && /* @__PURE__ */ a.createElement("title", null, u), /* @__PURE__ */ a.createElement("path", {
    fill: "#005EB8",
    d: "M0 0h513v342H0z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#FFF",
    d: "M0 302.1V342h59.9l196.6-131.1L453.1 342H513v-39.9L316.4 171 513 39.9V0h-59.9L256.5 131.1 59.9 0H0v39.9L196.7 171z"
  }));
}, gi = function(i) {
  var u = i.title, f = d(i, Wm);
  return /* @__PURE__ */ a.createElement("svg", s({
    xmlns: "http://www.w3.org/2000/svg",
    viewBox: "0 0 513 342"
  }, f), u && /* @__PURE__ */ a.createElement("title", null, u), /* @__PURE__ */ a.createElement("path", {
    fill: "#FFF",
    d: "M0 0h513v171H0z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#529E3C",
    d: "M0 171h513v171H0z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#D11C1C",
    d: "m201 259.8 28.2-4.8-21.8-10.3 14.9-8.4s25.2 21.2 25.2 14.4c0-7.3 23.7-4.1 22.7-14.4-1.3-14.1-26.2-1-30.6-18.7-2.5-9.9-10.3-8.6-10.3-8.6l-25.1 8.6-12.5 18.7-6.2-18.7s-14.6 11.9-19.5 18.7c-5.2 7.3-10.7 23.5-10.7 23.5l25.6 10.7-37.3-6.6-27.2 6.6-16.7 4.6 7.3-7.7-15-7.6 15-9-7.3-6.1 32.3 6.1s11.8-1.2 16.3-6.1c5.6-6.2 10.1-27.1 10.1-27.1l-14.8-8.6-11.6 21s-8-19.9-15.6-31c-5.7-8.3-24.3-27.3-24.3-27.3l-24 12.6 13.4-26.7s10.6-9.3 3.9-18.8c-6.8-9.5-12.4-30.9-12.4-30.9s14.1 24.4 19.2 22.5c7.2-2.7-9-25 0-28.9 6.5-2.9 7.6 25.5 7.6 25.5l7.3-13.9v17.3s-4.3 20.7 3 33c7.2 12.3 28.7 20.9 28.7 20.9s-5.6-12.3 0-36c3.8-16 17.2-43.4 23.6-52.1 3.3-4.6-26.7 17-26.7 17v-17l-28.6-2.9-7.3 8.3-18.3-30L104 83.1h34.6l-6.7-8.3H104s5.9-12.1 34.6-12.1l13.6-9.2s18.6.5 29 .9c9.3.4 26.1-11.5 26.1-11.5l4.7 11.5-11 17.3 11.1 11.4-4.7 7 8.1 11.5H201l11.1 17.9-11.1-6.3 6.4 17.3-6.4 17.8 28.2-9.5s0-25.6 10.3-37.2C271.1 69.2 322.6 43 322.6 43s-2.7 23.5 4.9 25.4c11.1 2.7 59.4-19.4 59.4-19.4s-29 31.3-23.1 34.1c3.2 1.5 8.5 7 8.5 7s-25.1 20.5-29.3 29.3c-4.2 8.8 6.1 19.4 6.1 19.4s-21.7 0-32.5 9.5c32.5 0 59.1 15.4 74.8 4 10.5-7.6-37.7-2.9-31.4-21.9 2.4-7.1 8.5-15.2 22.6-17.3s19.1 6.3 19.1 6.3l7.6-11.5h-22.4l40.6-39.6 5.3 51.1-13.7-11.4-6.2 19.2c14.6 44.6-52.8 54.1-52.8 54.1l41.6 27.8-14.8 4.2-4.2 41.7 19.1 15.5-25-6.6-49.2 11.2 9.8-15.3-20.6 4.1 13.7-13.1-13.7-6.1 17.6-4.9 22.1 15.2s11-14.2 12.2-21.7c1.3-7.8-4.8-24.2-4.8-24.2s-32.6-.7-44.1-3.5-18.2-11.9-18.2-11.9l-13.1 15.4s45.5 17.1 34.1 24.2c-2.6 1.7-15.7-3.2-15.7-3.2s-22.4 26.2-36.8 29.7c-6.5 1.6 18.3 10.7 18.3 10.7s-21.2-3.4-32-6.6c-11.3-3.4-44.4 6.6-44.4 6.6l-11-10.7zM383.9 138c3.1 0 5.7-2.6 5.7-5.7s-2.6-5.7-5.7-5.7-5.7 2.6-5.7 5.7 2.5 5.7 5.7 5.7z"
  }));
}, Mi = function(i) {
  var u = i.title, f = d(i, $m);
  return /* @__PURE__ */ a.createElement("svg", s({
    xmlns: "http://www.w3.org/2000/svg",
    viewBox: "0 0 513 342"
  }, f), u && /* @__PURE__ */ a.createElement("title", null, u), /* @__PURE__ */ a.createElement("g", {
    fill: "#FFF"
  }, /* @__PURE__ */ a.createElement("path", {
    d: "M0 0h513v341.3H0V0z"
  }), /* @__PURE__ */ a.createElement("path", {
    d: "M311.7 230 513 341.3v-31.5L369.3 230h-57.6zM200.3 111.3 0 0v31.5l143.7 79.8h56.6z"
  })), /* @__PURE__ */ a.createElement("path", {
    d: "M393.8 230 513 295.7V230H393.8zm-82.1 0L513 341.3v-31.5L369.3 230h-57.6zm146.9 111.3-147-81.7v81.7h147zM90.3 230 0 280.2V230h90.3zm110 14.2v97.2H25.5l174.8-97.2zm-82.1-132.9L0 45.6v65.7h118.2zm82.1 0L0 0v31.5l143.7 79.8h56.6zM53.4 0l147 81.7V0h-147zm368.3 111.3L513 61.1v50.2h-91.3zm-110-14.2V0h174.9L311.7 97.1z",
    fill: "#0052B4"
  }), /* @__PURE__ */ a.createElement("g", {
    fill: "#D80027"
  }, /* @__PURE__ */ a.createElement("path", {
    d: "M288 0h-64v138.7H0v64h224v138.7h64V202.7h224v-64H288V0z"
  }), /* @__PURE__ */ a.createElement("path", {
    d: "M311.7 230 513 341.3v-31.5L369.3 230h-57.6zm-168 0L0 309.9v31.5L200.3 230h-56.6zm56.6-118.7L0 0v31.5l143.7 79.8h56.6zm168 0L513 31.5V0L311.7 111.3h56.6z"
  })));
}, wi = function(i) {
  var u = i.title, f = d(i, Im);
  return /* @__PURE__ */ a.createElement("svg", s({
    xmlns: "http://www.w3.org/2000/svg",
    viewBox: "0 0 513 342"
  }, f), u && /* @__PURE__ */ a.createElement("title", null, u), /* @__PURE__ */ a.createElement("path", {
    fill: "#c60a0a",
    d: "M0 0h513v342H0z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#3E8446",
    d: "m256.5 170.7-212 126.1V44.5z"
  }), /* @__PURE__ */ a.createElement("path", {
    d: "m256.5 170.7 211 126.1h-423zm211-126.2-211 126.2-212-126.2z",
    fill: "#FFDA44"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#3E8446",
    d: "M467.5 44.5v252.3l-211-126.1z"
  }), /* @__PURE__ */ a.createElement("path", {
    d: "m256.5 10.4 2.8 10.2H270l-8.6 6.3 3.3 10.2-8.2-6.3-9.2 6.3 3.3-10.2-8.6-6.3h10.7zm-86.4 0 3.3 10.2h10.7l-8.6 6.3 3.3 10.2-8.7-6.3-8.7 6.3 3.3-10.2-8.6-6.3h10.7zm171.8 0 3.3 10.2H356l-8.7 6.3 3.3 10.2-8.7-6.3-8.7 6.3 3.3-10.2-8.6-6.3h10.7zm-85.4 293.8 2.8 10.2H270l-8.6 6.3 3.3 10.2-8.2-6.3-9.2 6.3 3.3-10.2-8.6-6.3h10.7zm-86.4 0 3.3 10.2h10.7l-8.6 6.3 3.3 10.2-8.7-6.3-8.7 6.3 3.3-10.2-8.6-6.3h10.7zm171.8 0 3.3 10.2H356l-8.7 6.3 3.3 10.2-8.7-6.3-8.7 6.3 3.3-10.2-8.6-6.3h10.7z",
    fill: "#FFDA44"
  }), /* @__PURE__ */ a.createElement("circle", {
    fill: "#c60a0a",
    cx: 244.5,
    cy: 170.7,
    r: 76.2
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#FFDA44",
    d: "m244.5 110.1 13.6 41.8h44l-35.6 25.8 13.6 41.8-35.6-25.8-35.5 25.8 13.6-41.8-35.6-25.8h44zm-136.8 57.7c4.4 6.9 2.3 16.1-4.6 20.5s-16.1 2.3-20.5-4.6c-7.9-12.5-3.3-33-3.3-33s20.4 4.6 28.4 17.1z"
  }), /* @__PURE__ */ a.createElement("circle", {
    fill: "#A2001D",
    cx: 99.1,
    cy: 182.1,
    r: 7.4
  }));
}, xi = function(i) {
  var u = i.title, f = d(i, Pm);
  return /* @__PURE__ */ a.createElement("svg", s({
    xmlns: "http://www.w3.org/2000/svg",
    viewBox: "0 0 22.5 15"
  }, f), u && /* @__PURE__ */ a.createElement("title", null, u), /* @__PURE__ */ a.createElement("path", {
    fill: "#fff",
    d: "M0 0h22.5v15H0z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#eb000e",
    d: "M9.8 6H0v3h9.8v6h3V9h9.8V6h-9.8V0h-3v6z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#eb000e",
    d: "m17.3 2.7-.2-1.2h1l-.2 1.2 1.2-.2v1l-1.2-.2.2 1.2h-1l.2-1.2-1.2.2v-1l1.2.2zm-12.7 0-.2-1.2h1l-.2 1.2 1.2-.2v1l-1.2-.2.2 1.2h-1l.2-1.2-1.2.2v-1l1.2.2zm0 9-.2-1.2h1l-.2 1.2 1.2-.2v1l-1.2-.2.2 1.2h-1l.2-1.2-1.2.2v-1l1.2.2zm12.7 0-.2-1.2h1l-.2 1.2 1.2-.2v1l-1.2-.2.2 1.2h-1l.2-1.2-1.2.2v-1l1.2.2z"
  }));
}, Hi = function(i) {
  var u = i.title, f = d(i, ev);
  return /* @__PURE__ */ a.createElement("svg", s({
    xmlns: "http://www.w3.org/2000/svg",
    viewBox: "0 0 90 60"
  }, f), u && /* @__PURE__ */ a.createElement("title", null, u), /* @__PURE__ */ a.createElement("path", {
    fill: "#FDEB01",
    d: "m0 0 90 60H0z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#58A846",
    d: "M90 0v60L0 0z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#ED3D24",
    d: "m50.7 33.3 9.3-6.7H48.5L45 15.7l-3.5 10.9H30l9.3 6.7-3.6 11 9.3-6.7 9.3 6.7z"
  }));
}, yi = function(i) {
  var u = i.title, f = d(i, tv);
  return /* @__PURE__ */ a.createElement("svg", s({
    xmlns: "http://www.w3.org/2000/svg",
    viewBox: "0 0 513 342"
  }, f), u && /* @__PURE__ */ a.createElement("title", null, u), /* @__PURE__ */ a.createElement("path", {
    fill: "#FFF",
    d: "M0 0h513v342H0z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#D80027",
    d: "M308 0H204v118.7H0v104h204V342h104V222.7h205v-104H308z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#FFDA44",
    d: "m368.6 188 26 17.3V136l-26 17.3h-95.3V58.1l17.3-26h-69.2l17.3 26v95.2h-95.3l-26-17.3v69.3l26-17.3h95.3v95.2l-17.3 26h69.2l-17.3-26V188z"
  }));
}, Fi = function(i) {
  var u = i.title, f = d(i, lv);
  return /* @__PURE__ */ a.createElement("svg", s({
    xmlns: "http://www.w3.org/2000/svg",
    viewBox: "0 0 513 342"
  }, f), u && /* @__PURE__ */ a.createElement("title", null, u), /* @__PURE__ */ a.createElement("path", {
    fill: "#FFDA44",
    d: "M0 0h513v342H0z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#D80027",
    d: "M0 0h513v114H0z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#496E2D",
    d: "M0 228h513v114H0z"
  }), /* @__PURE__ */ a.createElement("path", {
    d: "m255.9 113.8 14.1 43.4 40.4 3.2-37 26.9 19.5 40.3-37-26.9-37 26.9 14.1-43.5-36.9-26.9h45.7z"
  }));
}, Di = function(i) {
  var u = i.title, f = d(i, av);
  return /* @__PURE__ */ a.createElement("svg", s({
    xmlns: "http://www.w3.org/2000/svg",
    viewBox: "0 0 513 342"
  }, f), u && /* @__PURE__ */ a.createElement("title", null, u), /* @__PURE__ */ a.createElement("path", {
    fill: "#FFF",
    d: "M0 0h513v342H0z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#D80027",
    d: "M0 230h513v112H0z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#D80027",
    stroke: "#000",
    strokeMiterlimit: 10,
    d: "M363.1 131.8V99.1H374V77.3h-21.9v10.9h-21.8V77.3h-21.8v21.8h10.9v32.7h-21.8V55.5h10.9V33.7h-21.8v10.9h-10.9V33.7H254v10.9h-10.9V33.7h-21.8v21.8h10.9v76.3h-21.8V99.1h10.9V77.3h-21.8v10.9h-21.8V77.3h-21.8v21.8h10.9v32.7H145V219h239.9v-87.2z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#FFDA44",
    stroke: "#000",
    strokeMiterlimit: 10,
    d: "m264.9 235.5-24.2 18.2 19.1 14.3v31.3h-23.2v9h23v7.9h-23v9.3H270V268l19.1-14.3-24.2-18.2zm0 25.3-10.1-7.1 10.1-7.1 10.1 7.1-10.1 7.1z"
  }), /* @__PURE__ */ a.createElement("path", {
    d: "M239.6 209.7v-27.9s.1-22.3 25-22.3c24.8 0 25.7 22 25.7 22v28.2h-50.7zm-69.1 0v-24.3s.1-18.7 19.6-18.7 20.2 18.4 20.2 18.4v24.5h-39.8z"
  }), /* @__PURE__ */ a.createElement("path", {
    d: "M169.1 209.7v-24.3s.1-18.7 19.6-18.7 20.2 18.4 20.2 18.4v24.5h-39.8zm151.8 0v-24.3s.1-18.7 19.6-18.7 20.2 18.4 20.2 18.4v24.5h-39.8zm8.4-77.7v-15.7s.1-12.1 11.3-12.1c11.3 0 11.7 11.9 11.7 11.9V132h-23zm-78.8 0v-23.7s.1-18.3 14.3-18.3c14.2 0 14.7 18 14.7 18v24h-29zm-73 0v-15.7s.1-12.1 11.3-12.1 11.7 11.9 11.7 11.9V132h-23z"
  }));
}, Si = function(i) {
  var u = i.title, f = d(i, nv);
  return /* @__PURE__ */ a.createElement("svg", s({
    xmlns: "http://www.w3.org/2000/svg",
    viewBox: "0 0 513 342"
  }, f), u && /* @__PURE__ */ a.createElement("title", null, u), /* @__PURE__ */ a.createElement("path", {
    fill: "#D80027",
    d: "M0 0h513v342H0z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#FFF",
    d: "M513 0v171H0V0z"
  }), /* @__PURE__ */ a.createElement("circle", {
    fill: "#FFF",
    cx: 185.8,
    cy: 171.2,
    r: 117.8
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#D80027",
    d: "M68 171c0-65.1 52.8-117.8 117.8-117.8 65.1 0 117.8 52.8 117.8 117.8"
  }));
}, Ai = function(i) {
  var u = i.title, f = d(i, cv);
  return /* @__PURE__ */ a.createElement("svg", s({
    xmlns: "http://www.w3.org/2000/svg",
    viewBox: "0 0 513 342"
  }, f), u && /* @__PURE__ */ a.createElement("title", null, u), /* @__PURE__ */ a.createElement("path", {
    fill: "#FFF",
    d: "M0 0h513v342H0z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#cf0d19",
    d: "M0 0h513v100H0z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#0052B4",
    d: "M0 121h513v100H0z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#1a7e25",
    d: "M0 242h513v100H0z"
  }));
}, Bi = function(i) {
  var u = i.title, f = d(i, iv);
  return /* @__PURE__ */ a.createElement("svg", s({
    xmlns: "http://www.w3.org/2000/svg",
    viewBox: "0 0 513 342"
  }, f), u && /* @__PURE__ */ a.createElement("title", null, u), /* @__PURE__ */ a.createElement("path", {
    fill: "#FFDA44",
    d: "M0 0h512v342H0z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#6DA544",
    d: "M342 0h171v342H342z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#D80027",
    d: "M0 0h171v342H0z"
  }));
}, bi = function(i) {
  var u = i.title, f = d(i, uv);
  return /* @__PURE__ */ a.createElement("svg", s({
    xmlns: "http://www.w3.org/2000/svg",
    viewBox: "0 0 513 342"
  }, f), u && /* @__PURE__ */ a.createElement("title", null, u), /* @__PURE__ */ a.createElement("path", {
    fill: "#FFF",
    d: "M0 0h513v342H0z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#00318A",
    d: "M0 0h171v342H0z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#D80027",
    d: "M342 0h171v342H342z"
  }));
}, Ti = function(i) {
  var u = i.title, f = d(i, rv);
  return /* @__PURE__ */ a.createElement("svg", s({
    xmlns: "http://www.w3.org/2000/svg",
    viewBox: "0 0 513 342"
  }, f), u && /* @__PURE__ */ a.createElement("title", null, u), /* @__PURE__ */ a.createElement("path", {
    fill: "#FFF",
    d: "M0 0h513v342H0z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#6DA544",
    d: "M0 0h513v113.8H0z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#D80027",
    d: "M0 227.6h513V342H0z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#0070C8",
    d: "M126 171 0 342V0z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "none",
    stroke: "#000",
    strokeMiterlimit: 10,
    d: "M233.8 139.4v40.4c0 35.6 35.6 35.6 35.6 35.6s35.6 0 35.6-35.6v-40.4h-71.2z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#786145",
    d: "M264.5 179.8h9.8l4 25.8h-17.8z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#6DA544",
    d: "M287.2 162c0-9.8-8-14.8-17.8-14.8s-17.8 5-17.8 14.8c-4.9 0-8.9 4-8.9 8.9s4 8.9 8.9 8.9h35.6c4.9 0 8.9-4 8.9-8.9s-4-8.9-8.9-8.9z"
  }), /* @__PURE__ */ a.createElement("path", {
    d: "m230.7 120 1.9 3.3h3.8l-1.9 3.3 1.9 3.2h-3.8l-1.9 3.3-1.9-3.3H225l1.9-3.2-1.9-3.3h3.8zm15.3 0 1.9 3.3h3.7l-1.9 3.3 1.9 3.2h-3.7l-1.9 3.3-1.9-3.3h-3.8l1.9-3.2-1.9-3.3h3.8zm15.3 0 1.9 3.3h3.7l-1.9 3.3 1.9 3.2h-3.7l-1.9 3.3-1.9-3.3h-3.8l1.9-3.2-1.9-3.3h3.8zm15.8 0 1.9 3.3h3.8l-1.9 3.3 1.9 3.2H279l-1.9 3.3-1.9-3.3h-3.7l1.8-3.2-1.8-3.3h3.7zm16 0 1.9 3.3h3.8l-1.9 3.3 1.9 3.2H295l-1.9 3.3-1.9-3.3h-3.7l1.8-3.2-1.8-3.3h3.7zm15 0 1.9 3.3h3.7l-1.9 3.3 1.9 3.2H310l-1.9 3.3-1.9-3.3h-3.8l1.9-3.2-1.9-3.3h3.8z",
    fill: "#FFDA00",
    stroke: "#000",
    strokeMiterlimit: 10
  }));
}, Li = function(i) {
  var u = i.title, f = d(i, fv);
  return /* @__PURE__ */ a.createElement("svg", s({
    xmlns: "http://www.w3.org/2000/svg",
    viewBox: "0 0 513 342"
  }, f), u && /* @__PURE__ */ a.createElement("title", null, u), /* @__PURE__ */ a.createElement("path", {
    fill: "#FFF",
    d: "M0 0h513v342H0z"
  }), /* @__PURE__ */ a.createElement("g", {
    fill: "#0d5eaf"
  }, /* @__PURE__ */ a.createElement("path", {
    d: "M0 0h513v38H0zm0 76h513v38H0zm0 76h513v38H0zm0 76h513v38H0zm0 76h513v38H0z"
  }), /* @__PURE__ */ a.createElement("path", {
    d: "M0 0h190v190H0z"
  })), /* @__PURE__ */ a.createElement("g", {
    fill: "#FFF"
  }, /* @__PURE__ */ a.createElement("path", {
    d: "M0 76h190v38H0z"
  }), /* @__PURE__ */ a.createElement("path", {
    d: "M76 0h38v190H76z"
  })));
}, Vi = function(i) {
  var u = i.title, f = d(i, hv);
  return /* @__PURE__ */ a.createElement("svg", s({
    xmlns: "http://www.w3.org/2000/svg",
    viewBox: "0 0 513 342"
  }, f), u && /* @__PURE__ */ a.createElement("title", null, u), /* @__PURE__ */ a.createElement("path", {
    fill: "#0052B4",
    d: "M0 0h513v342H0z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#FFF",
    d: "M256 0v117.4h-46.1l46.1 30.7v22.6h-22.6L160 121.8v48.9H96v-48.9l-73.4 48.9H0v-22.6l46.1-30.7H0v-64h46.1L0 22.7V0h22.6L96 48.9V0h64v48.9L233.4 0z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#D80027",
    d: "M144 0h-32v69.4H0v32h112v69.3h32v-69.3h112v-32H144z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#2E52B2",
    d: "M256 22.7v30.7h-46.1z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#D80027",
    d: "M0 170.7v-11l62.5-42.3h22.6L7.3 170.7zM7.3.1l77.8 53.2H62.5L0 11.1V.1zM256 .1v11l-62.5 42.3h-22.6L248.7.1zm-7.3 170.6-77.8-53.3h22.6l62.5 42.3v11z"
  }), /* @__PURE__ */ a.createElement("ellipse", {
    fill: "#FFF",
    cx: 443.4,
    cy: 233.6,
    rx: 29,
    ry: 43.5
  }), /* @__PURE__ */ a.createElement("ellipse", {
    fill: "#FFCE00",
    cx: 406.9,
    cy: 317.7,
    rx: 22,
    ry: 9.3
  }), /* @__PURE__ */ a.createElement("ellipse", {
    fill: "#FFCE00",
    cx: 364,
    cy: 317.7,
    rx: 22,
    ry: 9.3
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#39B200",
    d: "m342 190.1 43.4 86.9 43.5-86.9z"
  }), /* @__PURE__ */ a.createElement("ellipse", {
    fill: "#9B9B9B",
    cx: 327.5,
    cy: 233.6,
    rx: 29,
    ry: 43.5
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#757575",
    d: "m371 175.6-14.5 14.5h57.9l-14.5-14.5 14.5-43.5s-13-29-29-29-29 29-29 29l14.6 43.5z"
  }), /* @__PURE__ */ a.createElement("circle", {
    fill: "#C6B56F",
    cx: 385.4,
    cy: 67,
    r: 29
  }), /* @__PURE__ */ a.createElement("circle", {
    fill: "#A54A00",
    cx: 386,
    cy: 88.7,
    r: 29
  }), /* @__PURE__ */ a.createElement("circle", {
    fill: "#FFF",
    cx: 443.4,
    cy: 117.7,
    r: 29
  }), /* @__PURE__ */ a.createElement("circle", {
    fill: "#0049FF",
    cx: 342.2,
    cy: 146.7,
    r: 29
  }), /* @__PURE__ */ a.createElement("circle", {
    fill: "#0041F9",
    cx: 429.8,
    cy: 146.7,
    r: 29
  }), /* @__PURE__ */ a.createElement("circle", {
    fill: "#F7D71E",
    cx: 386,
    cy: 233.6,
    r: 29
  }), /* @__PURE__ */ a.createElement("circle", {
    fill: "#CEC851",
    cx: 457.9,
    cy: 190.1,
    r: 29
  }), /* @__PURE__ */ a.createElement("circle", {
    fill: "#FFF",
    cx: 385.4,
    cy: 289.5,
    r: 29
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#7C0B29",
    d: "m371 135.7 14.5-3.5 14.5 3.5c0 7.5-14.5 11-14.5 11s-14.5-3.5-14.5-11z"
  }), /* @__PURE__ */ a.createElement("circle", {
    fill: "#FFF",
    cx: 327.5,
    cy: 117.4,
    r: 29
  }));
}, Ci = function(i) {
  var u = i.title, f = d(i, mv);
  return /* @__PURE__ */ a.createElement("svg", s({
    xmlns: "http://www.w3.org/2000/svg",
    viewBox: "0 0 513 342"
  }, f), u && /* @__PURE__ */ a.createElement("title", null, u), /* @__PURE__ */ a.createElement("path", {
    fill: "#FFF",
    d: "M0 0h513v342H0z"
  }), /* @__PURE__ */ a.createElement("path", {
    d: "M0 0h171v342H0zm342 0h171v342H342z",
    fill: "#338AF3"
  }), /* @__PURE__ */ a.createElement("circle", {
    fill: "#DCC26D",
    cx: 256,
    cy: 155.8,
    r: 25.5
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#628A40",
    d: "M194.2 155.9c0 22.1 11.8 42.5 30.8 53.5 5.9 3.4 13.5 1.4 16.9-4.5 3.4-5.9 1.4-13.5-4.5-16.9-11.5-6.6-18.5-18.9-18.5-32.1 0-6.8-5.5-12.4-12.4-12.4s-12.3 5.5-12.3 12.4zm95.1 52.1c17.8-11.4 28.6-31 28.5-52.1 0-6.8-5.5-12.4-12.4-12.4-6.8 0-12.4 5.5-12.4 12.4 0 12.7-6.5 24.5-17.1 31.3-5.8 3.6-7.7 11.2-4.1 17 3.6 5.8 11.2 7.7 17 4.1.2-.1.3-.2.5-.3z"
  }));
}, Oi = function(i) {
  var u = i.title, f = d(i, vv);
  return /* @__PURE__ */ a.createElement("svg", s({
    xmlns: "http://www.w3.org/2000/svg",
    viewBox: "0 0 513 342"
  }, f), u && /* @__PURE__ */ a.createElement("title", null, u), /* @__PURE__ */ a.createElement("path", {
    fill: "#D80027",
    d: "M0 0h513v342H0z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#0052B4",
    d: "M11.1 11.1h489.7v319.1H11.1z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#7DBEF1",
    d: "M256 285.3s76.4-51.3 76.4-114.6S256 56 256 56s-76.4 51.3-76.4 114.6S256 285.3 256 285.3z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#1C8AE6",
    d: "M179.8 180.7h152.6l-29.3 64.9h-95.2z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#FFF042",
    d: "M192.3 205.9s40.5 38.2 51 38.2c12.4 0 12.6-18.4 25.5-25.5 20.2-11.1 51-12.7 51-12.7L297 248.7l-41 36.6-46.8-39.7-16.9-39.7z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#259C7B",
    d: "m256 157.5-22.5 15.6 7.9-26.2-21.8-16.5 27.4-.5 9-25.9 9 25.9 27.4.5-21.8 16.6 7.9 26.2-22.5-15.7z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#8E5715",
    d: "M249.7 144.6c-.9 9.2-1.5 18.4-1.7 27.6-.3 11.9.3 20.7 2 26 2.2 6.6 7.2 12.9 14.2 18.9 5.3 4.6 10.6 8.1 14.2 10.1 3.1 1.7 7 .6 8.7-2.4 1.7-3.1.6-7-2.4-8.7-4.3-2.5-8.4-5.4-12.2-8.7-5.4-4.6-9.1-9.2-10.4-13.3-1.1-3.4-1.6-11.3-1.4-21.7.2-8.9.8-17.8 1.7-26.6.4-3.5-2.2-6.6-5.7-7-3.6-.3-6.7 2.3-7 5.8z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#FFF",
    d: "m217.8 170.7 25.5 38.2h-25.5v-38.2z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "none",
    stroke: "#D80027",
    strokeWidth: 12,
    strokeMiterlimit: 10,
    d: "M256 285.3s76.4-51.3 76.4-114.6S256 56 256 56s-76.4 51.3-76.4 114.6S256 285.3 256 285.3z"
  }));
}, Ni = function(i) {
  var u = i.title, f = d(i, sv);
  return /* @__PURE__ */ a.createElement("svg", s({
    xmlns: "http://www.w3.org/2000/svg",
    viewBox: "0 0 513 342"
  }, f), u && /* @__PURE__ */ a.createElement("title", null, u), /* @__PURE__ */ a.createElement("path", {
    fill: "#6DA544",
    d: "M0 0h512v342H0z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#FFDA44",
    d: "M0 0h512v171H0z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#D80027",
    d: "M0 0h182v342H0z"
  }), /* @__PURE__ */ a.createElement("path", {
    d: "m98.3 109.8 15.1 46.5h48.9L122.8 185l15.1 46.5-39.6-28.7-39.5 28.7L73.9 185l-39.6-28.7h48.9z"
  }));
}, _i = function(i) {
  var u = i.title, f = d(i, dv);
  return /* @__PURE__ */ a.createElement("svg", s({
    xmlns: "http://www.w3.org/2000/svg",
    viewBox: "0 0 900 600"
  }, f), u && /* @__PURE__ */ a.createElement("title", null, u), /* @__PURE__ */ a.createElement("path", {
    fill: "#009E49",
    d: "M0 0h900v600H0V0z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#FFF",
    d: "m0 0 947 300L0 600V0z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#FFD00D",
    d: "M0 26.1 870 300 0 573.9V26.1z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#2D2D2D",
    d: "m0 0 450 300L0 600V0z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#D3132F",
    d: "m0 35 397.5 265L0 565V35z"
  }));
}, Ui = function(i) {
  var u = i.title, f = d(i, ov);
  return /* @__PURE__ */ a.createElement("svg", s({
    viewBox: "0 0.5 21 14",
    xmlns: "http://www.w3.org/2000/svg"
  }, f), u && /* @__PURE__ */ a.createElement("title", null, u), /* @__PURE__ */ a.createElement("g", {
    fill: "none",
    fillRule: "evenodd"
  }, /* @__PURE__ */ a.createElement("path", {
    fill: "#FFF",
    d: "M0 0h21v15H0z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#ee1c25",
    d: "M0 0h21v15H0z"
  }), /* @__PURE__ */ a.createElement("path", {
    d: "M12 7.19c-.798-.5-1 .409-1 0 0-.828.895-1.5 2-1.5s2 .672 2 1.5c-.949 0-1.044.5-1.5.5-.56 0-.702 0-1.5-.5zM13.25 7a.25.25 0 1 0 0-.5.25.25 0 0 0 0 .5zm-1.81 1.962c.228-.913-.698-.824-.31-.95.788-.257 1.703.387 2.045 1.438.341 1.05-.021 2.11-.809 2.366-.293-.903-.798-.838-.939-1.272-.173-.533-.217-.668.012-1.582zm.566 1.13a.25.25 0 1 0 .476-.154.25.25 0 0 0-.476.154zM9.58 8.977c.94-.065.57-.919.81-.588.486.67.157 1.74-.737 2.389-.894.65-2.013.632-2.5-.038.768-.558.55-1.018.92-1.286.453-.33.568-.413 1.507-.477zm-.899.888a.25.25 0 1 0 .294.405.25.25 0 0 0-.294-.405zm.312-2.652c.351.874 1.049.258.809.588-.487.67-1.606.687-2.5.038-.894-.65-1.223-1.719-.736-2.39.767.559 1.138.21 1.507.478.453.33.568.413.92 1.286zm-1.124-.58a.25.25 0 1 0-.293.404.25.25 0 0 0 .293-.404zm2.619-.524c-.722.605.08 1.078-.309.951-.788-.256-1.15-1.315-.809-2.365.342-1.05 1.257-1.695 2.045-1.439-.293.903.153 1.147.012 1.581-.173.533-.217.668-.939 1.272zm.205-1.247a.25.25 0 1 0-.475-.155.25.25 0 0 0 .475.155z",
    fill: "#FFF"
  })));
}, Ri = function(i) {
  var u = i.title, f = d(i, Ev);
  return /* @__PURE__ */ a.createElement("svg", s({
    xmlns: "http://www.w3.org/2000/svg",
    viewBox: "0 0 513 342"
  }, f), u && /* @__PURE__ */ a.createElement("title", null, u), /* @__PURE__ */ a.createElement("path", {
    fill: "#0052B4",
    d: "M0 0h513v342H0z"
  }), /* @__PURE__ */ a.createElement("g", {
    fill: "#FFF"
  }, /* @__PURE__ */ a.createElement("path", {
    d: "M223.4 170.7c.3-.3.5-.6.8-.9-.3.3-.5.6-.8.9zm-34.2 41.9 11 22.9 24.7-5.7-11.1 22.8 19.9 15.8L209 274v25.4l-19.8-15.9-19.8 15.9.1-25.4-24.8-5.6 19.9-15.8-11.1-22.8 24.8 5.7zm197.9 28.5 5.2 10.9 11.8-2.7-5.3 10.9 9.5 7.5-11.8 2.6v12.2l-9.4-7.6-9.5 7.6.1-12.2-11.9-2.6 9.5-7.5-5.3-10.9 11.9 2.7zm-48.6-116 5.2 10.9 11.8-2.7-5.3 10.9 9.5 7.5-11.8 2.7v12.1l-9.4-7.6-9.5 7.6v-12.1l-11.8-2.7 9.5-7.5-5.3-10.9 11.8 2.7zm48.6-66.2 5.2 10.9 11.8-2.7-5.3 10.9 9.5 7.5-11.8 2.7v12.1l-9.4-7.6-9.5 7.6.1-12.1-11.9-2.7 9.5-7.5-5.3-10.9 11.9 2.7zm42.4 49.7 5.3 10.9 11.8-2.7-5.3 10.9 9.5 7.5-11.8 2.6V150l-9.5-7.6-9.4 7.6v-12.2l-11.8-2.6 9.5-7.5-5.3-10.9 11.8 2.7zm-30.3 57.9 4.1 12.7h13.3l-10.8 7.8 4.1 12.6-10.7-7.8-10.8 7.8 4.1-12.6-10.7-7.8h13.3z"
  }), /* @__PURE__ */ a.createElement("path", {
    d: "M256 0v30.6l-45.2 25.1H256V115h-59.1l59.1 32.8v22.9h-26.7l-73.5-40.9v40.9h-55.6v-48.6l-87.5 48.6H0v-30.6L45.2 115H0V55.7h59.1L0 22.8V0h26.7l73.5 40.8V0h55.6v48.6L243.3 0z"
  })), /* @__PURE__ */ a.createElement("path", {
    fill: "#D80027",
    d: "M144 0h-32v69.3H0v32h112v69.4h32v-69.4h112v-32H144z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#0052B4",
    d: "M155.8 115 256 170.7v-15.8L184.2 115z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#FFF",
    d: "M155.8 115 256 170.7v-15.8L184.2 115z"
  }), /* @__PURE__ */ a.createElement("path", {
    d: "M155.8 115 256 170.7v-15.8L184.2 115zm-84 0L0 154.9v15.8L100.2 115z",
    fill: "#D80027"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#0052B4",
    d: "M100.2 55.6 0 0v15.7l71.8 39.9z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#FFF",
    d: "M100.2 55.6 0 0v15.7l71.8 39.9z"
  }), /* @__PURE__ */ a.createElement("path", {
    d: "M100.2 55.6 0 0v15.7l71.8 39.9zm84 0L256 15.7V0L155.8 55.6z",
    fill: "#D80027"
  }));
}, ji = function(i) {
  var u = i.title, f = d(i, zv);
  return /* @__PURE__ */ a.createElement("svg", s({
    xmlns: "http://www.w3.org/2000/svg",
    viewBox: "0 0 513 342"
  }, f), u && /* @__PURE__ */ a.createElement("title", null, u), /* @__PURE__ */ a.createElement("path", {
    fill: "#FFF",
    d: "M0 0h513v342H0z"
  }), /* @__PURE__ */ a.createElement("path", {
    d: "M0 0h513v114H0zm0 228h513v114H0zm203.5-110.1 6.7 16.6 17.6 1-14 10.9 4.7 17.7-15-9.9-15 9.9 4.7-17.7-14-10.9 17.6-1zm105 0 6.7 16.6 17.6 1-14 10.9 4.7 17.7-15-9.9-15 9.9 4.7-17.7-14-10.9 17.6-1zM256 147.6l6.7 16.6 17.6 1-14 10.9 4.7 17.7-15-9.9-15 9.9 4.7-17.7-14-10.9 17.6-1zm-52.5 29.7 6.7 16.6 17.6 1-14 10.9 4.7 17.6-15-9.9-15 9.9 4.7-17.6-14-10.9 17.6-1zm105 0 6.7 16.6 17.6 1-14 10.9 4.7 17.6-15-9.9-15 9.9 4.7-17.6-14-10.9 17.6-1z",
    fill: "#338AF3"
  }));
}, Gi = function(i) {
  var u = i.title, f = d(i, pv);
  return /* @__PURE__ */ a.createElement("svg", s({
    xmlns: "http://www.w3.org/2000/svg",
    viewBox: "0 0 513 342"
  }, f), u && /* @__PURE__ */ a.createElement("title", null, u), /* @__PURE__ */ a.createElement("path", {
    fill: "#FFF",
    d: "M0 0h513v342H0z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#D80027",
    d: "M0 0h513v114H0z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#0052B4",
    d: "M0 228h513v114H0z"
  }), /* @__PURE__ */ a.createElement("path", {
    d: "M309.3 113.2v-44l17.5-14.4 17.7 14.4v44zm-105.6 0v-44l-17.3-14.4-17.9 14.4v44zm35.2 0v-44l17.6-14.4 17.6 14.4v44z",
    fill: "#338AF3"
  }), /* @__PURE__ */ a.createElement("path", {
    d: "M309.3 113.2h-35.2v-44l17.5-14.4 17.7 14.4zm-70.4 0h-35.2v-44l17.5-14.4 17.7 14.4z",
    fill: "#0052B4"
  }), /* @__PURE__ */ a.createElement("path", {
    stroke: "#D80027",
    fill: "#FFF",
    d: "M168.5 113.2v101.9c0 24.3 14.4 46.2 35.4 59.4 21.3 13.4 42.1 14.7 52.6 14.7s31.4-1.7 52.6-14.8c21-13 35.4-35.1 35.4-59.3V113.2h-176z"
  }), /* @__PURE__ */ a.createElement("path", {
    d: "M168.5 113.2h35.2v35.2h-35.2zm70.4 0h35.2v35.2h-35.2zm70.4 0h35.2v35.2h-35.2zm-105.6 35.2h35.2v35.2h-35.2zm70.4 0h35.2v35.2h-35.2zm-105.6 35.2h35.2v35.2h-35.2zm35.2 35.2h35.2V254h-35.2zm35.2-35.2h35.2v35.2h-35.2zm70.4 0h35.2v35.2h-35.2zm-35.2 35.2h35.2V254h-35.2zm35.2 55.5c8.6-5.4 16.2-12.3 22-20.3h-22v20.3zM181.7 254c5.8 8 13.3 14.9 22 20.4V254h-22zm57.2 0v33.7c7.2 1.2 13.3 1.5 17.6 1.5 4.3 0 10.4-.3 17.6-1.6V254h-35.2z",
    fill: "#D80027"
  }));
}, Yi = function(i) {
  var u = i.title, f = d(i, gv);
  return /* @__PURE__ */ a.createElement("svg", s({
    xmlns: "http://www.w3.org/2000/svg",
    viewBox: "0 0 513 342"
  }, f), u && /* @__PURE__ */ a.createElement("title", null, u), /* @__PURE__ */ a.createElement("path", {
    fill: "#A2001D",
    d: "M0 0h513v172H0z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#0052B4",
    d: "M0 172h513v172H0z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#FFF",
    d: "m381.4 251.5-110.7-13.8-110.8 13.8V85.4h221.5z"
  }), /* @__PURE__ */ a.createElement("circle", {
    fill: "#0052B4",
    cx: 270.7,
    cy: 182.3,
    r: 55.4
  }), /* @__PURE__ */ a.createElement("circle", {
    fill: "#A2001D",
    cx: 270.7,
    cy: 182.3,
    r: 27.7
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#6DA544",
    d: "M229.1 113.1h83.1l-41.5 41.5z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#FFDA44",
    d: "M256.8 140.8h27.7v83h-27.7z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#6DA544",
    d: "M314.9 215.5h-88.5l-66.5 36h221.5z"
  }));
}, qi = function(i) {
  var u = i.title, f = d(i, Mv);
  return /* @__PURE__ */ a.createElement("svg", s({
    xmlns: "http://www.w3.org/2000/svg",
    viewBox: "0 0 513 342"
  }, f), u && /* @__PURE__ */ a.createElement("title", null, u), /* @__PURE__ */ a.createElement("path", {
    fill: "#FFF",
    d: "M0 0h513v342H0z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#D80027",
    d: "M0 0h513v114H0z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#6DA544",
    d: "M0 228h513v114H0z"
  }));
}, Qi = function(i) {
  var u = i.title, f = d(i, wv);
  return /* @__PURE__ */ a.createElement("svg", s({
    xmlns: "http://www.w3.org/2000/svg",
    viewBox: "0 0 300 200"
  }, f), u && /* @__PURE__ */ a.createElement("title", null, u), /* @__PURE__ */ a.createElement("path", {
    fill: "#fc0",
    d: "M0 0h300v200H0z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#0768a9",
    d: "M0 0h200v200H0z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#fff",
    d: "M0 0h100v200H0z"
  }));
}, Xi = function(i) {
  var u = i.title, f = d(i, xv);
  return /* @__PURE__ */ a.createElement("svg", s({
    xmlns: "http://www.w3.org/2000/svg",
    viewBox: "0 0 513 342"
  }, f), u && /* @__PURE__ */ a.createElement("title", null, u), /* @__PURE__ */ a.createElement("path", {
    fill: "#FFF",
    d: "M0 0h513v342H0z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#E00",
    d: "M0 0h513v171H0z"
  }));
}, Zi = function(i) {
  var u = i.title, f = d(i, Hv);
  return /* @__PURE__ */ a.createElement("svg", s({
    xmlns: "http://www.w3.org/2000/svg",
    viewBox: "0 0 513 342"
  }, f), u && /* @__PURE__ */ a.createElement("title", null, u), /* @__PURE__ */ a.createElement("path", {
    fill: "#FFF",
    d: "M0 0h513v342H0z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#6DA544",
    d: "M0 0h171v342H0z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#FF9811",
    d: "M342 0h171v342H342z"
  }));
}, Ki = function(i) {
  var u = i.title, f = d(i, yv);
  return /* @__PURE__ */ a.createElement("svg", s({
    xmlns: "http://www.w3.org/2000/svg",
    viewBox: "0 0 513 342"
  }, f), u && /* @__PURE__ */ a.createElement("title", null, u), /* @__PURE__ */ a.createElement("path", {
    fill: "#FFF",
    d: "M0 0h513v342H0z"
  }), /* @__PURE__ */ a.createElement("path", {
    d: "M340.6 122.4h-56.1l-28-48.6-28 48.6h-56.1l28 48.6-28 48.6h56.1l28 48.6 28-48.6h56.1l-28-48.6 28-48.6zM293.2 171 276 204.2h-38.9L219.8 171l17.2-33.2h38.9l17.3 33.2zm-36.7-71.8 11.9 23.3h-23.9l12-23.3zm-58.3 38.6h23.9l-10.8 21-13.1-21zm0 66.4 13-22.1 11.9 22.1h-24.9zm58.3 37.5-11.9-22.1h23.9l-12 22.1zm59.4-37.5h-25l11.9-22.1 13.1 22.1zm-26.1-66.4h26.1l-13 22.1-13.1-22.1zM0 21.3h512V64H0zm0 256h512V320H0z",
    fill: "#2E52B2"
  }));
}, Ji = function(i) {
  var u = i.title, f = d(i, Fv);
  return /* @__PURE__ */ a.createElement("svg", s({
    xmlns: "http://www.w3.org/2000/svg",
    viewBox: "0 0 513 342"
  }, f), u && /* @__PURE__ */ a.createElement("title", null, u), /* @__PURE__ */ a.createElement("path", {
    fill: "#E52D42",
    d: "M0 0h513v342H0z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#FFF",
    d: "M393.3 68.2h-45.8l-33.8 80.2-34.4-22.9s-24.7-59.1-34.4-68.2c-9.7-9.7-18.7-.6-48.9 7.8-29.6 8.4-30.8 18.7-42.2 18.7-4.8 0-19.9-17.5-27.7-22.3-12.1-7.8-16.9-4.2-13.9 7.2 1.2 4.8 10.9 10.9 16.9 19.9 7.8 11.5 12.7 25.9 12.7 25.9s10.9-10.3 16.9-12.1c9-2.4 19.3 1.8 31.4 0 15.1-2.4 31.4-10.9 31.4-10.9l4.2 43.4s-54.3 50.7-49.5 70.6 56.1 44.6 68.2 62.7c12.1 17.5-7.2 24.1-7.2 32.6s-1.8 19.9 7.2 16.3c9-3.6 10.3-18.1 18.7-31.4 6-9 10.3-13.9 11.5-17.5 1.8-9-23.5-32-37.4-48.9-6.6-7.8-19.9-18.7-19.9-18.7l39.2-29.6s67.6 27.7 82.6 18.7c15.1-9 19.3-98.3 19.3-98.3l47-11.5-12.1-11.7z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#F8DD4E",
    d: "M279.3 262.4c-6 0-11.5-4.8-11.5-11.5s4.8-11.5 11.5-11.5c6 0 11.5 4.8 11.5 11.5s-5.5 11.5-11.5 11.5zm45.8-182.8c-6 0-11.5-4.8-11.5-11.5s4.8-11.5 11.5-11.5c6 0 11.5 4.8 11.5 11.5s-5.5 11.5-11.5 11.5zm0 91.1c-6 0-11.5-4.8-11.5-11.5 0-6 4.8-11.5 11.5-11.5 6 0 11.5 4.8 11.5 11.5-.6 6.7-5.5 11.5-11.5 11.5zm-136.9 45.8c-6 0-11.5-4.8-11.5-11.5 0-6 4.8-11.5 11.5-11.5 6.6 0 11.5 4.8 11.5 11.5s-5.5 11.5-11.5 11.5zm45.2-136.9c-6 0-11.5-4.8-11.5-11.5s4.8-11.5 11.5-11.5c6 0 11.5 4.8 11.5 11.5s-4.8 11.5-11.5 11.5zm22.9 91.1c-6 0-11.5-10.3-11.5-22.9s4.8-22.9 11.5-22.9c6 0 11.5 10.3 11.5 22.9s-4.8 22.9-11.5 22.9zm-114-45.2c-6 0-11.5-4.8-11.5-11.5 0-6.6 4.8-11.5 11.5-11.5 6.6 0 11.5 4.8 11.5 11.5 0 6.6-4.8 11.5-11.5 11.5z"
  }));
}, ki = function(i) {
  var u = i.title, f = d(i, Dv);
  return /* @__PURE__ */ a.createElement("svg", s({
    xmlns: "http://www.w3.org/2000/svg",
    viewBox: "0 0 513 342"
  }, f), u && /* @__PURE__ */ a.createElement("title", null, u), /* @__PURE__ */ a.createElement("path", {
    fill: "#F93",
    d: "M0 0h513v114H0z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#fff",
    d: "M0 114h513v114H0z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#138808",
    d: "M0 228h513v114H0z"
  }), /* @__PURE__ */ a.createElement("circle", {
    cx: 256.5,
    cy: 171,
    r: 34.2,
    stroke: "navy",
    strokeWidth: 4,
    fill: "none"
  }), /* @__PURE__ */ a.createElement("circle", {
    cx: 256.5,
    cy: 171,
    r: 3.42,
    fill: "navy"
  }), /* @__PURE__ */ a.createElement("path", {
    d: "m265.3 138.159-17.6 65.682m17.6-65.682-17.6 65.682m25.8-62.286-34 58.89m41.042-53.487-48.084 48.084M285.945 154l-58.89 34m62.286-25.8-65.682 17.6M290.5 171h-68m66.841 8.8-65.682-17.6m62.286 25.8-58.89-34m53.487 41.042-48.084-48.084m41.042 53.487-34-58.89m25.8 62.286-17.6-65.682M256.5 205v-68m-8.8 66.841 17.6-65.682m-25.8 62.286 34-58.89m-41.042 53.487 48.084-48.084M227.055 188l58.89-34m-62.286 25.8 65.682-17.6M222.5 171h68m-66.841-8.8 65.682 17.6M227.055 154l58.89 34m-53.487-41.042 48.084 48.084M239.5 141.555l34 58.89m-25.8-62.286 17.6 65.682",
    stroke: "navy",
    strokeWidth: 2
  }));
}, Wi = function(i) {
  var u = i.title, f = d(i, Sv);
  return /* @__PURE__ */ a.createElement("svg", s({
    xmlns: "http://www.w3.org/2000/svg",
    viewBox: "0 0 513 342"
  }, f), u && /* @__PURE__ */ a.createElement("title", null, u), /* @__PURE__ */ a.createElement("path", {
    fill: "#FFF",
    d: "M0-.3h513V342H0V-.3z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#0052B4",
    d: "M462.9 198.1c-4 0-14-5-21-9-11-6-21-12-31.1-12s-20 6-31.1 12c-7 4-16 9-20 9s-14-5-20-9c-11-6-22-12-32.1-12-9 0-20 6-31.1 12-7 4-16 9-20 9s-13-5-20-9c-11-6-22-12-31.1-12-10 0-21 6-32.1 12-6 4-16 9-20 9s-13-5-20-9c-11-6-21-12-31.1-12s-20 6-31.1 12c-7 4-17 9-21 9s-13-5-20-9c-10-6-20-12-30.1-12v22l19 10c11 6 21 12 31.1 12s21-6 32.1-12c6-4 16-10 20-10s13 6 20 10c11 6 21 12 31.1 12s20-6 31.1-12c7-4 17-10 21-10s13 6 20 10c11 6 21 12 31.1 12s20-6 31.1-12c7-4 16-10 20-10s14 6 21 10c11 6 21 12 31.1 12s20-6 31.1-12c7-4 16-10 20-10s14 6 20 10c11 6 22 12 32.1 12s20-6 31.1-12l19-10v-22c-10 0-20 6-30.1 12-7 4-16 9-20 9zM0 31.7l19 10c11 6 21 12 31.1 12s21-6 32.1-12c6-4 16-10 20-10s13 6 20 10c11 6 21 12 31.1 12s20-6 31.1-12c7-4 17-10 21-10s13 6 20 10c11 6 21 12 31.1 12s20-6 31.1-12c7-4 16-10 20-10s14 6 21 10c11 6 21 12 31.1 12s20-6 31.1-12c7-4 16-10 20-10s14 6 20 10c11 6 22 12 32.1 12s20-6 31.1-12l19-10v-22c-10 1-20 6-30.1 12-7 4-16 9-20 9s-14-5-21-9c-11-6-21-12-31.1-12s-20 6-31.1 12c-7 4-16 9-20 9s-14-5-20-9c-11-6-22-12-32.1-12s-20 6-31.1 12c-7 4-16 9-20 9s-13-5-20-9c-11-6-22-12-31.1-12s-21 6-32.1 12c-6 4-16 9-20 9s-13-5-20-9c-11-6-21-12-31.1-12s-20 6-31.1 12c-7 4-17 9-21 9s-13-5-20-9c-10-6-20-11-30.1-12v22zm462.9 55.2c-4 0-14-5-21-9-11-6-21-12-31.1-12s-20 6-31.1 12c-7 4-16 9-20 9s-14-5-20-9c-11-6-22-12-32.1-12-9 0-20 6-31.1 12-7 4-16 9-20 9s-13-5-20-9c-11-6-22-12-31.1-12-10 0-21 6-32.1 12-6 4-16 9-20 9s-13-5-20-9c-11-6-21-12-31.1-12s-20 6-31.1 12c-7 4-17 9-21 9s-13-5-20-9c-10-6-20-12-30.1-12v22l19 9c11 6 21 12 31.1 12s21-6 32.1-12c6-4 16-9 20-9s13 5 20 9c11 6 21 12 31.1 12s20-6 31.1-12c7-4 17-9 21-9s13 5 20 9c11 6 21 12 31.1 12s20-6 31.1-12c7-4 16-9 20-9s14 5 21 9c11 6 21 12 31.1 12s20-6 31.1-12c7-4 16-9 20-9s14 5 20 9c11 6 22 12 32.1 12s20-6 31.1-12l19-9v-22c-10 0-20 6-30.1 12-7 3.9-16 9-20 9zm50.1 34c-10 1-20 6-30.1 12-7 4-16 10-20 10s-14-6-21-10c-11-6-21-12-31.1-12s-20 6-31.1 12c-7 4-16 10-20 10s-14-6-20-10c-11-6-22-12-32.1-12s-20 6-31.1 12c-7 4-16 10-20 10s-13-6-20-10c-11-6-22-12-31.1-12s-21 6-32.1 12c-6 4-16 10-20 10s-13-6-20-10c-11-6-21-12-31.1-12s-20 6-31.1 12c-7 4-17 10-21 10s-13-6-20-10c-10-6-20-11-30.1-12v23c4 1 13 5 19 9 11 6 21 12 31.1 12s21-6 32.1-12c6-4 16-9 20-9s13 5 20 9c11 6 21 12 31.1 12s20-6 31.1-12c7-4 17-9 21-9s13 5 20 9c11 6 21 12 31.1 12s20-6 31.1-12c7-4 16-9 20-9s14 5 21 9c11 6 21 12 31.1 12s20-6 31.1-12c7-4 16-9 20-9s14 5 20 9c11 6 22 12 32.1 12s20-6 31.1-12c6-4 15-8 19-9v-23zm-50.1 133.3c-4 0-14-5-21-9-11-6-21-12-31.1-12s-20 6-31.1 12c-7 4-16 9-20 9s-14-5-20-9c-11-6-22-12-32.1-12-9 0-20 6-31.1 12-7 4-16 9-20 9s-13-5-20-9c-11-6-22-12-31.1-12-10 0-21 6-32.1 12-6 4-16 9-20 9s-13-5-20-9c-11-6-21-12-31.1-12s-20 6-31.1 12c-7 4-17 9-21 9s-13-5-20-9c-10-6-20-12-30.1-12v22l19 9c11 6 21 12 31.1 12s21-6 32.1-12c6-4 16-9 20-9s13 5 20 9c11 6 21 12 31.1 12s20-6 31.1-12c7-4 17-9 21-9s13 5 20 9c11 6 21 12 31.1 12s20-6 31.1-12c7-4 16-9 20-9s14 5 21 9c11 6 21 12 31.1 12s20-6 31.1-12c7-4 16-9 20-9s14 5 20 9c11 6 22 12 32.1 12s20-6 31.1-12l19-9v-22c-10 0-20 6-30.1 12-7 4-16 9-20 9zm50.1 34c-10 1-20 6-30.1 12-7 4-16 10-20 10s-14-6-21-10c-11-6-21-12-31.1-12s-20 6-31.1 12c-7 4-16 10-20 10s-14-6-20-10c-11-6-22-12-32.1-12s-20 6-31.1 12c-7 4-16 10-20 10s-13-6-20-10c-11-6-22-12-31.1-12s-21 6-32.1 12c-6 4-16 10-20 10s-13-6-20-10c-11-6-21-12-31.1-12s-20 6-31.1 12c-7 4-17 10-21 10s-13-6-20-10c-10-6-20-11-30.1-12v23c4 0 13 5 19 9 11 6 21 12 31.1 12s21-6 32.1-12c6-4 16-9 20-9s13 5 20 9c11 6 21 12 31.1 12s20-6 31.1-12c7-4 17-9 21-9s13 5 20 9c11 6 21 12 31.1 12s20-6 31.1-12c7-4 16-9 20-9s14 5 21 9c11 6 21 12 31.1 12s20-6 31.1-12c7-4 16-9 20-9s14 5 20 9c11 6 22 12 32.1 12s20-6 31.1-12c6-4 15-9 19-9v-23z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#FFF",
    d: "M0-.3h256v171H0V-.3z"
  }), /* @__PURE__ */ a.createElement("path", {
    d: "M389.8 119.9H408v78.8h-18.3l.1-78.8zm0 117.8H408v75.1h-18.3l.1-75.1z",
    fill: "#A2001D"
  }), /* @__PURE__ */ a.createElement("g", {
    fill: "#D80027"
  }, /* @__PURE__ */ a.createElement("path", {
    d: "M144-.3h-32v70H0v32h112v69h32v-69h112v-32H144v-70z"
  }), /* @__PURE__ */ a.createElement("path", {
    d: "M0-.3v15l57 39h23L0-.3zm256 0v15l-57 39h-23l80-54z"
  }), /* @__PURE__ */ a.createElement("path", {
    d: "M0-.3v15l57 39h23L0-.3zm256 0v15l-57 39h-23l80-54zM0 170.7v-15l57-38h23l-80 53zm256 0v-15l-57-38h-23l80 53z"
  })), /* @__PURE__ */ a.createElement("g", {
    fill: "#2E52B2"
  }, /* @__PURE__ */ a.createElement("path", {
    d: "M0 22.7v31h46l-46-31zm96-23v49L23-.3h73zm160 23v31h-46l46-31zm-96-23v49l73-49h-73z"
  }), /* @__PURE__ */ a.createElement("path", {
    d: "M0 22.7v31h46l-46-31zm96-23v49L23-.3h73zm160 23v31h-46l46-31zm-96-23v49l73-49h-73zM0 147.7v-30h46l-46 30zm96 23v-49l-73 49h73zm160-23v-30h-46l46 30zm-96 23v-49l73 49h-73z"
  })), /* @__PURE__ */ a.createElement("path", {
    fill: "#5DA51E",
    stroke: "#45602C",
    strokeWidth: 4,
    strokeMiterlimit: 10,
    d: "M462.8 91.5h-29.1l25.7-25.7c4.5-4.4 4.5-11.4 0-15.8l-3.4-3.4c-4.4-4.4-11.4-4.4-15.8 0l-25.9 25.9V37.4c0-6.2-5.1-11.3-11.3-11.3h-4.7c-6.2 0-11.3 5.1-11.3 11.3v31.7l-23.5-23.8c-4.7-4.7-12.2-4.7-16.9 0l-3.6 3.6c-4.7 4.7-4.7 12.4 0 17.2l25 25.4h-30.6c-6 0-10.9 4.9-10.9 10.9v4.6c0 6 4.9 10.9 10.9 10.9H369l-20.7 20.7c-4.4 4.4-4.4 11.4 0 15.8l3.4 3.4c4.4 4.4 11.4 4.4 15.8 0l19.6-19.6V167c0 6.1 5.1 11.1 11.3 11.3h4.7c6.2 0 11.3-5.1 11.3-11.3v-28.3l22.2 22.6c4.7 4.7 12.4 4.7 17.1 0l3.6-3.6c4.7-4.7 4.7-12.4 0-17.2L435.2 118h27.6c5.9 0 10.7-4.9 10.9-10.9v-4.6c0-6.1-4.9-11-10.9-11z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#E2DD24",
    stroke: "#525625",
    strokeWidth: 4,
    strokeMiterlimit: 10,
    d: "M439.2 249.1h-79.8s-.9-13.4-6-21.8c-6.2-10.4-13.9-28.1-1.4-36.7 15.9-10.9 33.7.8 48.2.8 11.8.1 26.9-14 45-.8 12.2 8.9 3.9 28.2-1.3 36.7-7 11.5-4.7 21.8-4.7 21.8z"
  }));
}, $i = function(i) {
  var u = i.title, f = d(i, Av);
  return /* @__PURE__ */ a.createElement("svg", s({
    xmlns: "http://www.w3.org/2000/svg",
    viewBox: "0 0 513 342"
  }, f), u && /* @__PURE__ */ a.createElement("title", null, u), /* @__PURE__ */ a.createElement("path", {
    fill: "#FFF",
    d: "M0 0h513v342H0z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#CE1126",
    d: "M0 0h513v114H0z"
  }), /* @__PURE__ */ a.createElement("path", {
    d: "M0 228h513v114H0z"
  }), /* @__PURE__ */ a.createElement("path", {
    d: "M219.2 160.7h-29.3c1.5-5.7 6.6-9.9 12.8-9.9v-19.9c-18.3 0-33.1 14.9-33.1 33.1v16.5h49.6c1.8 0 3.3 1.5 3.3 3.3v6.6h-66.2v19.9h86.1v-26.5c0-12.7-10.4-23.1-23.2-23.1zm49.6 29.8v-59.6H249v79.5h33.1v-19.9zm66.2 0v-59.6h-19.8v59.6h-6.6v-19.8h-19.9v39.7h59.6v-19.9z",
    fill: "#547C31"
  }));
}, Ii = function(i) {
  var u = i.title, f = d(i, Bv);
  return /* @__PURE__ */ a.createElement("svg", s({
    xmlns: "http://www.w3.org/2000/svg",
    viewBox: "0 0 513 342"
  }, f), u && /* @__PURE__ */ a.createElement("title", null, u), /* @__PURE__ */ a.createElement("path", {
    fill: "#FFF",
    d: "M0 0h512v342H0z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#6DA544",
    d: "M0 0h513v114H0z"
  }), /* @__PURE__ */ a.createElement("path", {
    d: "M0 227.9h513v114H0zm278.8-93.1c.1 2 8.7 26.2 4.4 39.4-6.6 20.3-15.8 21.8-19.8 24.5V134l-6.9-4.2-6.9 4.2v64.7c-4-2.7-12.4-2.4-19.8-24.5-4.3-12.7 5.7-37.3 5.8-39.2 0 0-9.5 8.1-15.8 24-5.9 14.8 1.9 49.6 29.5 54.8 2.3.4 4.7 5.6 7.2 5.6 2.1 0 4.1-5.2 6-5.5 28.4-4.6 35-41.7 29.9-55.6-5.4-14.6-13.6-23.5-13.6-23.5z",
    fill: "#D80027"
  }), /* @__PURE__ */ a.createElement("path", {
    d: "M44.6 98.9h22.3v24.4H44.6zM0 98.9h22.3v24.4H0zm89.2 0h22.3v24.4H89.2zm44.6 0h22.3v24.4h-22.3zm44.6 0h22.3v24.4h-22.3zm44.6 0h22.3v24.4H223zm44.7 0H290v24.4h-22.3zm44.6 0h22.3v24.4h-22.3zm44.6 0h22.3v24.4h-22.3zm44.6 0h22.3v24.4h-22.3zm44.6 0h22.3v24.4h-22.3zm44.6 0H513v24.4h-22.3zm-446.1 118h22.3v25.5H44.6zm-44.6 0h22.3v25.5H0zm89.2 0h22.3v25.5H89.2zm44.6 0h22.3v25.5h-22.3zm44.6 0h22.3v25.5h-22.3zm44.6 0h22.3v25.5H223zm44.7 0H290v25.5h-22.3zm44.6 0h22.3v25.5h-22.3zm44.6 0h22.3v25.5h-22.3zm44.6 0h22.3v25.5h-22.3zm44.6 0h22.3v25.5h-22.3zm44.6 0H513v25.5h-22.3z",
    fill: "#FFF",
    opacity: 0.5
  }));
}, Pi = function(i) {
  var u = i.title, f = d(i, bv);
  return /* @__PURE__ */ a.createElement("svg", s({
    xmlns: "http://www.w3.org/2000/svg",
    viewBox: "0 0 513 342"
  }, f), u && /* @__PURE__ */ a.createElement("title", null, u), /* @__PURE__ */ a.createElement("path", {
    fill: "#0052B4",
    d: "M0 0h513v342H0z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#FFF",
    d: "M513 210.6H202.2v130.7h-79.8V210.6H0v-79.8h122.4V0h79.8v130.8H513v18.5l-1 42.7z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#D80027",
    d: "M513 149.3V192H183.7v149.3H141V192H0v-42.7h141V0h42.7v149.3z"
  }));
}, e8 = function(i) {
  var u = i.title, f = d(i, Tv);
  return /* @__PURE__ */ a.createElement("svg", s({
    xmlns: "http://www.w3.org/2000/svg",
    viewBox: "0 0 513 342"
  }, f), u && /* @__PURE__ */ a.createElement("title", null, u), /* @__PURE__ */ a.createElement("path", {
    fill: "#F4F5F0",
    d: "M342 0H0v341.3h512V0z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#008C45",
    d: "M0 0h171v342H0z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#CD212A",
    d: "M342 0h171v342H342z"
  }));
}, t8 = function(i) {
  var u = i.title, f = d(i, Lv);
  return /* @__PURE__ */ a.createElement("svg", s({
    xmlns: "http://www.w3.org/2000/svg",
    viewBox: "0 0 513 342"
  }, f), u && /* @__PURE__ */ a.createElement("title", null, u), /* @__PURE__ */ a.createElement("path", {
    fill: "#FFF",
    d: "M0 0h513v342H0z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#D80027",
    d: "M301.8 171 513 30.2V0h-45.3L256.5 140.8 45.3 0H0v30.2L211.2 171 0 311.8V342h45.3l211.2-140.8L467.7 342H513v-30.2z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#FFDA44",
    d: "m233.7 44.6 22.3 5.6 22.3-5.6 4-20.9-14.9 6.5L256 14.8l-11.4 15.4-14.9-6.5z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#D80027",
    d: "M233.7 44.6s-4 12.9-4 29.9c0 27.9 26.3 41.3 26.3 41.3s26.3-15.5 26.3-41.3c0-15.4-4-29.9-4-29.9s-8.1-5-22.3-5-22.3 5-22.3 5z"
  }));
}, l8 = function(i) {
  var u = i.title, f = d(i, Vv);
  return /* @__PURE__ */ a.createElement("svg", s({
    xmlns: "http://www.w3.org/2000/svg",
    viewBox: "0 0 513 342"
  }, f), u && /* @__PURE__ */ a.createElement("title", null, u), /* @__PURE__ */ a.createElement("path", {
    fill: "#6DA544",
    d: "M0 0h513v342H0z"
  }), /* @__PURE__ */ a.createElement("path", {
    d: "M215.9 171 0 314.6V26.8zM513 26.8v287.8L296.1 171z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#0052B4",
    d: "M513 26.8 296.1 171 513 314.6V342h-41.1L256 197.4 40.1 342H0v-27.4L215.9 171 0 26.8V0h40.1L256 143.9 471.9 0H513z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#FFDA44",
    d: "M513 26.8 296.1 171 513 314.6V342h-41.1L256 197.4 40.1 342H0v-27.4L215.9 171 0 26.8V0h40.1L256 143.9 471.9 0H513z"
  }));
}, a8 = function(i) {
  var u = i.title, f = d(i, Cv);
  return /* @__PURE__ */ a.createElement("svg", s({
    xmlns: "http://www.w3.org/2000/svg",
    viewBox: "0 0 513 342"
  }, f), u && /* @__PURE__ */ a.createElement("title", null, u), /* @__PURE__ */ a.createElement("path", {
    fill: "#FFF",
    d: "M0 0h513v342H0z"
  }), /* @__PURE__ */ a.createElement("path", {
    d: "M0 0h513v114H0z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#6DA544",
    d: "M0 228h513v114H0z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#D80027",
    d: "M256.5 170.7 0 341.3V0z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#FFF",
    d: "m77.9 139.5 7.9 16.4 17.8-4.1-8 16.5 14.3 11.3-17.8 4 .1 18.3-14.3-11.5-14.2 11.5v-18.3l-17.8-4 14.3-11.3-7.9-16.5 17.7 4.1z"
  }));
}, n8 = function(i) {
  var u = i.title, f = d(i, Ov);
  return /* @__PURE__ */ a.createElement("svg", s({
    xmlns: "http://www.w3.org/2000/svg",
    viewBox: "0 0 513 342"
  }, f), u && /* @__PURE__ */ a.createElement("title", null, u), /* @__PURE__ */ a.createElement("path", {
    fill: "#FFF",
    d: "M0 0h512v342H0z"
  }), /* @__PURE__ */ a.createElement("circle", {
    fill: "#D80027",
    cx: 256.5,
    cy: 171,
    r: 96
  }));
}, c8 = function(i) {
  var u = i.title, f = d(i, Nv);
  return /* @__PURE__ */ a.createElement("svg", s({
    xmlns: "http://www.w3.org/2000/svg",
    viewBox: "0 0 513 342"
  }, f), u && /* @__PURE__ */ a.createElement("title", null, u), /* @__PURE__ */ a.createElement("path", {
    fill: "#FFF",
    d: "M0 0h513v342H0z"
  }), /* @__PURE__ */ a.createElement("path", {
    d: "M0 0h512v90.579H0z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#496E2D",
    d: "M0 251h513v91H0z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#A2001D",
    d: "M0 114h513v114H0z"
  }), /* @__PURE__ */ a.createElement("g", {
    fill: "#FFF"
  }, /* @__PURE__ */ a.createElement("path", {
    d: "m323.3 54.5-25.6-11.8L256 137l-41.7-94.3-25.6 11.8 51.8 116.2-51.8 116.2 25.6 11.8 41.7-94.4 41.7 94.4 25.6-11.8-51.8-116.2z"
  }), /* @__PURE__ */ a.createElement("path", {
    d: "M273.4 65.6c-9.9-10.8-17.4-17-17.4-17s-7.5 6.2-17.4 17v210.1c9.9 10.8 17.4 17 17.4 17s7.5-6.2 17.4-17V65.6z"
  })), /* @__PURE__ */ a.createElement("path", {
    d: "M209 105.9v129.5c10.5 18.5 23.3 33.7 32.9 43.8V62.1c-9.6 10.1-22.4 25.3-32.9 43.8zm94 0c-10.5-18.5-23.3-33.7-32.9-43.8v217.2c9.6-10.1 22.4-25.3 32.9-43.8V105.9z",
    fill: "#A2001D"
  }), /* @__PURE__ */ a.createElement("path", {
    d: "M303 105.9v129.5c10.6-18.8 18.8-41 18.8-64.8s-8.2-45.9-18.8-64.7zm-94 0v129.5c-10.6-18.8-18.8-41-18.8-64.8s8.2-45.9 18.8-64.7z"
  }));
}, i8 = function(i) {
  var u = i.title, f = d(i, _v);
  return /* @__PURE__ */ a.createElement("svg", s({
    xmlns: "http://www.w3.org/2000/svg",
    viewBox: "0 0 512 342"
  }, f), u && /* @__PURE__ */ a.createElement("title", null, u), /* @__PURE__ */ a.createElement("g", {
    fill: "#D80027"
  }, /* @__PURE__ */ a.createElement("path", {
    d: "M0 0h513v342H0z"
  }), /* @__PURE__ */ a.createElement("circle", {
    cx: 256,
    cy: 170.7,
    r: 170.7
  })), /* @__PURE__ */ a.createElement("path", {
    fill: "#FFDA44",
    d: "M382.4 170.7 330.8 195l27.5 50-56.1-10.7-7.1 56.6-39.1-41.7-39.1 41.7-7.1-56.6-56.1 10.7 27.5-50-51.6-24.3 51.6-24.3-27.5-50.1 56.1 10.8 7.1-56.7L256 92.1l39.1-41.7 7.1 56.7 56.1-10.8-27.5 50.1z"
  }), /* @__PURE__ */ a.createElement("circle", {
    fill: "#D80027",
    cx: 257.4,
    cy: 170.7,
    r: 71.6
  }), /* @__PURE__ */ a.createElement("path", {
    d: "M214.2 170.7c-2.1 0-4.1.1-6.2.3.1 12 4.4 22.9 11.6 31.5 3.8-10.3 9.5-19.6 16.7-27.7-6.9-2.7-14.3-4.1-22.1-4.1zm26.6 46.5c5.2 1.9 10.8 2.9 16.6 2.9 5.8 0 11.4-1 16.6-2.9-2.8-11.1-8.7-21-16.6-28.8-7.9 7.8-13.8 17.7-16.6 28.8zm59.4-71.2c-8.5-14.8-24.5-24.7-42.8-24.7-18.3 0-34.2 9.9-42.8 24.7 15.6.1 30.2 4.3 42.8 11.6 12.6-7.3 27.2-11.6 42.8-11.6zm-21.7 28.8c7.2 8 12.9 17.4 16.7 27.7 7.2-8.5 11.5-19.5 11.6-31.5-2-.2-4.1-.3-6.2-.3-7.7 0-15.2 1.4-22.1 4.1z",
    fill: "#FFDA44"
  }));
}, u8 = function(i) {
  var u = i.title, f = d(i, Uv);
  return /* @__PURE__ */ a.createElement("svg", s({
    xmlns: "http://www.w3.org/2000/svg",
    viewBox: "0 0 513 342"
  }, f), u && /* @__PURE__ */ a.createElement("title", null, u), /* @__PURE__ */ a.createElement("path", {
    fill: "#D80027",
    d: "M0 0h513v342H0z"
  }), /* @__PURE__ */ a.createElement("path", {
    d: "M0 0h513v114H0zm0 228h513v114H0z",
    fill: "#0052B4"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#FFF",
    d: "M303.5 196.6v-17.8h-11.9v-23.7l-11.9-11.9-11.8 11.9v-23.8L256 119.5l-11.9 11.8v23.8l-11.8-11.9-11.9 11.9v23.7h-11.9v17.8h-11.9v17.8h118.8v-17.8z"
  }));
}, r8 = function(i) {
  var u = i.title, f = d(i, Rv);
  return /* @__PURE__ */ a.createElement("svg", s({
    xmlns: "http://www.w3.org/2000/svg",
    viewBox: "0 0 450 300"
  }, f), u && /* @__PURE__ */ a.createElement("title", null, u), /* @__PURE__ */ a.createElement("path", {
    fill: "#CE1126",
    d: "M0 0h450v300H0V0z"
  }), /* @__PURE__ */ a.createElement("g", {
    fill: "#FCD116"
  }, /* @__PURE__ */ a.createElement("circle", {
    cx: 227.6,
    cy: 161.2,
    r: 56.2
  }), /* @__PURE__ */ a.createElement("path", {
    d: "m254.1 73.4-23.7 26 28.6 6.5zm-51.5.2-4.8 33.1 28.5-8zm94.3 26.7-34.7 8.5 20.1 23.7zm-138.3-1.8 15.5 32.7 20.2-22.7zm159 45.3-34.4-9.9 5.6 29.5zm-182-2.6 31.9 19.8 5.2-27.7z"
  })), /* @__PURE__ */ a.createElement("path", {
    fill: "#003F87",
    d: "M0 175h450v125H0V175z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#FFF",
    d: "M0 267c15 0 30 13 45 13s30-13 45-13 30 13 45 13 30-13 45-13 30 13 45 13 30-13 45-13 30 13 45 13 30-13 45-13 30 13 45 13 30-13 45-13v-21c-15 0-30 13-45 13s-30-13-45-13-30 13-45 13-30-13-45-13-30 13-45 13-30-13-45-13-30 13-45 13-30-13-45-13-30 13-45 13-30-13-45-13v21zm0-43c15 0 30 13 45 13s30-13 45-13 30 13 45 13 30-13 45-13 30 13 45 13 30-13 45-13 30 13 45 13 30-13 45-13 30 13 45 13 30-13 45-13v-21c-15 0-30 13-45 13s-30-13-45-13-30 13-45 13-30-13-45-13-30 13-45 13-30-13-45-13-30 13-45 13-30-13-45-13-30 13-45 13-30-13-45-13v21zm0-43c15 0 30 13 45 13s30-13 45-13 30 13 45 13 30-13 45-13 30 13 45 13 30-13 45-13 30 13 45 13 30-13 45-13 30 13 45 13 30-13 45-13v-21c-15 0-30 13-45 13s-30-13-45-13-30 13-45 13-30-13-45-13-30 13-45 13-30-13-45-13-30 13-45 13-30-13-45-13-30 13-45 13-30-13-45-13v21z"
  }), /* @__PURE__ */ a.createElement("path", {
    fillRule: "evenodd",
    clipRule: "evenodd",
    fill: "#FCD116",
    d: "m183.2 48.3-1.1-5.3 22.4-3.7s-30.1-7.1-40-10.7c-14.3-5.1-16.3-12.1-16.3-12.1s44.7 11 64.8 11c4.3 0 12.8 10.7 12.8 10.7s14.8-14.9 21.9-15.4c32.1-2.3 66.6-6.4 66.6-6.4s-10 6.7-16 8.5c-16.2 4.9-50.6 11.7-50.6 11.7l-4.8 7.5H277l-14.4 4.3 7.4 6.9s-7.8-4.1-18.1-2.1c-7.1 1.3-15.4 6-22.9 8-18.7 4.8-29.9-12.8-29.9-12.8l-15.9-.1z"
  }));
}, f8 = function(i) {
  var u = i.title, f = d(i, jv);
  return /* @__PURE__ */ a.createElement("svg", s({
    xmlns: "http://www.w3.org/2000/svg",
    viewBox: "0 0 513 342"
  }, f), u && /* @__PURE__ */ a.createElement("title", null, u), /* @__PURE__ */ a.createElement("path", {
    fill: "#0052B4",
    d: "M0 0h513v342H0z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#FFDA44",
    d: "M0 0h513v85.5H0z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#FFF",
    d: "M0 85.5h513V171H0z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#D80027",
    d: "M0 171h513v85.5H0z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#6DA544",
    d: "M256.5 171 0 342V0z"
  }), /* @__PURE__ */ a.createElement("g", {
    fill: "#FFF"
  }, /* @__PURE__ */ a.createElement("path", {
    d: "M68.6 170.7c0-24.9 17.5-45.6 40.8-50.7-3.6-.8-7.3-1.2-11.1-1.2-28.7 0-51.9 23.3-51.9 51.9s23.3 51.9 51.9 51.9c3.8 0 7.5-.4 11.1-1.2-23.3-5.1-40.8-25.9-40.8-50.7z"
  }), /* @__PURE__ */ a.createElement("path", {
    d: "m108.9 126.1 2.7 8.5h9l-7.3 5.3 2.8 8.5-7.2-5.2-7.3 5.2 2.8-8.5-7.2-5.3h8.9zm0 22.3 2.7 8.5h9l-7.3 5.3 2.8 8.5-7.2-5.3-7.3 5.3 2.8-8.5-7.2-5.3h8.9z"
  }), /* @__PURE__ */ a.createElement("path", {
    d: "m108.9 170.7 2.7 8.5h9l-7.3 5.2 2.8 8.5-7.2-5.2-7.3 5.2 2.8-8.5-7.2-5.2h8.9zm0 22.2 2.7 8.5h9l-7.3 5.3 2.8 8.5-7.2-5.3-7.3 5.3 2.8-8.5-7.2-5.3h8.9z"
  })));
}, h8 = function(i) {
  var u = i.title, f = d(i, Gv);
  return /* @__PURE__ */ a.createElement("svg", s({
    xmlns: "http://www.w3.org/2000/svg",
    viewBox: "0 0 513 342"
  }, f), u && /* @__PURE__ */ a.createElement("title", null, u), /* @__PURE__ */ a.createElement("path", {
    fill: "#D80027",
    d: "M0 0h513v342H0z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#6DA544",
    d: "M512 0H0v342"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#FFDA44",
    d: "M307.7 0 0 194.5V342h206.9L513 148.5V0z"
  }), /* @__PURE__ */ a.createElement("path", {
    d: "M513 0H385.2L0 249.4V341l126.2 1L513 91.6z"
  }), /* @__PURE__ */ a.createElement("path", {
    d: "m141.1 187 31.2 24 32.3-22.2-13.1 37 31.1 23.9-39.2-1.1-13.2 37-11.1-37.6-39.2-1.1 32.3-22.2zM310.6 70.8l31.2 23.9 32.3-22.2-13.1 37 31.1 23.9-39.2-1.1-13.2 37-11.1-37.6-39.2-1.1 32.4-22.2z",
    fill: "#FFF"
  }));
}, m8 = function(i) {
  var u = i.title, f = d(i, Yv);
  return /* @__PURE__ */ a.createElement("svg", s({
    xmlns: "http://www.w3.org/2000/svg",
    viewBox: "0 0 513 342"
  }, f), u && /* @__PURE__ */ a.createElement("title", null, u), /* @__PURE__ */ a.createElement("path", {
    fill: "#91DC5A",
    d: "M0 0h513v342H0z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#FFF",
    d: "M0 0h513v342H0z"
  }), /* @__PURE__ */ a.createElement("path", {
    d: "M0 0h513v57.188H0zm0 284.1h513v57.177H0z",
    fill: "#0052B4"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#D80027",
    d: "M0 79.9h513v181.582H0z"
  }), /* @__PURE__ */ a.createElement("circle", {
    fill: "#FFF",
    cx: 190.33,
    cy: 171,
    r: 65.668
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#D80027",
    d: "m190.3 105 14.8 45.3h47.7l-38.6 28.1 14.8 45.5-38.7-28.2-38.6 28.1 14.8-45.4-38.6-28.1h47.7z"
  }));
}, v8 = function(i) {
  var u = i.title, f = d(i, qv);
  return /* @__PURE__ */ a.createElement("svg", s({
    xmlns: "http://www.w3.org/2000/svg",
    viewBox: "0 0 900 600"
  }, f), u && /* @__PURE__ */ a.createElement("title", null, u), /* @__PURE__ */ a.createElement("path", {
    fill: "#FFF",
    d: "M0 0h900v600H0z"
  }), /* @__PURE__ */ a.createElement("g", {
    transform: "rotate(-56.31)"
  }, /* @__PURE__ */ a.createElement("path", {
    d: "M-75 228.3H75m-150 37.5H75m-150 37.5H75m-150 475H75m-150 37.5H75m-150 37.5H75",
    stroke: "#000",
    strokeWidth: 25
  }), /* @__PURE__ */ a.createElement("path", {
    stroke: "#FFF",
    strokeWidth: 12.5,
    d: "M0 753.3v125"
  }), /* @__PURE__ */ a.createElement("circle", {
    fill: "#ca163a",
    cy: 540.8,
    r: 150
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#0e4896",
    d: "M0 390.8c-41.4 0-75 33.6-75 75s33.6 75 75 75 75 33.6 75 75-33.6 75-75 75c-82.8 0-150-67.2-150-150s67.2-150 150-150z"
  })), /* @__PURE__ */ a.createElement("path", {
    d: "m231.56 535.73-83.205-124.808M262.76 514.928l-83.205-124.807m114.407 104.006-83.205-124.808m478.43-138.675-83.205-124.807M720.39 209.843 637.184 85.036m114.407 104.006L668.386 64.234",
    stroke: "#000",
    strokeWidth: 25
  }), /* @__PURE__ */ a.createElement("path", {
    stroke: "#FFF",
    strokeWidth: 12.5,
    d: "m205.6 462.897 31.202-20.8m389.981-259.989 36.444-24.296m31.202-20.801 31.202-20.801"
  }));
}, s8 = function(i) {
  var u = i.title, f = d(i, Qv);
  return /* @__PURE__ */ a.createElement("svg", s({
    xmlns: "http://www.w3.org/2000/svg",
    viewBox: "0 0 513 342"
  }, f), u && /* @__PURE__ */ a.createElement("title", null, u), /* @__PURE__ */ a.createElement("path", {
    fill: "#FFF",
    d: "M0 0h513v342H0z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#6DA544",
    d: "M0 0h513v114H0z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#D80027",
    d: "M0 228h513v114H0z"
  }), /* @__PURE__ */ a.createElement("path", {
    d: "M167 227.6 0 341.3V0l167 113.8z"
  }));
}, d8 = function(i) {
  var u = i.title, f = d(i, Xv);
  return /* @__PURE__ */ a.createElement("svg", s({
    xmlns: "http://www.w3.org/2000/svg",
    viewBox: "0 0 513 342"
  }, f), u && /* @__PURE__ */ a.createElement("title", null, u), /* @__PURE__ */ a.createElement("path", {
    fill: "#0052B4",
    d: "M0 0h513v342H0z"
  }), /* @__PURE__ */ a.createElement("circle", {
    fill: "#6DA544",
    cx: 384,
    cy: 96.5,
    r: 29.7
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#D80027",
    d: "M332.1 89H436v44.5H332.1z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#496E2D",
    d: "M435.9 170.7z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#FFDA44",
    d: "M332.1 200.3V230h15.6c9.4 9.2 22.2 14.8 36.3 14.8 14.1 0 27-5.7 36.3-14.8h15.6v-29.7H332.1z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#338AF3",
    d: "M332.1 126.1v44.5c0 39.8 51.9 51.9 51.9 51.9s51.9-12.2 51.9-51.9v-44.5H332.1z"
  }), /* @__PURE__ */ a.createElement("path", {
    d: "M384 149.9c-13 0-13 11.9-26 11.9s-13-11.9-26-11.9v20.8c13 0 13 11.9 26 11.9s13-11.9 26-11.9 13 11.9 26 11.9 13-11.9 26-11.9v-20.8c-13 0-13 11.9-26 11.9s-13-11.9-26-11.9zm0-41.6c-13 0-13 11.9-26 11.9s-13-11.9-26-11.9v20.8c13 0 13 11.9 26 11.9s13-11.9 26-11.9 13 11.9 26 11.9 13-11.9 26-11.9v-20.8c-13 0-13 11.9-26 11.9s-13-11.9-26-11.9z",
    fill: "#F3F3F3"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#FFF",
    d: "M256 0v22.6l-46.1 30.7H256v64h-46.1L256 148v22.7h-22.6l-73.4-49v49H96v-49l-73.4 49H0V148l46.1-30.7H0v-64h46.1L0 22.6V0h22.6L96 48.9V0h64v48.9L233.4 0z"
  }), /* @__PURE__ */ a.createElement("g", {
    fill: "#D80027"
  }, /* @__PURE__ */ a.createElement("path", {
    d: "M144 0h-32v69.3H0v32h112v69.4h32v-69.4h112v-32H144z"
  }), /* @__PURE__ */ a.createElement("path", {
    d: "M0 0v15.1l57.4 38.2H80zm256 0v15.1l-57.4 38.2H176z"
  }), /* @__PURE__ */ a.createElement("path", {
    d: "M0 0v15.1l57.4 38.2H80zm256 0v15.1l-57.4 38.2H176zM0 170.7v-15.1l57.4-38.3H80zm256 0v-15.1l-57.4-38.3H176z"
  })));
}, o8 = function(i) {
  var u = i.title, f = d(i, Zv);
  return /* @__PURE__ */ a.createElement("svg", s({
    xmlns: "http://www.w3.org/2000/svg",
    viewBox: "0 0 513 342"
  }, f), u && /* @__PURE__ */ a.createElement("title", null, u), /* @__PURE__ */ a.createElement("path", {
    fill: "#36B6CC",
    d: "M0 0h513v342H0V0z"
  }), /* @__PURE__ */ a.createElement("circle", {
    fill: "#FFD400",
    cx: 256.5,
    cy: 170.9,
    r: 68.4
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#FFD400",
    d: "m256.5 251.5-27.9 41.7-7.1-49.7-43.2 25.5 15.2-47.8-50 4.2 34.4-36.5-46.9-18 46.8-18-34.4-36.5 50 4.2-15.2-47.8 43.2 25.4 7.1-49.7 27.9 41.7 27.9-41.7 7.1 49.7 43.2-25.4-15.2 47.8 50-4.2-34.3 36.6 46.8 17.9-46.8 18 34.4 36.5-50-4.2 15.2 47.8-43.2-25.4-7.1 49.7c0-.1-27.9-41.8-27.9-41.8zm0-.8c44.1 0 79.9-35.8 79.9-79.9S300.6 91 256.5 91s-79.9 35.8-79.9 79.9 35.8 79.8 79.9 79.8zM22.8 28.3c0-3.2 2.4-5.7 5.7-5.7h11.3c3.2 0 5.7 2.4 5.7 5.7v11.3c.1 3.1-2.4 5.7-5.6 5.7H28.5c-3.1 0-5.7-2.5-5.7-5.6V28.3zm0 45.7c0-3.2 2.4-5.7 5.7-5.7h11.3c3.2 0 5.7 2.4 5.7 5.7v11.3c.1 3.1-2.4 5.7-5.6 5.7H28.5c-3.1 0-5.7-2.5-5.7-5.6V74zm0 45.6c0-3.2 2.4-5.7 5.7-5.7h11.3c3.2 0 5.7 2.4 5.7 5.7v11.3c.1 3.1-2.4 5.7-5.6 5.7H28.5c-3.1 0-5.7-2.5-5.7-5.6v-11.4zm0 45.6c0-3.2 2.4-5.7 5.7-5.7h11.3c3.2 0 5.7 2.4 5.7 5.7v11.3c.1 3.1-2.4 5.7-5.6 5.7H28.5c-3.1 0-5.7-2.5-5.7-5.6v-11.4zm0 45.7c0-3.2 2.4-5.7 5.7-5.7h11.3c3.2 0 5.7 2.4 5.7 5.7v11.3c.1 3.1-2.4 5.7-5.6 5.7H28.5c-3.1 0-5.7-2.5-5.7-5.6v-11.4zm0 45.6c0-3.2 2.4-5.7 5.7-5.7h11.3c3.2 0 5.7 2.4 5.7 5.7v11.3c.1 3.1-2.4 5.7-5.6 5.7H28.5c-3.1 0-5.7-2.5-5.7-5.6v-11.4zm0 45.6c0-3.2 2.4-5.7 5.7-5.7h11.3c3.2 0 5.7 2.4 5.7 5.7v11.3c.1 3.1-2.4 5.7-5.6 5.7H28.5c-3.1 0-5.7-2.5-5.7-5.6v-11.4zm22.8-22.8c0-3.2 2.4-5.7 5.7-5.7h11.3c3.2 0 5.7 2.4 5.7 5.7v11.3c.1 3.1-2.4 5.7-5.6 5.7H51.3c-3.1 0-5.7-2.5-5.7-5.6v-11.4zm0-45.6c0-3.2 2.4-5.7 5.7-5.7h11.3c3.2 0 5.7 2.4 5.7 5.7V245c.1 3.1-2.4 5.7-5.6 5.7H51.3c-3.1 0-5.7-2.5-5.7-5.6v-11.4zm0-45.7c0-3.2 2.4-5.7 5.7-5.7h11.3c3.2 0 5.7 2.4 5.7 5.7v11.3c.1 3.1-2.4 5.7-5.6 5.7H51.3c-3.1 0-5.7-2.5-5.7-5.6V188zm0-45.6c0-3.2 2.4-5.7 5.7-5.7h11.3c3.2 0 5.7 2.4 5.7 5.7v11.3c.1 3.1-2.4 5.7-5.6 5.7H51.3c-3.1 0-5.7-2.5-5.7-5.6v-11.4zm0-45.6c0-3.2 2.4-5.7 5.7-5.7h11.3c3.2 0 5.7 2.4 5.7 5.7v11.3c.1 3.1-2.4 5.7-5.6 5.7H51.3c-3.1 0-5.7-2.5-5.7-5.6V96.8zm0-45.6c0-3.2 2.4-5.7 5.7-5.7h11.3c3.2 0 5.7 2.4 5.7 5.7v11.3c.1 3.1-2.4 5.7-5.6 5.7H51.3c-3.1 0-5.7-2.5-5.7-5.6V51.2z"
  }));
}, E8 = function(i) {
  var u = i.title, f = d(i, Kv);
  return /* @__PURE__ */ a.createElement("svg", s({
    xmlns: "http://www.w3.org/2000/svg",
    viewBox: "0 0 513 342"
  }, f), u && /* @__PURE__ */ a.createElement("title", null, u), /* @__PURE__ */ a.createElement("path", {
    fill: "#0052B4",
    d: "M0 0h513v342H0z"
  }), /* @__PURE__ */ a.createElement("path", {
    d: "M0 .1h513v90.7H0zm0 251.2h513V342H0z",
    fill: "#D80027"
  }), /* @__PURE__ */ a.createElement("circle", {
    fill: "#FFF",
    cx: 256.5,
    cy: 171,
    r: 65.9
  }));
}, z8 = function(i) {
  var u = i.title, f = d(i, Jv);
  return /* @__PURE__ */ a.createElement("svg", s({
    xmlns: "http://www.w3.org/2000/svg",
    viewBox: "0 0 513 342"
  }, f), u && /* @__PURE__ */ a.createElement("title", null, u), /* @__PURE__ */ a.createElement("path", {
    fill: "#FFF",
    d: "M0 0h513v342H0z"
  }), /* @__PURE__ */ a.createElement("path", {
    d: "M0 .2h513v90.7H0zm.5 251.1H513V342H.5z",
    fill: "#D80027"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#6DA544",
    d: "M290.3 193.5 256 130.7l-34.3 62.8h25.7v17.2h17.2v-17.2z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#14AF5A",
    d: "M241.3 213.1c4.4-4.4 4.4-11.6-.1-16l4.7 4.7c-5.2-4.7-12.2-6.6-19.1-5.3l-23.5 5.9c-6.1 1.5-6.8-.4-1.8-4.2l27.1-20.3c5.1-3.8 4.2-6.9-2.3-6.9h-11c-6.4 0-7-2.3-1.5-5.1l25.3-12.7c5.6-2.8 5.1-5.1-1.5-5.1h-11c-6.4 0-7.4-3.1-2.3-6.9l27.1-20.3c5.6-3.8 12.9-3.8 18.5 0l27.1 20.3c5.1 3.8 4.2 6.9-2.3 6.9h-11c-6.4 0-7 2.3-1.5 5.1l25.3 12.7c5.6 2.8 5.1 5.1-1.5 5.1h-11c-6.4 0-7.4 3.1-2.3 6.9l27.1 20.3c5.1 3.8 4.4 5.7-1.8 4.2l-23.5-5.9c-6.8-1.3-13.9.6-19.1 5.3l4.7-4.7c-4.4 4.4-4.5 11.5-.1 16l6.8 6.8c4.4 4.4 3.1 8-3.3 8h-45.8c-6.2 0-7.7-3.6-3.3-8 .1.1 6.9-6.8 6.9-6.8z"
  }));
}, p8 = function(i) {
  var u = i.title, f = d(i, kv);
  return /* @__PURE__ */ a.createElement("svg", s({
    xmlns: "http://www.w3.org/2000/svg",
    viewBox: "0 0 513 342"
  }, f), u && /* @__PURE__ */ a.createElement("title", null, u), /* @__PURE__ */ a.createElement("path", {
    fill: "#55B2FF",
    d: "M0 0h513v342H0z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#F3F3F3",
    d: "M148.5 298.1h216l-108-254.2z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#333",
    d: "m186 272.7 70.5-160.3L327 272.7z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#FFDA44",
    d: "M148.5 298.1h216l-108-101.7z"
  }));
}, g8 = function(i) {
  var u = i.title, f = d(i, Wv);
  return /* @__PURE__ */ a.createElement("svg", s({
    xmlns: "http://www.w3.org/2000/svg",
    viewBox: "0 0 513 342"
  }, f), u && /* @__PURE__ */ a.createElement("title", null, u), /* @__PURE__ */ a.createElement("path", {
    fill: "#D80027",
    d: "M0 0h513v342H0z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#0052B4",
    d: "M0 0h513v171H0z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#FFDA44",
    d: "M149.3 98.1c0-14-11.3-25.3-25.3-25.3-6.5 0-12.4 2.4-16.9 6.4V64.4h8.4V47.5h-8.4v-8.4H90.3v8.4h-8.4v16.9h8.4v14.9c-4.5-4-10.4-6.4-16.9-6.4-14 0-25.3 11.3-25.3 25.3 0 7.5 3.3 14.2 8.4 18.8v14.9h84.3V117c5.3-4.7 8.5-11.5 8.5-18.9z"
  }));
}, M8 = function(i) {
  var u = i.title, f = d(i, $v);
  return /* @__PURE__ */ a.createElement("svg", s({
    xmlns: "http://www.w3.org/2000/svg",
    viewBox: "0 0 513 342"
  }, f), u && /* @__PURE__ */ a.createElement("title", null, u), /* @__PURE__ */ a.createElement("path", {
    fill: "#FFDA44",
    d: "M0 0h513v342H0z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#A2001D",
    d: "M222.6 22.3h267.1v296.8H222.6z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#D80027",
    d: "M22.3 22.3h200.3v296.8H22.3z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#FFDA44",
    d: "M200.3 0h22.3v342h-22.3z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#6DA544",
    d: "M22.3 22.3h89v296.8h-89z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#FF9811",
    d: "M111.3 22.3h89v296.8h-89z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#FFDA44",
    d: "M368.2 156.8c-1.4-2 0-37.9 0-37.9.9-7.4.1-8.4 1-11.4.9-2.9 5.4-11.6 5.4-11.6s-6-2.3-10.3-.6c-4.3 1.7-24.7-7.5-31.2 3.4-4.4 7-23.6 16.2-24.6 14-1-2.2-5.9-5-6.9-2.9s-.7 13.9 0 16c.8 2.6 9.8 2 12.3 3.1 3.7 1.8 12.2 8.2 11.9 10.7-.3 2.5-22.5-6-24.2-4.5-1.4 1.2-1.9 13.2-.4 15 1.8 2.1 23.4 5.6 25 7.5 1.6 1.9-13.1 7.4-12.3 25 .7 14.7 8 17.4 8 17.4s-2.4 3.6-7.8 3.1c-6.2-.6-27.4-17.7-27.4-17.7s-3.5-109 3.6-121.4c4.9-8.5-22.7 10.1-22.7 50.8V151c0 6.6-1.4 22.4 2.6 34 0 0-.4 46.9 1.5 49.6s12.7 2.2 14.8 0 0-30 0-30c7.7.2 18.3 17.5 32.5 16.4 20.6-1.6 28.7-9.9 34.3-5 8.1 7.1 5.5 23.2.4 31.5-2.2 3.5-14.3 4.7-15.7 7.6-1.4 2.9.5 5.4.5 5.4h29.7s1.4-20.5 2.7-26.1c1.3-5.7-.6-10.7 5.9-10.4 23.5 1.1 39.8-15.3 45.5-8 3.2 4.1 5.6 29.9 3.3 33.3-2.2 3.4-13.8 2.8-15.2 6.7-1.4 4 .8 4.4.8 4.4h29.7s-.4-21.2 2.5-27.7 4.9-10.4 9.3-17.2c4.4-6.8 11.3-15.7 11.3-36.2 0-14.2-8.1-22.8-8.1-22.8H417c-14.1.2-47.4 2.3-48.8.3zm-136-124.5c10.7-10.6 20.2-2.7 22.2 2.1 1.9 4.8 2.1 8.2 2.8 13.6.9 6.9 4.9 14 4.9 14s-7.1-3.8-14.7-4.2c-4.7-.3-9-.6-13.6-2.5-5.7-2.3-11.7-12.9-1.6-23zm248.4-.1c10.7 10.6 2.8 20.2-2 22.2s-8.2 2.1-13.6 2.8c-6.9.9-14 5-14 5s3.7-7.1 4.2-14.7c.3-4.7.6-9.1 2.4-13.6 2.2-5.7 12.8-11.8 23-1.7zm.1 276.9c-10.7 10.6-20.2 2.6-22.2-2.2-1.9-4.8-2.1-8.2-2.7-13.6-.8-6.9-4.9-14.1-4.9-14.1s7 3.8 14.7 4.3c4.7.3 9 .6 13.6 2.5 5.7 2.4 11.7 13.1 1.5 23.1zm-248.2-.1c-10.6-10.7-2.7-20.2 2.1-22.2 4.8-1.9 8.2-2.1 13.6-2.8 6.9-.9 14-4.9 14-4.9s-3.8 7.1-4.2 14.7c-.3 4.7-.6 9-2.5 13.6-2.3 5.7-12.9 11.8-23 1.6z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "none",
    stroke: "#FFDA44",
    strokeWidth: 6,
    strokeMiterlimit: 10,
    d: "M446.9 162.4s7.5-2.6 8.4-16.1c.6-8.6-19.8-16-39-11.5-19.7 4.6-34-3.1-34-14.6 0-22.9 29.7-16.2 38.9-11.3 9.1 4.9 29.7 23.5 35.1 9.4"
  }));
}, w8 = function(i) {
  var u = i.title, f = d(i, Iv);
  return /* @__PURE__ */ a.createElement("svg", s({
    xmlns: "http://www.w3.org/2000/svg",
    viewBox: "0 0 513 342"
  }, f), u && /* @__PURE__ */ a.createElement("title", null, u), /* @__PURE__ */ a.createElement("path", {
    fill: "#FFF",
    d: "M0 0h513v342H0z"
  }), /* @__PURE__ */ a.createElement("path", {
    d: "M0 0h513v31.1H0zm0 62.2h513v31.1H0zm0 62.3h513v31.1H0zm0 62.3h513v31.1H0zm0 62.5h513v31.1H0zm0 61.6h513V342H0z",
    fill: "#bf0a30"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#002868",
    d: "M0 0h155.6v155.6H0z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#FFF",
    d: "m77.8 30.2 10.7 32.9h34.6l-28 20.4 10.7 32.9-28-20.3-28 20.3 10.7-32.9-28-20.4h34.6z"
  }));
}, x8 = function(i) {
  var u = i.title, f = d(i, Pv);
  return /* @__PURE__ */ a.createElement("svg", s({
    xmlns: "http://www.w3.org/2000/svg",
    viewBox: "0 0 513 342"
  }, f), u && /* @__PURE__ */ a.createElement("title", null, u), /* @__PURE__ */ a.createElement("path", {
    fill: "#FFF",
    d: "M0 0h513v342H0z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#0052B4",
    d: "M0 0h513v114H0z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#6DA544",
    d: "M0 227.6h513v114H0z"
  }), /* @__PURE__ */ a.createElement("path", {
    d: "m245.2 171 7.7-23.1c2-5.9 5.1-6 7.1 0l7.7 23.1 19.4 29.1c1.7 2.6 1.3 6.3-1.2 8 0 0-6.9 8-29.5 8s-29.5-8-29.5-8c-2.4-2-2.9-5.4-1.2-8l19.5-29.1z"
  }), /* @__PURE__ */ a.createElement("path", {
    d: "M256.5 171c-12.5 0-22.6-10.1-22.6-22.6 0-12.5 10.1-22.6 22.6-22.6s22.6 10.1 22.6 22.6c0 12.5-10.2 22.6-22.6 22.6zm0 0c6.2 0 11.3-16.3 11.3-22.6 0-6.2-5.1-11.3-11.3-11.3-6.2 0-11.3 5.1-11.3 11.3 0 6.3 5 22.6 11.3 22.6z"
  }));
}, H8 = function(i) {
  var u = i.title, f = d(i, es);
  return /* @__PURE__ */ a.createElement("svg", s({
    xmlns: "http://www.w3.org/2000/svg",
    viewBox: "0 0 513 342"
  }, f), u && /* @__PURE__ */ a.createElement("title", null, u), /* @__PURE__ */ a.createElement("path", {
    fill: "#6DA544",
    d: "M0 0h513v342H0z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#FFDA44",
    d: "M0 0h513v114H0z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#D80027",
    d: "M0 228h513v114H0z"
  }));
}, y8 = function(i) {
  var u = i.title, f = d(i, ts);
  return /* @__PURE__ */ a.createElement("svg", s({
    xmlns: "http://www.w3.org/2000/svg",
    viewBox: "0 0 513 342"
  }, f), u && /* @__PURE__ */ a.createElement("title", null, u), /* @__PURE__ */ a.createElement("path", {
    fill: "#FFF",
    d: "M0 0h513v342H0z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#D80027",
    d: "M0 0h513v114H0z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#338AF3",
    d: "M0 228h513v114H0z"
  }));
}, F8 = function(i) {
  var u = i.title, f = d(i, ls);
  return /* @__PURE__ */ a.createElement("svg", s({
    xmlns: "http://www.w3.org/2000/svg",
    viewBox: "0 0 513 342"
  }, f), u && /* @__PURE__ */ a.createElement("title", null, u), /* @__PURE__ */ a.createElement("path", {
    fill: "#FFF",
    d: "M0 0h513v342H0z"
  }), /* @__PURE__ */ a.createElement("path", {
    className: "prefix__st1",
    d: "M0 0h513v127.6H0zm0 214.4h513V342H0z",
    fill: "#A2001D"
  }));
}, D8 = function(i) {
  var u = i.title, f = d(i, as);
  return /* @__PURE__ */ a.createElement("svg", s({
    xmlns: "http://www.w3.org/2000/svg",
    viewBox: "0 0 513 342"
  }, f), u && /* @__PURE__ */ a.createElement("title", null, u), /* @__PURE__ */ a.createElement("path", {
    d: "M0 0h513v342H0z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#D80027",
    d: "M0 0h513v114H0z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#496E2D",
    d: "M0 228h513v114H0z"
  }), /* @__PURE__ */ a.createElement("g", {
    fill: "#FFF"
  }, /* @__PURE__ */ a.createElement("path", {
    d: "m281.4 150.7 9 12.3 14.5-4.7-9 12.4 9 12.3-14.5-4.7-9 12.3v-15.2l-14.5-4.7 14.5-4.7z"
  }), /* @__PURE__ */ a.createElement("path", {
    d: "M257 201.5c-17 0-30.9-13.8-30.9-30.9s13.8-30.9 30.9-30.9c5.3 0 10.3 1.3 14.7 3.7-6.9-6.7-16.2-10.8-26.6-10.8-21 0-38 17-38 38s17 38 38 38c10.3 0 19.7-4.1 26.6-10.8-4.4 2.4-9.4 3.7-14.7 3.7z"
  })));
}, S8 = function(i) {
  var u = i.title, f = d(i, ns);
  return /* @__PURE__ */ a.createElement("svg", s({
    xmlns: "http://www.w3.org/2000/svg",
    viewBox: "0 0 513 342"
  }, f), u && /* @__PURE__ */ a.createElement("title", null, u), /* @__PURE__ */ a.createElement("path", {
    fill: "#d12a46",
    d: "M0 .3V342h513V.3z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#316525",
    d: "M359.8 148.9h-73.3l-22.7-69.7-22.7 69.7h-73.3l59.3 43.1-22.7 69.7 59.3-43.1 59.3 43.1-22.5-69.7 59.3-43.1zm-116.1 37.7 7.7-23.6h24.8l7.7 23.6-20.1 14.6-20.1-14.6zm27.9-37.7H256l7.8-24 7.8 24zm24.3 29-4.8-14.9h25.3l-20.5 14.9zM236.6 163l-4.8 14.9-20.5-14.9h25.3zm-5.3 61.8 7.8-24 12.6 9.2-20.4 14.8zm44.5-14.9 12.6-9.2 7.8 24-20.4-14.8z"
  }));
}, A8 = function(i) {
  var u = i.title, f = d(i, cs);
  return /* @__PURE__ */ a.createElement("svg", s({
    xmlns: "http://www.w3.org/2000/svg",
    viewBox: "0 0 513 342"
  }, f), u && /* @__PURE__ */ a.createElement("title", null, u), /* @__PURE__ */ a.createElement("path", {
    fill: "#FFF",
    d: "M0 171h513v171H0z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#ce1126",
    d: "M0 0h513v171H0z"
  }));
}, B8 = function(i) {
  var u = i.title, f = d(i, is);
  return /* @__PURE__ */ a.createElement("svg", s({
    xmlns: "http://www.w3.org/2000/svg",
    viewBox: "0 0 513 342"
  }, f), u && /* @__PURE__ */ a.createElement("title", null, u), /* @__PURE__ */ a.createElement("path", {
    fill: "#FFDA44",
    d: "M0 0h513v342H0z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#D80027",
    d: "M342 0h171v342H342z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#0052B4",
    d: "M0 0h171v342H0z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#AF7F59",
    stroke: "#231F20",
    strokeWidth: 3,
    strokeMiterlimit: 10,
    d: "m240.4 105.7 9.7 11.6-7.5 17.4-26.1-24.3-12.7 13.2v106.1l24.6-36 20.6 12.4-18 27.8 25.2 15.8 26.5-15.8-19.7-27.8 22.3-12.4 23.9 33.8v-106l-12.4-12-26.8 24.7-6.5-16.5 3.5-12.3-11.7-7.5z"
  }));
}, b8 = function(i) {
  var u = i.title, f = d(i, us);
  return /* @__PURE__ */ a.createElement("svg", s({
    xmlns: "http://www.w3.org/2000/svg",
    viewBox: "0 0 513 342"
  }, f), u && /* @__PURE__ */ a.createElement("title", null, u), /* @__PURE__ */ a.createElement("path", {
    fill: "#ffc000",
    d: "M0 0h513v342H0z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#e30000",
    d: "M20 20h473v302H20z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#ffc000",
    d: "M248.5 112.9c4.4 4.4 11.6 4.4 16 0l-4.7 4.7 22.7-22.7c4.9-4.5 12.1-5.6 18.2-2.8l2.1 1c5.7 2.9 6.7 8.7 2.3 13.1l4.7-4.7-22.8 22.8c-4.5 4.9-5.6 12.1-2.8 18.2l1 2.1c2.9 5.7 5.1 15.4 5.1 21.6v-11.3c0 6.2 4.4 9.1 10.2 6.2l2.1-1c5.7-2.9 6.7-8.7 2.3-13.1l4.7 4.7c-4.4-4.4-4.4-11.6 0-16l-4.7 4.7c5.2-5.5 9.6-11.6 13.1-18.2l1-2.1c2.9-5.7 6.3-15.1 7.9-21.2l5.8-23.3c1.5-6 5.9-6.8 9.6-1.8l20.2 26.9c3.8 5.1 6.9 14 6.9 20.6v22.4c0 6.3-2.2 15.8-5.1 21.7l-1 2.1c-3.5 6.6-7.9 12.7-13.1 18.2l-6.8 6.8c-5.4 5.2-11.5 9.6-18.2 13.1l-2.1 1c-5.7 2.9-15.5 5.1-21.6 5.1-6.2 0-6.9 2.2-1.1 5.1l2.1 1c5.7 2.9 13.8 8.7 18.2 13.1l-4.7-4.7c4.4 4.4 11.6 4.4 16 0l-4.7 4.7c4.4-4.4 11.6-4.4 16 0L339 226c4.6 4.9 5.7 12.1 2.9 18.2l-1 2.1c-2.9 5.7-10.4 10.2-16.4 10.2-7.5-.3-14.8-2-21.6-5.1l-2.1-1c-6.5-3.6-11.8-8.9-15.4-15.4l-1-2.1c-2.9-5.7-8.7-6.7-13.1-2.3l4.7-4.7c-4.4 4.7-5.2 11.8-1.8 17.3l10.3 15.4c3.4 5.1 2 12.1-3.1 15.5L266 284.4c-5.7 3.4-12.8 3.4-18.6 0L232 274.1c-5.1-3.4-6.5-10.3-3.1-15.5l10.3-15.4c3.4-5.5 2.6-12.6-1.8-17.3l4.7 4.7c-4.4-4.4-10.2-3.6-13.1 2.3l-1 2.1c-2.9 5.7-9.5 12.4-15.4 15.4l-2.1 1c-5.7 2.9-15.5 5.1-21.6 5.1-6.9-.3-13.1-4.2-16.4-10.2l-1-2.1c-2.8-6.1-1.6-13.3 2.9-18.2l-4.7 4.7c4.4-4.4 11.6-4.4 16 0L181 226c4.4 4.4 11.6 4.4 16 0l-4.7 4.7c5.5-5.2 11.6-9.6 18.2-13.1l2.1-1c5.7-2.9 5-5.1-1.1-5.1-7.5-.3-14.8-2-21.6-5.1l-2.1-1c-6.6-3.5-12.7-7.9-18.2-13.1l-6.8-6.8c-5.2-5.4-9.6-11.5-13.1-18.2l-1-2.1c-3.1-6.8-4.9-14.2-5.1-21.7V121c0-6.3 3.1-15.6 6.9-20.6l20.2-26.9c3.8-5.1 8.1-4.3 9.6 1.8l5.8 23.3c2 7.3 4.6 14.4 7.9 21.2l1 2.1c2.9 5.7 8.7 13.8 13.1 18.2l-4.7-4.7c4.4 4.4 4.4 11.6 0 16l4.7-4.7c-4.4 4.4-3.6 10.2 2.3 13.1l2.1 1c5.7 2.9 10.2 0 10.2-6.2v11.3c0-6.2 2.2-15.7 5.1-21.6l1-2.1c2.8-6.1 1.7-13.2-2.8-18.2l-22.7-22.7 4.7 4.7c-4.4-4.4-3.6-10.2 2.3-13.1l2.1-1c6.1-2.8 13.2-1.7 18.2 2.8l22.7 22.7-4.8-4.5zm-18-40.7c-4.4-4.4-3.2-9.5 2.8-11.5l12.4-4.2c3-1 6.9-7.5 10.8-7.5 3.9 0 7.8 6.5 10.8 7.5l12.4 4.2c6 2 7.2 7.1 2.8 11.5l-6.8 6.8c-5.3 4.7-12.1 7.5-19.2 7.9-6.2 0-14.8-3.5-19.2-7.9l-6.8-6.8z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#0371ba",
    d: "M275 201.3h-37c-4.9 0-8.8-3.9-8.8-8.8v-34.2c0-4.9 3.9-8.8 8.8-8.8h37c4.9 0 8.8 3.9 8.8 8.8v34.2c0 4.8-3.9 8.8-8.8 8.8z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#39B54A",
    d: "m230.8 212.8 24.4 17.9c.8.6 1.8.6 2.6 0l24.4-17.9c2.1-1.6 1.2-5.4-1.3-5.4h-48.8c-2.5 0-3.4 3.9-1.3 5.4z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#ffc000",
    d: "M262.8 186.5h-12.5c-3.3 0-5.9-2.6-5.9-5.9V170c0-3.3 2.6-5.9 5.9-5.9h12.5c3.3 0 5.9 2.6 5.9 5.9v10.6c-.1 3.3-2.7 5.9-5.9 5.9z"
  }));
}, T8 = function(i) {
  var u = i.title, f = d(i, rs);
  return /* @__PURE__ */ a.createElement("svg", s({
    xmlns: "http://www.w3.org/2000/svg",
    viewBox: "0 0 108 72"
  }, f), u && /* @__PURE__ */ a.createElement("title", null, u), /* @__PURE__ */ a.createElement("path", {
    d: "M0 0h108v72H0z",
    fill: "#003787"
  }), /* @__PURE__ */ a.createElement("path", {
    d: "M0 0h108L60 48v24H48V48z",
    fill: "#fff"
  }), /* @__PURE__ */ a.createElement("circle", {
    cx: 54,
    cy: 30,
    r: 8,
    fill: "#f9d90f"
  }), /* @__PURE__ */ a.createElement("path", {
    d: "M44 30h20L54 48z",
    fill: "#fff"
  }), /* @__PURE__ */ a.createElement("path", {
    d: "M54 48 38 33h32z",
    fill: "#cf142b"
  }));
}, L8 = function(i) {
  var u = i.title, f = d(i, fs);
  return /* @__PURE__ */ a.createElement("svg", s({
    xmlns: "http://www.w3.org/2000/svg",
    viewBox: "0 0 513 342"
  }, f), u && /* @__PURE__ */ a.createElement("title", null, u), /* @__PURE__ */ a.createElement("path", {
    fill: "#f9423a",
    d: "M0 0h513v171H0z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#00843d",
    d: "M0 171h513v171H0z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#FFF",
    d: "M0 0h171v342H0z"
  }));
}, V8 = function(i) {
  var u = i.title, f = d(i, hs);
  return /* @__PURE__ */ a.createElement("svg", s({
    xmlns: "http://www.w3.org/2000/svg",
    viewBox: "0 0 513 342"
  }, f), u && /* @__PURE__ */ a.createElement("title", null, u), /* @__PURE__ */ a.createElement("path", {
    fill: "#1E509C",
    d: "M0 0h513v342H0z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#FFF",
    d: "M513 176.1V81.3L0 342l513-165.9z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#F18D36",
    d: "M513 0v81.3L0 342 513 0z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#FFF",
    d: "m126.7 99.3-7.8-54.1-7.7 54.1-20.8-23.7 10.7 31.6-31.5-10.7 23.6 20.8-54.1 7.7 54.1 7.7-23.6 20.8 31.5-10.7-10.7 31.6 20.8-23.7 7.7 54.1 7.8-54.1 20.7 23.7-10.6-31.6 31.5 10.7-23.6-20.8 54-7.7-54-7.7 23.6-20.8-31.5 10.7 10.6-31.6z"
  }));
}, C8 = function(i) {
  var u = i.title, f = d(i, ms);
  return /* @__PURE__ */ a.createElement("svg", s({
    xmlns: "http://www.w3.org/2000/svg",
    viewBox: "0 0 513 342"
  }, f), u && /* @__PURE__ */ a.createElement("title", null, u), /* @__PURE__ */ a.createElement("path", {
    fill: "#FFDA44",
    d: "M0 0h513v342H0z"
  }), /* @__PURE__ */ a.createElement("g", {
    fill: "#D80027"
  }, /* @__PURE__ */ a.createElement("path", {
    d: "M384.6 342 256.5 167.3 283.4 342zM133.8 0l122.7 167.3L230.8 0zm95.8 342 26.9-174.7L128.4 342zM0 0v127.8l256.5 39.5L28.4 0zm0 342h18.2l238.3-174.7L0 206.7zm256.5-174.7L513 127.8V0h-28.4zM282.2 0l-25.7 167.3L379.2 0z"
  }), /* @__PURE__ */ a.createElement("path", {
    d: "M513 342V206.7l-256.5-39.4L494.8 342z"
  }), /* @__PURE__ */ a.createElement("circle", {
    cx: 256.5,
    cy: 171,
    r: 59.5
  })), /* @__PURE__ */ a.createElement("circle", {
    fill: "#FFDA44",
    cx: 256.5,
    cy: 171,
    r: 44.5
  }));
}, O8 = function(i) {
  var u = i.title, f = d(i, vs);
  return /* @__PURE__ */ a.createElement("svg", s({
    xmlns: "http://www.w3.org/2000/svg",
    viewBox: "0 0 513 342"
  }, f), u && /* @__PURE__ */ a.createElement("title", null, u), /* @__PURE__ */ a.createElement("path", {
    fill: "#14b53a",
    d: "M0 0h171v342H0z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#fcd116",
    d: "M171 0h171v342H171z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#ce1126",
    d: "M342 0h171v342H342z"
  }));
}, N8 = function(i) {
  var u = i.title, f = d(i, ss);
  return /* @__PURE__ */ a.createElement("svg", s({
    xmlns: "http://www.w3.org/2000/svg",
    viewBox: "0 0 513 342"
  }, f), u && /* @__PURE__ */ a.createElement("title", null, u), /* @__PURE__ */ a.createElement("path", {
    fill: "#fecb00",
    d: "M0 0h513v114H0z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#34b233",
    d: "M0 114h513v114H0z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#ea2839",
    d: "M0 228h513v114H0z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#FFF",
    d: "M384.8 142.2h-98L256.5 49l-30.3 93.2h-98l79.3 57.6-30.3 93.2 79.3-56.9 79.3 56.9-30.3-93.2 79.3-57.6z"
  }));
}, _8 = function(i) {
  var u = i.title, f = d(i, ds);
  return /* @__PURE__ */ a.createElement("svg", s({
    xmlns: "http://www.w3.org/2000/svg",
    viewBox: "0 0 513 342"
  }, f), u && /* @__PURE__ */ a.createElement("title", null, u), /* @__PURE__ */ a.createElement("path", {
    fill: "#0066b2",
    d: "M0 0h513v342H0z"
  }), /* @__PURE__ */ a.createElement("path", {
    d: "M342 0h171v342H342zM0 0h171v342H0z",
    fill: "#da2031"
  }), /* @__PURE__ */ a.createElement("g", {
    fill: "#ffd300"
  }, /* @__PURE__ */ a.createElement("path", {
    d: "M108.3 166.3h14.8v74.2h-14.8zm-59.4 0h14.8v74.2H48.9z"
  }), /* @__PURE__ */ a.createElement("circle", {
    cx: 86,
    cy: 203.4,
    r: 14.8
  }), /* @__PURE__ */ a.createElement("path", {
    d: "M71.2 225.7h29.7v14.8H71.2zm0-59.4h29.7v14.8H71.2z"
  }), /* @__PURE__ */ a.createElement("circle", {
    cx: 86,
    cy: 144,
    r: 14.8
  }), /* @__PURE__ */ a.createElement("path", {
    d: "M76.3 123.9h19.5L86 108.3z"
  })));
}, U8 = function(i) {
  var u = i.title, f = d(i, os);
  return /* @__PURE__ */ a.createElement("svg", s({
    xmlns: "http://www.w3.org/2000/svg",
    viewBox: "0 85.333 512 341.333"
  }, f), u && /* @__PURE__ */ a.createElement("title", null, u), /* @__PURE__ */ a.createElement("path", {
    fill: "#00745a",
    d: "M0 85.331h512v341.337H0z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#FFF",
    d: "M285.682 248.878c-4.19 0-8.166.836-11.861 2.164 2.023-4.509 3.379-9.527 3.379-14.885 0-21.201-21.201-37.101-21.201-37.101s-21.201 15.901-21.201 37.101c0 5.357 1.356 10.375 3.379 14.885-3.693-1.328-7.671-2.164-11.861-2.164-21.201 0-37.101 21.201-37.101 21.201s15.901 21.201 37.101 21.201c12.429 0 23.031-7.286 29.682-13.315 6.65 6.03 17.251 13.315 29.682 13.315 21.201 0 37.101-21.201 37.101-21.201s-15.899-21.201-37.099-21.201z"
  }), /* @__PURE__ */ a.createElement("path", {
    d: "m256 152.111 4.38 13.476h14.171l-11.464 8.332 4.376 13.476-11.463-8.33-11.463 8.33 4.376-13.476-11.464-8.332h14.171zm-53.888 23.572 8.425 7.585 9.818-5.667-4.61 10.355 8.423 7.588-11.274-1.185-4.611 10.355-2.358-11.088-11.275-1.185 9.818-5.669zm-39.45 39.449 11.09 2.358 5.668-9.818 1.186 11.274 11.089 2.358-10.357 4.61 1.186 11.274-7.587-8.423-10.357 4.61 5.669-9.818zm147.227-39.449-8.425 7.585-9.818-5.667 4.611 10.355-8.425 7.588 11.275-1.185 4.61 10.355 2.359-11.088 11.275-1.185-9.818-5.669zm39.449 39.449-11.089 2.358-5.669-9.818-1.186 11.274-11.088 2.358 10.357 4.61-1.185 11.274 7.586-8.423 10.357 4.61-5.669-9.818z",
    fill: "#FFDA44"
  }), /* @__PURE__ */ a.createElement("path", {
    d: "M256 350.642c16.84 0 32.363-5.619 44.816-15.073h-89.63c12.451 9.454 27.974 15.073 44.814 15.073zm-68.023-44.522a74.138 74.138 0 0 0 8.85 15.073h118.345a74.13 74.13 0 0 0 8.85-15.073H187.977z",
    fill: "#FFF"
  }));
}, R8 = function(i) {
  var u = i.title, f = d(i, Es);
  return /* @__PURE__ */ a.createElement("svg", s({
    xmlns: "http://www.w3.org/2000/svg",
    viewBox: "0 0 750 500"
  }, f), u && /* @__PURE__ */ a.createElement("title", null, u), /* @__PURE__ */ a.createElement("path", {
    fill: "#0071BC",
    d: "M0 0h750v500H0V0z"
  }), /* @__PURE__ */ a.createElement("g", {
    fill: "#FFF",
    stroke: "#000",
    strokeWidth: 1.9
  }, /* @__PURE__ */ a.createElement("circle", {
    cx: 375,
    cy: 245,
    r: 225,
    fill: "#fff"
  }), /* @__PURE__ */ a.createElement("circle", {
    cx: 375,
    cy: 245,
    r: 165,
    fill: "#0071BC"
  })), /* @__PURE__ */ a.createElement("g", {
    stroke: "#000"
  }, /* @__PURE__ */ a.createElement("path", {
    fill: "#8C8A8C",
    strokeWidth: 1.9,
    d: "M444.7 450c.7 11-7.8 20-18.8 20H323.6c-11 0-19.5-9-19-20l16.7-325c.6-11 10-20 21-20h61.4c11 0 20.5 9 21.2 20l19.8 325z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#FFF",
    strokeWidth: 2,
    strokeLinejoin: "round",
    d: "m373 114 30 93h97l-78 56 29 92-78-56-78 57 30-93-79-57h97l30-92z"
  })));
}, j8 = function(i) {
  var u = i.title, f = d(i, zs);
  return /* @__PURE__ */ a.createElement("svg", s({
    viewBox: "0 0 900 600",
    xmlns: "http://www.w3.org/2000/svg"
  }, f), u && /* @__PURE__ */ a.createElement("title", null, u), /* @__PURE__ */ a.createElement("path", {
    fill: "#231f1e",
    d: "M0 0h900v600H0z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#00a650",
    d: "M0 0h900v300H0z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#ef1923",
    d: "m0 0 450 300L0 600z"
  }));
}, G8 = function(i) {
  var u = i.title, f = d(i, ps);
  return /* @__PURE__ */ a.createElement("svg", s({
    xmlns: "http://www.w3.org/2000/svg",
    viewBox: "0 100 513 342"
  }, f), u && /* @__PURE__ */ a.createElement("title", null, u), /* @__PURE__ */ a.createElement("path", {
    fill: "#006233",
    d: "M0 100h513v342H0z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#cd2a3e",
    d: "M0 100h513v46H0zm0 296h513v46H0z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#ffc400",
    d: "M256 298.851c-45.956 0-84.348-32.298-93.767-75.429A96.288 96.288 0 0 0 160 243.994c0 53.02 42.979 96 96 96s96-42.98 96-96c0-7.066-.785-13.942-2.233-20.572-9.419 43.131-47.811 75.429-93.767 75.429z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#ffc400",
    d: "m255.999 171.994 8.935 27.502h28.918l-23.394 16.996 8.936 27.503-23.395-16.998-23.394 16.998 8.937-27.503-23.394-16.996h28.915z"
  }));
}, Y8 = function(i) {
  var u = i.title, f = d(i, gs);
  return /* @__PURE__ */ a.createElement("svg", s({
    xmlns: "http://www.w3.org/2000/svg",
    viewBox: "0 0 513 342"
  }, f), u && /* @__PURE__ */ a.createElement("title", null, u), /* @__PURE__ */ a.createElement("path", {
    fill: "#1B4991",
    d: "M0 0h513v342H0z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#00A2B8",
    stroke: "#FFF",
    strokeWidth: 3,
    d: "M318.2 106.7v106.5c0 51.9 67.8 67.8 67.8 67.8s67.8-15.9 67.8-67.8V106.7H318.2z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#A35023",
    d: "M319.7 212.7c0 50.8 66.3 66.3 66.3 66.3s66.3-15.6 66.3-66.3H319.7z"
  }), /* @__PURE__ */ a.createElement("path", {
    d: "M415.1 155.1h-19.4v-19.4h-19.4v19.4H357v19.4h19.3v58.1h19.4v-58.1h19.4z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#FFF",
    d: "M256 0v30.6l-45.2 25.1H256V115h-59.1l59.1 32.8v22.9h-26.7l-73.5-40.9v40.9h-55.6v-48.6l-87.5 48.6H0v-30.6L45.2 115H0V55.7h59.1L0 22.8V0h26.7l73.5 40.8V0h55.6v48.6L243.3 0z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#D80027",
    d: "M144 0h-32v69.3H0v32h112v69.4h32v-69.4h112v-32H144z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#0052B4",
    d: "M155.8 115 256 170.7v-15.8L184.2 115z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#FFF",
    d: "M155.8 115 256 170.7v-15.8L184.2 115z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#D80027",
    d: "M155.8 115 256 170.7v-15.8L184.2 115zm-84 0L0 154.9v15.8L100.2 115z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#0052B4",
    d: "M100.2 55.6 0 0v15.7l71.8 39.9z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#FFF",
    d: "M100.2 55.6 0 0v15.7l71.8 39.9z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#D80027",
    d: "M100.2 55.6 0 0v15.7l71.8 39.9zm84 0L256 15.7V0L155.8 55.6z"
  }));
}, q8 = function(i) {
  var u = i.title, f = d(i, Ms);
  return /* @__PURE__ */ a.createElement("svg", s({
    xmlns: "http://www.w3.org/2000/svg",
    viewBox: "0 0 513 342"
  }, f), u && /* @__PURE__ */ a.createElement("title", null, u), /* @__PURE__ */ a.createElement("path", {
    fill: "#FFF",
    d: "M0 0h513v342H0z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#C31B28",
    d: "M256 0h256.5v342H256z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#ACABB1",
    stroke: "#C31B28",
    strokeWidth: 2,
    d: "M101.2 68.2V33H66v35.2H30.8v35.2H66v35.2h35.2v-35.2h35.2V68.2z"
  }));
}, Q8 = function(i) {
  var u = i.title, f = d(i, ws);
  return /* @__PURE__ */ a.createElement("svg", s({
    xmlns: "http://www.w3.org/2000/svg",
    viewBox: "0 85.333 512 341.333"
  }, f), u && /* @__PURE__ */ a.createElement("title", null, u), /* @__PURE__ */ a.createElement("path", {
    fill: "#f7ce00",
    d: "M0 85.331h512v341.326H0z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#e32737",
    d: "M0 85.331h512v85.337H0z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#191f6a",
    d: "M0 170.657h512v85.337H0z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#00a04e",
    d: "M0 341.331h512v85.337H0z"
  }));
}, X8 = function(i) {
  var u = i.title, f = d(i, xs);
  return /* @__PURE__ */ a.createElement("svg", s({
    xmlns: "http://www.w3.org/2000/svg",
    viewBox: "0 85.333 512 341.333"
  }, f), u && /* @__PURE__ */ a.createElement("title", null, u), /* @__PURE__ */ a.createElement("path", {
    fill: "#d01920",
    d: "M0 85.331h512v341.333H0z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#017c3b",
    d: "M144.696 174.375h222.609v163.25H144.696z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#FFF",
    d: "M283.484 304.226c-26.637 0-48.232-21.594-48.232-48.232s21.594-48.232 48.232-48.232c8.306 0 16.12 2.1 22.943 5.797-10.703-10.467-25.341-16.927-41.494-16.927-32.784 0-59.362 26.577-59.362 59.362s26.578 59.362 59.362 59.362c16.154 0 30.791-6.461 41.494-16.927-6.822 3.698-14.636 5.797-22.943 5.797z"
  }));
}, Z8 = function(i) {
  var u = i.title, f = d(i, Hs);
  return /* @__PURE__ */ a.createElement("svg", s({
    xmlns: "http://www.w3.org/2000/svg",
    viewBox: "0 0 512 342"
  }, f), u && /* @__PURE__ */ a.createElement("title", null, u), /* @__PURE__ */ a.createElement("path", {
    fill: "#272727",
    d: "M0 0h512v114H0z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#e40112",
    d: "M0 114h512v114H0z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#07893f",
    d: "M0 228h512v114H0z"
  }), /* @__PURE__ */ a.createElement("circle", {
    fill: "#e40112",
    cx: 256,
    cy: 125,
    r: 95
  }));
}, K8 = function(i) {
  var u = i.title, f = d(i, ys);
  return /* @__PURE__ */ a.createElement("svg", s({
    xmlns: "http://www.w3.org/2000/svg",
    viewBox: "0 0 513 342"
  }, f), u && /* @__PURE__ */ a.createElement("title", null, u), /* @__PURE__ */ a.createElement("path", {
    fill: "#dc2339",
    d: "M342 0H0v341.3h513V0z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#11865d",
    d: "M0 0h171v342H0z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#FFF",
    d: "M171 0h171v342H171z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#8C9157",
    d: "M195.8 171.2c0 21.6 11.5 41.7 30.3 52.5 5.8 3.4 13.2 1.4 16.6-4.4 3.4-5.8 1.4-13.2-4.4-16.6-11.3-6.5-18.2-18.5-18.2-31.5 0-6.7-5.4-12.1-12.1-12.1-6.7 0-12.2 5.4-12.2 12.1zm93.4 51.1c17.5-11.1 28-30.4 28-51.1 0-6.7-5.4-12.1-12.1-12.1s-12.1 5.4-12.1 12.1c0 12.4-6.3 24-16.8 30.7-5.7 3.5-7.5 10.9-4.1 16.7s10.9 7.5 16.7 4.1c0-.2.2-.3.4-.4z"
  }), /* @__PURE__ */ a.createElement("ellipse", {
    fill: "#C59262",
    cx: 256.5,
    cy: 159.1,
    rx: 24.3,
    ry: 36.4
  }));
}, J8 = function(i) {
  var u = i.title, f = d(i, Fs);
  return /* @__PURE__ */ a.createElement("svg", s({
    xmlns: "http://www.w3.org/2000/svg",
    viewBox: "0 0 513 342"
  }, f), u && /* @__PURE__ */ a.createElement("title", null, u), /* @__PURE__ */ a.createElement("path", {
    fill: "#FFF",
    d: "M0 0h513v342H0z"
  }), /* @__PURE__ */ a.createElement("path", {
    d: "M0 0h513v24.4H0zm0 48.9h513v24.4H0zm0 48.8h513v24.4H0zm0 48.9h513V171H0zm0 48.8h513v24.4H0zm0 48.9h513v24.4H0zm0 48.8h513v24.4H0z",
    fill: "#c00"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#006",
    d: "M0 0h256.5v195.4H0z"
  }), /* @__PURE__ */ a.createElement("path", {
    d: "M153.3 42.1C122.6 30.7 88.5 46.3 77.1 77s4.2 64.8 34.9 76.2c13.3 5 28 5 41.4 0-30.7 24.5-75.4 19.6-100-11.1s-19.6-75.4 11.1-100c26-20.7 62.9-20.7 88.8 0zm26.7 75-20.6 23.3 5.4-30.6-31-1.6 27.3-14.9L143 68l28.6 12 8.4-29.9 8.4 29.9L217 68l-18 25.4 27.3 14.9-31 1.6 5.4 30.6-20.7-23.4z",
    fill: "#fc0"
  }));
}, k8 = function(i) {
  var u = i.title, f = d(i, Ds);
  return /* @__PURE__ */ a.createElement("svg", s({
    xmlns: "http://www.w3.org/2000/svg",
    viewBox: "0 85.333 513 342"
  }, f), u && /* @__PURE__ */ a.createElement("title", null, u), /* @__PURE__ */ a.createElement("path", {
    fill: "#FFF",
    d: "M0 85.333h513v342H0z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#f4d900",
    d: "M0 323.333h513v104H0z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#006d66",
    d: "M0 85.333h513v104H0z"
  }), /* @__PURE__ */ a.createElement("path", {
    d: "M0 204.333h513v104H0z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#cb0f31",
    d: "M256 256.006 0 426.668V85.331z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#f4d900",
    d: "m83.477 195.132 15.107 46.498h48.894l-39.554 28.739 15.107 46.499-39.554-28.738-39.555 28.738 15.11-46.499-39.554-28.739H68.37z"
  }));
}, W8 = function(i) {
  var u = i.title, f = d(i, Ss);
  return /* @__PURE__ */ a.createElement("svg", s({
    xmlns: "http://www.w3.org/2000/svg",
    viewBox: "0 85.333 512 341.333"
  }, f), u && /* @__PURE__ */ a.createElement("title", null, u), /* @__PURE__ */ a.createElement("path", {
    fill: "#496E2D",
    d: "M0 85.337h512v341.326H0z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#0052B4",
    d: "M0 426.663V85.337h512"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#FFF",
    d: "M512 152.222V85.337H411.67L0 359.778v66.885h100.33z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#A2001D",
    d: "M512 85.337v40.125L60.193 426.663H0v-40.125L451.807 85.337z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#FFDA44",
    d: "m187.737 189.212-22.741 10.696 12.11 22.024-24.693-4.724-3.129 24.945-17.199-18.347-17.2 18.347-3.129-24.945-24.693 4.723 12.109-22.023-22.739-10.696 22.74-10.697-12.11-22.022 24.693 4.722 3.13-24.944 17.199 18.347 17.2-18.347 3.128 24.944 24.693-4.722-12.108 22.024z"
  }));
}, $8 = function(i) {
  var u = i.title, f = d(i, As);
  return /* @__PURE__ */ a.createElement("svg", s({
    xmlns: "http://www.w3.org/2000/svg",
    viewBox: "0 0 640 426.7"
  }, f), u && /* @__PURE__ */ a.createElement("title", null, u), /* @__PURE__ */ a.createElement("path", {
    fill: "#009543",
    d: "M0 293h640v133.7H0V293z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#ED4135",
    d: "M0 132h640v161.3H0V132z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#0035AD",
    d: "M0 0h640v133.3H0V0z"
  }), /* @__PURE__ */ a.createElement("circle", {
    fill: "#FAE600",
    stroke: "#000",
    strokeWidth: 5.3,
    cx: 240,
    cy: 213.3,
    r: 157.3
  }), /* @__PURE__ */ a.createElement("path", {
    d: "m307.9 354.5-31.8 12-36.1 4.2-37.8-2.4-35.9-15.5s28.6-29.3 40.7-33.5c8.6-3 22.4-3.7 22.4-3.7V122l-.3-66H246l-.5 67.5v191.7s15.3 1.2 22.1 4.1c11.3 4.9 40.3 35.2 40.3 35.2z"
  }), /* @__PURE__ */ a.createElement("path", {
    d: "m212.727 102.375 42.955-21.418 5.577 11.187-42.955 21.417z"
  }), /* @__PURE__ */ a.createElement("ellipse", {
    cx: 237.8,
    cy: 280.5,
    rx: 16.8,
    ry: 26.5
  }), /* @__PURE__ */ a.createElement("circle", {
    cx: 237.4,
    cy: 143.4,
    r: 20.9
  }), /* @__PURE__ */ a.createElement("ellipse", {
    cx: 238.4,
    cy: 211.1,
    rx: 19.9,
    ry: 12.1
  }), /* @__PURE__ */ a.createElement("path", {
    d: "M213.8 177h47v10h-47zm0 57h47v10h-47z"
  }));
}, I8 = function(i) {
  var u = i.title, f = d(i, Bs);
  return /* @__PURE__ */ a.createElement("svg", s({
    xmlns: "http://www.w3.org/2000/svg",
    viewBox: "0 85.333 512 341.333"
  }, f), u && /* @__PURE__ */ a.createElement("title", null, u), /* @__PURE__ */ a.createElement("path", {
    fill: "#FFF",
    d: "M0 85.337h512v341.326H0z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#e05206",
    d: "M0 85.337h512v113.775H0z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#0db02b",
    d: "M0 312.888h512v113.775H0z"
  }), /* @__PURE__ */ a.createElement("circle", {
    fill: "#e05206",
    cx: 256,
    cy: 256,
    r: 32
  }));
}, P8 = function(i) {
  var u = i.title, f = d(i, bs);
  return /* @__PURE__ */ a.createElement("svg", s({
    xmlns: "http://www.w3.org/2000/svg",
    viewBox: "0 85.5 513 342"
  }, f), u && /* @__PURE__ */ a.createElement("title", null, u), /* @__PURE__ */ a.createElement("path", {
    fill: "#FFF",
    d: "M0 85.5h513v342H0z"
  }), /* @__PURE__ */ a.createElement("path", {
    d: "M0 85.5h171v342H0zm342 0h171v342H342zm-38 226.495-48-152.001-48 152.001h36v39.999h24v-39.999z",
    fill: "#007b23"
  }));
}, eu = function(i) {
  var u = i.title, f = d(i, Ts);
  return /* @__PURE__ */ a.createElement("svg", s({
    xmlns: "http://www.w3.org/2000/svg",
    viewBox: "0 85.5 513 342"
  }, f), u && /* @__PURE__ */ a.createElement("title", null, u), /* @__PURE__ */ a.createElement("path", {
    fill: "#FFF",
    d: "M0 85.5h513v342H0z"
  }), /* @__PURE__ */ a.createElement("path", {
    d: "M0 85.5h171v342H0zm342 0h171v342H342z",
    fill: "#007b23"
  }));
}, tu = function(i) {
  var u = i.title, f = d(i, Ls);
  return /* @__PURE__ */ a.createElement("svg", s({
    xmlns: "http://www.w3.org/2000/svg",
    viewBox: "0 85.333 512 341.333"
  }, f), u && /* @__PURE__ */ a.createElement("title", null, u), /* @__PURE__ */ a.createElement("path", {
    fill: "#FFF",
    d: "M0 85.337h512v341.326H0z"
  }), /* @__PURE__ */ a.createElement("path", {
    d: "M0 85.337h512v113.775H0zm0 227.551h512v113.775H0z",
    fill: "#338AF3"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#FFDA44",
    d: "M256 214.447c-22.949 0-41.553 18.603-41.553 41.553S233.05 297.553 256 297.553c22.949 0 41.553-18.603 41.553-41.553S278.949 214.447 256 214.447zm0 65.298c-13.114 0-23.745-10.631-23.745-23.745s10.631-23.745 23.745-23.745 23.745 10.631 23.745 23.745-10.631 23.745-23.745 23.745z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#0052B4",
    d: "M276.563 261.936 256 256l-20.563 5.936-6.855 11.873h54.836z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#338AF3",
    d: "m256 226.32-13.709 23.744L256 256l13.709-5.936z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#6DA544",
    d: "M235.437 261.936h41.126l-6.854-11.872h-27.418z"
  }));
}, lu = function(i) {
  var u = i.title, f = d(i, Vs);
  return /* @__PURE__ */ a.createElement("svg", s({
    xmlns: "http://www.w3.org/2000/svg",
    viewBox: "0 0 513 342"
  }, f), u && /* @__PURE__ */ a.createElement("title", null, u), /* @__PURE__ */ a.createElement("path", {
    fill: "#FFF",
    d: "M0 114h513v114H0z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#cd1f2a",
    d: "M0 0h513v114H0z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#1d4185",
    d: "M0 228h513v114H0z"
  }));
}, au = function(i) {
  var u = i.title, f = d(i, Cs);
  return /* @__PURE__ */ a.createElement("svg", s({
    xmlns: "http://www.w3.org/2000/svg",
    viewBox: "0 85.333 512 341.333"
  }, f), u && /* @__PURE__ */ a.createElement("title", null, u), /* @__PURE__ */ a.createElement("path", {
    fill: "#D80027",
    d: "M0 85.334h512v341.337H0z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#FFF",
    d: "M512 295.883H202.195v130.783h-79.76V295.883H0v-79.772h122.435V85.329h79.76v130.782H512v61.218z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#2E52B2",
    d: "M512 234.666v42.663H183.652v149.337h-42.674V277.329H0v-42.663h140.978V85.329h42.674v149.337z"
  }));
}, nu = function(i) {
  var u = i.title, f = d(i, Os);
  return /* @__PURE__ */ a.createElement("svg", s({
    xmlns: "http://www.w3.org/2000/svg",
    viewBox: "0 85.333 512 341.333"
  }, f), u && /* @__PURE__ */ a.createElement("title", null, u), /* @__PURE__ */ a.createElement("path", {
    fill: "#FFF",
    d: "M0 85.337h512v341.326H0z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#0052B4",
    d: "M0 426.663V85.337l280.419 174.75H87.61l183.576 166.576z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#D80027",
    d: "M244.769 249.888 10.199 103.71v312.754h234.57L61.193 249.888z"
  }), /* @__PURE__ */ a.createElement("g", {
    fill: "#FFF"
  }, /* @__PURE__ */ a.createElement("path", {
    d: "m98.003 324.433-14.589-6.863 7.77-14.13-15.844 3.03-2.008-16.005-11.035 11.772-11.036-11.772-2.008 16.005-15.843-3.031 7.77 14.131-14.591 6.863 14.591 6.862-7.77 14.13 15.844-3.029 2.007 16.004 11.036-11.772L73.332 358.4l2.008-16.004 15.843 3.03-7.769-14.13zm-9.735-132.771-10.612-4.991 5.651-10.278-11.523 2.205-1.461-11.641-8.026 8.561-8.026-8.561-1.46 11.641-11.525-2.205 5.652 10.278-10.613 4.991 25.972 5.194z"
  }), /* @__PURE__ */ a.createElement("path", {
    d: "M93.462 191.662c0 17.212-13.953 31.165-31.165 31.165s-31.165-13.953-31.165-31.165"
  })));
}, cu = function(i) {
  var u = i.title, f = d(i, Ns);
  return /* @__PURE__ */ a.createElement("svg", s({
    xmlns: "http://www.w3.org/2000/svg",
    viewBox: "0 85.333 512 341.333"
  }, f), u && /* @__PURE__ */ a.createElement("title", null, u), /* @__PURE__ */ a.createElement("path", {
    fill: "#0052B4",
    d: "M0 85.337h512v341.326H0z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#FFDA44",
    d: "M0 239.994h512v32H0z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#FFF",
    d: "m174.802 341.329-19.124 8.996 10.184 18.521-20.767-3.973-2.631 20.978L128 370.422l-14.465 15.429-2.63-20.978-20.767 3.973 10.183-18.521-19.123-8.996 19.124-8.995-10.184-18.52 20.766 3.971 2.632-20.978L128 312.236l14.465-15.429 2.63 20.978 20.767-3.971-10.183 18.521z"
  }));
}, iu = function(i) {
  var u = i.title, f = d(i, _s);
  return /* @__PURE__ */ a.createElement("svg", s({
    xmlns: "http://www.w3.org/2000/svg",
    viewBox: "0 85.333 512 341.333"
  }, f), u && /* @__PURE__ */ a.createElement("title", null, u), /* @__PURE__ */ a.createElement("path", {
    fill: "#FFDA44",
    d: "M0 85.334h512v341.337H0z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#FFF",
    d: "M0 85.334h256v170.663H0z"
  }), /* @__PURE__ */ a.createElement("g", {
    fill: "#D80027"
  }, /* @__PURE__ */ a.createElement("path", {
    d: "M144 85.33h-32v69.333H0v32h112v69.334h32v-69.334h112v-32H144z"
  }), /* @__PURE__ */ a.createElement("path", {
    d: "M0 85.329v15.083l57.377 38.251H80z"
  })), /* @__PURE__ */ a.createElement("path", {
    d: "M0 107.951v30.712h46.069zm96-22.62v48.913L22.628 85.331z",
    fill: "#2E52B2"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#D80027",
    d: "M256 85.329v15.083l-57.377 38.251H176z"
  }), /* @__PURE__ */ a.createElement("path", {
    d: "M256 107.951v30.712h-46.069zm-96-22.62v48.913l73.372-48.913z",
    fill: "#2E52B2"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#D80027",
    d: "M0 85.329v15.083l57.377 38.251H80z"
  }), /* @__PURE__ */ a.createElement("path", {
    d: "M0 107.951v30.712h46.069zm96-22.62v48.913L22.628 85.331z",
    fill: "#2E52B2"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#D80027",
    d: "M256 85.329v15.083l-57.377 38.251H176z"
  }), /* @__PURE__ */ a.createElement("path", {
    d: "M256 107.951v30.712h-46.069zm-96-22.62v48.913l73.372-48.913z",
    fill: "#2E52B2"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#D80027",
    d: "M0 255.997v-15.082l57.377-38.252H80z"
  }), /* @__PURE__ */ a.createElement("path", {
    d: "M0 233.376v-30.713h46.069zm96 22.618v-48.912l-73.372 48.912z",
    fill: "#2E52B2"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#D80027",
    d: "M256 255.997v-15.082l-57.377-38.252H176z"
  }), /* @__PURE__ */ a.createElement("g", {
    fill: "#2E52B2"
  }, /* @__PURE__ */ a.createElement("path", {
    d: "M256 233.376v-30.713h-46.069zm-96 22.618v-48.912l73.372 48.912z"
  }), /* @__PURE__ */ a.createElement("circle", {
    cx: 128,
    cy: 170.66,
    r: 22.627
  })), /* @__PURE__ */ a.createElement("path", {
    d: "m128 154.663 3.97 12.222h12.854l-10.398 7.556 3.971 12.222L128 179.109l-10.398 7.554 3.972-12.222-10.398-7.556h12.853zm0 65.14 2.364 7.278h7.653l-6.19 4.498 2.364 7.278-6.191-4.498-6.191 4.498 2.364-7.278-6.19-4.498h7.652zm0-117.334 2.364 7.278h7.653l-6.19 4.498 2.364 7.278-6.191-4.498-6.191 4.498 2.364-7.278-6.19-4.498h7.652zm69.931 58.667 2.365 7.277h7.653l-6.192 4.499 2.365 7.279-6.191-4.498-6.191 4.498 2.365-7.279-6.191-4.499h7.652zm-139.862 0 2.364 7.277h7.653l-6.191 4.499 2.364 7.279-6.19-4.498-6.191 4.498 2.365-7.279-6.192-4.499h7.653z",
    fill: "#FFDA44"
  }));
}, uu = function(i) {
  var u = i.title, f = d(i, Us);
  return /* @__PURE__ */ a.createElement("svg", s({
    xmlns: "http://www.w3.org/2000/svg",
    viewBox: "0 85.333 512 341.333"
  }, f), u && /* @__PURE__ */ a.createElement("title", null, u), /* @__PURE__ */ a.createElement("path", {
    fill: "#0052B4",
    d: "M0 85.334h512v341.337H0z"
  }), /* @__PURE__ */ a.createElement("path", {
    d: "m425.301 233.745 3.388 10.428h10.963l-8.87 6.444 3.388 10.427-8.869-6.444-8.871 6.444 3.388-10.427-8.87-6.444h10.963zm-39.194 75.072 5.083 15.642h16.445l-13.305 9.667 5.082 15.64-13.305-9.667-13.305 9.667 5.083-15.64-13.305-9.667h16.445zm1.481-122.846 4.236 13.036h13.704l-11.088 8.054 4.235 13.034-11.087-8.056-11.088 8.056 4.235-13.034-11.087-8.054h13.704zm-37.712 47.32 5.082 15.641h16.446l-13.306 9.666 5.084 15.641-13.306-9.666-13.305 9.666 5.082-15.641-13.305-9.666h16.445z",
    fill: "#D80027"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#FFF",
    d: "M256.003 85.329v30.564l-45.178 25.088h45.178v59.359H196.89l59.113 32.846v22.806h-26.69l-73.484-40.826v40.826h-55.652v-48.573l-87.429 48.573H.003v-30.553l45.168-25.099H.003v-59.359h59.103L.003 108.147V85.329h26.68l73.494 40.838V85.329h55.652v48.573l87.43-48.573z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#D80027",
    d: "M144 85.33h-32v69.334H0v32h112v69.334h32v-69.334h112v-32H144z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#0052B4",
    d: "M155.826 200.344 256 255.998v-15.739l-71.847-39.915z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#FFF",
    d: "M155.826 200.344 256 255.998v-15.739l-71.847-39.915z"
  }), /* @__PURE__ */ a.createElement("path", {
    d: "M155.826 200.344 256 255.998v-15.739l-71.847-39.915zm-83.98 0L0 240.259v15.739l100.174-55.654z",
    fill: "#D80027"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#0052B4",
    d: "M100.174 140.983 0 85.33v15.738l71.847 39.915z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#FFF",
    d: "M100.174 140.983 0 85.33v15.738l71.847 39.915z"
  }), /* @__PURE__ */ a.createElement("path", {
    d: "M100.174 140.983 0 85.33v15.738l71.847 39.915zm83.98 0L256 101.068V85.33l-100.174 55.653z",
    fill: "#D80027"
  }));
}, ru = function(i) {
  var u = i.title, f = d(i, Rs);
  return /* @__PURE__ */ a.createElement("svg", s({
    xmlns: "http://www.w3.org/2000/svg",
    viewBox: "0 0 513 342"
  }, f), u && /* @__PURE__ */ a.createElement("title", null, u), /* @__PURE__ */ a.createElement("path", {
    fill: "#D80027",
    d: "M0 0h513v342H0z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#FFF",
    d: "M138 0h377v114H138z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#4A7C3A",
    d: "M138 230h377v114H138z"
  }), /* @__PURE__ */ a.createElement("g", {
    fill: "none",
    stroke: "#FFF",
    strokeWidth: 10,
    strokeMiterlimit: 2
  }, /* @__PURE__ */ a.createElement("path", {
    d: "M40.3 35.2s37.1 48 50.8 54.5c13.7 6.5 17.1 5.7 17.1 5.7"
  }), /* @__PURE__ */ a.createElement("path", {
    d: "M100 35.2S62.9 80.6 53.3 87.6c-9.6 7-18.7 7.8-18.7 7.8m17.1-30.1h37.9M70.7 35.2v30.1"
  })));
}, fu = function(i) {
  var u = i.title, f = d(i, js);
  return /* @__PURE__ */ a.createElement("svg", s({
    xmlns: "http://www.w3.org/2000/svg",
    viewBox: "0 85.333 512 341.333"
  }, f), u && /* @__PURE__ */ a.createElement("title", null, u), /* @__PURE__ */ a.createElement("path", {
    fill: "#FFF",
    d: "M0 85.337h512v341.326H0z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#0052B4",
    d: "M256 256v170.663H0V256h256z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#D80027",
    d: "M512 85.337V256H256V85.337h256z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#0052B4",
    d: "m128 123.034 11.824 36.389h38.263l-30.955 22.491 11.823 36.389L128 195.813l-30.955 22.49 11.823-36.389-30.955-22.491h38.263z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#D80027",
    d: "m384 293.697 11.824 36.389h38.263l-30.955 22.491 11.823 36.389L384 366.476l-30.955 22.49 11.823-36.389-30.955-22.491h38.263z"
  }));
}, hu = function(i) {
  var u = i.title, f = d(i, Gs);
  return /* @__PURE__ */ a.createElement("svg", s({
    xmlns: "http://www.w3.org/2000/svg",
    viewBox: "0 85.333 512 341.333"
  }, f), u && /* @__PURE__ */ a.createElement("title", null, u), /* @__PURE__ */ a.createElement("path", {
    fill: "#FFF",
    d: "M0 85.331h512v341.326H0z"
  }), /* @__PURE__ */ a.createElement("path", {
    d: "M0 85.331h170.663v341.337H0zm341.337 0H512v341.337H341.337z",
    fill: "#D80027"
  }));
}, mu = function(i) {
  var u = i.title, f = d(i, Ys);
  return /* @__PURE__ */ a.createElement("svg", s({
    xmlns: "http://www.w3.org/2000/svg",
    viewBox: "0 85.333 512 341.333"
  }, f), u && /* @__PURE__ */ a.createElement("title", null, u), /* @__PURE__ */ a.createElement("path", {
    fill: "#FFF",
    d: "M0 85.337h512v341.326H0z"
  }), /* @__PURE__ */ a.createElement("path", {
    d: "M0 85.337h512v113.775H0zm0 227.551h512v113.775H0z",
    fill: "#D80027"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#FFDA44",
    d: "M293.991 256c0 20.982-17.01 33.243-37.992 33.243S218.008 276.982 218.008 256s17.01-37.992 37.992-37.992 37.991 17.01 37.991 37.992z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#0052B4",
    d: "M293.991 256c0 20.982-17.01 37.992-37.992 37.992s-37.992-17.01-37.992-37.992"
  }), /* @__PURE__ */ a.createElement("path", {
    d: "M232.259 246.506h9.498v19h-9.498zm37.988 0h9.498v19h-9.498zm-19-14.247h9.498v33.243h-9.498z",
    fill: "#D80027"
  }));
}, vu = function(i) {
  var u = i.title, f = d(i, qs);
  return /* @__PURE__ */ a.createElement("svg", s({
    xmlns: "http://www.w3.org/2000/svg",
    viewBox: "0 85.333 512 341.333"
  }, f), u && /* @__PURE__ */ a.createElement("title", null, u), /* @__PURE__ */ a.createElement("path", {
    d: "M0 85.333h512V426.67H0z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#A2001D",
    d: "M0 85.33h512v341.332"
  }), /* @__PURE__ */ a.createElement("path", {
    d: "m204.631 326.488 2.762 8.504h8.941l-7.233 5.255 2.763 8.502-7.233-5.254-7.233 5.254 2.762-8.502-7.233-5.255h8.94zm-22.834-81.622 4.605 14.172h14.901l-12.056 8.757 4.605 14.172-12.055-8.759-12.055 8.759 4.606-14.172-12.056-8.757h14.901zm0 103.883 4.604 14.173h14.902l-12.057 8.757 4.606 14.173-12.055-8.759-12.055 8.759 4.604-14.173-12.054-8.757h14.899zm43.566-66.782 4.606 14.172h14.901l-12.056 8.758 4.605 14.172-12.056-8.759-12.054 8.759 4.606-14.172-12.056-8.758h14.901zm-87.133 0 4.606 14.172h14.899l-12.054 8.758 4.603 14.172-12.054-8.759-12.055 8.759 4.605-14.172-12.055-8.758h14.9z",
    fill: "#FFF"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#FFDA44",
    d: "M376.526 204.163c-7.628-7.628-17.538-12.133-28.189-12.908l31.88-24.795c-12.698-12.698-29.714-18.431-46.319-17.218a44.31 44.31 0 0 0-11.396-19.511l-12.593 25.186a18.47 18.47 0 0 0-3.148-4.197c-7.245-7.245-18.991-7.244-26.234 0s-7.245 18.99 0 26.234a18.536 18.536 0 0 0 4.197 3.148l-25.186 12.593a44.303 44.303 0 0 0 19.519 11.399c-1.217 16.606 4.511 33.619 17.209 46.317L324.12 214.6a11.053 11.053 0 0 1 7.807-3.202c2.973 0 5.768 1.158 7.87 3.26a11.055 11.055 0 0 1 3.26 7.87c0 2.974-1.158 5.768-3.26 7.87l10.494 10.494c4.905-4.905 7.607-11.428 7.607-18.364 0-5.675-1.81-11.071-5.153-15.534a29.492 29.492 0 0 1 13.288 7.662c11.573 11.572 11.573 30.403 0 41.975l10.494 10.494c8.409-8.409 13.039-19.59 13.039-31.481-.001-11.892-4.632-23.072-13.04-31.481z"
  }));
}, su = function(i) {
  var u = i.title, f = d(i, Qs);
  return /* @__PURE__ */ a.createElement("svg", s({
    xmlns: "http://www.w3.org/2000/svg",
    viewBox: "0 85.333 512 341.333"
  }, f), u && /* @__PURE__ */ a.createElement("title", null, u), /* @__PURE__ */ a.createElement("path", {
    fill: "#FFF",
    d: "M0 85.337h512v341.326H0z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#0052B4",
    d: "M512 85.337V256H256L0 85.337z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#D80027",
    d: "M512 256v170.663H0L256 256z"
  }), /* @__PURE__ */ a.createElement("path", {
    d: "m161.908 256-27.288-12.835 14.532-26.428-29.632 5.668-3.755-29.933-20.64 22.015-20.639-22.015-3.755 29.933-29.631-5.669 14.531 26.428L28.343 256l27.288 12.835L41.1 295.263l29.633-5.668 3.753 29.933 20.639-22.015 20.64 22.015 3.755-29.933 29.631 5.669-14.532-26.427zM21.789 117.607l9.081 12.696 14.879-4.714-9.268 12.56 9.081 12.694-14.809-4.932-9.268 12.559.117-15.608-14.809-4.934 14.88-4.712zm0 235.923 9.081 12.696 14.879-4.714-9.268 12.56 9.081 12.695-14.809-4.933-9.268 12.558.117-15.607-14.809-4.934 14.88-4.712zm188.606-117.961-9.081 12.695-14.879-4.713 9.268 12.559-9.081 12.696 14.809-4.933 9.268 12.558-.117-15.607 14.809-4.934-14.88-4.713z",
    fill: "#FFDA44"
  }));
}, du = function(i) {
  var u = i.title, f = d(i, Xs);
  return /* @__PURE__ */ a.createElement("svg", s({
    xmlns: "http://www.w3.org/2000/svg",
    viewBox: "0 85.333 512 341.333"
  }, f), u && /* @__PURE__ */ a.createElement("title", null, u), /* @__PURE__ */ a.createElement("g", {
    fill: "#FFF"
  }, /* @__PURE__ */ a.createElement("path", {
    d: "m393.508 170.516 17.512 18.875 23.363-10.821-12.541 22.487 17.513 18.876-25.263-4.978-12.539 22.488-3.073-25.564-25.263-4.978 23.363-10.82z"
  }), /* @__PURE__ */ a.createElement("path", {
    d: "M0 85.343h512v341.326H0z"
  })), /* @__PURE__ */ a.createElement("path", {
    fill: "#01411c",
    d: "M128 85.331h384v341.337H128z"
  }), /* @__PURE__ */ a.createElement("path", {
    d: "M361.909 298.793c-31.037 22.426-74.378 15.446-96.804-15.592-22.427-31.038-15.446-74.379 15.593-96.804 9.677-6.992 20.55-11.125 31.613-12.563-21.283-3.183-43.777 1.613-62.598 15.211-38.2 27.602-46.792 80.944-19.191 119.145 27.601 38.199 80.944 46.792 119.145 19.189 18.82-13.598 30.436-33.448 34.096-54.655-4.839 10.05-12.176 19.076-21.854 26.069zm-1.329-125.904 17.484 18.842 23.322-10.802-12.519 22.447 17.483 18.844-25.219-4.968-12.519 22.45-3.067-25.521-25.22-4.969 23.323-10.802z",
    fill: "#FFF"
  }));
}, ou = function(i) {
  var u = i.title, f = d(i, Zs);
  return /* @__PURE__ */ a.createElement("svg", s({
    xmlns: "http://www.w3.org/2000/svg",
    viewBox: "0 85.333 512 341.333"
  }, f), u && /* @__PURE__ */ a.createElement("title", null, u), /* @__PURE__ */ a.createElement("g", {
    fill: "#FFF"
  }, /* @__PURE__ */ a.createElement("path", {
    d: "M0 85.337h512v341.326H0z"
  }), /* @__PURE__ */ a.createElement("path", {
    d: "M0 85.337h512V256H0z"
  })), /* @__PURE__ */ a.createElement("path", {
    fill: "#D80027",
    d: "M0 256h512v170.663H0z"
  }));
}, Eu = function(i) {
  var u = i.title, f = d(i, Ks);
  return /* @__PURE__ */ a.createElement("svg", s({
    xmlns: "http://www.w3.org/2000/svg",
    viewBox: "0 0 513 342"
  }, f), u && /* @__PURE__ */ a.createElement("title", null, u), /* @__PURE__ */ a.createElement("path", {
    fill: "#FFF",
    d: "M0 0h513v342H0z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#00318A",
    d: "M0 0h171v342H0z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#D80027",
    d: "M342 0h171v342H342z"
  }));
}, zu = function(i) {
  var u = i.title, f = d(i, Js);
  return /* @__PURE__ */ a.createElement("svg", s({
    xmlns: "http://www.w3.org/2000/svg",
    viewBox: "0 85.333 512 341.333"
  }, f), u && /* @__PURE__ */ a.createElement("title", null, u), /* @__PURE__ */ a.createElement("path", {
    fill: "#0052B4",
    d: "M0 85.333h512V426.67H0z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#ACABB1",
    d: "M400.696 219.822 384 225.387l-16.696-5.565-5.565-13.913 5.565-13.913h33.392l-5.566 13.913z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#338AF3",
    d: "M345.043 219.822v61.217c0 29.821 38.957 38.957 38.957 38.957s38.957-9.137 38.957-38.957v-61.217h-77.914z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#6DA544",
    d: "M348.555 295.541C358.131 313.927 384 319.996 384 319.996s25.869-6.069 35.445-24.455L384 236.518l-35.445 59.023z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#FFDA44",
    d: "M422.957 280.421 384 219.822l-38.957 60.599v.618c0 5.518 1.337 10.328 3.512 14.503L384 240.405l35.445 55.137c2.175-4.175 3.512-8.983 3.512-14.503v-.618z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#FFF",
    d: "M256 85.333v30.553l-45.167 25.099H256v59.359h-59.103L256 233.179v22.817h-26.68l-73.494-40.826v40.826h-55.652v-48.573l-87.43 48.573H0v-30.554l45.167-25.098H0v-59.359h59.103L0 108.139V85.333h26.68l73.494 40.825V85.333h55.652v48.572l87.43-48.572z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#D80027",
    d: "M144 85.33h-32v69.332H0v32h112v69.334h32v-69.334h112v-32H144z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#0052B4",
    d: "M155.826 200.344 256 255.996v-15.737l-71.847-39.915z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#FFF",
    d: "M155.826 200.344 256 255.996v-15.737l-71.847-39.915z"
  }), /* @__PURE__ */ a.createElement("path", {
    d: "M155.826 200.344 256 255.996v-15.737l-71.847-39.915zm-83.98 0L0 240.259v15.737l100.174-55.652z",
    fill: "#D80027"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#0052B4",
    d: "M100.174 140.982 0 85.33v15.737l71.847 39.915z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#FFF",
    d: "M100.174 140.982 0 85.33v15.737l71.847 39.915z"
  }), /* @__PURE__ */ a.createElement("path", {
    d: "M100.174 140.982 0 85.33v15.737l71.847 39.915zm83.98 0L256 101.067V85.33l-100.174 55.652z",
    fill: "#D80027"
  }));
}, pu = function(i) {
  var u = i.title, f = d(i, ks);
  return /* @__PURE__ */ a.createElement("svg", s({
    xmlns: "http://www.w3.org/2000/svg",
    viewBox: "0 85.333 512 341.333"
  }, f), u && /* @__PURE__ */ a.createElement("title", null, u), /* @__PURE__ */ a.createElement("path", {
    fill: "#FFF",
    d: "M0 85.337h512v341.326H0z"
  }), /* @__PURE__ */ a.createElement("path", {
    d: "M0 85.337h512V153.6H0zm0 136.526h512v68.263H0zM0 358.4h512v68.263H0z",
    fill: "#D80027"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#0052B4",
    d: "M256 256.006 0 426.668V85.331z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#FFF",
    d: "m83.477 195.132 15.107 46.498h48.894l-39.554 28.739 15.107 46.499-39.554-28.738-39.555 28.738 15.11-46.499-39.554-28.739H68.37z"
  }));
}, gu = function(i) {
  var u = i.title, f = d(i, Ws);
  return /* @__PURE__ */ a.createElement("svg", s({
    xmlns: "http://www.w3.org/2000/svg",
    viewBox: "0 85.333 512 341.333"
  }, f), u && /* @__PURE__ */ a.createElement("title", null, u), /* @__PURE__ */ a.createElement("path", {
    fill: "#FFF",
    d: "M0 85.337h512v341.326H0z"
  }), /* @__PURE__ */ a.createElement("path", {
    d: "M0 85.337h512v113.775H0z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#268024",
    d: "M0 312.888h512v113.775H0z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#e4312b",
    d: "M256 256.006 0 426.668V85.331z"
  }));
}, Mu = function(i) {
  var u = i.title, f = d(i, $s);
  return /* @__PURE__ */ a.createElement("svg", s({
    xmlns: "http://www.w3.org/2000/svg",
    viewBox: "0 85.333 512 341.333"
  }, f), u && /* @__PURE__ */ a.createElement("title", null, u), /* @__PURE__ */ a.createElement("path", {
    fill: "#D80027",
    d: "M0 85.337h512v341.326H0z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#6DA544",
    d: "M196.641 85.337v341.326H0V85.337z"
  }), /* @__PURE__ */ a.createElement("circle", {
    fill: "#FFDA44",
    cx: 196.641,
    cy: 256,
    r: 64
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#D80027",
    d: "M160.638 224v40.001c0 19.882 16.118 36 36 36s36-16.118 36-36V224h-72z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#FFF",
    d: "M196.638 276c-6.617 0-12-5.383-12-12v-16h24.001v16c-.001 6.616-5.385 12-12.001 12z"
  }));
}, wu = function(i) {
  var u = i.title, f = d(i, Is);
  return /* @__PURE__ */ a.createElement("svg", s({
    xmlns: "http://www.w3.org/2000/svg",
    viewBox: "0 85.333 512 341.333"
  }, f), u && /* @__PURE__ */ a.createElement("title", null, u), /* @__PURE__ */ a.createElement("path", {
    fill: "#338AF3",
    d: "M0 85.331h512v341.337H0z"
  }), /* @__PURE__ */ a.createElement("circle", {
    fill: "#FFDA44",
    cx: 218.902,
    cy: 255.994,
    r: 74.207
  }));
}, xu = function(i) {
  var u = i.title, f = d(i, Ps);
  return /* @__PURE__ */ a.createElement("svg", s({
    xmlns: "http://www.w3.org/2000/svg",
    viewBox: "0 85.333 512 341.333"
  }, f), u && /* @__PURE__ */ a.createElement("title", null, u), /* @__PURE__ */ a.createElement("path", {
    fill: "#FFF",
    d: "M0 85.337h512v341.326H0z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#D80027",
    d: "M0 85.337h512v113.775H0z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#0052B4",
    d: "M0 312.888h512v113.775H0z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#6DA544",
    d: "m289.579 216.738-12.592 12.592c5.37 5.372 8.693 12.792 8.693 20.988 0 16.392-13.289 29.68-29.68 29.68-16.392 0-29.68-13.289-29.68-29.68 0-8.195 3.322-15.616 8.693-20.988l-12.592-12.592c-8.594 8.594-13.91 20.466-13.91 33.579 0 26.228 21.261 47.489 47.489 47.489s47.489-21.261 47.489-47.489c0-13.112-5.316-24.985-13.91-33.579z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#FFDA44",
    d: "m256 232.51 4.421 13.605h14.304l-11.573 8.408 4.421 13.604L256 259.719l-11.573 8.408 4.421-13.604-11.573-8.408h14.304z"
  }));
}, Hu = function(i) {
  var u = i.title, f = d(i, ed);
  return /* @__PURE__ */ a.createElement("svg", s({
    xmlns: "http://www.w3.org/2000/svg",
    viewBox: "0 0 513 342"
  }, f), u && /* @__PURE__ */ a.createElement("title", null, u), /* @__PURE__ */ a.createElement("path", {
    fill: "#751A46",
    d: "M0 0h512v342H0z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#FFF",
    d: "M0 0v342h150.3l37.7-19.6-37.7-18.9 37.7-19-37.7-18.9 37.7-19-37.7-19 37.7-18.9-37.7-19 37.7-19-37.7-18.9 37.7-19-37.7-18.9 37.7-19-37.7-19L188 57l-37.7-19L188 19.1 150.3 0z"
  }));
}, yu = function(i) {
  var u = i.title, f = d(i, td);
  return /* @__PURE__ */ a.createElement("svg", s({
    xmlns: "http://www.w3.org/2000/svg",
    viewBox: "0 0 513 342"
  }, f), u && /* @__PURE__ */ a.createElement("title", null, u), /* @__PURE__ */ a.createElement("path", {
    fill: "#FFF",
    d: "M0 0h513v342H0z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#00318A",
    d: "M0 0h171v342H0z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#D80027",
    d: "M342 0h171v342H342z"
  }));
}, Fu = function(i) {
  var u = i.title, f = d(i, ld);
  return /* @__PURE__ */ a.createElement("svg", s({
    xmlns: "http://www.w3.org/2000/svg",
    viewBox: "0 85.333 512 341.333"
  }, f), u && /* @__PURE__ */ a.createElement("title", null, u), /* @__PURE__ */ a.createElement("path", {
    fill: "#FFDA44",
    d: "M0 85.331h512v341.326H0z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#0052B4",
    d: "M0 85.331h170.663v341.337H0z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#D80027",
    d: "M341.337 85.331H512v341.337H341.337z"
  }));
}, Du = function(i) {
  var u = i.title, f = d(i, ad);
  return /* @__PURE__ */ a.createElement("svg", s({
    xmlns: "http://www.w3.org/2000/svg",
    viewBox: "0 85.333 512 341.333"
  }, f), u && /* @__PURE__ */ a.createElement("title", null, u), /* @__PURE__ */ a.createElement("path", {
    fill: "#0052B4",
    d: "M0 85.331h512v341.337H0z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#D80027",
    d: "M0 85.331h512v113.775H0z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#FFF",
    d: "M0 312.882h512v113.775H0z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#D80027",
    d: "M129.468 181.793v85.136c0 48.429 63.267 63.267 63.267 63.267S256 315.356 256 266.929v-85.136H129.468z"
  }), /* @__PURE__ */ a.createElement("path", {
    d: "M155.634 196.634h74.201v-29.681l-14.841 7.42-22.261-22.26-22.259 22.26-14.84-7.42zm85.526 82.148-48.231-48.231-48.231 48.231 15.741 15.74 32.49-32.49 32.49 32.49z",
    fill: "#FFDA44"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#FFF",
    d: "M241.16 233.734h-22.504a14.74 14.74 0 0 0 2.001-7.418c0-8.196-6.645-14.84-14.84-14.84-5.663 0-10.581 3.172-13.083 7.836-2.502-4.663-7.421-7.836-13.083-7.836-8.195 0-14.84 6.644-14.84 14.84 0 2.706.736 5.235 2.001 7.418h-22.114c0 8.196 7.139 14.84 15.334 14.84h-.494c0 8.196 6.644 14.84 14.84 14.84 0 7.257 5.211 13.286 12.094 14.576l-11.694 26.401a48.075 48.075 0 0 0 18.149 3.544 48.079 48.079 0 0 0 18.149-3.544l-11.694-26.401c6.883-1.29 12.094-7.319 12.094-14.576 8.196 0 14.84-6.644 14.84-14.84h-.494c8.199 0 15.338-6.644 15.338-14.84z"
  }));
}, Su = function(i) {
  var u = i.title, f = d(i, nd);
  return /* @__PURE__ */ a.createElement("svg", s({
    xmlns: "http://www.w3.org/2000/svg",
    viewBox: "0 85.333 512 341.333"
  }, f), u && /* @__PURE__ */ a.createElement("title", null, u), /* @__PURE__ */ a.createElement("path", {
    fill: "#FFF",
    d: "M0 85.33v341.332h512V85.33z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#0052B4",
    d: "M0 85.333h512V426.67H0z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#FFF",
    d: "M0 85.333h512v113.775H0z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#D80027",
    d: "M0 312.884h512v113.775H0z"
  }));
}, Au = function(i) {
  var u = i.title, f = d(i, cd);
  return /* @__PURE__ */ a.createElement("svg", s({
    xmlns: "http://www.w3.org/2000/svg",
    viewBox: "0 85.333 512 341.333"
  }, f), u && /* @__PURE__ */ a.createElement("title", null, u), /* @__PURE__ */ a.createElement("path", {
    fill: "#338AF3",
    d: "M0 85.331h512v341.337H0z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#FFDA44",
    d: "M0 255.994h512v81.619H0z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#496E2D",
    d: "M0 337.614h512v89.043H0z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#FFDA44",
    d: "m278.261 185.209 20.844 9.804-11.099 20.186 22.632-4.33 2.868 22.865 15.765-16.816 15.766 16.816 2.867-22.865 22.633 4.33-11.099-20.186 20.843-9.804-20.844-9.805 11.1-20.185-22.633 4.329-2.868-22.864-15.765 16.816-15.766-16.816-2.867 22.864-22.634-4.329 11.1 20.187z"
  }));
}, Bu = function(i) {
  var u = i.title, f = d(i, id);
  return /* @__PURE__ */ a.createElement("svg", s({
    xmlns: "http://www.w3.org/2000/svg",
    viewBox: "0 85.333 512 341.333"
  }, f), u && /* @__PURE__ */ a.createElement("title", null, u), /* @__PURE__ */ a.createElement("path", {
    fill: "#055e1c",
    d: "M0 85.333h512v341.333H0z"
  }), /* @__PURE__ */ a.createElement("g", {
    fill: "#FFF"
  }, /* @__PURE__ */ a.createElement("path", {
    d: "M183.548 289.386c0 12.295 9.731 22.261 21.736 22.261h65.208c0 10.244 8.11 18.551 18.114 18.551h21.736c10.004 0 18.114-8.306 18.114-18.551v-22.261H183.548zm146.716-107.595v51.942c0 8.183-6.5 14.84-14.491 14.84v22.261c19.976 0 36.226-16.643 36.226-37.101v-51.942h-21.735zm-155.773 51.943c0 8.183-6.5 14.84-14.491 14.84v22.261c19.976 0 36.226-16.643 36.226-37.101v-51.942H174.49v51.942z"
  }), /* @__PURE__ */ a.createElement("path", {
    d: "M297.661 181.788h21.736v51.942h-21.736zm-32.604 29.685c0 2.046-1.625 3.71-3.623 3.71-1.998 0-3.623-1.664-3.623-3.71v-29.682h-21.736v29.682c0 2.046-1.625 3.71-3.623 3.71s-3.623-1.664-3.623-3.71v-29.682h-21.736v29.682c0 14.32 11.376 25.971 25.358 25.971 5.385 0 10.38-1.733 14.491-4.677 4.11 2.944 9.106 4.677 14.491 4.677 1.084 0 2.15-.078 3.2-.215-1.54 6.499-7.255 11.345-14.068 11.345v22.261c19.976 0 36.226-16.643 36.226-37.101v-51.943h-21.736l.002 29.682z"
  }), /* @__PURE__ */ a.createElement("path", {
    d: "M207.093 248.57h32.601v22.261h-32.601z"
  })));
}, bu = function(i) {
  var u = i.title, f = d(i, ud);
  return /* @__PURE__ */ a.createElement("svg", s({
    xmlns: "http://www.w3.org/2000/svg",
    viewBox: "0 85.333 512 341.333"
  }, f), u && /* @__PURE__ */ a.createElement("title", null, u), /* @__PURE__ */ a.createElement("path", {
    fill: "#496E2D",
    d: "M0 85.334h512v341.337H0z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#0052B4",
    d: "M0 426.663V85.329h512"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#FFDA44",
    d: "M0 396.494v30.169h45.255L512 115.499v-30.17h-45.255z"
  }), /* @__PURE__ */ a.createElement("path", {
    d: "m85.688 108.787 5.12 15.756h16.566l-13.403 9.737 5.12 15.757-13.403-9.738-13.405 9.738 5.12-15.757L64 124.543h16.567zm84.624 0 5.121 15.756H192l-13.403 9.737 5.12 15.757-13.405-9.738-13.403 9.738 5.12-15.757-13.403-9.737h16.566zm-84.624 82.502 5.12 15.756h16.566l-13.403 9.738 5.12 15.757-13.403-9.739-13.405 9.739 5.12-15.757L64 207.045h16.567zm84.624 0 5.121 15.756H192l-13.403 9.738 5.12 15.757-13.405-9.739-13.403 9.739 5.12-15.757-13.403-9.738h16.566zM128 150.037l5.12 15.756h16.568l-13.405 9.738 5.12 15.758L128 181.55l-13.403 9.739 5.12-15.758-13.405-9.738h16.568z",
    fill: "#FFF"
  }));
}, Tu = function(i) {
  var u = i.title, f = d(i, rd);
  return /* @__PURE__ */ a.createElement("svg", s({
    xmlns: "http://www.w3.org/2000/svg",
    viewBox: "0 85.333 512 341.333"
  }, f), u && /* @__PURE__ */ a.createElement("title", null, u), /* @__PURE__ */ a.createElement("path", {
    fill: "#FFF",
    d: "M0 85.337h512v341.326H0z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#FFDA44",
    d: "M235.454 85.337 0 426.663 427.345 85.337z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#6DA544",
    d: "M512 329.393 0 426.663h512z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#D80027",
    d: "M512 85.337h-84.655L0 426.663l512-204.512z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#0052B4",
    d: "M0 85.337v341.326L235.454 85.337z"
  }));
}, Lu = function(i) {
  var u = i.title, f = d(i, fd);
  return /* @__PURE__ */ a.createElement("svg", s({
    xmlns: "http://www.w3.org/2000/svg",
    viewBox: "0 85.333 512 341.333"
  }, f), u && /* @__PURE__ */ a.createElement("title", null, u), /* @__PURE__ */ a.createElement("path", {
    fill: "#FFF",
    d: "M0 85.337h512v341.326H0z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#D80027",
    d: "M0 85.337h512v113.775H0z"
  }), /* @__PURE__ */ a.createElement("path", {
    d: "M0 312.888h512v113.775H0z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#496E2D",
    d: "M0 426.668V85.331l256 170.675z"
  }));
}, Vu = function(i) {
  var u = i.title, f = d(i, hd);
  return /* @__PURE__ */ a.createElement("svg", s({
    xmlns: "http://www.w3.org/2000/svg",
    viewBox: "0 85.333 512 341.333"
  }, f), u && /* @__PURE__ */ a.createElement("title", null, u), /* @__PURE__ */ a.createElement("path", {
    fill: "#0052B4",
    d: "M0 85.333h512V426.67H0z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#FFDA44",
    d: "M192 85.33h-64v138.666H0v64h128v138.666h64V287.996h320v-64H192z"
  }));
}, Cu = function(i) {
  var u = i.title, f = d(i, md);
  return /* @__PURE__ */ a.createElement("svg", s({
    xmlns: "http://www.w3.org/2000/svg",
    viewBox: "0 85.333 512 341.333"
  }, f), u && /* @__PURE__ */ a.createElement("title", null, u), /* @__PURE__ */ a.createElement("path", {
    fill: "#FFF",
    d: "M0 85.337h512v341.326H0z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#D80027",
    d: "M0 85.337h512V256H0z"
  }), /* @__PURE__ */ a.createElement("g", {
    fill: "#FFF"
  }, /* @__PURE__ */ a.createElement("path", {
    d: "M83.478 170.666c0-24.865 17.476-45.637 40.812-50.734a52.059 52.059 0 0 0-11.13-1.208c-28.688 0-51.942 23.254-51.942 51.941s23.255 51.942 51.942 51.942c3.822 0 7.543-.425 11.13-1.208-23.336-5.095-40.812-25.867-40.812-50.733zm66.783-48.231 3.684 11.337h11.921l-9.645 7.007 3.684 11.337-9.644-7.006-9.645 7.006 3.685-11.337-9.645-7.007h11.921z"
  }), /* @__PURE__ */ a.createElement("path", {
    d: "m121.344 144.696 3.683 11.337h11.921l-9.645 7.007 3.684 11.337-9.643-7.006-9.645 7.006 3.685-11.337-9.645-7.007h11.921zm57.834 0 3.684 11.337h11.921l-9.645 7.007 3.684 11.337-9.644-7.006-9.644 7.006 3.685-11.337-9.645-7.007h11.921zm-11.131 33.391 3.684 11.337h11.921l-9.644 7.007 3.684 11.337-9.645-7.006-9.643 7.006 3.684-11.337-9.644-7.007h11.92zm-35.573 0 3.683 11.337h11.921l-9.644 7.007 3.684 11.337-9.644-7.006-9.644 7.006 3.684-11.337-9.644-7.007h11.92z"
  })));
}, Ou = function(i) {
  var u = i.title, f = d(i, vd);
  return /* @__PURE__ */ a.createElement("svg", s({
    xmlns: "http://www.w3.org/2000/svg",
    viewBox: "0 0 512 341.3"
  }, f), u && /* @__PURE__ */ a.createElement("title", null, u), /* @__PURE__ */ a.createElement("path", {
    fill: "#0052B4",
    d: "M0 0h512v341.3H0z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#FFF",
    d: "M256 0v117.4h-46.1l46.1 30.7v22.6h-22.6L160 121.8v48.9H96v-48.9l-73.4 48.9H0v-22.6l46.1-30.7H0v-64h46.1L0 22.7V0h22.6L96 48.9V0h64v48.9L233.4 0z"
  }), /* @__PURE__ */ a.createElement("g", {
    fill: "#D80027"
  }, /* @__PURE__ */ a.createElement("path", {
    d: "M144 0h-32v69.4H0v32h112v69.3h32v-69.3h112v-32H144z"
  }), /* @__PURE__ */ a.createElement("path", {
    d: "M0 0v15.1l57.4 38.3H80zm256 0v15.1l-57.4 38.3H176z"
  })), /* @__PURE__ */ a.createElement("path", {
    fill: "#2E52B2",
    d: "M256 22.7v30.7h-46.1z"
  }), /* @__PURE__ */ a.createElement("path", {
    d: "M0 0v15.1l57.4 38.3H80zm256 0v15.1l-57.4 38.3H176z",
    fill: "#D80027"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#2E52B2",
    d: "M256 22.7v30.7h-46.1z"
  }), /* @__PURE__ */ a.createElement("path", {
    d: "M0 170.7v-15.1l57.4-38.2H80zm256 0v-15.1l-57.4-38.2H176z",
    fill: "#D80027"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#29DBFF",
    d: "M449 139.7c-.1 44.4-7.2 92.1-65 114.9-57.8-22.8-64.9-70.5-65-114.9h130z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#ffda44",
    d: "M449 139.7c0-16.7-.9-32.9-.5-47.1C426.9 83 398.4 81 383.9 81s-42.9 2-64.6 11.6c.4 14.2-.5 30.4-.5 47.1H449z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#BF521B",
    d: "m369.5 204.5.3 10.3-12.8.2 4.9 13.2h-17.3c-18-20.2-23.4-42.4-24.9-68.1l9.1-9.7 8.3 14.3 10.8-12.8 7 7.8.8 15.7 13.8 29.1z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#474747",
    d: "m436.6 192.5-8.6 18.3h-47l-29.2-19.5 25.7 10.5h25.1l3.7-6.5 7.2.2 2-3z"
  }));
}, Nu = function(i) {
  var u = i.title, f = d(i, sd);
  return /* @__PURE__ */ a.createElement("svg", s({
    xmlns: "http://www.w3.org/2000/svg",
    viewBox: "0 85.333 512 341.333"
  }, f), u && /* @__PURE__ */ a.createElement("title", null, u), /* @__PURE__ */ a.createElement("path", {
    fill: "#FFF",
    d: "M0 85.337h512v341.326H0z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#0052B4",
    d: "M0 199.112h512v113.775H0z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#D80027",
    d: "M0 312.888h512v113.775H0z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#FFF",
    d: "M233.606 196.639v14.837c0 34.081-44.522 44.522-44.522 44.522s-44.522-10.44-44.522-44.522v-14.837l-.145-44.188 89.043-.266.146 44.454z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#0052B4",
    d: "M233.606 196.639v14.837c0 34.081-44.522 44.522-44.522 44.522s-44.522-10.44-44.522-44.522v-14.837l-.145-44.188 89.043-.266.146 44.454z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#FFF",
    d: "M233.606 196.639v14.837c0 34.081-44.522 44.522-44.522 44.522s-44.522-10.44-44.522-44.522v-14.837l14.848 14.837 29.674-22.261 29.685 22.261 14.837-14.837z"
  }));
}, _u = function(i) {
  var u = i.title, f = d(i, dd);
  return /* @__PURE__ */ a.createElement("svg", s({
    xmlns: "http://www.w3.org/2000/svg",
    viewBox: "0 85.333 512 341.333"
  }, f), u && /* @__PURE__ */ a.createElement("title", null, u), /* @__PURE__ */ a.createElement("path", {
    fill: "#D80027",
    d: "M0 85.334h512v341.337H0z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#FFF",
    d: "M512 295.883H202.195v130.783h-79.76V295.883H0v-79.772h122.435V85.329h79.76v130.782H512v61.218z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#2E52B2",
    d: "M512 234.666v42.663H183.652v149.337h-42.674V277.329H0v-42.663h140.978V85.329h42.674v149.337z"
  }));
}, Uu = function(i) {
  var u = i.title, f = d(i, od);
  return /* @__PURE__ */ a.createElement("svg", s({
    xmlns: "http://www.w3.org/2000/svg",
    viewBox: "0 85.333 512 341.333"
  }, f), u && /* @__PURE__ */ a.createElement("title", null, u), /* @__PURE__ */ a.createElement("path", {
    fill: "#FFF",
    d: "M0 85.337h512v341.326H0z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#0052B4",
    d: "M0 196.641h512v118.717H0z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#D80027",
    d: "M0 315.359h512v111.304H0z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#FFF",
    d: "M129.468 181.799v85.136c0 48.429 63.267 63.267 63.267 63.267S256 315.362 256 266.935v-85.136H129.468z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#D80027",
    d: "M146.126 184.294v81.941c0 5.472 1.215 10.64 3.623 15.485h85.97c2.408-4.844 3.623-10.012 3.623-15.485v-81.941h-93.216z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#FFF",
    d: "M221.301 241.427h-21.425v-14.283h14.284v-14.283h-14.284v-14.284h-14.283v14.284h-14.282v14.283h14.282v14.283h-21.426v14.284h21.426v14.283h14.283v-14.283h21.425z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#0052B4",
    d: "M169.232 301.658c9.204 5.783 18.66 9.143 23.502 10.636 4.842-1.494 14.298-4.852 23.502-10.636 9.282-5.833 15.79-12.506 19.484-19.939a24.878 24.878 0 0 0-14.418-4.583c-1.956 0-3.856.232-5.682.657-3.871-8.796-12.658-14.94-22.884-14.94-10.227 0-19.013 6.144-22.884 14.94a25.048 25.048 0 0 0-5.682-.657 24.88 24.88 0 0 0-14.418 4.583c3.691 7.433 10.198 14.106 19.48 19.939z"
  }));
}, Ru = function(i) {
  var u = i.title, f = d(i, Ed);
  return /* @__PURE__ */ a.createElement("svg", s({
    xmlns: "http://www.w3.org/2000/svg",
    viewBox: "0 85.333 512 341.333"
  }, f), u && /* @__PURE__ */ a.createElement("title", null, u), /* @__PURE__ */ a.createElement("path", {
    fill: "#FFF",
    d: "M0 85.337h512v341.326H0z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#6DA544",
    d: "M0 85.337h512v113.775H0z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#338AF3",
    d: "M0 312.888h512v113.775H0z"
  }));
}, ju = function(i) {
  var u = i.title, f = d(i, zd);
  return /* @__PURE__ */ a.createElement("svg", s({
    xmlns: "http://www.w3.org/2000/svg",
    viewBox: "0 85.333 512 341.333"
  }, f), u && /* @__PURE__ */ a.createElement("title", null, u), /* @__PURE__ */ a.createElement("path", {
    fill: "#338AF3",
    d: "M0 85.34h512v341.326H0z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#FFF",
    d: "M512 85.334v166.69L0 256.175V85.334z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#6DA544",
    d: "M323.744 203.099 256 270.843l-67.744-67.744a81.156 81.156 0 0 0-13.879 45.483v22.261c0 35.744 23.097 66.193 55.148 77.213-4.277 8.385-3.556 18.848 2.712 26.671l24.258-19.439 24.258 19.439c6.342-7.915 7.011-18.534 2.564-26.968 31.614-11.261 54.308-41.485 54.308-76.916v-22.261a81.174 81.174 0 0 0-13.881-45.483z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#FFDA44",
    d: "M256 330.206c-32.732 0-59.362-26.63-59.362-59.362v-22.261c0-32.733 26.63-59.363 59.362-59.363s59.362 26.63 59.362 59.362v22.261c0 32.733-26.63 59.363-59.362 59.363z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#338AF3",
    d: "M293.101 270.843v-22.261c0-20.458-16.643-37.101-37.101-37.101s-37.101 16.643-37.101 37.101v22.261L256 278.264l37.101-7.421z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#6DA544",
    d: "M218.899 270.843c0 20.458 16.643 37.101 37.101 37.101s37.101-16.643 37.101-37.101h-74.202z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#FFDA44",
    d: "M300.522 189.22c0-12.295-9.966-22.261-22.261-22.261a22.173 22.173 0 0 0-14.84 5.672v-13.093h7.421v-14.84h-7.421v-7.421h-14.84v7.421h-7.421v14.84h7.421v13.093a22.177 22.177 0 0 0-14.841-5.672c-12.295 0-22.261 9.966-22.261 22.261 0 6.591 2.867 12.512 7.421 16.589v13.093h74.203v-13.093c4.552-4.077 7.419-9.997 7.419-16.589z"
  }));
}, Gu = function(i) {
  var u = i.title, f = d(i, pd);
  return /* @__PURE__ */ a.createElement("svg", s({
    xmlns: "http://www.w3.org/2000/svg",
    viewBox: "0 85.333 512 341.333"
  }, f), u && /* @__PURE__ */ a.createElement("title", null, u), /* @__PURE__ */ a.createElement("path", {
    fill: "#FFDA44",
    d: "M0 85.331h512v341.326H0z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#D80027",
    d: "M330.207 85.331H512v341.337H330.207z"
  }), /* @__PURE__ */ a.createElement("path", {
    d: "M0 85.331h181.793v341.337H0zm255.999 111.301 14.733 45.347h47.685l-38.576 28.029 14.734 45.348-38.576-28.026-38.577 28.026 14.737-45.348-38.576-28.029h47.681z",
    fill: "#496E2D"
  }));
}, Yu = function(i) {
  var u = i.title, f = d(i, gd);
  return /* @__PURE__ */ a.createElement("svg", s({
    xmlns: "http://www.w3.org/2000/svg",
    viewBox: "0 85.333 512 341.333"
  }, f), u && /* @__PURE__ */ a.createElement("title", null, u), /* @__PURE__ */ a.createElement("path", {
    fill: "#338AF3",
    d: "M0 85.331h512v341.337H0z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#FFF",
    d: "m256 157.273 22.663 69.748H352l-59.332 43.106 22.664 69.749L256 296.769l-59.332 43.107 22.664-69.749L160 227.021h73.337z"
  }));
}, qu = function(i) {
  var u = i.title, f = d(i, Md);
  return /* @__PURE__ */ a.createElement("svg", s({
    xmlns: "http://www.w3.org/2000/svg",
    viewBox: "0 85.333 512 341.333"
  }, f), u && /* @__PURE__ */ a.createElement("title", null, u), /* @__PURE__ */ a.createElement("path", {
    fill: "#FFF",
    d: "M0 85.331h512v341.337H0z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#A2001D",
    d: "M0 196.636h512v118.728H0z"
  }), /* @__PURE__ */ a.createElement("path", {
    d: "M0 352.462h512v74.207H0zM0 85.331h512v74.207H0z",
    fill: "#6DA544"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#FFDA44",
    d: "m256.742 218.003 9.43 29.021h30.518L272 264.963l9.431 29.023-24.689-17.937-24.689 17.937 9.431-29.023-24.69-17.939h30.518z"
  }));
}, Qu = function(i) {
  var u = i.title, f = d(i, wd);
  return /* @__PURE__ */ a.createElement("svg", s({
    xmlns: "http://www.w3.org/2000/svg",
    viewBox: "0 85.333 512 341.333"
  }, f), u && /* @__PURE__ */ a.createElement("title", null, u), /* @__PURE__ */ a.createElement("path", {
    fill: "#FFF",
    d: "M0 85.337h512v341.326H0z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#496E2D",
    d: "M0 322.783h512v103.88H0z"
  }), /* @__PURE__ */ a.createElement("path", {
    d: "M0 85.337h512v104.515H0z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#A2001D",
    d: "M0 210.877h512v89.656H0z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#0052B4",
    d: "M256 256.006 0 426.668V85.331z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#FFDA44",
    d: "m73.178 209.188 20.831 29.067 34.084-10.83-21.207 28.795 20.83 29.069-33.939-11.271-21.208 28.794.234-35.762-33.94-11.273 34.083-10.83z"
  }));
}, Xu = function(i) {
  var u = i.title, f = d(i, xd);
  return /* @__PURE__ */ a.createElement("svg", s({
    xmlns: "http://www.w3.org/2000/svg",
    viewBox: "0 85.333 512 341.333"
  }, f), u && /* @__PURE__ */ a.createElement("title", null, u), /* @__PURE__ */ a.createElement("path", {
    fill: "#FFDA44",
    d: "M0 85.337h512v341.326H0z"
  }), /* @__PURE__ */ a.createElement("path", {
    d: "M0 85.337h512v113.775H0zm0 227.551h512v113.775H0z",
    fill: "#6DA544"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#D80027",
    d: "M256 256.006 0 426.668V85.331z"
  }), /* @__PURE__ */ a.createElement("path", {
    d: "m302.049 226.318 7.368 22.674h23.842l-19.288 14.016 7.366 22.674-19.288-14.015-19.287 14.015 7.366-22.674-19.288-14.016h23.842zm74.203 0 7.367 22.674h23.842l-19.288 14.016 7.367 22.674-19.288-14.015-19.288 14.015 7.367-22.674-19.288-14.016h23.842z"
  }));
}, Zu = function(i) {
  var u = i.title, f = d(i, Hd);
  return /* @__PURE__ */ a.createElement("svg", s({
    xmlns: "http://www.w3.org/2000/svg",
    viewBox: "0 85.333 512 341.333"
  }, f), u && /* @__PURE__ */ a.createElement("title", null, u), /* @__PURE__ */ a.createElement("path", {
    fill: "#FFF",
    d: "M0 85.337h512v341.326H0z"
  }), /* @__PURE__ */ a.createElement("path", {
    d: "M0 85.337h512v113.775H0zm0 227.551h512v113.775H0z",
    fill: "#0052B4"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#FFDA44",
    d: "M228.582 261.936 256 214.447l27.418 47.489z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#6DA544",
    d: "M291.616 277.616 256 295.425l-35.616-17.809v-23.744h71.232z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#FFDA44",
    d: "m289.579 216.485-12.592 12.592c5.37 5.372 8.693 12.791 8.693 20.988 0 16.392-13.289 29.68-29.68 29.68-16.392 0-29.68-13.289-29.68-29.68 0-8.195 3.322-15.616 8.693-20.988l-12.592-12.592c-8.594 8.594-13.91 20.466-13.91 33.579 0 26.228 21.261 47.489 47.489 47.489s47.489-21.261 47.489-47.489c0-13.114-5.316-24.987-13.91-33.579z"
  }));
}, Ku = function(i) {
  var u = i.title, f = d(i, yd);
  return /* @__PURE__ */ a.createElement("svg", s({
    xmlns: "http://www.w3.org/2000/svg",
    viewBox: "0 85.333 512 341.333"
  }, f), u && /* @__PURE__ */ a.createElement("title", null, u), /* @__PURE__ */ a.createElement("path", {
    fill: "#0052B4",
    d: "M0 85.337h512v341.326H0z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#D80027",
    d: "M512 85.331v166.69L0 256.173V85.331z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#FFF",
    d: "M256 256.006 0 426.668V85.331z"
  }), /* @__PURE__ */ a.createElement("g", {
    fill: "#FFDA44"
  }, /* @__PURE__ */ a.createElement("path", {
    d: "M59.621 256a59.546 59.546 0 0 0-.193 4.57c0 32.821 26.607 59.429 59.429 59.429s59.429-26.607 59.429-59.429c0-1.539-.078-3.061-.193-4.57H59.621z"
  }), /* @__PURE__ */ a.createElement("circle", {
    cx: 118.862,
    cy: 210.287,
    r: 18.286
  })), /* @__PURE__ */ a.createElement("path", {
    fill: "#D80027",
    d: "M77.715 205.714v59.429c0 31.494 41.144 41.143 41.144 41.143s41.144-9.649 41.144-41.143v-59.429H77.715z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#338AF3",
    d: "M118.877 287.148c-7.632-2.746-22.876-9.767-22.876-22.006v-41.144h45.715v41.144c-.001 12.28-15.244 19.283-22.839 22.006z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#F3F3F3",
    d: "M128.001 246.856v-9.142l-9.144-4.571-9.142 4.571v9.142l-4.571 4.573v18.285h27.428v-18.285z"
  }));
}, Ju = function(i) {
  var u = i.title, f = d(i, Fd);
  return /* @__PURE__ */ a.createElement("svg", s({
    xmlns: "http://www.w3.org/2000/svg",
    viewBox: "0 0 900 600"
  }, f), u && /* @__PURE__ */ a.createElement("title", null, u), /* @__PURE__ */ a.createElement("path", {
    d: "M0 0h900v600H0z",
    fill: "#0f0f0f"
  }), /* @__PURE__ */ a.createElement("path", {
    d: "M0 0h900v400H0z",
    fill: "#fff"
  }), /* @__PURE__ */ a.createElement("path", {
    d: "M0 0h900v200H0z",
    fill: "#047a3f"
  }), /* @__PURE__ */ a.createElement("path", {
    d: "m176.26 375 48.738-150 48.738 150-127.6-92.705h157.72M626.256 375l48.738-150 48.738 150-127.6-92.705h157.72M401.252 375l48.738-150 48.738 150-127.6-92.705h157.72",
    fill: "#ce1126"
  }));
}, ku = function(i) {
  var u = i.title, f = d(i, Dd);
  return /* @__PURE__ */ a.createElement("svg", s({
    xmlns: "http://www.w3.org/2000/svg",
    viewBox: "0 0 900 600"
  }, f), u && /* @__PURE__ */ a.createElement("title", null, u), /* @__PURE__ */ a.createElement("path", {
    fill: "#2B5DEA",
    d: "M0 0h900v600H0V0z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#FFDF29",
    d: "M0 100h900v400H0V100z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#D70000",
    d: "M0 150h900v300H0V150z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#FFF",
    d: "M450 171.4v257.2c114.3 0 171.4-85.7 214.3-128.6-42.9-42.9-100-128.6-214.3-128.6z"
  }), /* @__PURE__ */ a.createElement("path", {
    d: "M450 171.4c-100 0-171.4 85.7-214.3 128.6C278.6 342.9 350 428.6 450 428.6V171.4z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#FFF",
    d: "M346.3 254.3h21v91.3h-21zm51.9 0h21v91.3h-21z"
  }), /* @__PURE__ */ a.createElement("path", {
    d: "M477.8 254.3h21v91.3h-21zm51.9 0h21v91.3h-21z"
  }));
}, Wu = function(i) {
  var u = i.title, f = d(i, Sd);
  return /* @__PURE__ */ a.createElement("svg", s({
    xmlns: "http://www.w3.org/2000/svg",
    viewBox: "0 0 512 341.3"
  }, f), u && /* @__PURE__ */ a.createElement("title", null, u), /* @__PURE__ */ a.createElement("path", {
    fill: "#0052B4",
    d: "M0 0h512v341.3H0z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#FFF",
    d: "M256 0v117.4h-46.1l46.1 30.7v22.6h-22.6L160 121.8v48.9H96v-48.9l-73.4 48.9H0v-22.6l46.1-30.7H0v-64h46.1L0 22.7V0h22.6L96 48.9V0h64v48.9L233.4 0z"
  }), /* @__PURE__ */ a.createElement("g", {
    fill: "#D80027"
  }, /* @__PURE__ */ a.createElement("path", {
    d: "M144 0h-32v69.4H0v32h112v69.3h32v-69.3h112v-32H144z"
  }), /* @__PURE__ */ a.createElement("path", {
    d: "M0 0v15.1l57.4 38.3H80zm256 0v15.1l-57.4 38.3H176z"
  })), /* @__PURE__ */ a.createElement("path", {
    fill: "#2E52B2",
    d: "M256 22.7v30.7h-46.1z"
  }), /* @__PURE__ */ a.createElement("path", {
    d: "M0 0v15.1l57.4 38.3H80zm256 0v15.1l-57.4 38.3H176z",
    fill: "#D80027"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#2E52B2",
    d: "M256 22.7v30.7h-46.1z"
  }), /* @__PURE__ */ a.createElement("path", {
    d: "M0 170.7v-15.1l57.4-38.2H80zm256 0v-15.1l-57.4-38.2H176z",
    fill: "#D80027"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#29DBFF",
    d: "M448.9 169.5c0 9.6-.3 29.6-1.4 39.2-4.1 34.9-23.5 68.8-62.1 85.9-45.3-17.9-60.8-51-64.9-85.9-1.1-9.6-1.5-19.4-1.5-29l.3-47.1h129.2l.4 36.9z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#FFF",
    d: "M447.5 208.7c-.2 1.6-.4 3.3-.6 4.9-4.8 33.1-22.9 65.4-61.5 81-43.2-17-59.4-47.9-64.2-81-.2-1.6-.4-3.2-.6-4.9"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#29DBFF",
    d: "m385.4 251.7-22.9-43h45.8z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#FFF",
    d: "m385.4 165.8-22.9 42.9h45.8z"
  }), /* @__PURE__ */ a.createElement("ellipse", {
    fill: "#FFBE57",
    cx: 474.8,
    cy: 236.8,
    rx: 16.8,
    ry: 43.3
  }), /* @__PURE__ */ a.createElement("ellipse", {
    fill: "#FFBE57",
    cx: 295.3,
    cy: 236.8,
    rx: 16.8,
    ry: 43.3
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#FFF",
    d: "m385.4 31.5-33.4 44h68.5zM315.5 280s33.8 29.5 69.9 29.5 67.1-29.5 67.1-29.5l8.5 14.6S439.2 326 385.4 326 307 294.6 307 294.6l8.5-14.6z"
  }), /* @__PURE__ */ a.createElement("ellipse", {
    fill: "#A5A5A5",
    cx: 386.3,
    cy: 104.3,
    rx: 34.3,
    ry: 23.3
  }));
}, $u = function(i) {
  var u = i.title, f = d(i, Ad);
  return /* @__PURE__ */ a.createElement("svg", s({
    xmlns: "http://www.w3.org/2000/svg",
    viewBox: "0 85.333 512 341.333"
  }, f), u && /* @__PURE__ */ a.createElement("title", null, u), /* @__PURE__ */ a.createElement("path", {
    fill: "#0052B4",
    d: "M0 85.333h512V426.67H0z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#FFDA44",
    d: "M332.058 191.996v78.221c0 38.103 51.942 49.779 51.942 49.779s51.942-11.675 51.942-49.779v-78.221H332.058z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#FF9811",
    d: "M376.579 220.44c0 7.855-6.644 28.445-14.84 28.445s-14.84-20.589-14.84-28.445c0-7.856 14.84-14.222 14.84-14.222s14.84 6.367 14.84 14.222z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#A2001D",
    d: "M415.961 235.93c2.394-5.6 4.257-13.785 4.257-17.86 0-6.546-8.904-11.852-8.904-11.852s-8.904 5.306-8.904 11.852c0 4.075 1.862 12.26 4.257 17.86l-5.141 11.123a26.898 26.898 0 0 0 9.788 1.831c3.463 0 6.766-.654 9.788-1.831l-5.141-11.123z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#6DA544",
    d: "M372.87 270.217s-7.421 14.222-7.421 28.445h37.101c0-14.222-7.421-28.445-7.421-28.445l-11.13-7.111-11.129 7.111z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#D80027",
    d: "M395.13 270.217v-3.555c0-5.891-4.983-10.666-11.13-10.666-6.147 0-11.13 4.776-11.13 10.666v3.555h22.26z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#FFF",
    d: "M256 85.333v30.553l-45.167 25.099H256v59.359h-59.103L256 233.179v22.817h-26.68l-73.494-40.826v40.826h-55.652v-48.573l-87.43 48.573H0v-30.554l45.167-25.098H0v-59.359h59.103L0 108.139V85.333h26.68l73.494 40.825V85.333h55.652v48.572l87.43-48.572z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#D80027",
    d: "M144 85.33h-32v69.332H0v32h112v69.334h32v-69.334h112v-32H144z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#0052B4",
    d: "M155.826 200.344 256 255.996v-15.737l-71.847-39.915z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#FFF",
    d: "M155.826 200.344 256 255.996v-15.737l-71.847-39.915z"
  }), /* @__PURE__ */ a.createElement("path", {
    d: "M155.826 200.344 256 255.996v-15.737l-71.847-39.915zm-83.98 0L0 240.259v15.737l100.174-55.652z",
    fill: "#D80027"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#0052B4",
    d: "M100.174 140.982 0 85.33v15.737l71.847 39.915z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#FFF",
    d: "M100.174 140.982 0 85.33v15.737l71.847 39.915z"
  }), /* @__PURE__ */ a.createElement("path", {
    d: "M100.174 140.982 0 85.33v15.737l71.847 39.915zm83.98 0L256 101.067V85.33l-100.174 55.652z",
    fill: "#D80027"
  }));
}, Iu = function(i) {
  var u = i.title, f = d(i, Bd);
  return /* @__PURE__ */ a.createElement("svg", s({
    xmlns: "http://www.w3.org/2000/svg",
    viewBox: "0 85.333 512 341.333"
  }, f), u && /* @__PURE__ */ a.createElement("title", null, u), /* @__PURE__ */ a.createElement("path", {
    fill: "#FFDA44",
    d: "M0 85.331h512v341.337H0z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#0052B4",
    d: "M0 85.331h170.663v341.337H0z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#D80027",
    d: "M341.337 85.331H512v341.337H341.337z"
  }));
}, Pu = function(i) {
  var u = i.title, f = d(i, bd);
  return /* @__PURE__ */ a.createElement("svg", s({
    xmlns: "http://www.w3.org/2000/svg",
    viewBox: "0 0 22.5 15"
  }, f), u && /* @__PURE__ */ a.createElement("title", null, u), /* @__PURE__ */ a.createElement("path", {
    fill: "#FFF",
    d: "M0 0h21v15H0V0z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#0A388A",
    d: "M0 0h22.5v15H0V0z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#FFF",
    d: "M0 0h11.3v7.5H0V0z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#D80027",
    d: "M7.1 0h3.6v7H7.1V0z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#0A388A",
    d: "M0 0h3.6v7H0V0z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#FFF",
    d: "M3.6 0h3.6v7H3.6V0zm10.9 6h5L19 7h-1.5v.5h1l-.5 1h-.5V10h-1V7H15l-.5-1zm4 2.5 1 1.5h-2l1-1.5zm-3 0 1 1.5h-2l1-1.5zm1.5 3L16 10h2l-1 1.5zM20 8c-.3 0-.5-.2-.5-.5s.2-.5.5-.5.5.2.5.5-.2.5-.5.5zm-6 0c-.3 0-.5-.2-.5-.5s.2-.5.5-.5.5.2.5.5-.2.5-.5.5zm1 4c-.3 0-.5-.2-.5-.5s.2-.5.5-.5.5.2.5.5-.2.5-.5.5zm4 0c-.3 0-.5-.2-.5-.5s.2-.5.5-.5.5.2.5.5-.2.5-.5.5zm-2 1c-.3 0-.5-.2-.5-.5s.2-.5.5-.5.5.2.5.5-.2.5-.5.5z"
  }));
}, e9 = function(i) {
  var u = i.title, f = d(i, Td);
  return /* @__PURE__ */ a.createElement("svg", s({
    xmlns: "http://www.w3.org/2000/svg",
    viewBox: "0 85.333 512 341.333"
  }, f), u && /* @__PURE__ */ a.createElement("title", null, u), /* @__PURE__ */ a.createElement("path", {
    fill: "#FFDA44",
    d: "M0 85.337h512v341.326H0z"
  }), /* @__PURE__ */ a.createElement("path", {
    d: "M0 85.337h512V153.6H0zM0 358.4h512v68.263H0zm0-136.537h512v68.263H0z",
    fill: "#496E2D"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#D80027",
    d: "M0 85.337h204.054v204.054H0z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#FFF",
    d: "m102.026 133.938 13.26 40.812h42.916l-34.718 25.226 13.26 40.814-34.718-25.224-34.719 25.224 13.263-40.814-34.718-25.226h42.913z"
  }));
}, t9 = function(i) {
  var u = i.title, f = d(i, Ld);
  return /* @__PURE__ */ a.createElement("svg", s({
    xmlns: "http://www.w3.org/2000/svg",
    viewBox: "0 85.333 512 341.333"
  }, f), u && /* @__PURE__ */ a.createElement("title", null, u), /* @__PURE__ */ a.createElement("path", {
    fill: "#FFF",
    d: "M0 85.334h512V426.66H0z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#0052B4",
    d: "M0 194.056h512v123.882H0z"
  }), /* @__PURE__ */ a.createElement("path", {
    d: "M0 85.334h512v54.522H0zm0 286.809h512v54.522H0z",
    fill: "#D80027"
  }));
}, l9 = function(i) {
  var u = i.title, f = d(i, Vd);
  return /* @__PURE__ */ a.createElement("svg", s({
    xmlns: "http://www.w3.org/2000/svg",
    viewBox: "0 85.333 512 341.333"
  }, f), u && /* @__PURE__ */ a.createElement("title", null, u), /* @__PURE__ */ a.createElement("path", {
    fill: "#FFF",
    d: "M0 85.337h512v341.326H0z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#D80027",
    d: "M0 85.337h512v113.775H0z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#6DA544",
    d: "M0 312.888h512v113.775H0z"
  }), /* @__PURE__ */ a.createElement("path", {
    d: "M226.318 300.522h59.364v-25.23l-11.873 5.937L256 263.421l-17.809 17.808-11.873-5.937zm-33.963-29.682 2.763 8.504h8.94l-7.233 5.255 2.763 8.502-7.233-5.255-7.234 5.255 2.763-8.502-7.233-5.255h8.94zm8.566-29.68 2.763 8.502h8.94l-7.233 5.255 2.763 8.504-7.233-5.256-7.234 5.256 2.763-8.504-7.233-5.255h8.94zm24.488-22.261 2.763 8.502h8.94l-7.233 5.255 2.763 8.504-7.233-5.256-7.234 5.256 2.763-8.504-7.233-5.255h8.94zm94.236 51.941-2.763 8.504h-8.94l7.233 5.255-2.763 8.502 7.233-5.255 7.234 5.255-2.763-8.502 7.233-5.255h-8.94zm-8.566-29.68-2.763 8.502h-8.94l7.233 5.255-2.763 8.504 7.233-5.256 7.234 5.256-2.763-8.504 7.233-5.255h-8.94zm-24.488-22.261-2.763 8.502h-8.94l7.233 5.255-2.763 8.504 7.233-5.256 7.234 5.256-2.763-8.504 7.233-5.255h-8.94zM256 207.767l2.763 8.503h8.941l-7.234 5.256 2.763 8.502-7.233-5.254-7.233 5.254 2.763-8.502-7.234-5.256h8.941z",
    fill: "#FFDA44"
  }));
}, a9 = function(i) {
  var u = i.title, f = d(i, Cd);
  return /* @__PURE__ */ a.createElement("svg", s({
    xmlns: "http://www.w3.org/2000/svg",
    viewBox: "0 85.333 512 341.333"
  }, f), u && /* @__PURE__ */ a.createElement("title", null, u), /* @__PURE__ */ a.createElement("path", {
    fill: "#0052B4",
    d: "M0 85.337h512v341.326H0z"
  }), /* @__PURE__ */ a.createElement("path", {
    d: "m210.715 174.377 3.684 11.338h11.919l-9.643 7.006 3.684 11.337-9.644-7.007-9.644 7.007 3.684-11.337-9.645-7.006h11.921zm-93.899 107.594 4.605 14.172h14.9l-12.055 8.757 4.606 14.173-12.056-8.76-12.055 8.76 4.604-14.173-12.054-8.757h14.899zm27.88-162.292 4.604 14.172h14.901l-12.056 8.758 4.606 14.171-12.055-8.759-12.056 8.759 4.606-14.171-12.055-8.758h14.9zm-75.157 55.652 4.604 14.172h14.9l-12.055 8.758 4.606 14.171-12.055-8.758-12.056 8.758 4.606-14.171-12.055-8.758h14.9z",
    fill: "#FFF"
  }), /* @__PURE__ */ a.createElement("path", {
    d: "M483.386 354.503H117.801s109.234-88.562 310.593-220.938c.001 0-88.443 128.935 54.992 220.938zm-365.585 11.586c-11.177 0-11.195 17.37 0 17.37h365.585c11.177 0 11.195-17.37 0-17.37H117.801z",
    fill: "#FFDA44"
  }));
}, n9 = function(i) {
  var u = i.title, f = d(i, Od);
  return /* @__PURE__ */ a.createElement("svg", s({
    xmlns: "http://www.w3.org/2000/svg",
    viewBox: "0 85.333 512 341.333"
  }, f), u && /* @__PURE__ */ a.createElement("title", null, u), /* @__PURE__ */ a.createElement("path", {
    fill: "#D80027",
    d: "M0 85.337h512v341.326H0z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#FFDA44",
    d: "M256 256 0 90.691v44.242L155.826 256 0 377.067v44.242z"
  }), /* @__PURE__ */ a.createElement("path", {
    d: "M0 90.691v330.618L189.217 256z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#FFF",
    d: "m44.184 213.36 24.912 23.577 30.121-16.41-14.723 30.98 24.911 23.575-34.012-4.43L60.67 301.63l-6.296-33.716-34.012-4.43 30.119-16.408z"
  }));
}, c9 = function(i) {
  var u = i.title, f = d(i, Nd);
  return /* @__PURE__ */ a.createElement("svg", s({
    xmlns: "http://www.w3.org/2000/svg",
    viewBox: "0 85.333 512 341.333"
  }, f), u && /* @__PURE__ */ a.createElement("title", null, u), /* @__PURE__ */ a.createElement("path", {
    fill: "#496E2D",
    d: "M0 85.337h512v341.326H0z"
  }), /* @__PURE__ */ a.createElement("g", {
    fill: "#FFF"
  }, /* @__PURE__ */ a.createElement("path", {
    d: "M357.208 183.679c8.397-23.404-1.036-48.858-21.281-61.536a52.131 52.131 0 0 1 10.884 2.621c27.002 9.688 41.038 39.428 31.35 66.431s-39.428 41.038-66.431 31.35a52.182 52.182 0 0 1-10.069-4.895c23.686 3.084 47.15-10.566 55.547-33.971zm-46.571-67.95-7.297 9.427-11.22-4.026 6.712 9.852-7.296 9.427 11.443-3.338 6.712 9.852.361-11.914 11.444-3.339-11.221-4.025z"
  }), /* @__PURE__ */ a.createElement("path", {
    d: "m330.338 146.448-7.296 9.427-11.221-4.026 6.712 9.852-7.296 9.427 11.443-3.339 6.712 9.853.36-11.915 11.445-3.34-11.221-4.024zM275.9 126.916l-7.296 9.427-11.219-4.024 6.711 9.851-7.296 9.426 11.443-3.338 6.712 9.852.361-11.915 11.444-3.337-11.221-4.027zm-.799 35.189-7.296 9.427-11.221-4.026 6.712 9.852-7.296 9.427 11.444-3.338 6.711 9.852.362-11.915 11.443-3.338-11.22-4.026zm33.484 12.013-7.296 9.427-11.22-4.026 6.712 9.853-7.297 9.426 11.444-3.338 6.712 9.852.36-11.915 11.443-3.338-11.219-4.025z"
  })), /* @__PURE__ */ a.createElement("path", {
    fill: "#D80027",
    d: "M83.478 85.337h89.043v341.326H83.478z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#FFF",
    d: "m117.458 175.191-14.908-11.105v-15.705l14.908-11.105h21.084l14.908 11.105v15.705l-14.908 11.105z"
  }), /* @__PURE__ */ a.createElement("path", {
    d: "M128 137.276h-10.542l-14.908 11.105v7.678H128zm0 37.915h10.542l14.908-11.105v-8.012H128z",
    fill: "#FF9811"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#FFF",
    d: "m117.458 374.725-14.908-11.106v-15.704l14.908-11.105h21.084l14.908 11.105v15.704l-14.908 11.106z"
  }), /* @__PURE__ */ a.createElement("path", {
    d: "M128 336.81h-10.542l-14.908 11.105v7.678H128zm0 37.915h10.542l14.908-11.106v-8.011H128z",
    fill: "#FF9811"
  }), /* @__PURE__ */ a.createElement("path", {
    d: "m117.458 274.957-14.908-11.105v-15.703l14.908-11.106h21.084l14.908 11.106v15.703l-14.908 11.105zm36.513 24.434h-6.493v-6.493h-11.411l-8.068-8.068-8.067 8.068h-11.41v6.493h-6.493v12.986h6.493v6.492h11.41l8.068 8.069 8.068-8.069h11.41v-6.492h6.493zm0-99.768h-6.493v-6.492h-11.411l-8.068-8.069-8.067 8.069h-11.41v6.492h-6.493v12.986h6.493v6.492h11.41l8.068 8.07 8.068-8.07h11.41v-6.492h6.493z",
    fill: "#496E2D"
  }), /* @__PURE__ */ a.createElement("g", {
    fill: "#D80027"
  }, /* @__PURE__ */ a.createElement("path", {
    d: "M120.576 248.576h14.84v14.84h-14.84z"
  }), /* @__PURE__ */ a.createElement("circle", {
    cx: 128,
    cy: 206.113,
    r: 7.421
  }), /* @__PURE__ */ a.createElement("circle", {
    cx: 128,
    cy: 305.887,
    r: 7.421
  })));
}, i9 = function(i) {
  var u = i.title, f = d(i, _d);
  return /* @__PURE__ */ a.createElement("svg", s({
    xmlns: "http://www.w3.org/2000/svg",
    viewBox: "0 85.333 512 341.333"
  }, f), u && /* @__PURE__ */ a.createElement("title", null, u), /* @__PURE__ */ a.createElement("path", {
    fill: "#D80027",
    d: "M0 85.331h512v341.337H0z"
  }), /* @__PURE__ */ a.createElement("circle", {
    fill: "#FFF",
    cx: 256,
    cy: 255.994,
    r: 96
  }), /* @__PURE__ */ a.createElement("g", {
    fill: "#D80027"
  }, /* @__PURE__ */ a.createElement("path", {
    d: "m267.826 219.291 16.47 22.695 26.673-8.649-16.496 22.676 16.468 22.695-26.664-8.681-16.495 22.676.017-28.04-26.664-8.682 26.674-8.648z"
  }), /* @__PURE__ */ a.createElement("path", {
    d: "M277.818 312.724c-31.33 0-56.727-25.397-56.727-56.727s25.397-56.727 56.727-56.727c9.769 0 18.96 2.47 26.985 6.819-12.589-12.31-29.804-19.909-48.803-19.909-38.558 0-69.818 31.259-69.818 69.818s31.26 69.818 69.818 69.818c18.999 0 36.215-7.599 48.803-19.909-8.026 4.347-17.216 6.817-26.985 6.817z"
  })));
}, u9 = function(i) {
  var u = i.title, f = d(i, Ud);
  return /* @__PURE__ */ a.createElement("svg", s({
    xmlns: "http://www.w3.org/2000/svg",
    viewBox: "0 85.333 512 341.333"
  }, f), u && /* @__PURE__ */ a.createElement("title", null, u), /* @__PURE__ */ a.createElement("path", {
    fill: "#D80027",
    d: "M0 85.331h512v341.337H0z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#FFF",
    d: "M0 85.331h256v170.663H0z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#D80027",
    d: "M141.357 157.303V130.59h-26.714v26.713H87.93v26.713h26.713v26.713h26.714v-26.713h26.713v-26.713z"
  }));
}, r9 = function(i) {
  var u = i.title, f = d(i, Rd);
  return /* @__PURE__ */ a.createElement("svg", s({
    xmlns: "http://www.w3.org/2000/svg",
    viewBox: "0 0 513 342"
  }, f), u && /* @__PURE__ */ a.createElement("title", null, u), /* @__PURE__ */ a.createElement("path", {
    fill: "#E30A17",
    d: "M0 0h513v342H0z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#FFF",
    d: "M259.7 118.6c-13.1-9.5-29-14.6-45.3-14.5-40.8 0-73.8 30.8-73.8 68.9s33.1 68.9 73.8 68.9c17.1 0 32.9-5.4 45.3-14.5-30 38.6-85.7 45.6-124.3 15.5s-45.6-85.7-15.5-124.3 85.7-45.6 124.3-15.5c5.8 4.5 11 9.8 15.5 15.5zm39.9 65.8-18.1 21.9 1.2-28.4-26.4-10.4 27.3-7.6 1.8-28.3 15.6 23.7 27.5-7.1-17.5 22 15.3 23.9-26.7-9.7z"
  }));
}, f9 = function(i) {
  var u = i.title, f = d(i, jd);
  return /* @__PURE__ */ a.createElement("svg", s({
    xmlns: "http://www.w3.org/2000/svg",
    viewBox: "0 85.333 512 341.333"
  }, f), u && /* @__PURE__ */ a.createElement("title", null, u), /* @__PURE__ */ a.createElement("path", {
    fill: "#D80027",
    d: "M0 85.337h512v341.326H0z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#FFF",
    d: "m6.066 85.337 207.961 212.636 131.584 128.69h160.323L297.973 214.027 166.389 85.337z"
  }), /* @__PURE__ */ a.createElement("path", {
    d: "M43.364 85.337 384.69 426.663h83.946L127.31 85.337z"
  }));
}, h9 = function(i) {
  var u = i.title, f = d(i, Gd);
  return /* @__PURE__ */ a.createElement("svg", s({
    xmlns: "http://www.w3.org/2000/svg",
    viewBox: "0 0 900 600"
  }, f), u && /* @__PURE__ */ a.createElement("title", null, u), /* @__PURE__ */ a.createElement("path", {
    fill: "#0052B4",
    d: "M0 0h900v600H0z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#00B2EE",
    d: "M0 300h450V0h450v600H0V300z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#FFDA44",
    d: "m345.3 480.9 29.4 90.6-77.1-56H393l-77.1 56 29.4-90.6zM706.4 340l29.4 90.6-77.1-56H754l-77.1 56 29.5-90.6zm106.4-78.5 29.4 90.6-77.1-56h95.3l-77.1 56 29.5-90.6zm0-224.1 29.4 90.6-77.1-56h95.3l-77.1 56 29.5-90.6z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#FFF",
    d: "M449.9 0v206.3h-81l81 54V300h-39.7l-129-85.9V300H168.7v-85.9L39.7 300H0v-39.7l81-54H0V93.8h81L0 39.9V0h39.7l129 85.9V0h112.5v85.9L410.2 0z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#D80027",
    d: "M253.1 0h-56.3v122H0v56.2h196.8V300h56.3V178.2h196.8V122H253.1z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#2E52B2",
    d: "M449.9 39.9v53.9h-81z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#D80027",
    d: "M0 300v-19.3l109.9-74.4h39.7L12.8 300zM12.8.1l136.8 93.7h-39.7L0 19.4V.1zm437.1.1v19.3L340 93.8h-39.7L437.1.2zM437.1 300l-136.8-93.7H340l109.9 74.4V300z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#FFDA44",
    d: "m719.9 131.6 29.4 90.6-77.1-56h95.3l-77.1 56 29.5-90.6zM584 187.7l29.4 90.6-77.1-56h95.3l-77.1 56 29.5-90.6zm30 242.9 29.4 90.6-77.1-56h95.3l-77.1 56 29.5-90.6zM488 332l29.4 90.6-77.1-56h95.3l-77.1 56L488 332zm0 132 29.4 90.6-77.1-56h95.3l-77.1 56L488 464z"
  }));
}, m9 = function(i) {
  var u = i.title, f = d(i, Yd);
  return /* @__PURE__ */ a.createElement("svg", s({
    xmlns: "http://www.w3.org/2000/svg",
    viewBox: "0 85.333 512 341.333"
  }, f), u && /* @__PURE__ */ a.createElement("title", null, u), /* @__PURE__ */ a.createElement("path", {
    fill: "#D80027",
    d: "M0 85.337h512v341.326H0z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#0052B4",
    d: "M0 85.337h256V256H0z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#FFF",
    d: "M186.435 170.669 162.558 181.9l12.714 23.125-25.927-4.961-3.286 26.192L128 206.993l-18.06 19.263-3.285-26.192-25.927 4.96 12.714-23.125-23.877-11.23 23.877-11.231-12.714-23.125 25.927 4.96 3.286-26.192L128 134.344l18.06-19.263 3.285 26.192 25.928-4.96-12.715 23.125z"
  }), /* @__PURE__ */ a.createElement("circle", {
    fill: "#0052B4",
    cx: 128,
    cy: 170.674,
    r: 29.006
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#FFF",
    d: "M128 190.06c-10.692 0-19.391-8.7-19.391-19.391 0-10.692 8.7-19.391 19.391-19.391 10.692 0 19.391 8.7 19.391 19.391 0 10.691-8.699 19.391-19.391 19.391z"
  }));
}, v9 = function(i) {
  var u = i.title, f = d(i, qd);
  return /* @__PURE__ */ a.createElement("svg", s({
    xmlns: "http://www.w3.org/2000/svg",
    viewBox: "0 85.333 512 341.333"
  }, f), u && /* @__PURE__ */ a.createElement("title", null, u), /* @__PURE__ */ a.createElement("path", {
    fill: "#338AF3",
    d: "M0 85.337h512v341.326H0z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#6DA544",
    d: "M0 426.663V85.337h512"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#FFDA44",
    d: "M512 152.222V85.337H411.67L0 359.778v66.885h100.33z"
  }), /* @__PURE__ */ a.createElement("path", {
    d: "M512 85.337v40.125L60.193 426.663H0v-40.125L451.807 85.337z"
  }));
}, s9 = function(i) {
  var u = i.title, f = d(i, Qd);
  return /* @__PURE__ */ a.createElement("svg", s({
    xmlns: "http://www.w3.org/2000/svg",
    viewBox: "0 85.333 512 341.333"
  }, f), u && /* @__PURE__ */ a.createElement("title", null, u), /* @__PURE__ */ a.createElement("path", {
    fill: "#FFDA44",
    d: "M0 85.337h512v341.326H0z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#338AF3",
    d: "M0 85.337h512V256H0z"
  }));
}, d9 = function(i) {
  var u = i.title, f = d(i, Xd);
  return /* @__PURE__ */ a.createElement("svg", s({
    xmlns: "http://www.w3.org/2000/svg",
    viewBox: "0 0 512 341.3"
  }, f), u && /* @__PURE__ */ a.createElement("title", null, u), /* @__PURE__ */ a.createElement("path", {
    fill: "#232323",
    d: "M0 0h512v341.3H0z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#FFDA44",
    d: "M0 56.9h512v56.9H0z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#D32300",
    d: "M0 113.8h512v56.9H0z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#FFDA44",
    d: "M0 227.6h512v56.9H0z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#D32300",
    d: "M0 284.4h512v56.9H0z"
  }), /* @__PURE__ */ a.createElement("circle", {
    fill: "#FFF",
    cx: 256,
    cy: 170.7,
    r: 80.7
  }), /* @__PURE__ */ a.createElement("path", {
    d: "M234.5 127.5c.9-1.4-19.6-2-19.6-2 1.7-2.5 18.4-10.5 18.4-10.5s-.9-6 2.7-9.8l-4.5-9.8s7-3.4 18.4-3.4 19.9 7 21 13.1l-6.4 3.4c-.1 4.7 1.7 11.6-4.3 17.3-5.9 5.7-8.3 6.9-8.5 18.8 0 3.9 2.3 8.2 8.8 11.7 17.4 9.3 38.6 28.2 44.6 33.9 6.1 5.7 5.1 19.2 2.6 25.2s-12.1 14.3-14.3 13.6c-2.2-.6 1.2-9.2-3.5-11.6 0 0-8.9-7.6-16.8.6s-.2 25.2 3.9 28c4.1 2.8 1.7 5-1.9 5H247c-3.8 0-5.4-2.3-1.9-5 7.6-3.8 12.4-15.3 7.4-20.3-4.9-4.9-18.3 1.5-21.5 6.3-2.8 2.8-8.8 3.6-12.9-1.4s-4.1-10.4-1.1-12.5c7-4.8 0 0 .1-.1 0 0 11.9-8.6 29.9-9 3.9 0 4.8-2.2 2.2-4.8 0 0-30.9-27.4-28.1-44.2 3-18.3 20-26.3 13.4-32.5-1.3-1.8 0 0 0 0z"
  }));
}, o9 = function(i) {
  var u = i.title, f = d(i, Zd);
  return /* @__PURE__ */ a.createElement("svg", s({
    xmlns: "http://www.w3.org/2000/svg",
    viewBox: "0 0 513 342"
  }, f), u && /* @__PURE__ */ a.createElement("title", null, u), /* @__PURE__ */ a.createElement("path", {
    fill: "#FFF",
    d: "M0 0h513v342H0z"
  }), /* @__PURE__ */ a.createElement("path", {
    d: "M0 0h513v26.3H0zm0 52.6h513v26.3H0zm0 52.6h513v26.3H0zm0 52.6h513v26.3H0zm0 52.7h513v26.3H0zm0 52.6h513v26.3H0zm0 52.6h513V342H0z",
    fill: "#D80027"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#2E52B2",
    d: "M0 0h256.5v184.1H0z"
  }), /* @__PURE__ */ a.createElement("path", {
    d: "m47.8 138.9-4-12.8-4.4 12.8H26.2l10.7 7.7-4 12.8 10.9-7.9 10.6 7.9-4.1-12.8 10.9-7.7zm56.3 0-4.1-12.8-4.2 12.8H82.6l10.7 7.7-4 12.8 10.7-7.9 10.8 7.9-4-12.8 10.7-7.7zm56.5 0-4.3-12.8-4 12.8h-13.5l11 7.7-4.2 12.8 10.7-7.9 11 7.9-4.2-12.8 10.7-7.7zm56.2 0-4-12.8-4.2 12.8h-13.3l10.8 7.7-4 12.8 10.7-7.9 10.8 7.9-4.3-12.8 11-7.7zM100 75.3l-4.2 12.8H82.6L93.3 96l-4 12.6 10.7-7.8 10.8 7.8-4-12.6 10.7-7.9h-13.4zm-56.2 0-4.4 12.8H26.2L36.9 96l-4 12.6 10.9-7.8 10.6 7.8L50.3 96l10.9-7.9H47.8zm112.5 0-4 12.8h-13.5l11 7.9-4.2 12.6 10.7-7.8 11 7.8-4.2-12.6 10.7-7.9h-13.2zm56.5 0-4.2 12.8h-13.3l10.8 7.9-4 12.6 10.7-7.8 10.8 7.8-4.3-12.6 11-7.9h-13.5zm-169-50.6-4.4 12.6H26.2l10.7 7.9-4 12.7L43.8 50l10.6 7.9-4.1-12.7 10.9-7.9H47.8zm56.2 0-4.2 12.6H82.6l10.7 7.9-4 12.7L100 50l10.8 7.9-4-12.7 10.7-7.9h-13.4zm56.3 0-4 12.6h-13.5l11 7.9-4.2 12.7 10.7-7.9 11 7.9-4.2-12.7 10.7-7.9h-13.2zm56.5 0-4.2 12.6h-13.3l10.8 7.9-4 12.7 10.7-7.9 10.8 7.9-4.3-12.7 11-7.9h-13.5z",
    fill: "#FFF"
  }));
}, E9 = function(i) {
  var u = i.title, f = d(i, Kd);
  return /* @__PURE__ */ a.createElement("svg", s({
    xmlns: "http://www.w3.org/2000/svg",
    viewBox: "0 0 513 342"
  }, f), u && /* @__PURE__ */ a.createElement("title", null, u), /* @__PURE__ */ a.createElement("path", {
    fill: "#FFF",
    d: "M0 0h513v342H0z"
  }), /* @__PURE__ */ a.createElement("path", {
    d: "M0 0h513v26.3H0zm0 52.6h513v26.3H0zm0 52.6h513v26.3H0zm0 52.6h513v26.3H0zm0 52.7h513v26.3H0zm0 52.6h513v26.3H0zm0 52.6h513V342H0z",
    fill: "#D80027"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#2E52B2",
    d: "M0 0h256.5v184.1H0z"
  }), /* @__PURE__ */ a.createElement("path", {
    d: "m47.8 138.9-4-12.8-4.4 12.8H26.2l10.7 7.7-4 12.8 10.9-7.9 10.6 7.9-4.1-12.8 10.9-7.7zm56.3 0-4.1-12.8-4.2 12.8H82.6l10.7 7.7-4 12.8 10.7-7.9 10.8 7.9-4-12.8 10.7-7.7zm56.5 0-4.3-12.8-4 12.8h-13.5l11 7.7-4.2 12.8 10.7-7.9 11 7.9-4.2-12.8 10.7-7.7zm56.2 0-4-12.8-4.2 12.8h-13.3l10.8 7.7-4 12.8 10.7-7.9 10.8 7.9-4.3-12.8 11-7.7zM100 75.3l-4.2 12.8H82.6L93.3 96l-4 12.6 10.7-7.8 10.8 7.8-4-12.6 10.7-7.9h-13.4zm-56.2 0-4.4 12.8H26.2L36.9 96l-4 12.6 10.9-7.8 10.6 7.8L50.3 96l10.9-7.9H47.8zm112.5 0-4 12.8h-13.5l11 7.9-4.2 12.6 10.7-7.8 11 7.8-4.2-12.6 10.7-7.9h-13.2zm56.5 0-4.2 12.8h-13.3l10.8 7.9-4 12.6 10.7-7.8 10.8 7.8-4.3-12.6 11-7.9h-13.5zm-169-50.6-4.4 12.6H26.2l10.7 7.9-4 12.7L43.8 50l10.6 7.9-4.1-12.7 10.9-7.9H47.8zm56.2 0-4.2 12.6H82.6l10.7 7.9-4 12.7L100 50l10.8 7.9-4-12.7 10.7-7.9h-13.4zm56.3 0-4 12.6h-13.5l11 7.9-4.2 12.7 10.7-7.9 11 7.9-4.2-12.7 10.7-7.9h-13.2zm56.5 0-4.2 12.6h-13.3l10.8 7.9-4 12.7 10.7-7.9 10.8 7.9-4.3-12.7 11-7.9h-13.5z",
    fill: "#FFF"
  }));
}, z9 = function(i) {
  var u = i.title, f = d(i, Jd);
  return /* @__PURE__ */ a.createElement("svg", s({
    xmlns: "http://www.w3.org/2000/svg",
    viewBox: "0 0 513 342"
  }, f), u && /* @__PURE__ */ a.createElement("title", null, u), /* @__PURE__ */ a.createElement("path", {
    fill: "#FFF",
    d: "M0 0h513v342H0z"
  }), /* @__PURE__ */ a.createElement("path", {
    d: "M0 38h513v38H0zm0 76h513v38H0zm0 76h513v38H0zm0 76h513v38H0z",
    fill: "#0038a8"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#FFF",
    d: "M0 0h256.5v190H0z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#FED443",
    d: "m128.3 138.7-15.1 22.6-3.8-26.9L86 148.2l8.2-25.9-27.2 2.2 18.6-19.8L60.3 95l25.4-9.7L67 65.5l27.1 2.3-8.2-25.9 23.4 13.8 3.9-26.9 15.1 22.6 15.1-22.6 3.8 26.9 23.4-13.8-8.2 25.9 27.1-2.2-18.6 19.8 25.4 9.7-25.4 9.7 18.6 19.8-27.1-2.2 8.2 25.9-23.4-13.8-3.9 26.9-15-22.7zm0-.5c23.9.9 44-17.6 44.9-41.5.9-23.9-17.6-44-41.5-44.9h-3.4c-23.9.8-42.6 20.8-41.8 44.6.6 22.8 19 41.1 41.8 41.8zm0-10.6c-18.8 0-34-15.2-34-34s15.2-34 34-34 34 15.2 34 34-15.3 34-34 34z"
  }));
}, p9 = function(i) {
  var u = i.title, f = d(i, kd);
  return /* @__PURE__ */ a.createElement("svg", s({
    xmlns: "http://www.w3.org/2000/svg",
    viewBox: "0 85.333 512 341.333"
  }, f), u && /* @__PURE__ */ a.createElement("title", null, u), /* @__PURE__ */ a.createElement("path", {
    fill: "#D80027",
    d: "M0 85.337h512v341.326H0z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#6DA544",
    d: "M0 322.783h512v103.88H0z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#338AF3",
    d: "M0 85.337h512v104.515H0z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#FFF",
    d: "M0 210.877h512v89.656H0z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#338AF3",
    d: "M0 85.337h512v104.515H0z"
  }), /* @__PURE__ */ a.createElement("path", {
    d: "M188.688 137.589c0-15.984 11.234-29.339 26.236-32.614a33.531 33.531 0 0 0-7.155-.777c-18.442 0-33.391 14.949-33.391 33.391s14.949 33.391 33.391 33.391c2.458 0 4.85-.273 7.155-.777-15.002-3.275-26.236-16.63-26.236-32.614zm45.97 15.177 2.261 6.957h7.315l-5.918 4.301 2.261 6.956-5.919-4.3-5.918 4.3 2.261-6.956-5.918-4.301h7.315zm23.348 0 2.26 6.957h7.315l-5.918 4.301 2.261 6.956-5.918-4.3-5.918 4.3 2.26-6.956-5.917-4.301h7.314zm23.347 0 2.26 6.957h7.315l-5.917 4.301 2.26 6.956-5.918-4.3-5.918 4.3 2.26-6.956-5.918-4.301h7.315zm23.347 0 2.262 6.957h7.315l-5.919 4.301 2.261 6.956-5.919-4.3-5.918 4.3 2.262-6.956-5.919-4.301h7.314zm23.348 0 2.26 6.957h7.315l-5.918 4.301 2.261 6.956-5.918-4.3-5.918 4.3 2.26-6.956-5.917-4.301h7.314zm-70.042-24.284 2.26 6.956h7.315l-5.918 4.3 2.261 6.957-5.918-4.299-5.918 4.299 2.26-6.957-5.917-4.3h7.314zm23.347 0 2.26 6.956h7.315l-5.917 4.3 2.26 6.957-5.918-4.299-5.918 4.299 2.26-6.957-5.918-4.3h7.315zm23.347 0 2.262 6.956h7.315l-5.919 4.3 2.261 6.957-5.919-4.299-5.918 4.299 2.262-6.957-5.919-4.3h7.314zm23.348 0 2.26 6.956h7.315l-5.918 4.3 2.261 6.957-5.918-4.299-5.918 4.299 2.26-6.957-5.917-4.3h7.314zm-46.695-24.284 2.26 6.956h7.315l-5.917 4.3 2.26 6.957-5.918-4.3-5.918 4.3 2.26-6.957-5.918-4.3h7.315zm23.347 0 2.262 6.956h7.315l-5.919 4.3 2.261 6.957-5.919-4.3-5.918 4.3 2.262-6.957-5.919-4.3h7.314zm23.348 0 2.26 6.956h7.315l-5.918 4.3 2.261 6.957-5.918-4.3-5.918 4.3 2.26-6.957-5.917-4.3h7.314z",
    fill: "#FFF"
  }));
}, g9 = function(i) {
  var u = i.title, f = d(i, Wd);
  return /* @__PURE__ */ a.createElement("svg", s({
    xmlns: "http://www.w3.org/2000/svg",
    viewBox: "0 85.333 512 341.333"
  }, f), u && /* @__PURE__ */ a.createElement("title", null, u), /* @__PURE__ */ a.createElement("path", {
    fill: "#FFDA44",
    d: "M0 85.331h512v341.326H0z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#FFF",
    d: "M256 85.331h256v341.337H256z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#ACABB1",
    d: "m321.353 233.837 32.073 42.43c-5.053 7.651-5.026 17.961.817 25.692 7.414 9.807 21.374 11.748 31.182 4.335 9.807-7.414 11.748-21.374 4.334-31.182-5.843-7.731-15.756-10.568-24.495-7.795l-49.988-66.129-11.838 8.949-17.759 13.424 17.899 23.677 17.775-13.401zm46.175 48.78a7.421 7.421 0 1 1 8.95 11.84 7.421 7.421 0 0 1-8.95-11.84z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#FFDA44",
    d: "m376.367 247.24 17.899-23.677-17.759-13.424-11.838-8.949-49.988 66.129c-8.74-2.775-18.651.063-24.495 7.795-7.414 9.808-5.473 23.768 4.334 31.182 9.808 7.414 23.768 5.473 31.182-4.335 5.845-7.731 5.871-18.041.817-25.692l32.073-42.43 17.775 13.401zm-62.504 45.771a7.42 7.42 0 1 1-11.84-8.95 7.42 7.42 0 0 1 11.84 8.95z"
  }));
}, M9 = function(i) {
  var u = i.title, f = d(i, $d);
  return /* @__PURE__ */ a.createElement("svg", s({
    xmlns: "http://www.w3.org/2000/svg",
    viewBox: "0 85.333 512 341.333"
  }, f), u && /* @__PURE__ */ a.createElement("title", null, u), /* @__PURE__ */ a.createElement("path", {
    fill: "#FFDA44",
    d: "M0 85.331h512v341.337H0z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#338AF3",
    d: "M0 85.331h170.663v341.337H0z"
  }), /* @__PURE__ */ a.createElement("path", {
    d: "M341.337 85.331H512v341.337H341.337zM214.261 283.82l-33.393-50.086 33.392-50.087 33.392 50.087zm83.478 0-33.391-50.086 33.391-50.087 33.393 50.087zM256 350.603l-33.391-50.087L256 250.429l33.391 50.087z",
    fill: "#6DA544"
  }));
}, w9 = function(i) {
  var u = i.title, f = d(i, Id);
  return /* @__PURE__ */ a.createElement("svg", s({
    xmlns: "http://www.w3.org/2000/svg",
    viewBox: "0 0 22.5 15"
  }, f), u && /* @__PURE__ */ a.createElement("title", null, u), /* @__PURE__ */ a.createElement("path", {
    fill: "#FFCE00",
    d: "M0 0h22.5v5H0z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#203899",
    d: "M0 5h22.5v5H0z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#D82B2B",
    d: "M0 10h22.5v5H0z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#FFF",
    d: "m12.13 5.38.14.46.45.14-.39.27.01.48-.38-.29-.45.15.15-.44-.28-.38h.47zm-1.77 0 .27.39h.47l-.28.38.15.45-.45-.16-.38.28.01-.47-.39-.28.45-.13zm3.44.56-.01.48.39.27-.46.14-.14.45-.27-.38h-.48l.29-.38-.15-.45.45.16zm-5.11.02.4.27.44-.18-.13.46.3.36-.47.02-.26.4-.16-.44-.46-.12.37-.3zm6.55 1.17-.2.44.25.4-.47-.05-.31.36-.1-.46-.43-.18.41-.24.03-.47.35.31zm.19 1.12.21.43.47.07-.34.33.08.46-.42-.22-.42.22.08-.46-.34-.33.47-.07zM7 8.2l.21.43.47.06-.34.33.08.47L7 9.27l-.42.22.08-.47-.34-.33.47-.06zm.31-1.15.45.14.38-.29v.48l.39.26-.45.15-.13.46-.28-.38-.47.01.27-.38z"
  }));
}, x9 = function(i) {
  var u = i.title, f = d(i, Pd);
  return /* @__PURE__ */ a.createElement("svg", s({
    xmlns: "http://www.w3.org/2000/svg",
    viewBox: "0 85.333 512 341.333"
  }, f), u && /* @__PURE__ */ a.createElement("title", null, u), /* @__PURE__ */ a.createElement("path", {
    fill: "#0052B4",
    d: "M0 85.334h512v341.337H0z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#FFF",
    d: "M256 85.334V202.66h-46.069L256 233.38v22.617h-22.628L160 207.078v48.919H96v-48.919l-73.372 48.919H0V233.38l46.069-30.72H0v-64h46.069L0 107.951V85.334h22.628L96 134.241V85.334h64v48.907l73.372-48.907z"
  }), /* @__PURE__ */ a.createElement("g", {
    fill: "#D80027"
  }, /* @__PURE__ */ a.createElement("path", {
    d: "M144 85.33h-32v69.333H0v32h112v69.334h32v-69.334h112v-32H144z"
  }), /* @__PURE__ */ a.createElement("path", {
    d: "M0 85.329v15.083l57.377 38.251H80zm256 0v15.083l-57.377 38.251H176z"
  })), /* @__PURE__ */ a.createElement("path", {
    fill: "#2E52B2",
    d: "M256 107.951v30.712h-46.069z"
  }), /* @__PURE__ */ a.createElement("path", {
    d: "M0 85.329v15.083l57.377 38.251H80zm256 0v15.083l-57.377 38.251H176z",
    fill: "#D80027"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#2E52B2",
    d: "M256 107.951v30.712h-46.069z"
  }), /* @__PURE__ */ a.createElement("path", {
    d: "M0 255.997v-15.082l57.377-38.252H80zm256 0v-15.082l-57.377-38.252H176z",
    fill: "#D80027"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#FFDA44",
    d: "m384 259.706-46.129 46.129c8.645 16.675 26.051 28.074 46.129 28.074s37.484-11.4 46.129-28.074L384 259.706z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#6DA544",
    d: "M332.058 178.084v81.624c.001 39.759 51.942 51.941 51.942 51.941s51.941-12.182 51.942-51.942v-81.623H332.058z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#FFF",
    d: "M372.87 215.181h22.261v59.359H372.87z"
  }), /* @__PURE__ */ a.createElement("circle", {
    fill: "#A2001D",
    cx: 384,
    cy: 215.181,
    r: 11.13
  }), /* @__PURE__ */ a.createElement("path", {
    d: "M346.902 192.92h14.84v14.84h-14.84zm0 33.392h14.84v14.84h-14.84zm0 33.391h14.84v14.84h-14.84zm59.359-66.783h14.84v14.84h-14.84zm0 33.392h14.84v14.84h-14.84zm0 33.391h14.84v14.84h-14.84z",
    fill: "#FFDA44"
  }));
}, H9 = function(i) {
  var u = i.title, f = d(i, eo);
  return /* @__PURE__ */ a.createElement("svg", s({
    xmlns: "http://www.w3.org/2000/svg",
    viewBox: "0 0 512 341.3"
  }, f), u && /* @__PURE__ */ a.createElement("title", null, u), /* @__PURE__ */ a.createElement("path", {
    fill: "#FFF",
    d: "M0 0h512v341.3H0z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#1583C4",
    d: "m397.7 166.3-51 97.7c-1.1 2-.3 4.5 1.8 5.6 2 1.1 4.5.3 5.6-1.8l51-97.7c1-2.1.1-4.6-2-5.5-2-.9-4.3-.2-5.4 1.7z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#1583C4",
    d: "m404.9 188-57.8 75.4c-1.3 1.9-.8 4.5 1.1 5.8 1.7 1.2 4.1.9 5.5-.7l57.8-75.4c1.4-1.8 1.1-4.4-.8-5.8-1.8-1.5-4.4-1.1-5.8.7z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#1583C4",
    d: "m379 181.3-32.5 83.1c-.8 2.1.2 4.6 2.4 5.4 2.1.8 4.6-.2 5.4-2.4l32.5-83.1c.7-2.2-.4-4.5-2.6-5.3-2.1-.6-4.3.4-5.2 2.3z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#409347",
    d: "M122.1 171.9c.4 1.7 1.2 4.7 2.2 8.5 1.7 6.4 3.6 12.8 5.6 18.7 2.3 6.9 14.7 31.4 20.2 39.1 6.9 9.7 14.1 19.3 21.5 28.6 1.6 1.9 4.5 2.1 6.4.5 1.8-1.6 2.1-4.3.7-6.2-7.3-9.2-14.4-18.6-21.2-28.2-4.2-5.8-17.2-31.1-19-36.7-2.9-8.7-5.4-17.6-7.6-26.5-.5-2.4-2.9-4-5.3-3.5s-4 2.9-3.5 5.3c-.1.2 0 .3 0 .4z"
  }), /* @__PURE__ */ a.createElement("g", {
    fill: "#FFD836",
    stroke: "#231F20",
    strokeMiterlimit: 10
  }, /* @__PURE__ */ a.createElement("path", {
    d: "M297.1 114.9S279.5 42.8 261 42.8c-6.9 0-11.9-.6-15.4 6.4-.9 1.7-20.7-1.6-19.1 20.8.5 6.9 1.1-9.8 14.9-3.4 6.3 3-16.4 48.2-16.4 48.2h72.1z"
  }), /* @__PURE__ */ a.createElement("path", {
    d: "m466.1 48.4-90.9 22c-75.3 18.3-42.1 44.4-42.1 44.4l-72.1 12-72.1-12s33.2-26.1-42.1-44.4l-90.9-22C43.1 45.3 38.1 51.8 45.3 63c0 0 106.6 104 126.8 112 20.2 8.1 40.8 0 40.8 0s-14.9 8.7-24 24c-9.2 15.3-4.6 34.7-38.9 40.2-5.5 6.6-5 14 2.9 19.9 7.9 5.8 72.1-48.1 72.1-48.1s-24 24.4-24 36.1c0 5.1 44.1 53.8 60.1 53.8s60.1-48.7 60.1-53.8c0-11.7-24-36.1-24-36.1s64.2 53.9 72.1 48.1 11.4-19.3 2.8-19.9c-28-1.8-29.7-24.9-38.9-40.2-9.2-15.3-24-24-24-24s20.6 8.1 40.8 0S476.7 63 476.7 63c7.2-11.2 2.3-17.7-10.6-14.6z"
  })), /* @__PURE__ */ a.createElement("path", {
    fill: "#FFF",
    stroke: "#231F20",
    strokeMiterlimit: 10,
    d: "M201.3 111.8v80.3c0 45.7 59.7 59.7 59.7 59.7s59.7-14 59.7-59.7v-80.3H201.3z"
  }), /* @__PURE__ */ a.createElement("path", {
    d: "M225.2 137.7V235c8.2 6 17 10.2 23.9 12.9V137.7h-23.9zm71.6 0V235c-8.2 6-17 10.2-23.9 12.9V137.7h23.9z",
    fill: "#D80027"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#0052B4",
    d: "M201.3 111.8h119.4v37.9H201.3z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#1583C4",
    d: "m60.1 210.5-21.9-58.1H15.7l31.8 80.8h25.2l31.5-80.8H81.7zm391.4-58.6h18.9v81.7h-18.9z"
  }));
}, y9 = function(i) {
  var u = i.title, f = d(i, to);
  return /* @__PURE__ */ a.createElement("svg", s({
    xmlns: "http://www.w3.org/2000/svg",
    viewBox: "0 85.333 512 341.333"
  }, f), u && /* @__PURE__ */ a.createElement("title", null, u), /* @__PURE__ */ a.createElement("path", {
    fill: "#D80027",
    d: "M196.641 85.337H0v341.326h512V85.337z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#FFDA44",
    d: "m256 157.279 22.663 69.747H352l-59.332 43.106 22.664 69.749L256 296.774l-59.332 43.107 22.664-69.749L160 227.026h73.337z"
  }));
}, F9 = function(i) {
  var u = i.title, f = d(i, lo);
  return /* @__PURE__ */ a.createElement("svg", s({
    xmlns: "http://www.w3.org/2000/svg",
    viewBox: "0 85.333 512 341.333"
  }, f), u && /* @__PURE__ */ a.createElement("title", null, u), /* @__PURE__ */ a.createElement("path", {
    d: "M0 85.337h512v341.326H0z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#6DA544",
    d: "M512 256v170.663H0L215.185 256z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#D80027",
    d: "M512 85.337V256H215.185L0 85.337z"
  }), /* @__PURE__ */ a.createElement("path", {
    d: "M221.001 239.304 26.868 85.337H8.956l208.168 165.098H512v-11.131zM8.956 426.663h17.912l194.133-153.967H512v-11.131H217.124zM0 92.44v14.206L188.317 256 0 405.354v14.205L206.229 256z"
  }), /* @__PURE__ */ a.createElement("g", {
    fill: "#FFDA44"
  }, /* @__PURE__ */ a.createElement("path", {
    d: "M8.956 85.337H0v7.103L206.229 256 0 419.559v7.104h8.956l208.168-165.098H512v-11.13H217.124z"
  }), /* @__PURE__ */ a.createElement("path", {
    d: "M63.718 292.382v-14.295c14.265 0 25.87-11.606 25.87-25.869 0-10.092-8.211-18.303-18.304-18.303-6.875 0-12.469 5.593-12.469 12.469 0 4.397 3.577 7.974 7.974 7.974a4.514 4.514 0 0 0 4.508-4.508h14.295c0 10.368-8.435 18.804-18.802 18.804-12.279-.002-22.269-9.993-22.269-22.271 0-14.758 12.006-26.764 26.764-26.764 17.975 0 32.599 14.623 32.599 32.599 0 22.145-18.018 40.164-40.166 40.164z"
  })));
}, D9 = function(i) {
  var u = i.title, f = d(i, ao);
  return /* @__PURE__ */ a.createElement("svg", s({
    xmlns: "http://www.w3.org/2000/svg",
    viewBox: "0 0 150 100"
  }, f), u && /* @__PURE__ */ a.createElement("title", null, u), /* @__PURE__ */ a.createElement("path", {
    fill: "#D80B30",
    d: "M0 0h150v100H0z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#00318A",
    d: "M0 0h20v44.33H0z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#FFF",
    d: "M20 0h20v44.33H20z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "none",
    stroke: "#FFF",
    strokeWidth: 3,
    d: "M0 44.33h62.75V0"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#FFF",
    d: "M108.08 43.29 87.96 23.17h40.25l-20.13 20.12zm-6.7 6.71L81.25 29.88v40.25L101.38 50zm6.7 6.71L87.96 76.83h40.25l-20.13-20.12zm6.71-6.71 20.13-20.13v40.25L114.79 50z"
  }));
}, S9 = function(i) {
  var u = i.title, f = d(i, no);
  return /* @__PURE__ */ a.createElement("svg", s({
    xmlns: "http://www.w3.org/2000/svg",
    viewBox: "0 85.333 512 341.333"
  }, f), u && /* @__PURE__ */ a.createElement("title", null, u), /* @__PURE__ */ a.createElement("path", {
    fill: "#D80027",
    d: "M0 85.331h512v341.337H0z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#0052B4",
    d: "M0 85.331h256v170.663H0z"
  }), /* @__PURE__ */ a.createElement("path", {
    d: "m165.483 181.79 2.764 8.504h8.939l-7.232 5.254 2.763 8.503-7.234-5.255-7.233 5.255 2.763-8.503-7.233-5.254h8.94zm-44.904-66.783 4.606 14.173h14.9l-12.055 8.757 4.605 14.171-12.056-8.758-12.054 8.758 4.605-14.171-12.056-8.757h14.901zm44.522 14.841 4.606 14.172h14.9l-12.055 8.757 4.604 14.173-12.055-8.76-12.054 8.76 4.604-14.173-12.055-8.757h14.9zm-35.192 59.363 4.606 14.172h14.9l-12.055 8.758 4.604 14.171-12.055-8.759-12.054 8.759 4.604-14.171-12.055-8.758h14.9zm-39.01-37.103 4.605 14.173h14.9l-12.055 8.757 4.604 14.173-12.054-8.76-12.055 8.76 4.604-14.173-12.055-8.757h14.9z",
    fill: "#FFF"
  }));
}, A9 = function(i) {
  var u = i.title, f = d(i, co);
  return /* @__PURE__ */ a.createElement("svg", s({
    xmlns: "http://www.w3.org/2000/svg",
    viewBox: "0 0 513 342"
  }, f), u && /* @__PURE__ */ a.createElement("title", null, u), /* @__PURE__ */ a.createElement("path", {
    fill: "#FFF",
    d: "M0 0h513v342H0z"
  }), /* @__PURE__ */ a.createElement("path", {
    d: "M0 0h513v48.8H0zm0 97.5h513v48.8H0zM0 195h513v48.8H0zm0 97.6h513v48.8H0z",
    fill: "#6DA544"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#D80027",
    d: "M0 0h256.5v146.3H0z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#FFF",
    d: "m116.9 114.4-7.5-14.8V69.9L128 59l18.6 10.9v22.3l7.4-7.4 4.2 3-4.2 11.8-14.9 14.8z"
  }), /* @__PURE__ */ a.createElement("circle", {
    fill: "#FFF",
    cx: 82,
    cy: 82.8,
    r: 5.4
  }), /* @__PURE__ */ a.createElement("circle", {
    fill: "#FFF",
    cx: 90.8,
    cy: 61.7,
    r: 5.4
  }), /* @__PURE__ */ a.createElement("circle", {
    fill: "#FFF",
    cx: 106.6,
    cy: 46.2,
    r: 5.4
  }), /* @__PURE__ */ a.createElement("circle", {
    fill: "#FFF",
    cx: 128,
    cy: 40.8,
    r: 5.4
  }), /* @__PURE__ */ a.createElement("circle", {
    fill: "#FFF",
    cx: 149.4,
    cy: 46.2,
    r: 5.4
  }), /* @__PURE__ */ a.createElement("circle", {
    fill: "#FFF",
    cx: 165.2,
    cy: 61.7,
    r: 5.4
  }), /* @__PURE__ */ a.createElement("circle", {
    fill: "#FFF",
    cx: 174,
    cy: 82.8,
    r: 5.4
  }));
}, B9 = function(i) {
  var u = i.title, f = d(i, io);
  return /* @__PURE__ */ a.createElement("svg", s({
    xmlns: "http://www.w3.org/2000/svg",
    viewBox: "0 0 900 600",
    fill: "#e30a17"
  }, f), u && /* @__PURE__ */ a.createElement("title", null, u), /* @__PURE__ */ a.createElement("path", {
    d: "M0 0h900v600H0z",
    fill: "#fff"
  }), /* @__PURE__ */ a.createElement("circle", {
    cx: 300,
    cy: 300,
    r: 150
  }), /* @__PURE__ */ a.createElement("circle", {
    cx: 337.5,
    cy: 300,
    r: 120,
    fill: "#fff"
  }), /* @__PURE__ */ a.createElement("path", {
    d: "M0 60h900v60H0zm0 420h900v60H0zm417.5-180 135.676-44.084-83.852 115.413V228.671l83.852 115.413z"
  }));
}, b9 = function(i) {
  var u = i.title, f = d(i, uo);
  return /* @__PURE__ */ a.createElement("svg", s({
    xmlns: "http://www.w3.org/2000/svg",
    viewBox: "0 0 512 341.33"
  }, f), u && /* @__PURE__ */ a.createElement("title", null, u), /* @__PURE__ */ a.createElement("path", {
    fill: "#0052B4",
    d: "M0 0h512v341.34H0z"
  }), /* @__PURE__ */ a.createElement("path", {
    d: "m220.72 42.63 7.95 16.11 17.77 2.58-12.86 12.54 3.04 17.7-15.9-8.36-15.9 8.36 3.03-17.7-12.86-12.54 17.78-2.58zm70.56-.06 7.95 16.1 17.78 2.59-12.86 12.53 3.03 17.71-15.9-8.36-15.9 8.36 3.04-17.71-12.86-12.53 17.77-2.59zm62.08 11.36 7.95 16.11 17.77 2.58-12.86 12.54 3.04 17.7-15.9-8.35-15.9 8.35 3.04-17.7-12.87-12.54 17.78-2.58zm63.01 18.3 7.95 16.11 17.77 2.59-12.86 12.53 3.04 17.71-15.9-8.36-15.9 8.36 3.03-17.71-12.86-12.53 17.78-2.59zm-257.73-18.3 7.95 16.11 17.78 2.58-12.87 12.54 3.04 17.7-15.9-8.35-15.9 8.35 3.04-17.7-12.86-12.54 17.77-2.58zm-63.01 18.3 7.95 16.11 17.78 2.59-12.86 12.53 3.03 17.71-15.9-8.36-15.9 8.36 3.04-17.71-12.86-12.53 17.77-2.59z",
    fill: "#FFF"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#FFDA44",
    d: "m217.53 259.33-27.22-13.61-27.22-54.43h27.22l27.22-27.22 13.61-27.22 27.22-13.61 13.61 13.61 27.22 13.61v13.61l13.61 13.61 40.83 27.22L340 245.73l-40.83 40.83-13.61-27.22-40.83 27.22v27.22l-13.61-13.61-13.59-40.84z"
  }));
}, T9 = function(i) {
  var u = i.title, f = d(i, ro);
  return /* @__PURE__ */ a.createElement("svg", s({
    xmlns: "http://www.w3.org/2000/svg",
    viewBox: "0 0 450 300"
  }, f), u && /* @__PURE__ */ a.createElement("title", null, u), /* @__PURE__ */ a.createElement("path", {
    fill: "#FFF",
    d: "M0 0h450v300H0z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "red",
    d: "M0 100h450v200H0z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#FFDF00",
    d: "M0 200h450v100H0z"
  }));
}, L9 = function(i) {
  var u = i.title, f = d(i, fo);
  return /* @__PURE__ */ a.createElement("svg", s({
    xmlns: "http://www.w3.org/2000/svg",
    viewBox: "0 85.333 512 341.333"
  }, f), u && /* @__PURE__ */ a.createElement("title", null, u), /* @__PURE__ */ a.createElement("path", {
    fill: "#FFF",
    d: "M0 85.337h512v341.326H0z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#D80027",
    d: "M0 85.337h512v113.775H0z"
  }), /* @__PURE__ */ a.createElement("path", {
    d: "M0 312.888h512v113.775H0z"
  }));
}, V9 = function(i) {
  var u = i.title, f = d(i, ho);
  return /* @__PURE__ */ a.createElement("svg", s({
    xmlns: "http://www.w3.org/2000/svg",
    viewBox: "0 0 90 60"
  }, f), u && /* @__PURE__ */ a.createElement("title", null, u), /* @__PURE__ */ a.createElement("path", {
    fill: "#FFF",
    d: "M0 0h90v60H0z"
  }), /* @__PURE__ */ a.createElement("path", {
    stroke: "#7f7f7f",
    strokeWidth: 0.25,
    fill: "#ABABAB",
    d: "M75.6 39.6c-1.1.7 0 1.8-.6 1.9-.6.1-1.5-1.4-.2-2.5 1.6-1.5 5.8 0 5.9 4.3 0 2.2-.9 6.8-7.2 6.8-7.6 0-11.4-5.7-11.4-11.4 0-4.8 3.7-13.5 6.5-14.6 3.1-1.2 6.7-3.5 6.8-5 0-.5-1.4-.1-3.3.9-3.1 1.7-6.6 2.5-6.6 1.4s2.9-3.1 4-4.6c1.2-1.6.9-5.5 3-5.5 7.6 0 9.9 7.7 7 11.7-1.9 2.6 4.3 1 4.3 1s0 5.3-3.2 7.3c-3 1.8-4.8 3.9-4.8 3.9s.2-4.6-.5-3.9c-.6.7-11.1 4.5-10.3 12 .3 3 4.5 5.4 8.5 5.2s5.7-2.4 5.8-4.9c.1-3.9-3-4.4-3.7-4zm-64.9 4c.1 2.5 1.8 4.7 5.8 4.9s8.2-2.2 8.5-5.2c.8-7.5-9.7-11.3-10.3-12-.7-.7-.5 3.9-.5 3.9s-1.8-2.1-4.8-3.9c-3.2-2-3.2-7.3-3.2-7.3s6.2 1.6 4.3-1c-2.9-4-.6-11.7 7-11.7 2.1 0 1.8 3.9 3 5.5 1.1 1.5 4 3.5 4 4.6s-3.5.3-6.6-1.4c-1.9-1-3.3-1.4-3.3-.9.1 1.5 3.7 3.8 6.8 5 2.8 1.1 6.5 9.8 6.5 14.6 0 5.7-3.8 11.4-11.4 11.4-6.3 0-7.2-4.6-7.2-6.8.1-4.3 4.3-5.8 5.9-4.3 1.2 1.1.4 2.7-.2 2.5-.6-.1.6-1.2-.6-1.9-.6-.4-3.7.1-3.7 4z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#DE393A",
    d: "M31 28.8v14c0 1.9 2.1 1.6 2.1 1.6h9.3c1.2 0 2.6 2.3 2.6 2.3s1.4-2.3 2.5-2.3h9.6s2 .2 2-1.8V28.7H31z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#3951A3",
    d: "M31 13.1h28v15.7H31z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#FFF",
    d: "M50.1 16.5c-.4 2.4-2.5 4.2-5.1 4.2-2.5 0-4.6-1.8-5.1-4.2-.5.9-.8 1.9-.8 3 0 3.3 2.6 5.9 5.9 5.9 3.3 0 5.9-2.6 5.9-5.9 0-1.1-.3-2.1-.8-3z"
  }), /* @__PURE__ */ a.createElement("circle", {
    fill: "#FEE05F",
    cx: 39.1,
    cy: 36.5,
    r: 3
  }), /* @__PURE__ */ a.createElement("circle", {
    fill: "#FEE05F",
    cx: 50.9,
    cy: 36.5,
    r: 3
  }));
}, C9 = function(i) {
  var u = i.title, f = d(i, mo);
  return /* @__PURE__ */ a.createElement("svg", s({
    xmlns: "http://www.w3.org/2000/svg",
    viewBox: "0 85.333 512 341.333"
  }, f), u && /* @__PURE__ */ a.createElement("title", null, u), /* @__PURE__ */ a.createElement("path", {
    fill: "#FFF",
    d: "M0 85.337h512v341.326H0z"
  }), /* @__PURE__ */ a.createElement("path", {
    d: "M114.024 256.001 0 141.926v228.17z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#ffb915",
    d: "M161.192 256 0 94.7v47.226l114.024 114.075L0 370.096v47.138z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#007847",
    d: "M509.833 289.391c.058-.44.804-.878 2.167-1.318v-65.464H222.602L85.33 85.337H0V94.7L161.192 256 0 417.234v9.429h85.33l137.272-137.272h287.231z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#000c8a",
    d: "M503.181 322.783H236.433l-103.881 103.88H512v-103.88z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#e1392d",
    d: "M503.181 189.217H512V85.337H132.552l103.881 103.88z"
  }));
}, O9 = function(i) {
  var u = i.title, f = d(i, vo);
  return /* @__PURE__ */ a.createElement("svg", s({
    xmlns: "http://www.w3.org/2000/svg",
    viewBox: "0 85.333 512 341.333"
  }, f), u && /* @__PURE__ */ a.createElement("title", null, u), /* @__PURE__ */ a.createElement("path", {
    fill: "#496E2D",
    d: "M0 85.331h512v341.337H0z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#FF9811",
    d: "M490.668 195.476h-48c0-8.836-7.164-16-16-16s-16 7.164-16 16h-48c0 8.836 7.697 16 16.533 16h-.533c0 8.836 7.162 16 16 16 0 8.836 7.162 16 16 16h32c8.836 0 16-7.164 16-16 8.836 0 16-7.164 16-16h-.533c8.837 0 16.533-7.164 16.533-16z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#D80027",
    d: "M341.337 255.994h56.888v170.663h-56.888z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#FF9811",
    d: "M455.112 255.994H512v170.663h-56.888z"
  }), /* @__PURE__ */ a.createElement("path", {
    d: "M398.225 255.994h56.888v170.663h-56.888z"
  }));
}, N9 = function(i) {
  var u = i.title, f = d(i, so);
  return /* @__PURE__ */ a.createElement("svg", s({
    xmlns: "http://www.w3.org/2000/svg",
    viewBox: "0 85.333 512 341.333"
  }, f), u && /* @__PURE__ */ a.createElement("title", null, u), /* @__PURE__ */ a.createElement("path", {
    d: "M0 85.333h512V426.67H0z"
  }), /* @__PURE__ */ a.createElement("path", {
    d: "M0 134.196h512v48.868H0zm0 195.472h512v48.868H0z",
    fill: "#FFDA44"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#057f44",
    d: "M0 85.333h512v48.868H0z"
  }), /* @__PURE__ */ a.createElement("path", {
    d: "M0 183.069h512v48.868H0zm0 97.737h512v48.868H0z",
    fill: "#D80027"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#057f44",
    d: "M0 378.542h512v48.128H0z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#FFF",
    d: "M276.992 255.996 106.329 426.659H0V85.333h106.329z"
  }), /* @__PURE__ */ a.createElement("path", {
    d: "M256 255.996 85.334 426.662h20.987l170.667-170.666L106.321 85.33H85.334z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#D80027",
    d: "m102.465 202.57 13.259 40.812h42.917l-34.718 25.226 13.26 40.814-34.718-25.224-34.72 25.224 13.262-40.814-34.718-25.226h42.915z"
  }), /* @__PURE__ */ a.createElement("path", {
    fill: "#FFDA44",
    d: "m138.94 259.335-34.559-12.243s-2.553-23.955-2.708-24.766c-1.173-6.18-6.603-10.851-13.123-10.851-7.376 0-13.357 5.98-13.357 13.357 0 1.223.178 2.402.486 3.528l-9.689 9.755h17.229c0 17.882-13.344 17.882-13.344 35.691l7.402 17.809h44.522l7.422-17.809h-.004a17.782 17.782 0 0 0 1.381-5.231c6.397-2.589 8.342-9.24 8.342-9.24z"
  }));
};
const Eo = {
  AC: Y6,
  AD: q6,
  AE: Q6,
  AF: X6,
  AG: Z6,
  AI: K6,
  AL: J6,
  AM: k6,
  AO: W6,
  AQ: $6,
  AR: I6,
  AS: P6,
  AT: e7,
  AU: t7,
  AW: l7,
  AX: a7,
  AZ: n7,
  BA: c7,
  BB: i7,
  BD: u7,
  BE: r7,
  BF: f7,
  BG: h7,
  BH: m7,
  BI: v7,
  BJ: s7,
  BL: d7,
  BM: o7,
  BN: E7,
  BO: z7,
  BQ_BO: p7,
  BQ_SA: g7,
  BQ_SE: M7,
  BQ: w7,
  BR: x7,
  BS: H7,
  BT: y7,
  BV: F7,
  BW: D7,
  BY: S7,
  BZ: A7,
  CA: B7,
  CC: b7,
  CD: T7,
  CF: L7,
  CG: V7,
  CH: C7,
  CI: O7,
  CK: N7,
  CL: _7,
  CM: U7,
  CN: R7,
  CO: j7,
  CR: G7,
  CU: Y7,
  CV: q7,
  CW: Q7,
  CX: X7,
  CY: Z7,
  CZ: K7,
  DE: J7,
  DJ: k7,
  DK: W7,
  DM: $7,
  DO: I7,
  DZ: P7,
  EC: ei,
  EE: ti,
  EG: li,
  EH: ai,
  ER: ni,
  ES_CT: ci,
  ES: ii,
  ET: ui,
  EU: ri,
  FI: fi,
  FJ: hi,
  FK: mi,
  FM: vi,
  FO: si,
  FR: di,
  GA: oi,
  GB_ENG: Ei,
  GB_NIR: zi,
  GB_SCT: pi,
  GB_WLS: gi,
  GB: Mi,
  GD: wi,
  GE: xi,
  GF: Hi,
  GG: yi,
  GH: Fi,
  GI: Di,
  GL: Si,
  GM: Ai,
  GN: Bi,
  GP: bi,
  GQ: Ti,
  GR: Li,
  GS: Vi,
  GT: Ci,
  GU: Oi,
  GW: Ni,
  GY: _i,
  HK: Ui,
  HM: Ri,
  HN: ji,
  HR: Gi,
  HT: Yi,
  HU: qi,
  IC: Qi,
  ID: Xi,
  IE: Zi,
  IL: Ki,
  IM: Ji,
  IN: ki,
  IO: Wi,
  IQ: $i,
  IR: Ii,
  IS: Pi,
  IT: e8,
  JE: t8,
  JM: l8,
  JO: a8,
  JP: n8,
  KE: c8,
  KG: i8,
  KH: u8,
  KI: r8,
  KM: f8,
  KN: h8,
  KP: m8,
  KR: v8,
  KW: s8,
  KY: d8,
  KZ: o8,
  LA: E8,
  LB: z8,
  LC: p8,
  LI: g8,
  LK: M8,
  LR: w8,
  LS: x8,
  LT: H8,
  LU: y8,
  LV: F8,
  LY: D8,
  MA: S8,
  MC: A8,
  MD: B8,
  ME: b8,
  MF: T8,
  MG: L8,
  MH: V8,
  MK: C8,
  ML: O8,
  MM: N8,
  MN: _8,
  MO: U8,
  MP: R8,
  MQ: j8,
  MR: G8,
  MS: Y8,
  MT: q8,
  MU: Q8,
  MV: X8,
  MW: Z8,
  MX: K8,
  MY: J8,
  MZ: k8,
  NA: W8,
  NC: $8,
  NE: I8,
  NF: P8,
  NG: eu,
  NI: tu,
  NL: lu,
  NO: au,
  NP: nu,
  NR: cu,
  NU: iu,
  NZ: uu,
  OM: ru,
  PA: fu,
  PE: hu,
  PF: mu,
  PG: vu,
  PH: su,
  PK: du,
  PL: ou,
  PM: Eu,
  PN: zu,
  PR: pu,
  PS: gu,
  PT: Mu,
  PW: wu,
  PY: xu,
  QA: Hu,
  RE: yu,
  RO: Fu,
  RS: Du,
  RU: Su,
  RW: Au,
  SA: Bu,
  SB: bu,
  SC: Tu,
  SD: Lu,
  SE: Vu,
  SG: Cu,
  SH: Ou,
  SI: Nu,
  SJ: _u,
  SK: Uu,
  SL: Ru,
  SM: ju,
  SN: Gu,
  SO: Yu,
  SR: qu,
  SS: Qu,
  ST: Xu,
  SV: Zu,
  SX: Ku,
  SY: Ju,
  SZ: ku,
  TA: Wu,
  TC: $u,
  TD: Iu,
  TF: Pu,
  TG: e9,
  TH: t9,
  TJ: l9,
  TK: a9,
  TL: n9,
  TM: c9,
  TN: i9,
  TO: u9,
  TR: r9,
  TT: f9,
  TV: h9,
  TW: m9,
  TZ: v9,
  UA: s9,
  UG: d9,
  UM: o9,
  US: E9,
  UY: z9,
  UZ: p9,
  VA: g9,
  VC: M9,
  VE: w9,
  VG: x9,
  VI: H9,
  VN: y9,
  VU: F9,
  WF: D9,
  WS: S9,
  XA: A9,
  XC: B9,
  XK: b9,
  XO: T9,
  YE: L9,
  YT: V9,
  ZA: C9,
  ZM: O9,
  ZW: N9
}, zo = /* @__PURE__ */ Object.freeze(/* @__PURE__ */ Object.defineProperty({
  __proto__: null,
  AC: Y6,
  AD: q6,
  AE: Q6,
  AF: X6,
  AG: Z6,
  AI: K6,
  AL: J6,
  AM: k6,
  AO: W6,
  AQ: $6,
  AR: I6,
  AS: P6,
  AT: e7,
  AU: t7,
  AW: l7,
  AX: a7,
  AZ: n7,
  BA: c7,
  BB: i7,
  BD: u7,
  BE: r7,
  BF: f7,
  BG: h7,
  BH: m7,
  BI: v7,
  BJ: s7,
  BL: d7,
  BM: o7,
  BN: E7,
  BO: z7,
  BQ: w7,
  BQ_BO: p7,
  BQ_SA: g7,
  BQ_SE: M7,
  BR: x7,
  BS: H7,
  BT: y7,
  BV: F7,
  BW: D7,
  BY: S7,
  BZ: A7,
  CA: B7,
  CC: b7,
  CD: T7,
  CF: L7,
  CG: V7,
  CH: C7,
  CI: O7,
  CK: N7,
  CL: _7,
  CM: U7,
  CN: R7,
  CO: j7,
  CR: G7,
  CU: Y7,
  CV: q7,
  CW: Q7,
  CX: X7,
  CY: Z7,
  CZ: K7,
  DE: J7,
  DJ: k7,
  DK: W7,
  DM: $7,
  DO: I7,
  DZ: P7,
  EC: ei,
  EE: ti,
  EG: li,
  EH: ai,
  ER: ni,
  ES: ii,
  ES_CT: ci,
  ET: ui,
  EU: ri,
  FI: fi,
  FJ: hi,
  FK: mi,
  FM: vi,
  FO: si,
  FR: di,
  GA: oi,
  GB: Mi,
  GB_ENG: Ei,
  GB_NIR: zi,
  GB_SCT: pi,
  GB_WLS: gi,
  GD: wi,
  GE: xi,
  GF: Hi,
  GG: yi,
  GH: Fi,
  GI: Di,
  GL: Si,
  GM: Ai,
  GN: Bi,
  GP: bi,
  GQ: Ti,
  GR: Li,
  GS: Vi,
  GT: Ci,
  GU: Oi,
  GW: Ni,
  GY: _i,
  HK: Ui,
  HM: Ri,
  HN: ji,
  HR: Gi,
  HT: Yi,
  HU: qi,
  IC: Qi,
  ID: Xi,
  IE: Zi,
  IL: Ki,
  IM: Ji,
  IN: ki,
  IO: Wi,
  IQ: $i,
  IR: Ii,
  IS: Pi,
  IT: e8,
  JE: t8,
  JM: l8,
  JO: a8,
  JP: n8,
  KE: c8,
  KG: i8,
  KH: u8,
  KI: r8,
  KM: f8,
  KN: h8,
  KP: m8,
  KR: v8,
  KW: s8,
  KY: d8,
  KZ: o8,
  LA: E8,
  LB: z8,
  LC: p8,
  LI: g8,
  LK: M8,
  LR: w8,
  LS: x8,
  LT: H8,
  LU: y8,
  LV: F8,
  LY: D8,
  MA: S8,
  MC: A8,
  MD: B8,
  ME: b8,
  MF: T8,
  MG: L8,
  MH: V8,
  MK: C8,
  ML: O8,
  MM: N8,
  MN: _8,
  MO: U8,
  MP: R8,
  MQ: j8,
  MR: G8,
  MS: Y8,
  MT: q8,
  MU: Q8,
  MV: X8,
  MW: Z8,
  MX: K8,
  MY: J8,
  MZ: k8,
  NA: W8,
  NC: $8,
  NE: I8,
  NF: P8,
  NG: eu,
  NI: tu,
  NL: lu,
  NO: au,
  NP: nu,
  NR: cu,
  NU: iu,
  NZ: uu,
  OM: ru,
  PA: fu,
  PE: hu,
  PF: mu,
  PG: vu,
  PH: su,
  PK: du,
  PL: ou,
  PM: Eu,
  PN: zu,
  PR: pu,
  PS: gu,
  PT: Mu,
  PW: wu,
  PY: xu,
  QA: Hu,
  RE: yu,
  RO: Fu,
  RS: Du,
  RU: Su,
  RW: Au,
  SA: Bu,
  SB: bu,
  SC: Tu,
  SD: Lu,
  SE: Vu,
  SG: Cu,
  SH: Ou,
  SI: Nu,
  SJ: _u,
  SK: Uu,
  SL: Ru,
  SM: ju,
  SN: Gu,
  SO: Yu,
  SR: qu,
  SS: Qu,
  ST: Xu,
  SV: Zu,
  SX: Ku,
  SY: Ju,
  SZ: ku,
  TA: Wu,
  TC: $u,
  TD: Iu,
  TF: Pu,
  TG: e9,
  TH: t9,
  TJ: l9,
  TK: a9,
  TL: n9,
  TM: c9,
  TN: i9,
  TO: u9,
  TR: r9,
  TT: f9,
  TV: h9,
  TW: m9,
  TZ: v9,
  UA: s9,
  UG: d9,
  UM: o9,
  US: E9,
  UY: z9,
  UZ: p9,
  VA: g9,
  VC: M9,
  VE: w9,
  VG: x9,
  VI: H9,
  VN: y9,
  VU: F9,
  WF: D9,
  WS: S9,
  XA: A9,
  XC: B9,
  XK: b9,
  XO: T9,
  YE: L9,
  YT: V9,
  ZA: C9,
  ZM: O9,
  ZW: N9,
  default: Eo
}, Symbol.toStringTag, { value: "Module" })), k3 = ({ countryCode: m, size: i = 23 }) => {
  const u = zo[m];
  return u ? /* @__PURE__ */ F.jsx(
    "div",
    {
      className: "flex items-center justify-center rounded-full overflow-hidden shrink-0",
      style: {
        width: `${i}px`,
        height: `${i}px`
      },
      children: /* @__PURE__ */ F.jsx(
        u,
        {
          style: {
            width: `${i}px`,
            height: `${i}px`,
            display: "block"
          },
          title: m
        }
      )
    }
  ) : /* @__PURE__ */ F.jsx(
    "div",
    {
      className: "flex items-center justify-center rounded-full overflow-hidden shrink-0 bg-gray-200",
      style: { width: `${i}px`, height: `${i}px`, fontSize: `${i * 0.6}px` },
      title: `Bandeira não disponível: ${m}`,
      children: "🌏"
    }
  );
}, po = {
  BRL: "BR",
  USD: "US",
  EUR: "EU",
  GBP: "GB",
  JPY: "JP"
}, W3 = (m) => po[m], _9 = ({
  size: m = 20,
  color: i = "#525252",
  onClick: u,
  className: f = ""
}) => {
  const E = /* @__PURE__ */ F.jsx(
    "svg",
    {
      width: m,
      height: m,
      viewBox: "0 0 20 20",
      fill: "none",
      xmlns: "http://www.w3.org/2000/svg",
      className: "shrink-0",
      children: /* @__PURE__ */ F.jsx(
        "path",
        {
          d: "M5 7.5L10 12.5L15 7.5",
          stroke: i,
          strokeWidth: "1.5",
          strokeLinecap: "round",
          strokeLinejoin: "round"
        }
      )
    }
  );
  return u ? /* @__PURE__ */ F.jsx(
    "button",
    {
      type: "button",
      onClick: u,
      className: `chevron-button inline-flex items-center justify-center p-0 m-0 border-0 bg-transparent cursor-pointer hover:opacity-70 transition-opacity focus:outline-none focus:ring-0 active:bg-transparent ${f}`,
      style: {
        width: m,
        height: m,
        minWidth: m,
        minHeight: m,
        backgroundColor: "transparent",
        background: "transparent",
        border: "none",
        outline: "none",
        boxShadow: "none"
      },
      "aria-label": "Abrir dropdown",
      children: E
    }
  ) : /* @__PURE__ */ F.jsx("span", { className: f, children: E });
}, go = (m, i, u) => {
  const f = u[m], E = u[i];
  return !f || !E ? "-" : (E / f).toFixed(2);
}, b2 = ({
  currency: m,
  currencies: i,
  onCurrencyChange: u,
  exchangeRates: f = {}
}) => {
  const [E, O] = ye.useState(!1), [G, Q] = ye.useState({ top: 0, right: 0 }), T = ye.useRef(null), H = ye.useRef(null), R = ye.useRef(null), k = ye.useRef(null), Y = [...i].sort(
    (W, Fe) => W.code.localeCompare(Fe.code)
  ), je = (W) => {
    u(W), O(!1);
  };
  ye.useEffect(() => {
    if (E && T.current) {
      const W = T.current.getBoundingClientRect();
      Q({
        top: W.bottom + window.scrollY + 4,
        // 4px de margem
        right: window.innerWidth - W.right + window.scrollX
      });
    }
  }, [E]), ye.useEffect(() => {
    const W = (Fe) => {
      E && T.current && !T.current.contains(Fe.target) && H.current && !H.current.contains(Fe.target) && O(!1);
    };
    if (E)
      return document.addEventListener("mousedown", W), () => {
        document.removeEventListener("mousedown", W);
      };
  }, [E]);
  const Ve = E ? /* @__PURE__ */ F.jsx(
    "div",
    {
      ref: H,
      className: "fixed bg-white border border-gray-200 rounded-lg shadow-lg max-h-60 overflow-y-auto min-w-[120px] w-auto",
      style: {
        top: `${G.top}px`,
        right: `${G.right}px`,
        zIndex: 9999
      },
      children: Y.map((W) => /* @__PURE__ */ F.jsxs(
        "button",
        {
          type: "button",
          onClick: () => je(W),
          className: `w-full px-4 py-2 text-left hover:bg-gray-100 flex items-center justify-center gap-2 ${W.code === m.code ? "bg-blue-50" : ""}`,
          children: [
            /* @__PURE__ */ F.jsx("div", { className: "w-5 h-5 shrink-0 rounded-full overflow-hidden flex items-center justify-center", children: /* @__PURE__ */ F.jsx(
              k3,
              {
                countryCode: W3(W.code),
                size: 30
              }
            ) }),
            /* @__PURE__ */ F.jsx("span", { className: "font-inter text-sm", children: W.code }),
            f && Object.keys(f).length > 0 && /* @__PURE__ */ F.jsx("span", { className: "ml-auto text-xs text-gray-500", children: go(m.code, W.code, f) })
          ]
        },
        W.code
      ))
    }
  ) : null;
  return /* @__PURE__ */ F.jsxs(F.Fragment, { children: [
    /* @__PURE__ */ F.jsxs(
      "div",
      {
        ref: T,
        className: "relative w-auto min-w-[90px] sm:min-w-[100px] h-5 flex items-center justify-end gap-1 shrink-0",
        children: [
          /* @__PURE__ */ F.jsx(
            "div",
            {
              ref: R,
              className: "w-5 h-5 shrink-0 rounded-full overflow-hidden flex items-center justify-center",
              children: /* @__PURE__ */ F.jsx(
                k3,
                {
                  countryCode: W3(m.code),
                  size: 30
                }
              )
            }
          ),
          /* @__PURE__ */ F.jsx(
            "span",
            {
              ref: k,
              className: "font-inter font-semibold text-sm sm:text-base leading-5 text-wl-neutral-600 whitespace-nowrap",
              children: m.code
            }
          ),
          /* @__PURE__ */ F.jsx(
            _9,
            {
              size: 20,
              color: "#525252",
              onClick: () => O(!E)
            }
          )
        ]
      }
    ),
    typeof document < "u" && ph.createPortal(Ve, document.body)
  ] });
}, He = ({
  width: m = "100%",
  height: i = "1rem",
  borderRadius: u = "4px",
  className: f = "",
  variant: E = "rectangular",
  animation: O = "pulse"
}) => {
  const G = "bg-gray-200 dark:bg-gray-700", Q = {
    text: "h-4 rounded",
    circular: "rounded-full",
    rectangular: "rounded",
    input: "h-11 rounded-xl",
    button: "h-10 rounded-lg"
  }, T = {
    pulse: "animate-pulse",
    wave: "skeleton-wave",
    none: ""
  }, H = {
    width: typeof m == "number" ? `${m}px` : m,
    height: typeof i == "number" ? `${i}px` : i,
    borderRadius: E === "circular" ? "50%" : u
  };
  return /* @__PURE__ */ F.jsx(
    "div",
    {
      className: `${G} ${Q[E]} ${T[O]} ${f}`,
      style: H,
      "aria-label": "Carregando...",
      role: "status"
    }
  );
}, Mo = ({
  isMobile: m = !1
}) => m ? /* @__PURE__ */ F.jsxs("div", { className: "flex flex-col gap-5 bg-white border border-wl-neutral-200 rounded-lg shadow-sm w-full max-w-md mx-auto p-4 sm:p-6", children: [
  /* @__PURE__ */ F.jsx(He, { variant: "text", width: "60%", height: "28px", className: "mb-2" }),
  /* @__PURE__ */ F.jsxs("div", { className: "flex flex-col items-center gap-2 w-full", children: [
    /* @__PURE__ */ F.jsxs("div", { className: "flex flex-row items-center justify-between w-full h-11 px-3 sm:px-4 py-3 bg-white border border-wl-neutral-200 rounded-xl", children: [
      /* @__PURE__ */ F.jsx(He, { variant: "text", width: "40%", height: "20px" }),
      /* @__PURE__ */ F.jsxs("div", { className: "flex items-center gap-1", children: [
        /* @__PURE__ */ F.jsx(He, { variant: "circular", width: 20, height: 20 }),
        /* @__PURE__ */ F.jsx(He, { variant: "text", width: "30px", height: "16px" }),
        /* @__PURE__ */ F.jsx(He, { variant: "text", width: "8px", height: "8px" })
      ] })
    ] }),
    /* @__PURE__ */ F.jsx(He, { variant: "button", width: 48, height: 48, className: "rounded-full" }),
    /* @__PURE__ */ F.jsxs("div", { className: "flex flex-row items-center justify-between w-full h-11 px-3 sm:px-4 py-3 bg-white border border-wl-neutral-200 rounded-xl", children: [
      /* @__PURE__ */ F.jsx(He, { variant: "text", width: "40%", height: "20px" }),
      /* @__PURE__ */ F.jsxs("div", { className: "flex items-center gap-1", children: [
        /* @__PURE__ */ F.jsx(He, { variant: "circular", width: 20, height: 20 }),
        /* @__PURE__ */ F.jsx(He, { variant: "text", width: "30px", height: "16px" }),
        /* @__PURE__ */ F.jsx(He, { variant: "text", width: "8px", height: "8px" })
      ] })
    ] }),
    /* @__PURE__ */ F.jsx(He, { variant: "text", width: "80%", height: "16px", className: "mt-2" })
  ] })
] }) : /* @__PURE__ */ F.jsxs("div", { className: "flex flex-col gap-5 bg-white border border-wl-neutral-200 rounded-lg shadow-sm w-full max-w-6xl mx-auto p-4 sm:p-6", children: [
  /* @__PURE__ */ F.jsx(He, { variant: "text", width: "40%", height: "28px", className: "mb-2" }),
  /* @__PURE__ */ F.jsxs("div", { className: "flex flex-col sm:flex-row items-stretch sm:items-center gap-4 sm:gap-6 w-full", children: [
    /* @__PURE__ */ F.jsxs("div", { className: "flex flex-row items-center flex-1 min-w-0 h-11 px-3 sm:px-4 py-3 bg-white border border-wl-neutral-200 rounded-xl", children: [
      /* @__PURE__ */ F.jsx(He, { variant: "text", width: "50%", height: "20px" }),
      /* @__PURE__ */ F.jsxs("div", { className: "flex items-center gap-1 ml-auto", children: [
        /* @__PURE__ */ F.jsx(He, { variant: "circular", width: 20, height: 20 }),
        /* @__PURE__ */ F.jsx(He, { variant: "text", width: "30px", height: "16px" }),
        /* @__PURE__ */ F.jsx(He, { variant: "text", width: "8px", height: "8px" })
      ] })
    ] }),
    /* @__PURE__ */ F.jsx("div", { className: "flex justify-center sm:justify-start", children: /* @__PURE__ */ F.jsx(He, { variant: "button", width: 48, height: 48, className: "rounded-full" }) }),
    /* @__PURE__ */ F.jsxs("div", { className: "flex flex-row items-center flex-1 min-w-0 h-11 px-3 sm:px-4 py-3 bg-white border border-wl-neutral-200 rounded-xl", children: [
      /* @__PURE__ */ F.jsx(He, { variant: "text", width: "50%", height: "20px" }),
      /* @__PURE__ */ F.jsxs("div", { className: "flex items-center gap-1 ml-auto", children: [
        /* @__PURE__ */ F.jsx(He, { variant: "circular", width: 20, height: 20 }),
        /* @__PURE__ */ F.jsx(He, { variant: "text", width: "30px", height: "16px" }),
        /* @__PURE__ */ F.jsx(He, { variant: "text", width: "8px", height: "8px" })
      ] })
    ] })
  ] }),
  /* @__PURE__ */ F.jsxs("div", { className: "flex flex-col items-center gap-2 w-full", children: [
    /* @__PURE__ */ F.jsxs("div", { className: "flex flex-row flex-wrap justify-center items-center gap-2 w-full px-2", children: [
      /* @__PURE__ */ F.jsx(He, { variant: "text", width: "120px", height: "40px", className: "sm:h-12 lg:h-16" }),
      /* @__PURE__ */ F.jsx(He, { variant: "text", width: "20px", height: "32px", className: "sm:h-8" }),
      /* @__PURE__ */ F.jsx(He, { variant: "text", width: "120px", height: "40px", className: "sm:h-12 lg:h-16" })
    ] }),
    /* @__PURE__ */ F.jsx(He, { variant: "text", width: "60%", height: "16px" })
  ] })
] }), U9 = ye.createContext(void 0), wo = ({ children: m }) => {
  const [i, u] = a.useState(!1);
  return /* @__PURE__ */ F.jsx(U9.Provider, { value: { isLoading: i, setIsLoading: u }, children: m });
}, xo = () => {
  const m = ye.useContext(U9);
  if (!m)
    throw new Error("useSkeleton must be used within SkeletonProvider");
  return m;
}, R9 = ye.createContext(void 0), Ho = ({
  children: m,
  defaultDevice: i
}) => {
  const [u, f] = ye.useState(() => i || (typeof window < "u" && window.innerWidth < 768 ? "mobile" : "web")), [E, O] = ye.useState(() => typeof window < "u" ? window.innerWidth : 1024);
  ye.useEffect(() => {
    const T = () => {
      const H = window.innerWidth;
      if (O(H), !i) {
        const R = H < 768 ? "mobile" : "web";
        f(R);
      }
    };
    return T(), window.addEventListener("resize", T), () => {
      window.removeEventListener("resize", T);
    };
  }, [i]);
  const Q = {
    device: u,
    isMobile: u === "mobile",
    isWeb: u === "web",
    width: E,
    setDevice: (T) => {
      f(T);
    }
  };
  return /* @__PURE__ */ F.jsx(R9.Provider, { value: Q, children: m });
}, yo = () => {
  const m = ye.useContext(R9);
  if (!m)
    throw new Error("useDevice must be used within DeviceProvider");
  return m;
}, Fo = {
  JPY: 0,
  KRW: 0,
  VND: 0,
  HUF: 0,
  CLP: 0,
  ISK: 0,
  TWD: 0,
  KWD: 3,
  BHD: 3,
  JOD: 3,
  OMR: 3,
  TND: 3,
  LYD: 3
}, Wt = (m) => Fo[m] ?? 2, j9 = (m, i) => {
  const u = m.lastIndexOf(","), f = m.lastIndexOf(".");
  return u > f && u !== -1 ? "," : f > u && f !== -1 ? "." : i;
}, A2 = (m, i, u, f) => {
  if (m === 0)
    return i > 0 ? `0${u}${"0".repeat(i)}` : "0";
  const E = m.toFixed(i), [O, G = ""] = E.split("."), Q = O.replace(
    /\B(?=(\d{3})+(?!\d))/g,
    f
  );
  return G ? `${Q}${u}${G}` : Q;
}, b6 = (m, i, u) => {
  let f = m.replace(/[^\d.,]/g, "");
  if (f === "") return "";
  if (i === 0) return f;
  const E = j9(f, u), O = f.split(E), G = O[0], Q = O.slice(1).join("");
  return O.length === 1 ? G : Q === "" ? `${G}${E}` : `${G}${E}${Q}`;
}, T6 = (m, i, u) => {
  const f = m.trim();
  if (f === "" || f === u) return 0;
  if (i === 0) {
    const H = f.replace(/[^\d]/g, "");
    if (H === "") return 0;
    const R = parseInt(H, 10);
    return Number.isNaN(R) ? 0 : R;
  }
  const E = j9(f, u), O = E === "," ? "." : ",", Q = f.replace(new RegExp(`\\${O}`, "g"), "").replace(new RegExp(`\\${E}`), "."), T = parseFloat(Q);
  return Number.isNaN(T) ? 0 : T;
}, Do = () => /* @__PURE__ */ F.jsxs(
  "svg",
  {
    width: 50,
    height: 50,
    viewBox: "0 0 48 48",
    fill: "none",
    xmlns: "http://www.w3.org/2000/svg",
    children: [
      /* @__PURE__ */ F.jsx("circle", { cx: "24", cy: "24", r: "24", fill: "#007bff" }),
      /* @__PURE__ */ F.jsx("line", { x1: "18", y1: "18", x2: "30", y2: "18", stroke: "white", strokeWidth: "2", strokeLinecap: "round" }),
      /* @__PURE__ */ F.jsx(
        "path",
        {
          d: "M20 16 L18 18 L20 20",
          stroke: "white",
          strokeWidth: "2",
          strokeLinecap: "round",
          strokeLinejoin: "round",
          fill: "none"
        }
      ),
      /* @__PURE__ */ F.jsx("line", { x1: "18", y1: "30", x2: "30", y2: "30", stroke: "white", strokeWidth: "2", strokeLinecap: "round" }),
      /* @__PURE__ */ F.jsx(
        "path",
        {
          d: "M28 28 L30 30 L28 32",
          stroke: "white",
          strokeWidth: "2",
          strokeLinecap: "round",
          strokeLinejoin: "round",
          fill: "none"
        }
      )
    ]
  }
), So = () => /* @__PURE__ */ F.jsxs(
  "svg",
  {
    width: 50,
    height: 50,
    viewBox: "0 0 48 48",
    fill: "none",
    xmlns: "http://www.w3.org/2000/svg",
    children: [
      /* @__PURE__ */ F.jsx("circle", { cx: "24", cy: "24", r: "24", fill: "#007bff" }),
      /* @__PURE__ */ F.jsx("line", { x1: "18", y1: "14", x2: "18", y2: "26", stroke: "white", strokeWidth: "3", strokeLinecap: "round" }),
      /* @__PURE__ */ F.jsx(
        "path",
        {
          d: "M16 24 L18 26 L20 24",
          stroke: "white",
          strokeWidth: "3",
          strokeLinecap: "round",
          strokeLinejoin: "round",
          fill: "none"
        }
      ),
      /* @__PURE__ */ F.jsx("line", { x1: "30", y1: "34", x2: "30", y2: "22", stroke: "white", strokeWidth: "3", strokeLinecap: "round" }),
      /* @__PURE__ */ F.jsx(
        "path",
        {
          d: "M28 24 L30 22 L32 24",
          stroke: "white",
          strokeWidth: "3",
          strokeLinecap: "round",
          strokeLinejoin: "round",
          fill: "none"
        }
      )
    ]
  }
), Ao = ({ onClick: m }) => /* @__PURE__ */ F.jsx(
  "button",
  {
    type: "button",
    onClick: m,
    title: "Trocar moedas",
    className: "inline-flex items-center justify-center p-0 m-0 bg-transparent border-0 outline-none appearance-none cursor-pointer",
    style: {
      width: 50,
      height: 50,
      boxShadow: "none",
      background: "transparent",
      border: "none",
      padding: 0,
      margin: 0
    },
    "aria-label": "Trocar moedas",
    children: /* @__PURE__ */ F.jsx(Do, {})
  }
), Bo = ({ onClick: m }) => /* @__PURE__ */ F.jsx(
  "button",
  {
    type: "button",
    onClick: m,
    title: "Trocar moedas",
    className: "inline-flex items-center justify-center p-0 m-0 bg-transparent border-0 outline-none appearance-none cursor-pointer",
    style: {
      width: 50,
      height: 50,
      transform: "rotate(-160deg)",
      transformOrigin: "center center",
      boxShadow: "none",
      background: "transparent",
      border: "none",
      padding: 0,
      margin: 0,
      transition: "transform 0.2s ease"
    },
    "aria-label": "Trocar moedas",
    children: /* @__PURE__ */ F.jsx(So, {})
  }
), G9 = ({
  fromValue: m,
  toValue: i,
  fromCurrency: u,
  toCurrency: f,
  rate: E,
  currencies: O,
  exchangeRates: G,
  device: Q = "web",
  lastUpdated: T,
  onFromValueChange: H,
  onToValueChange: R,
  onFromCurrencyChange: k,
  onToCurrencyChange: Y,
  onSwap: je,
  converterData: Ve
}) => {
  const W = Ve?.currencies ?? O, Fe = Ve?.exchangeRates ?? G, ne = Ve?.lastUpdated ?? T, oe = E ?? (Fe ? (Fe[f.code] ?? 1) / (Fe[u.code] ?? 1) : 1), f1 = ne ?? j6(), $e = (navigator.language || "pt-BR").startsWith("pt"), ve = $e ? "," : ".", be = $e ? "." : ",", ce = a.useRef(null), [Ue, Pe] = a.useState(""), [h1, Ge] = a.useState(""), m1 = (ue) => {
    const he = Wt(u.code), Xe = b6(
      ue.target.value,
      he,
      ve
    );
    if (Pe(Xe), Xe.trim() === "" || Xe === ve) {
      H(0);
      return;
    }
    const H1 = T6(
      Xe,
      he,
      ve
    );
    if (Number.isNaN(H1)) {
      H(0);
      return;
    }
    H(H1);
  }, G1 = (ue) => {
    if (!R) return;
    const he = Wt(f.code), Xe = b6(
      ue.target.value,
      he,
      ve
    );
    if (Ge(Xe), Xe.trim() === "" || Xe === ve) {
      R(0);
      return;
    }
    const H1 = T6(
      Xe,
      he,
      ve
    );
    if (Number.isNaN(H1)) {
      R(0);
      return;
    }
    R(H1);
  }, { isMobile: v1 } = yo(), C = Q === "mobile" ? !0 : Q === "web" ? !1 : v1, { isLoading: b } = xo();
  if (b)
    return /* @__PURE__ */ F.jsx(Mo, { isMobile: C });
  const L = a.useRef(!1), I = a.useRef(!1), ge = Ue.trim() !== "", e1 = h1.trim() !== "", p = m, B = i, V = A2(
    m,
    Wt(u.code),
    ve,
    be
  ), _ = A2(
    i,
    Wt(f.code),
    ve,
    be
  );
  a.useEffect(() => {
    if (!L.current)
      if (m === 0)
        Pe("");
      else {
        const ue = A2(
          m,
          Wt(u.code),
          ve,
          be
        );
        Pe(ue);
      }
  }, [m, u.code, ve, be]), a.useEffect(() => {
    if (!I.current)
      if (i === 0)
        Ge("");
      else {
        const ue = A2(
          i,
          Wt(f.code),
          ve,
          be
        );
        Ge(ue);
      }
  }, [i, f.code, ve, be]);
  const J = (ue, he) => ue === 0 ? "" : A2(
    ue,
    Wt(he),
    ve,
    be
  ), $ = (ue, he, Xe, H1) => {
    Pe(J(ue, Xe)), Ge(J(he, H1));
  }, ie = () => {
    const ue = {
      fromValue: p,
      toValue: B,
      hasFromInput: ge,
      hasToInput: e1
    };
    $(
      ue.toValue,
      ue.fromValue,
      f.code,
      u.code
    ), je(ue);
  };
  return C ? /* @__PURE__ */ F.jsxs(
    "div",
    {
      className: "flex flex-col gap-5 bg-white border border-wl-neutral-200 rounded-lg shadow-sm w-full max-w-md mx-auto p-4 sm:p-6",
      children: [
        /* @__PURE__ */ F.jsx("h2", { className: "font-inter font-medium text-xl leading-7 tracking-tight-xs text-wl-neutral-950 m-0 text-center px-3 sm:px-4", children: "Conversor de Moedas" }),
        /* @__PURE__ */ F.jsxs("div", { className: "flex flex-col items-center gap-2 w-full", children: [
          /* @__PURE__ */ F.jsxs("div", { className: "flex flex-row items-center gap-2 w-full h-11 px-3 sm:px-4 py-3 bg-white border border-wl-neutral-600 rounded-xl relative transition-all hover:border-black hover:border-2 focus-within:border-black focus-within:border-2 overflow-hidden", children: [
            /* @__PURE__ */ F.jsx(
              "input",
              {
                type: "text",
                inputMode: "decimal",
                value: Ue,
                onChange: m1,
                onFocus: () => L.current = !0,
                onBlur: () => L.current = !1,
                placeholder: "0,00",
                className: "font-inter font-semibold text-sm sm:text-base leading-5 text-wl-neutral-600 min-w-0 w-full max-w-[calc(100%-90px)] h-5 border-0 outline-none bg-transparent placeholder-wl-neutral-400"
              }
            ),
            /* @__PURE__ */ F.jsx(
              b2,
              {
                currency: u,
                currencies: W,
                onCurrencyChange: k
              }
            )
          ] }),
          /* @__PURE__ */ F.jsx(Bo, { onClick: ie }),
          /* @__PURE__ */ F.jsxs("div", { className: "flex flex-row items-center gap-2 w-full h-11 px-3 sm:px-4 py-3 bg-white border border-wl-neutral-600 rounded-xl relative transition-all hover:border-black hover:border-2 focus-within:border-black focus-within:border-2 overflow-hidden", children: [
            /* @__PURE__ */ F.jsx(
              "input",
              {
                type: "text",
                inputMode: "decimal",
                value: h1,
                onChange: G1,
                onFocus: () => I.current = !0,
                onBlur: () => I.current = !1,
                placeholder: "0,00",
                className: "font-inter font-semibold text-sm sm:text-base leading-5 text-wl-neutral-600 min-w-0 w-full max-w-[calc(100%-90px)] h-5 border-0 outline-none bg-transparent placeholder-wl-neutral-400"
              }
            ),
            /* @__PURE__ */ F.jsx(
              b2,
              {
                currency: f,
                currencies: W,
                onCurrencyChange: Y
              }
            )
          ] }),
          /* @__PURE__ */ F.jsxs("div", { className: "flex flex-col items-center gap-1 w-full", children: [
            /* @__PURE__ */ F.jsxs("p", { className: "font-inter font-semibold text-sm leading-5 text-wl-neutral-700 text-center m-0 px-2", children: [
              u.symbol,
              " ",
              Ue || V,
              " = ",
              f.symbol,
              " ",
              h1 || _
            ] }),
            /* @__PURE__ */ F.jsxs("p", { className: "font-inter font-normal text-xs leading-4 text-wl-neutral-600 text-center m-0 px-2", children: [
              "1 ",
              u.code,
              " = ",
              oe.toFixed(2),
              " ",
              f.code,
              " · Atualizado em ",
              f1
            ] })
          ] })
        ] })
      ]
    }
  ) : /* @__PURE__ */ F.jsxs(
    "div",
    {
      className: "flex flex-col gap-5 bg-white border border-wl-neutral-200 rounded-lg shadow-sm w-full max-w-6xl mx-auto p-4 sm:p-6",
      children: [
        /* @__PURE__ */ F.jsx("h2", { className: "font-inter font-medium text-xl leading-7 tracking-tight-xs text-wl-neutral-950 m-0 text-left px-3 sm:px-4", children: "Conversor de Moedas" }),
        /* @__PURE__ */ F.jsxs(
          "div",
          {
            className: "flex flex-col sm:flex-row items-stretch sm:items-center gap-4 sm:gap-6 w-full",
            children: [
              /* @__PURE__ */ F.jsxs("div", { className: "flex flex-row items-center flex-1 min-w-0 h-11 px-4 py-3 bg-white border border-wl-neutral-600 rounded-xl transition-all hover:border-black hover:border-2 focus-within:border-black focus-within:border-2 overflow-hidden", children: [
                /* @__PURE__ */ F.jsx(
                  "input",
                  {
                    type: "text",
                    inputMode: "decimal",
                    value: Ue,
                    onChange: m1,
                    onFocus: () => L.current = !0,
                    onBlur: () => L.current = !1,
                    placeholder: "0,00",
                    className: "font-inter font-semibold text-sm sm:text-base leading-5 text-wl-neutral-600 min-w-0 flex-1 h-5 border-0 outline-none bg-transparent placeholder-wl-neutral-400"
                  }
                ),
                /* @__PURE__ */ F.jsx("div", { className: "shrink-0 ml-2", children: /* @__PURE__ */ F.jsx(
                  b2,
                  {
                    currency: u,
                    currencies: W,
                    onCurrencyChange: k
                  }
                ) })
              ] }),
              /* @__PURE__ */ F.jsx("div", { className: "flex justify-center sm:justify-start", children: /* @__PURE__ */ F.jsx(Ao, { onClick: ie }) }),
              /* @__PURE__ */ F.jsxs("div", { className: "flex flex-row items-center flex-1 min-w-0 h-11 px-4 py-3 bg-white border border-wl-neutral-600 rounded-xl transition-all hover:border-black hover:border-2 focus-within:border-black focus-within:border-2 overflow-hidden", children: [
                R ? /* @__PURE__ */ F.jsx(
                  "input",
                  {
                    ref: ce,
                    type: "text",
                    inputMode: "decimal",
                    value: h1,
                    onChange: G1,
                    onFocus: () => I.current = !0,
                    onBlur: () => I.current = !1,
                    placeholder: "0,00",
                    className: "min-w-0 flex-1 h-5 bg-transparent border-none font-inter font-semibold text-base leading-5 text-wl-neutral-600 placeholder-neutral-400 focus:outline-none p-0"
                  }
                ) : /* @__PURE__ */ F.jsx(
                  "span",
                  {
                    className: "min-w-0 flex-1 font-inter font-semibold text-base leading-5",
                    style: {
                      color: "var(--Colors-Content-One, rgba(82, 82, 82, 1))"
                    },
                    children: h1
                  }
                ),
                /* @__PURE__ */ F.jsx("div", { className: "shrink-0 ml-2", children: /* @__PURE__ */ F.jsx(
                  b2,
                  {
                    currency: f,
                    currencies: W,
                    onCurrencyChange: Y
                  }
                ) })
              ] })
            ]
          }
        ),
        /* @__PURE__ */ F.jsxs("div", { className: "flex flex-col items-center gap-2 w-full", children: [
          /* @__PURE__ */ F.jsxs("div", { className: "flex flex-row flex-wrap justify-center items-center gap-2 w-full px-2", children: [
            /* @__PURE__ */ F.jsxs("span", { className: "font-inter font-bold text-2xl sm:text-3xl lg:text-4xl leading-8 sm:leading-9 lg:leading-10 tracking-tight-md text-wl-neutral-600 text-center", children: [
              u.symbol,
              " ",
              V
            ] }),
            /* @__PURE__ */ F.jsx("span", { className: "font-inter font-semibold text-xl sm:text-2xl leading-7 sm:leading-8 tracking-tight-sm text-wl-neutral-600", children: "=" }),
            /* @__PURE__ */ F.jsxs("span", { className: "font-inter font-bold text-2xl sm:text-3xl lg:text-4xl leading-8 sm:leading-9 lg:leading-10 tracking-tight-md text-wl-neutral-600 text-center", children: [
              f.symbol,
              " ",
              _
            ] })
          ] }),
          /* @__PURE__ */ F.jsx("div", { className: "flex flex-col gap-1 w-full", children: /* @__PURE__ */ F.jsxs("p", { className: "font-inter font-normal text-xs leading-4 text-wl-neutral-600 text-center m-0 px-2", children: [
            "1 ",
            u.code,
            " = ",
            oe.toFixed(2),
            " ",
            f.code,
            " · Atualizado em ",
            f1
          ] }) })
        ] })
      ]
    }
  );
}, bo = /* @__PURE__ */ Object.freeze(/* @__PURE__ */ Object.defineProperty({
  __proto__: null,
  CurrencyConverter: G9
}, Symbol.toStringTag, { value: "Module" })), To = (m) => [
  "flex",
  "flex-row",
  "items-center",
  "gap-2",
  "w-[525px]",
  "h-11",
  "px-4",
  "py-3",
  "bg-white",
  "border",
  "border-[#525252]",
  "rounded-[12px]",
  "transition-all",
  "hover:ring-2",
  "hover:ring-black",
  "focus-within:ring-2",
  "focus-within:ring-black",
  "ring-offset-0",
  m ? "bg-[#F5F5F5] cursor-not-allowed" : "cursor-text"
].join(" "), Lo = (m) => [
  "flex-1",
  "bg-transparent",
  "font-inter",
  "font-semibold",
  "text-base",
  "leading-5",
  "placeholder-[#ABABA8]",
  "focus:outline-none",
  "border-none",
  m ? "cursor-not-allowed text-[#AAAAAA]" : ""
].filter(Boolean).join(" "), Xo = ({
  value: m,
  currency: i,
  currencies: u,
  disabled: f = !1,
  onValueChange: E,
  onCurrencyChange: O
}) => {
  const G = (Q) => {
    E(Number(Q.target.value) || 0);
  };
  return /* @__PURE__ */ F.jsxs("div", { className: To(f), children: [
    /* @__PURE__ */ F.jsx(
      b2,
      {
        currency: i,
        currencies: u,
        onCurrencyChange: O
      }
    ),
    /* @__PURE__ */ F.jsx("div", { className: "w-px h-6 bg-[#cccccc] shrink-0" }),
    /* @__PURE__ */ F.jsx(
      "input",
      {
        type: "number",
        value: m || "",
        onChange: G,
        disabled: f,
        placeholder: "0,00",
        className: Lo(f),
        style: {
          color: "var(--colors-Content-one, #525252)",
          opacity: 1
        }
      }
    )
  ] });
}, Vo = (m, i, u) => {
  const f = u[m], E = u[i];
  return !f || !E ? "-" : (E / f).toFixed(2);
}, Zo = ({
  currency: m,
  currencies: i,
  onCurrencyChange: u,
  listWidth: f,
  fromValue: E = 1,
  exchangeRates: O = {}
}) => {
  const [G, Q] = ye.useState(!1), [T, H] = ye.useState(-1), R = ye.useRef(null), k = ye.useMemo(
    () => [...i].sort((ne, oe) => ne.code.localeCompare(oe.code)),
    [i]
  ), Y = W3(m.code), je = (ne) => {
    R.current && (R.current.contains(ne.target) || Q(!1));
  };
  ye.useEffect(() => (document.addEventListener("mousedown", je), () => {
    document.removeEventListener("mousedown", je);
  }), []);
  const Ve = () => {
    Q((ne) => !ne), H(-1);
  }, W = (ne) => {
    u(ne), Q(!1), H(-1);
  }, Fe = (ne) => {
    if (ne.key === "Enter") {
      if (ne.preventDefault(), !G) {
        Q(!0), H(0);
        return;
      }
      const oe = k[T];
      oe && W(oe);
      return;
    }
    if (G) {
      if (ne.key === "Escape") {
        Q(!1), H(-1);
        return;
      }
      if (ne.key === "ArrowDown") {
        ne.preventDefault(), H((oe) => (oe + 1) % k.length);
        return;
      }
      if (ne.key === "ArrowUp") {
        ne.preventDefault(), H((oe) => (oe - 1 + k.length) % k.length);
        return;
      }
    }
  };
  return /* @__PURE__ */ F.jsxs("div", { children: [
    /* @__PURE__ */ F.jsxs("div", { className: "cs-root", ref: R, onKeyDown: Fe, children: [
      /* @__PURE__ */ F.jsxs(
        "div",
        {
          className: "cs-trigger",
          role: "combobox",
          "aria-expanded": G,
          "aria-haspopup": "listbox",
          children: [
            /* @__PURE__ */ F.jsx("span", { className: "cs-flag", "aria-hidden": !0, children: /* @__PURE__ */ F.jsx(k3, { countryCode: Y, size: 30 }) }),
            /* @__PURE__ */ F.jsx("span", { className: "cs-code", children: m.code }),
            /* @__PURE__ */ F.jsx(
              "button",
              {
                type: "button",
                "aria-label": "Abrir lista de moedas",
                "aria-expanded": G,
                onClick: Ve,
                className: "cs-chevron",
                children: /* @__PURE__ */ F.jsx(_9, { size: 20, color: "#525252" })
              }
            )
          ]
        }
      ),
      G && /* @__PURE__ */ F.jsx(
        "div",
        {
          role: "listbox",
          "aria-label": "Selecionar moeda",
          className: "cs-popover",
          style: {
            background: "#fff",
            width: f ? `${f}px` : void 0
          },
          children: /* @__PURE__ */ F.jsx("ul", { className: "cs-list", children: k.map((ne, oe) => {
            const f1 = W3(ne.code), Be = ne.code === m.code, $e = oe === T, ve = Vo(
              m.code,
              ne.code,
              O
            );
            return /* @__PURE__ */ F.jsxs(
              "li",
              {
                role: "option",
                "aria-selected": Be,
                className: "cs-option",
                "data-active": $e ? "true" : "false",
                onMouseEnter: () => H(oe),
                onClick: () => W(ne),
                children: [
                  /* @__PURE__ */ F.jsx("div", { className: "w-5 h-5 shrink-0 rounded-full overflow-hidden flex items-center justify-center", children: /* @__PURE__ */ F.jsx(k3, { countryCode: f1, size: 30 }) }),
                  /* @__PURE__ */ F.jsx("span", { className: "cs-option-code", children: ne.code }),
                  ve !== "-" && /* @__PURE__ */ F.jsxs("span", { className: "cs-option-rate", children: [
                    E,
                    " ",
                    m.code,
                    " = ",
                    ve,
                    " ",
                    ne.code
                  ] })
                ]
              },
              ne.code
            );
          }) })
        }
      )
    ] }),
    /* @__PURE__ */ F.jsxs("div", { className: "text-xs text-[#888888] mt-2", children: [
      "Atualizado em ",
      j6()
    ] })
  ] });
};
var p4 = { exports: {} }, B2 = {}, g4 = { exports: {} }, M4 = {};
var L6;
function Co() {
  return L6 || (L6 = 1, (function(m) {
    function i(C, b) {
      var L = C.length;
      C.push(b);
      e: for (; 0 < L; ) {
        var I = L - 1 >>> 1, ge = C[I];
        if (0 < E(ge, b))
          C[I] = b, C[L] = ge, L = I;
        else break e;
      }
    }
    function u(C) {
      return C.length === 0 ? null : C[0];
    }
    function f(C) {
      if (C.length === 0) return null;
      var b = C[0], L = C.pop();
      if (L !== b) {
        C[0] = L;
        e: for (var I = 0, ge = C.length, e1 = ge >>> 1; I < e1; ) {
          var p = 2 * (I + 1) - 1, B = C[p], V = p + 1, _ = C[V];
          if (0 > E(B, L))
            V < ge && 0 > E(_, B) ? (C[I] = _, C[V] = L, I = V) : (C[I] = B, C[p] = L, I = p);
          else if (V < ge && 0 > E(_, L))
            C[I] = _, C[V] = L, I = V;
          else break e;
        }
      }
      return b;
    }
    function E(C, b) {
      var L = C.sortIndex - b.sortIndex;
      return L !== 0 ? L : C.id - b.id;
    }
    if (m.unstable_now = void 0, typeof performance == "object" && typeof performance.now == "function") {
      var O = performance;
      m.unstable_now = function() {
        return O.now();
      };
    } else {
      var G = Date, Q = G.now();
      m.unstable_now = function() {
        return G.now() - Q;
      };
    }
    var T = [], H = [], R = 1, k = null, Y = 3, je = !1, Ve = !1, W = !1, Fe = !1, ne = typeof setTimeout == "function" ? setTimeout : null, oe = typeof clearTimeout == "function" ? clearTimeout : null, f1 = typeof setImmediate < "u" ? setImmediate : null;
    function Be(C) {
      for (var b = u(H); b !== null; ) {
        if (b.callback === null) f(H);
        else if (b.startTime <= C)
          f(H), b.sortIndex = b.expirationTime, i(T, b);
        else break;
        b = u(H);
      }
    }
    function $e(C) {
      if (W = !1, Be(C), !Ve)
        if (u(T) !== null)
          Ve = !0, ve || (ve = !0, Ge());
        else {
          var b = u(H);
          b !== null && v1($e, b.startTime - C);
        }
    }
    var ve = !1, be = -1, ce = 5, Ue = -1;
    function Pe() {
      return Fe ? !0 : !(m.unstable_now() - Ue < ce);
    }
    function h1() {
      if (Fe = !1, ve) {
        var C = m.unstable_now();
        Ue = C;
        var b = !0;
        try {
          e: {
            Ve = !1, W && (W = !1, oe(be), be = -1), je = !0;
            var L = Y;
            try {
              t: {
                for (Be(C), k = u(T); k !== null && !(k.expirationTime > C && Pe()); ) {
                  var I = k.callback;
                  if (typeof I == "function") {
                    k.callback = null, Y = k.priorityLevel;
                    var ge = I(
                      k.expirationTime <= C
                    );
                    if (C = m.unstable_now(), typeof ge == "function") {
                      k.callback = ge, Be(C), b = !0;
                      break t;
                    }
                    k === u(T) && f(T), Be(C);
                  } else f(T);
                  k = u(T);
                }
                if (k !== null) b = !0;
                else {
                  var e1 = u(H);
                  e1 !== null && v1(
                    $e,
                    e1.startTime - C
                  ), b = !1;
                }
              }
              break e;
            } finally {
              k = null, Y = L, je = !1;
            }
            b = void 0;
          }
        } finally {
          b ? Ge() : ve = !1;
        }
      }
    }
    var Ge;
    if (typeof f1 == "function")
      Ge = function() {
        f1(h1);
      };
    else if (typeof MessageChannel < "u") {
      var m1 = new MessageChannel(), G1 = m1.port2;
      m1.port1.onmessage = h1, Ge = function() {
        G1.postMessage(null);
      };
    } else
      Ge = function() {
        ne(h1, 0);
      };
    function v1(C, b) {
      be = ne(function() {
        C(m.unstable_now());
      }, b);
    }
    m.unstable_IdlePriority = 5, m.unstable_ImmediatePriority = 1, m.unstable_LowPriority = 4, m.unstable_NormalPriority = 3, m.unstable_Profiling = null, m.unstable_UserBlockingPriority = 2, m.unstable_cancelCallback = function(C) {
      C.callback = null;
    }, m.unstable_forceFrameRate = function(C) {
      0 > C || 125 < C ? console.log(
        "forceFrameRate takes a positive int between 0 and 125, forcing frame rates higher than 125 fps is not supported"
      ) : ce = 0 < C ? Math.floor(1e3 / C) : 5;
    }, m.unstable_getCurrentPriorityLevel = function() {
      return Y;
    }, m.unstable_next = function(C) {
      switch (Y) {
        case 1:
        case 2:
        case 3:
          var b = 3;
          break;
        default:
          b = Y;
      }
      var L = Y;
      Y = b;
      try {
        return C();
      } finally {
        Y = L;
      }
    }, m.unstable_requestPaint = function() {
      Fe = !0;
    }, m.unstable_runWithPriority = function(C, b) {
      switch (C) {
        case 1:
        case 2:
        case 3:
        case 4:
        case 5:
          break;
        default:
          C = 3;
      }
      var L = Y;
      Y = C;
      try {
        return b();
      } finally {
        Y = L;
      }
    }, m.unstable_scheduleCallback = function(C, b, L) {
      var I = m.unstable_now();
      switch (typeof L == "object" && L !== null ? (L = L.delay, L = typeof L == "number" && 0 < L ? I + L : I) : L = I, C) {
        case 1:
          var ge = -1;
          break;
        case 2:
          ge = 250;
          break;
        case 5:
          ge = 1073741823;
          break;
        case 4:
          ge = 1e4;
          break;
        default:
          ge = 5e3;
      }
      return ge = L + ge, C = {
        id: R++,
        callback: b,
        priorityLevel: C,
        startTime: L,
        expirationTime: ge,
        sortIndex: -1
      }, L > I ? (C.sortIndex = L, i(H, C), u(T) === null && C === u(H) && (W ? (oe(be), be = -1) : W = !0, v1($e, L - I))) : (C.sortIndex = ge, i(T, C), Ve || je || (Ve = !0, ve || (ve = !0, Ge()))), C;
    }, m.unstable_shouldYield = Pe, m.unstable_wrapCallback = function(C) {
      var b = Y;
      return function() {
        var L = Y;
        Y = b;
        try {
          return C.apply(this, arguments);
        } finally {
          Y = L;
        }
      };
    };
  })(M4)), M4;
}
var V6;
function Oo() {
  return V6 || (V6 = 1, g4.exports = Co()), g4.exports;
}
var C6;
function No() {
  if (C6) return B2;
  C6 = 1;
  var m = { env: {} };
  var i = Oo(), u = w4(), f = G6();
  function E(e) {
    var t = "https://react.dev/errors/" + e;
    if (1 < arguments.length) {
      t += "?args[]=" + encodeURIComponent(arguments[1]);
      for (var l = 2; l < arguments.length; l++)
        t += "&args[]=" + encodeURIComponent(arguments[l]);
    }
    return "Minified React error #" + e + "; visit " + t + " for the full message or use the non-minified dev environment for full errors and additional helpful warnings.";
  }
  function O(e) {
    return !(!e || e.nodeType !== 1 && e.nodeType !== 9 && e.nodeType !== 11);
  }
  function G(e) {
    var t = e, l = e;
    if (e.alternate) for (; t.return; ) t = t.return;
    else {
      e = t;
      do
        t = e, (t.flags & 4098) !== 0 && (l = t.return), e = t.return;
      while (e);
    }
    return t.tag === 3 ? l : null;
  }
  function Q(e) {
    if (e.tag === 13) {
      var t = e.memoizedState;
      if (t === null && (e = e.alternate, e !== null && (t = e.memoizedState)), t !== null) return t.dehydrated;
    }
    return null;
  }
  function T(e) {
    if (e.tag === 31) {
      var t = e.memoizedState;
      if (t === null && (e = e.alternate, e !== null && (t = e.memoizedState)), t !== null) return t.dehydrated;
    }
    return null;
  }
  function H(e) {
    if (G(e) !== e)
      throw Error(E(188));
  }
  function R(e) {
    var t = e.alternate;
    if (!t) {
      if (t = G(e), t === null) throw Error(E(188));
      return t !== e ? null : e;
    }
    for (var l = e, n = t; ; ) {
      var c = l.return;
      if (c === null) break;
      var r = c.alternate;
      if (r === null) {
        if (n = c.return, n !== null) {
          l = n;
          continue;
        }
        break;
      }
      if (c.child === r.child) {
        for (r = c.child; r; ) {
          if (r === l) return H(c), e;
          if (r === n) return H(c), t;
          r = r.sibling;
        }
        throw Error(E(188));
      }
      if (l.return !== n.return) l = c, n = r;
      else {
        for (var h = !1, v = c.child; v; ) {
          if (v === l) {
            h = !0, l = c, n = r;
            break;
          }
          if (v === n) {
            h = !0, n = c, l = r;
            break;
          }
          v = v.sibling;
        }
        if (!h) {
          for (v = r.child; v; ) {
            if (v === l) {
              h = !0, l = r, n = c;
              break;
            }
            if (v === n) {
              h = !0, n = r, l = c;
              break;
            }
            v = v.sibling;
          }
          if (!h) throw Error(E(189));
        }
      }
      if (l.alternate !== n) throw Error(E(190));
    }
    if (l.tag !== 3) throw Error(E(188));
    return l.stateNode.current === l ? e : t;
  }
  function k(e) {
    var t = e.tag;
    if (t === 5 || t === 26 || t === 27 || t === 6) return e;
    for (e = e.child; e !== null; ) {
      if (t = k(e), t !== null) return t;
      e = e.sibling;
    }
    return null;
  }
  var Y = Object.assign, je = /* @__PURE__ */ Symbol.for("react.element"), Ve = /* @__PURE__ */ Symbol.for("react.transitional.element"), W = /* @__PURE__ */ Symbol.for("react.portal"), Fe = /* @__PURE__ */ Symbol.for("react.fragment"), ne = /* @__PURE__ */ Symbol.for("react.strict_mode"), oe = /* @__PURE__ */ Symbol.for("react.profiler"), f1 = /* @__PURE__ */ Symbol.for("react.consumer"), Be = /* @__PURE__ */ Symbol.for("react.context"), $e = /* @__PURE__ */ Symbol.for("react.forward_ref"), ve = /* @__PURE__ */ Symbol.for("react.suspense"), be = /* @__PURE__ */ Symbol.for("react.suspense_list"), ce = /* @__PURE__ */ Symbol.for("react.memo"), Ue = /* @__PURE__ */ Symbol.for("react.lazy"), Pe = /* @__PURE__ */ Symbol.for("react.activity"), h1 = /* @__PURE__ */ Symbol.for("react.memo_cache_sentinel"), Ge = Symbol.iterator;
  function m1(e) {
    return e === null || typeof e != "object" ? null : (e = Ge && e[Ge] || e["@@iterator"], typeof e == "function" ? e : null);
  }
  var G1 = /* @__PURE__ */ Symbol.for("react.client.reference");
  function v1(e) {
    if (e == null) return null;
    if (typeof e == "function")
      return e.$$typeof === G1 ? null : e.displayName || e.name || null;
    if (typeof e == "string") return e;
    switch (e) {
      case Fe:
        return "Fragment";
      case oe:
        return "Profiler";
      case ne:
        return "StrictMode";
      case ve:
        return "Suspense";
      case be:
        return "SuspenseList";
      case Pe:
        return "Activity";
    }
    if (typeof e == "object")
      switch (e.$$typeof) {
        case W:
          return "Portal";
        case Be:
          return e.displayName || "Context";
        case f1:
          return (e._context.displayName || "Context") + ".Consumer";
        case $e:
          var t = e.render;
          return e = e.displayName, e || (e = t.displayName || t.name || "", e = e !== "" ? "ForwardRef(" + e + ")" : "ForwardRef"), e;
        case ce:
          return t = e.displayName || null, t !== null ? t : v1(e.type) || "Memo";
        case Ue:
          t = e._payload, e = e._init;
          try {
            return v1(e(t));
          } catch {
          }
      }
    return null;
  }
  var C = Array.isArray, b = u.__CLIENT_INTERNALS_DO_NOT_USE_OR_WARN_USERS_THEY_CANNOT_UPGRADE, L = f.__DOM_INTERNALS_DO_NOT_USE_OR_WARN_USERS_THEY_CANNOT_UPGRADE, I = {
    pending: !1,
    data: null,
    method: null,
    action: null
  }, ge = [], e1 = -1;
  function p(e) {
    return { current: e };
  }
  function B(e) {
    0 > e1 || (e.current = ge[e1], ge[e1] = null, e1--);
  }
  function V(e, t) {
    e1++, ge[e1] = e.current, e.current = t;
  }
  var _ = p(null), J = p(null), $ = p(null), ie = p(null);
  function ue(e, t) {
    switch (V($, t), V(J, e), V(_, null), t.nodeType) {
      case 9:
      case 11:
        e = (e = t.documentElement) && (e = e.namespaceURI) ? Kc(e) : 0;
        break;
      default:
        if (e = t.tagName, t = t.namespaceURI)
          t = Kc(t), e = Jc(t, e);
        else
          switch (e) {
            case "svg":
              e = 1;
              break;
            case "math":
              e = 2;
              break;
            default:
              e = 0;
          }
    }
    B(_), V(_, e);
  }
  function he() {
    B(_), B(J), B($);
  }
  function Xe(e) {
    e.memoizedState !== null && V(ie, e);
    var t = _.current, l = Jc(t, e.type);
    t !== l && (V(J, e), V(_, l));
  }
  function H1(e) {
    J.current === e && (B(_), B(J)), ie.current === e && (B(ie), H2._currentValue = I);
  }
  var $3, x4;
  function Tt(e) {
    if ($3 === void 0)
      try {
        throw Error();
      } catch (l) {
        var t = l.stack.trim().match(/\n( *(at )?)/);
        $3 = t && t[1] || "", x4 = -1 < l.stack.indexOf(`
    at`) ? " (<anonymous>)" : -1 < l.stack.indexOf("@") ? "@unknown:0:0" : "";
      }
    return `
` + $3 + e + x4;
  }
  var I3 = !1;
  function P3(e, t) {
    if (!e || I3) return "";
    I3 = !0;
    var l = Error.prepareStackTrace;
    Error.prepareStackTrace = void 0;
    try {
      var n = {
        DetermineComponentFrameRoot: function() {
          try {
            if (t) {
              var A = function() {
                throw Error();
              };
              if (Object.defineProperty(A.prototype, "props", {
                set: function() {
                  throw Error();
                }
              }), typeof Reflect == "object" && Reflect.construct) {
                try {
                  Reflect.construct(A, []);
                } catch (y) {
                  var x = y;
                }
                Reflect.construct(e, [], A);
              } else {
                try {
                  A.call();
                } catch (y) {
                  x = y;
                }
                e.call(A.prototype);
              }
            } else {
              try {
                throw Error();
              } catch (y) {
                x = y;
              }
              (A = e()) && typeof A.catch == "function" && A.catch(function() {
              });
            }
          } catch (y) {
            if (y && x && typeof y.stack == "string")
              return [y.stack, x.stack];
          }
          return [null, null];
        }
      };
      n.DetermineComponentFrameRoot.displayName = "DetermineComponentFrameRoot";
      var c = Object.getOwnPropertyDescriptor(
        n.DetermineComponentFrameRoot,
        "name"
      );
      c && c.configurable && Object.defineProperty(
        n.DetermineComponentFrameRoot,
        "name",
        { value: "DetermineComponentFrameRoot" }
      );
      var r = n.DetermineComponentFrameRoot(), h = r[0], v = r[1];
      if (h && v) {
        var o = h.split(`
`), w = v.split(`
`);
        for (c = n = 0; n < o.length && !o[n].includes("DetermineComponentFrameRoot"); )
          n++;
        for (; c < w.length && !w[c].includes(
          "DetermineComponentFrameRoot"
        ); )
          c++;
        if (n === o.length || c === w.length)
          for (n = o.length - 1, c = w.length - 1; 1 <= n && 0 <= c && o[n] !== w[c]; )
            c--;
        for (; 1 <= n && 0 <= c; n--, c--)
          if (o[n] !== w[c]) {
            if (n !== 1 || c !== 1)
              do
                if (n--, c--, 0 > c || o[n] !== w[c]) {
                  var D = `
` + o[n].replace(" at new ", " at ");
                  return e.displayName && D.includes("<anonymous>") && (D = D.replace("<anonymous>", e.displayName)), D;
                }
              while (1 <= n && 0 <= c);
            break;
          }
      }
    } finally {
      I3 = !1, Error.prepareStackTrace = l;
    }
    return (l = e ? e.displayName || e.name : "") ? Tt(l) : "";
  }
  function q9(e, t) {
    switch (e.tag) {
      case 26:
      case 27:
      case 5:
        return Tt(e.type);
      case 16:
        return Tt("Lazy");
      case 13:
        return e.child !== t && t !== null ? Tt("Suspense Fallback") : Tt("Suspense");
      case 19:
        return Tt("SuspenseList");
      case 0:
      case 15:
        return P3(e.type, !1);
      case 11:
        return P3(e.type.render, !1);
      case 1:
        return P3(e.type, !0);
      case 31:
        return Tt("Activity");
      default:
        return "";
    }
  }
  function H4(e) {
    try {
      var t = "", l = null;
      do
        t += q9(e, l), l = e, e = e.return;
      while (e);
      return t;
    } catch (n) {
      return `
Error generating stack: ` + n.message + `
` + n.stack;
    }
  }
  var e0 = Object.prototype.hasOwnProperty, t0 = i.unstable_scheduleCallback, l0 = i.unstable_cancelCallback, Q9 = i.unstable_shouldYield, X9 = i.unstable_requestPaint, s1 = i.unstable_now, Z9 = i.unstable_getCurrentPriorityLevel, y4 = i.unstable_ImmediatePriority, F4 = i.unstable_UserBlockingPriority, L2 = i.unstable_NormalPriority, K9 = i.unstable_LowPriority, D4 = i.unstable_IdlePriority, J9 = i.log, k9 = i.unstable_setDisableYieldValue, Ol = null, d1 = null;
  function it(e) {
    if (typeof J9 == "function" && k9(e), d1 && typeof d1.setStrictMode == "function")
      try {
        d1.setStrictMode(Ol, e);
      } catch {
      }
  }
  var o1 = Math.clz32 ? Math.clz32 : I9, W9 = Math.log, $9 = Math.LN2;
  function I9(e) {
    return e >>>= 0, e === 0 ? 32 : 31 - (W9(e) / $9 | 0) | 0;
  }
  var V2 = 256, C2 = 262144, O2 = 4194304;
  function Lt(e) {
    var t = e & 42;
    if (t !== 0) return t;
    switch (e & -e) {
      case 1:
        return 1;
      case 2:
        return 2;
      case 4:
        return 4;
      case 8:
        return 8;
      case 16:
        return 16;
      case 32:
        return 32;
      case 64:
        return 64;
      case 128:
        return 128;
      case 256:
      case 512:
      case 1024:
      case 2048:
      case 4096:
      case 8192:
      case 16384:
      case 32768:
      case 65536:
      case 131072:
        return e & 261888;
      case 262144:
      case 524288:
      case 1048576:
      case 2097152:
        return e & 3932160;
      case 4194304:
      case 8388608:
      case 16777216:
      case 33554432:
        return e & 62914560;
      case 67108864:
        return 67108864;
      case 134217728:
        return 134217728;
      case 268435456:
        return 268435456;
      case 536870912:
        return 536870912;
      case 1073741824:
        return 0;
      default:
        return e;
    }
  }
  function N2(e, t, l) {
    var n = e.pendingLanes;
    if (n === 0) return 0;
    var c = 0, r = e.suspendedLanes, h = e.pingedLanes;
    e = e.warmLanes;
    var v = n & 134217727;
    return v !== 0 ? (n = v & ~r, n !== 0 ? c = Lt(n) : (h &= v, h !== 0 ? c = Lt(h) : l || (l = v & ~e, l !== 0 && (c = Lt(l))))) : (v = n & ~r, v !== 0 ? c = Lt(v) : h !== 0 ? c = Lt(h) : l || (l = n & ~e, l !== 0 && (c = Lt(l)))), c === 0 ? 0 : t !== 0 && t !== c && (t & r) === 0 && (r = c & -c, l = t & -t, r >= l || r === 32 && (l & 4194048) !== 0) ? t : c;
  }
  function Nl(e, t) {
    return (e.pendingLanes & ~(e.suspendedLanes & ~e.pingedLanes) & t) === 0;
  }
  function P9(e, t) {
    switch (e) {
      case 1:
      case 2:
      case 4:
      case 8:
      case 64:
        return t + 250;
      case 16:
      case 32:
      case 128:
      case 256:
      case 512:
      case 1024:
      case 2048:
      case 4096:
      case 8192:
      case 16384:
      case 32768:
      case 65536:
      case 131072:
      case 262144:
      case 524288:
      case 1048576:
      case 2097152:
        return t + 5e3;
      case 4194304:
      case 8388608:
      case 16777216:
      case 33554432:
        return -1;
      case 67108864:
      case 134217728:
      case 268435456:
      case 536870912:
      case 1073741824:
        return -1;
      default:
        return -1;
    }
  }
  function S4() {
    var e = O2;
    return O2 <<= 1, (O2 & 62914560) === 0 && (O2 = 4194304), e;
  }
  function a0(e) {
    for (var t = [], l = 0; 31 > l; l++) t.push(e);
    return t;
  }
  function _l(e, t) {
    e.pendingLanes |= t, t !== 268435456 && (e.suspendedLanes = 0, e.pingedLanes = 0, e.warmLanes = 0);
  }
  function er(e, t, l, n, c, r) {
    var h = e.pendingLanes;
    e.pendingLanes = l, e.suspendedLanes = 0, e.pingedLanes = 0, e.warmLanes = 0, e.expiredLanes &= l, e.entangledLanes &= l, e.errorRecoveryDisabledLanes &= l, e.shellSuspendCounter = 0;
    var v = e.entanglements, o = e.expirationTimes, w = e.hiddenUpdates;
    for (l = h & ~l; 0 < l; ) {
      var D = 31 - o1(l), A = 1 << D;
      v[D] = 0, o[D] = -1;
      var x = w[D];
      if (x !== null)
        for (w[D] = null, D = 0; D < x.length; D++) {
          var y = x[D];
          y !== null && (y.lane &= -536870913);
        }
      l &= ~A;
    }
    n !== 0 && A4(e, n, 0), r !== 0 && c === 0 && e.tag !== 0 && (e.suspendedLanes |= r & ~(h & ~t));
  }
  function A4(e, t, l) {
    e.pendingLanes |= t, e.suspendedLanes &= ~t;
    var n = 31 - o1(t);
    e.entangledLanes |= t, e.entanglements[n] = e.entanglements[n] | 1073741824 | l & 261930;
  }
  function B4(e, t) {
    var l = e.entangledLanes |= t;
    for (e = e.entanglements; l; ) {
      var n = 31 - o1(l), c = 1 << n;
      c & t | e[n] & t && (e[n] |= t), l &= ~c;
    }
  }
  function b4(e, t) {
    var l = t & -t;
    return l = (l & 42) !== 0 ? 1 : n0(l), (l & (e.suspendedLanes | t)) !== 0 ? 0 : l;
  }
  function n0(e) {
    switch (e) {
      case 2:
        e = 1;
        break;
      case 8:
        e = 4;
        break;
      case 32:
        e = 16;
        break;
      case 256:
      case 512:
      case 1024:
      case 2048:
      case 4096:
      case 8192:
      case 16384:
      case 32768:
      case 65536:
      case 131072:
      case 262144:
      case 524288:
      case 1048576:
      case 2097152:
      case 4194304:
      case 8388608:
      case 16777216:
      case 33554432:
        e = 128;
        break;
      case 268435456:
        e = 134217728;
        break;
      default:
        e = 0;
    }
    return e;
  }
  function c0(e) {
    return e &= -e, 2 < e ? 8 < e ? (e & 134217727) !== 0 ? 32 : 268435456 : 8 : 2;
  }
  function T4() {
    var e = L.p;
    return e !== 0 ? e : (e = window.event, e === void 0 ? 32 : z6(e.type));
  }
  function L4(e, t) {
    var l = L.p;
    try {
      return L.p = e, t();
    } finally {
      L.p = l;
    }
  }
  var ut = Math.random().toString(36).slice(2), Ze = "__reactFiber$" + ut, l1 = "__reactProps$" + ut, $t = "__reactContainer$" + ut, i0 = "__reactEvents$" + ut, tr = "__reactListeners$" + ut, lr = "__reactHandles$" + ut, V4 = "__reactResources$" + ut, Ul = "__reactMarker$" + ut;
  function u0(e) {
    delete e[Ze], delete e[l1], delete e[i0], delete e[tr], delete e[lr];
  }
  function It(e) {
    var t = e[Ze];
    if (t) return t;
    for (var l = e.parentNode; l; ) {
      if (t = l[$t] || l[Ze]) {
        if (l = t.alternate, t.child !== null || l !== null && l.child !== null)
          for (e = t6(e); e !== null; ) {
            if (l = e[Ze]) return l;
            e = t6(e);
          }
        return t;
      }
      e = l, l = e.parentNode;
    }
    return null;
  }
  function Pt(e) {
    if (e = e[Ze] || e[$t]) {
      var t = e.tag;
      if (t === 5 || t === 6 || t === 13 || t === 31 || t === 26 || t === 27 || t === 3)
        return e;
    }
    return null;
  }
  function Rl(e) {
    var t = e.tag;
    if (t === 5 || t === 26 || t === 27 || t === 6) return e.stateNode;
    throw Error(E(33));
  }
  function el(e) {
    var t = e[V4];
    return t || (t = e[V4] = { hoistableStyles: /* @__PURE__ */ new Map(), hoistableScripts: /* @__PURE__ */ new Map() }), t;
  }
  function Ye(e) {
    e[Ul] = !0;
  }
  var C4 = /* @__PURE__ */ new Set(), O4 = {};
  function Vt(e, t) {
    tl(e, t), tl(e + "Capture", t);
  }
  function tl(e, t) {
    for (O4[e] = t, e = 0; e < t.length; e++)
      C4.add(t[e]);
  }
  var ar = RegExp(
    "^[:A-Z_a-z\\u00C0-\\u00D6\\u00D8-\\u00F6\\u00F8-\\u02FF\\u0370-\\u037D\\u037F-\\u1FFF\\u200C-\\u200D\\u2070-\\u218F\\u2C00-\\u2FEF\\u3001-\\uD7FF\\uF900-\\uFDCF\\uFDF0-\\uFFFD][:A-Z_a-z\\u00C0-\\u00D6\\u00D8-\\u00F6\\u00F8-\\u02FF\\u0370-\\u037D\\u037F-\\u1FFF\\u200C-\\u200D\\u2070-\\u218F\\u2C00-\\u2FEF\\u3001-\\uD7FF\\uF900-\\uFDCF\\uFDF0-\\uFFFD\\-.0-9\\u00B7\\u0300-\\u036F\\u203F-\\u2040]*$"
  ), N4 = {}, _4 = {};
  function nr(e) {
    return e0.call(_4, e) ? !0 : e0.call(N4, e) ? !1 : ar.test(e) ? _4[e] = !0 : (N4[e] = !0, !1);
  }
  function _2(e, t, l) {
    if (nr(t))
      if (l === null) e.removeAttribute(t);
      else {
        switch (typeof l) {
          case "undefined":
          case "function":
          case "symbol":
            e.removeAttribute(t);
            return;
          case "boolean":
            var n = t.toLowerCase().slice(0, 5);
            if (n !== "data-" && n !== "aria-") {
              e.removeAttribute(t);
              return;
            }
        }
        e.setAttribute(t, "" + l);
      }
  }
  function U2(e, t, l) {
    if (l === null) e.removeAttribute(t);
    else {
      switch (typeof l) {
        case "undefined":
        case "function":
        case "symbol":
        case "boolean":
          e.removeAttribute(t);
          return;
      }
      e.setAttribute(t, "" + l);
    }
  }
  function Y1(e, t, l, n) {
    if (n === null) e.removeAttribute(l);
    else {
      switch (typeof n) {
        case "undefined":
        case "function":
        case "symbol":
        case "boolean":
          e.removeAttribute(l);
          return;
      }
      e.setAttributeNS(t, l, "" + n);
    }
  }
  function y1(e) {
    switch (typeof e) {
      case "bigint":
      case "boolean":
      case "number":
      case "string":
      case "undefined":
        return e;
      case "object":
        return e;
      default:
        return "";
    }
  }
  function U4(e) {
    var t = e.type;
    return (e = e.nodeName) && e.toLowerCase() === "input" && (t === "checkbox" || t === "radio");
  }
  function cr(e, t, l) {
    var n = Object.getOwnPropertyDescriptor(
      e.constructor.prototype,
      t
    );
    if (!e.hasOwnProperty(t) && typeof n < "u" && typeof n.get == "function" && typeof n.set == "function") {
      var c = n.get, r = n.set;
      return Object.defineProperty(e, t, {
        configurable: !0,
        get: function() {
          return c.call(this);
        },
        set: function(h) {
          l = "" + h, r.call(this, h);
        }
      }), Object.defineProperty(e, t, {
        enumerable: n.enumerable
      }), {
        getValue: function() {
          return l;
        },
        setValue: function(h) {
          l = "" + h;
        },
        stopTracking: function() {
          e._valueTracker = null, delete e[t];
        }
      };
    }
  }
  function r0(e) {
    if (!e._valueTracker) {
      var t = U4(e) ? "checked" : "value";
      e._valueTracker = cr(
        e,
        t,
        "" + e[t]
      );
    }
  }
  function R4(e) {
    if (!e) return !1;
    var t = e._valueTracker;
    if (!t) return !0;
    var l = t.getValue(), n = "";
    return e && (n = U4(e) ? e.checked ? "true" : "false" : e.value), e = n, e !== l ? (t.setValue(e), !0) : !1;
  }
  function R2(e) {
    if (e = e || (typeof document < "u" ? document : void 0), typeof e > "u") return null;
    try {
      return e.activeElement || e.body;
    } catch {
      return e.body;
    }
  }
  var ir = /[\n"\\]/g;
  function F1(e) {
    return e.replace(
      ir,
      function(t) {
        return "\\" + t.charCodeAt(0).toString(16) + " ";
      }
    );
  }
  function f0(e, t, l, n, c, r, h, v) {
    e.name = "", h != null && typeof h != "function" && typeof h != "symbol" && typeof h != "boolean" ? e.type = h : e.removeAttribute("type"), t != null ? h === "number" ? (t === 0 && e.value === "" || e.value != t) && (e.value = "" + y1(t)) : e.value !== "" + y1(t) && (e.value = "" + y1(t)) : h !== "submit" && h !== "reset" || e.removeAttribute("value"), t != null ? h0(e, h, y1(t)) : l != null ? h0(e, h, y1(l)) : n != null && e.removeAttribute("value"), c == null && r != null && (e.defaultChecked = !!r), c != null && (e.checked = c && typeof c != "function" && typeof c != "symbol"), v != null && typeof v != "function" && typeof v != "symbol" && typeof v != "boolean" ? e.name = "" + y1(v) : e.removeAttribute("name");
  }
  function j4(e, t, l, n, c, r, h, v) {
    if (r != null && typeof r != "function" && typeof r != "symbol" && typeof r != "boolean" && (e.type = r), t != null || l != null) {
      if (!(r !== "submit" && r !== "reset" || t != null)) {
        r0(e);
        return;
      }
      l = l != null ? "" + y1(l) : "", t = t != null ? "" + y1(t) : l, v || t === e.value || (e.value = t), e.defaultValue = t;
    }
    n = n ?? c, n = typeof n != "function" && typeof n != "symbol" && !!n, e.checked = v ? e.checked : !!n, e.defaultChecked = !!n, h != null && typeof h != "function" && typeof h != "symbol" && typeof h != "boolean" && (e.name = h), r0(e);
  }
  function h0(e, t, l) {
    t === "number" && R2(e.ownerDocument) === e || e.defaultValue === "" + l || (e.defaultValue = "" + l);
  }
  function ll(e, t, l, n) {
    if (e = e.options, t) {
      t = {};
      for (var c = 0; c < l.length; c++)
        t["$" + l[c]] = !0;
      for (l = 0; l < e.length; l++)
        c = t.hasOwnProperty("$" + e[l].value), e[l].selected !== c && (e[l].selected = c), c && n && (e[l].defaultSelected = !0);
    } else {
      for (l = "" + y1(l), t = null, c = 0; c < e.length; c++) {
        if (e[c].value === l) {
          e[c].selected = !0, n && (e[c].defaultSelected = !0);
          return;
        }
        t !== null || e[c].disabled || (t = e[c]);
      }
      t !== null && (t.selected = !0);
    }
  }
  function G4(e, t, l) {
    if (t != null && (t = "" + y1(t), t !== e.value && (e.value = t), l == null)) {
      e.defaultValue !== t && (e.defaultValue = t);
      return;
    }
    e.defaultValue = l != null ? "" + y1(l) : "";
  }
  function Y4(e, t, l, n) {
    if (t == null) {
      if (n != null) {
        if (l != null) throw Error(E(92));
        if (C(n)) {
          if (1 < n.length) throw Error(E(93));
          n = n[0];
        }
        l = n;
      }
      l == null && (l = ""), t = l;
    }
    l = y1(t), e.defaultValue = l, n = e.textContent, n === l && n !== "" && n !== null && (e.value = n), r0(e);
  }
  function al(e, t) {
    if (t) {
      var l = e.firstChild;
      if (l && l === e.lastChild && l.nodeType === 3) {
        l.nodeValue = t;
        return;
      }
    }
    e.textContent = t;
  }
  var ur = new Set(
    "animationIterationCount aspectRatio borderImageOutset borderImageSlice borderImageWidth boxFlex boxFlexGroup boxOrdinalGroup columnCount columns flex flexGrow flexPositive flexShrink flexNegative flexOrder gridArea gridRow gridRowEnd gridRowSpan gridRowStart gridColumn gridColumnEnd gridColumnSpan gridColumnStart fontWeight lineClamp lineHeight opacity order orphans scale tabSize widows zIndex zoom fillOpacity floodOpacity stopOpacity strokeDasharray strokeDashoffset strokeMiterlimit strokeOpacity strokeWidth MozAnimationIterationCount MozBoxFlex MozBoxFlexGroup MozLineClamp msAnimationIterationCount msFlex msZoom msFlexGrow msFlexNegative msFlexOrder msFlexPositive msFlexShrink msGridColumn msGridColumnSpan msGridRow msGridRowSpan WebkitAnimationIterationCount WebkitBoxFlex WebKitBoxFlexGroup WebkitBoxOrdinalGroup WebkitColumnCount WebkitColumns WebkitFlex WebkitFlexGrow WebkitFlexPositive WebkitFlexShrink WebkitLineClamp".split(
      " "
    )
  );
  function q4(e, t, l) {
    var n = t.indexOf("--") === 0;
    l == null || typeof l == "boolean" || l === "" ? n ? e.setProperty(t, "") : t === "float" ? e.cssFloat = "" : e[t] = "" : n ? e.setProperty(t, l) : typeof l != "number" || l === 0 || ur.has(t) ? t === "float" ? e.cssFloat = l : e[t] = ("" + l).trim() : e[t] = l + "px";
  }
  function Q4(e, t, l) {
    if (t != null && typeof t != "object")
      throw Error(E(62));
    if (e = e.style, l != null) {
      for (var n in l)
        !l.hasOwnProperty(n) || t != null && t.hasOwnProperty(n) || (n.indexOf("--") === 0 ? e.setProperty(n, "") : n === "float" ? e.cssFloat = "" : e[n] = "");
      for (var c in t)
        n = t[c], t.hasOwnProperty(c) && l[c] !== n && q4(e, c, n);
    } else
      for (var r in t)
        t.hasOwnProperty(r) && q4(e, r, t[r]);
  }
  function m0(e) {
    if (e.indexOf("-") === -1) return !1;
    switch (e) {
      case "annotation-xml":
      case "color-profile":
      case "font-face":
      case "font-face-src":
      case "font-face-uri":
      case "font-face-format":
      case "font-face-name":
      case "missing-glyph":
        return !1;
      default:
        return !0;
    }
  }
  var rr = /* @__PURE__ */ new Map([
    ["acceptCharset", "accept-charset"],
    ["htmlFor", "for"],
    ["httpEquiv", "http-equiv"],
    ["crossOrigin", "crossorigin"],
    ["accentHeight", "accent-height"],
    ["alignmentBaseline", "alignment-baseline"],
    ["arabicForm", "arabic-form"],
    ["baselineShift", "baseline-shift"],
    ["capHeight", "cap-height"],
    ["clipPath", "clip-path"],
    ["clipRule", "clip-rule"],
    ["colorInterpolation", "color-interpolation"],
    ["colorInterpolationFilters", "color-interpolation-filters"],
    ["colorProfile", "color-profile"],
    ["colorRendering", "color-rendering"],
    ["dominantBaseline", "dominant-baseline"],
    ["enableBackground", "enable-background"],
    ["fillOpacity", "fill-opacity"],
    ["fillRule", "fill-rule"],
    ["floodColor", "flood-color"],
    ["floodOpacity", "flood-opacity"],
    ["fontFamily", "font-family"],
    ["fontSize", "font-size"],
    ["fontSizeAdjust", "font-size-adjust"],
    ["fontStretch", "font-stretch"],
    ["fontStyle", "font-style"],
    ["fontVariant", "font-variant"],
    ["fontWeight", "font-weight"],
    ["glyphName", "glyph-name"],
    ["glyphOrientationHorizontal", "glyph-orientation-horizontal"],
    ["glyphOrientationVertical", "glyph-orientation-vertical"],
    ["horizAdvX", "horiz-adv-x"],
    ["horizOriginX", "horiz-origin-x"],
    ["imageRendering", "image-rendering"],
    ["letterSpacing", "letter-spacing"],
    ["lightingColor", "lighting-color"],
    ["markerEnd", "marker-end"],
    ["markerMid", "marker-mid"],
    ["markerStart", "marker-start"],
    ["overlinePosition", "overline-position"],
    ["overlineThickness", "overline-thickness"],
    ["paintOrder", "paint-order"],
    ["panose-1", "panose-1"],
    ["pointerEvents", "pointer-events"],
    ["renderingIntent", "rendering-intent"],
    ["shapeRendering", "shape-rendering"],
    ["stopColor", "stop-color"],
    ["stopOpacity", "stop-opacity"],
    ["strikethroughPosition", "strikethrough-position"],
    ["strikethroughThickness", "strikethrough-thickness"],
    ["strokeDasharray", "stroke-dasharray"],
    ["strokeDashoffset", "stroke-dashoffset"],
    ["strokeLinecap", "stroke-linecap"],
    ["strokeLinejoin", "stroke-linejoin"],
    ["strokeMiterlimit", "stroke-miterlimit"],
    ["strokeOpacity", "stroke-opacity"],
    ["strokeWidth", "stroke-width"],
    ["textAnchor", "text-anchor"],
    ["textDecoration", "text-decoration"],
    ["textRendering", "text-rendering"],
    ["transformOrigin", "transform-origin"],
    ["underlinePosition", "underline-position"],
    ["underlineThickness", "underline-thickness"],
    ["unicodeBidi", "unicode-bidi"],
    ["unicodeRange", "unicode-range"],
    ["unitsPerEm", "units-per-em"],
    ["vAlphabetic", "v-alphabetic"],
    ["vHanging", "v-hanging"],
    ["vIdeographic", "v-ideographic"],
    ["vMathematical", "v-mathematical"],
    ["vectorEffect", "vector-effect"],
    ["vertAdvY", "vert-adv-y"],
    ["vertOriginX", "vert-origin-x"],
    ["vertOriginY", "vert-origin-y"],
    ["wordSpacing", "word-spacing"],
    ["writingMode", "writing-mode"],
    ["xmlnsXlink", "xmlns:xlink"],
    ["xHeight", "x-height"]
  ]), fr = /^[\u0000-\u001F ]*j[\r\n\t]*a[\r\n\t]*v[\r\n\t]*a[\r\n\t]*s[\r\n\t]*c[\r\n\t]*r[\r\n\t]*i[\r\n\t]*p[\r\n\t]*t[\r\n\t]*:/i;
  function j2(e) {
    return fr.test("" + e) ? "javascript:throw new Error('React has blocked a javascript: URL as a security precaution.')" : e;
  }
  function q1() {
  }
  var v0 = null;
  function s0(e) {
    return e = e.target || e.srcElement || window, e.correspondingUseElement && (e = e.correspondingUseElement), e.nodeType === 3 ? e.parentNode : e;
  }
  var nl = null, cl = null;
  function X4(e) {
    var t = Pt(e);
    if (t && (e = t.stateNode)) {
      var l = e[l1] || null;
      e: switch (e = t.stateNode, t.type) {
        case "input":
          if (f0(
            e,
            l.value,
            l.defaultValue,
            l.defaultValue,
            l.checked,
            l.defaultChecked,
            l.type,
            l.name
          ), t = l.name, l.type === "radio" && t != null) {
            for (l = e; l.parentNode; ) l = l.parentNode;
            for (l = l.querySelectorAll(
              'input[name="' + F1(
                "" + t
              ) + '"][type="radio"]'
            ), t = 0; t < l.length; t++) {
              var n = l[t];
              if (n !== e && n.form === e.form) {
                var c = n[l1] || null;
                if (!c) throw Error(E(90));
                f0(
                  n,
                  c.value,
                  c.defaultValue,
                  c.defaultValue,
                  c.checked,
                  c.defaultChecked,
                  c.type,
                  c.name
                );
              }
            }
            for (t = 0; t < l.length; t++)
              n = l[t], n.form === e.form && R4(n);
          }
          break e;
        case "textarea":
          G4(e, l.value, l.defaultValue);
          break e;
        case "select":
          t = l.value, t != null && ll(e, !!l.multiple, t, !1);
      }
    }
  }
  var d0 = !1;
  function Z4(e, t, l) {
    if (d0) return e(t, l);
    d0 = !0;
    try {
      var n = e(t);
      return n;
    } finally {
      if (d0 = !1, (nl !== null || cl !== null) && (S3(), nl && (t = nl, e = cl, cl = nl = null, X4(t), e)))
        for (t = 0; t < e.length; t++) X4(e[t]);
    }
  }
  function jl(e, t) {
    var l = e.stateNode;
    if (l === null) return null;
    var n = l[l1] || null;
    if (n === null) return null;
    l = n[t];
    e: switch (t) {
      case "onClick":
      case "onClickCapture":
      case "onDoubleClick":
      case "onDoubleClickCapture":
      case "onMouseDown":
      case "onMouseDownCapture":
      case "onMouseMove":
      case "onMouseMoveCapture":
      case "onMouseUp":
      case "onMouseUpCapture":
      case "onMouseEnter":
        (n = !n.disabled) || (e = e.type, n = !(e === "button" || e === "input" || e === "select" || e === "textarea")), e = !n;
        break e;
      default:
        e = !1;
    }
    if (e) return null;
    if (l && typeof l != "function")
      throw Error(
        E(231, t, typeof l)
      );
    return l;
  }
  var Q1 = !(typeof window > "u" || typeof window.document > "u" || typeof window.document.createElement > "u"), o0 = !1;
  if (Q1)
    try {
      var Gl = {};
      Object.defineProperty(Gl, "passive", {
        get: function() {
          o0 = !0;
        }
      }), window.addEventListener("test", Gl, Gl), window.removeEventListener("test", Gl, Gl);
    } catch {
      o0 = !1;
    }
  var rt = null, E0 = null, G2 = null;
  function K4() {
    if (G2) return G2;
    var e, t = E0, l = t.length, n, c = "value" in rt ? rt.value : rt.textContent, r = c.length;
    for (e = 0; e < l && t[e] === c[e]; e++) ;
    var h = l - e;
    for (n = 1; n <= h && t[l - n] === c[r - n]; n++) ;
    return G2 = c.slice(e, 1 < n ? 1 - n : void 0);
  }
  function Y2(e) {
    var t = e.keyCode;
    return "charCode" in e ? (e = e.charCode, e === 0 && t === 13 && (e = 13)) : e = t, e === 10 && (e = 13), 32 <= e || e === 13 ? e : 0;
  }
  function q2() {
    return !0;
  }
  function J4() {
    return !1;
  }
  function a1(e) {
    function t(l, n, c, r, h) {
      this._reactName = l, this._targetInst = c, this.type = n, this.nativeEvent = r, this.target = h, this.currentTarget = null;
      for (var v in e)
        e.hasOwnProperty(v) && (l = e[v], this[v] = l ? l(r) : r[v]);
      return this.isDefaultPrevented = (r.defaultPrevented != null ? r.defaultPrevented : r.returnValue === !1) ? q2 : J4, this.isPropagationStopped = J4, this;
    }
    return Y(t.prototype, {
      preventDefault: function() {
        this.defaultPrevented = !0;
        var l = this.nativeEvent;
        l && (l.preventDefault ? l.preventDefault() : typeof l.returnValue != "unknown" && (l.returnValue = !1), this.isDefaultPrevented = q2);
      },
      stopPropagation: function() {
        var l = this.nativeEvent;
        l && (l.stopPropagation ? l.stopPropagation() : typeof l.cancelBubble != "unknown" && (l.cancelBubble = !0), this.isPropagationStopped = q2);
      },
      persist: function() {
      },
      isPersistent: q2
    }), t;
  }
  var Ct = {
    eventPhase: 0,
    bubbles: 0,
    cancelable: 0,
    timeStamp: function(e) {
      return e.timeStamp || Date.now();
    },
    defaultPrevented: 0,
    isTrusted: 0
  }, Q2 = a1(Ct), Yl = Y({}, Ct, { view: 0, detail: 0 }), hr = a1(Yl), z0, p0, ql, X2 = Y({}, Yl, {
    screenX: 0,
    screenY: 0,
    clientX: 0,
    clientY: 0,
    pageX: 0,
    pageY: 0,
    ctrlKey: 0,
    shiftKey: 0,
    altKey: 0,
    metaKey: 0,
    getModifierState: M0,
    button: 0,
    buttons: 0,
    relatedTarget: function(e) {
      return e.relatedTarget === void 0 ? e.fromElement === e.srcElement ? e.toElement : e.fromElement : e.relatedTarget;
    },
    movementX: function(e) {
      return "movementX" in e ? e.movementX : (e !== ql && (ql && e.type === "mousemove" ? (z0 = e.screenX - ql.screenX, p0 = e.screenY - ql.screenY) : p0 = z0 = 0, ql = e), z0);
    },
    movementY: function(e) {
      return "movementY" in e ? e.movementY : p0;
    }
  }), k4 = a1(X2), mr = Y({}, X2, { dataTransfer: 0 }), vr = a1(mr), sr = Y({}, Yl, { relatedTarget: 0 }), g0 = a1(sr), dr = Y({}, Ct, {
    animationName: 0,
    elapsedTime: 0,
    pseudoElement: 0
  }), or = a1(dr), Er = Y({}, Ct, {
    clipboardData: function(e) {
      return "clipboardData" in e ? e.clipboardData : window.clipboardData;
    }
  }), zr = a1(Er), pr = Y({}, Ct, { data: 0 }), W4 = a1(pr), gr = {
    Esc: "Escape",
    Spacebar: " ",
    Left: "ArrowLeft",
    Up: "ArrowUp",
    Right: "ArrowRight",
    Down: "ArrowDown",
    Del: "Delete",
    Win: "OS",
    Menu: "ContextMenu",
    Apps: "ContextMenu",
    Scroll: "ScrollLock",
    MozPrintableKey: "Unidentified"
  }, Mr = {
    8: "Backspace",
    9: "Tab",
    12: "Clear",
    13: "Enter",
    16: "Shift",
    17: "Control",
    18: "Alt",
    19: "Pause",
    20: "CapsLock",
    27: "Escape",
    32: " ",
    33: "PageUp",
    34: "PageDown",
    35: "End",
    36: "Home",
    37: "ArrowLeft",
    38: "ArrowUp",
    39: "ArrowRight",
    40: "ArrowDown",
    45: "Insert",
    46: "Delete",
    112: "F1",
    113: "F2",
    114: "F3",
    115: "F4",
    116: "F5",
    117: "F6",
    118: "F7",
    119: "F8",
    120: "F9",
    121: "F10",
    122: "F11",
    123: "F12",
    144: "NumLock",
    145: "ScrollLock",
    224: "Meta"
  }, wr = {
    Alt: "altKey",
    Control: "ctrlKey",
    Meta: "metaKey",
    Shift: "shiftKey"
  };
  function xr(e) {
    var t = this.nativeEvent;
    return t.getModifierState ? t.getModifierState(e) : (e = wr[e]) ? !!t[e] : !1;
  }
  function M0() {
    return xr;
  }
  var Hr = Y({}, Yl, {
    key: function(e) {
      if (e.key) {
        var t = gr[e.key] || e.key;
        if (t !== "Unidentified") return t;
      }
      return e.type === "keypress" ? (e = Y2(e), e === 13 ? "Enter" : String.fromCharCode(e)) : e.type === "keydown" || e.type === "keyup" ? Mr[e.keyCode] || "Unidentified" : "";
    },
    code: 0,
    location: 0,
    ctrlKey: 0,
    shiftKey: 0,
    altKey: 0,
    metaKey: 0,
    repeat: 0,
    locale: 0,
    getModifierState: M0,
    charCode: function(e) {
      return e.type === "keypress" ? Y2(e) : 0;
    },
    keyCode: function(e) {
      return e.type === "keydown" || e.type === "keyup" ? e.keyCode : 0;
    },
    which: function(e) {
      return e.type === "keypress" ? Y2(e) : e.type === "keydown" || e.type === "keyup" ? e.keyCode : 0;
    }
  }), yr = a1(Hr), Fr = Y({}, X2, {
    pointerId: 0,
    width: 0,
    height: 0,
    pressure: 0,
    tangentialPressure: 0,
    tiltX: 0,
    tiltY: 0,
    twist: 0,
    pointerType: 0,
    isPrimary: 0
  }), $4 = a1(Fr), Dr = Y({}, Yl, {
    touches: 0,
    targetTouches: 0,
    changedTouches: 0,
    altKey: 0,
    metaKey: 0,
    ctrlKey: 0,
    shiftKey: 0,
    getModifierState: M0
  }), Sr = a1(Dr), Ar = Y({}, Ct, {
    propertyName: 0,
    elapsedTime: 0,
    pseudoElement: 0
  }), Br = a1(Ar), br = Y({}, X2, {
    deltaX: function(e) {
      return "deltaX" in e ? e.deltaX : "wheelDeltaX" in e ? -e.wheelDeltaX : 0;
    },
    deltaY: function(e) {
      return "deltaY" in e ? e.deltaY : "wheelDeltaY" in e ? -e.wheelDeltaY : "wheelDelta" in e ? -e.wheelDelta : 0;
    },
    deltaZ: 0,
    deltaMode: 0
  }), Tr = a1(br), Lr = Y({}, Ct, {
    newState: 0,
    oldState: 0
  }), Vr = a1(Lr), Cr = [9, 13, 27, 32], w0 = Q1 && "CompositionEvent" in window, Ql = null;
  Q1 && "documentMode" in document && (Ql = document.documentMode);
  var Or = Q1 && "TextEvent" in window && !Ql, I4 = Q1 && (!w0 || Ql && 8 < Ql && 11 >= Ql), P4 = " ", e5 = !1;
  function t5(e, t) {
    switch (e) {
      case "keyup":
        return Cr.indexOf(t.keyCode) !== -1;
      case "keydown":
        return t.keyCode !== 229;
      case "keypress":
      case "mousedown":
      case "focusout":
        return !0;
      default:
        return !1;
    }
  }
  function l5(e) {
    return e = e.detail, typeof e == "object" && "data" in e ? e.data : null;
  }
  var il = !1;
  function Nr(e, t) {
    switch (e) {
      case "compositionend":
        return l5(t);
      case "keypress":
        return t.which !== 32 ? null : (e5 = !0, P4);
      case "textInput":
        return e = t.data, e === P4 && e5 ? null : e;
      default:
        return null;
    }
  }
  function _r(e, t) {
    if (il)
      return e === "compositionend" || !w0 && t5(e, t) ? (e = K4(), G2 = E0 = rt = null, il = !1, e) : null;
    switch (e) {
      case "paste":
        return null;
      case "keypress":
        if (!(t.ctrlKey || t.altKey || t.metaKey) || t.ctrlKey && t.altKey) {
          if (t.char && 1 < t.char.length)
            return t.char;
          if (t.which) return String.fromCharCode(t.which);
        }
        return null;
      case "compositionend":
        return I4 && t.locale !== "ko" ? null : t.data;
      default:
        return null;
    }
  }
  var Ur = {
    color: !0,
    date: !0,
    datetime: !0,
    "datetime-local": !0,
    email: !0,
    month: !0,
    number: !0,
    password: !0,
    range: !0,
    search: !0,
    tel: !0,
    text: !0,
    time: !0,
    url: !0,
    week: !0
  };
  function a5(e) {
    var t = e && e.nodeName && e.nodeName.toLowerCase();
    return t === "input" ? !!Ur[e.type] : t === "textarea";
  }
  function n5(e, t, l, n) {
    nl ? cl ? cl.push(n) : cl = [n] : nl = n, t = C3(t, "onChange"), 0 < t.length && (l = new Q2(
      "onChange",
      "change",
      null,
      l,
      n
    ), e.push({ event: l, listeners: t }));
  }
  var Xl = null, Zl = null;
  function Rr(e) {
    Gc(e, 0);
  }
  function Z2(e) {
    var t = Rl(e);
    if (R4(t)) return e;
  }
  function c5(e, t) {
    if (e === "change") return t;
  }
  var i5 = !1;
  if (Q1) {
    var x0;
    if (Q1) {
      var H0 = "oninput" in document;
      if (!H0) {
        var u5 = document.createElement("div");
        u5.setAttribute("oninput", "return;"), H0 = typeof u5.oninput == "function";
      }
      x0 = H0;
    } else x0 = !1;
    i5 = x0 && (!document.documentMode || 9 < document.documentMode);
  }
  function r5() {
    Xl && (Xl.detachEvent("onpropertychange", f5), Zl = Xl = null);
  }
  function f5(e) {
    if (e.propertyName === "value" && Z2(Zl)) {
      var t = [];
      n5(
        t,
        Zl,
        e,
        s0(e)
      ), Z4(Rr, t);
    }
  }
  function jr(e, t, l) {
    e === "focusin" ? (r5(), Xl = t, Zl = l, Xl.attachEvent("onpropertychange", f5)) : e === "focusout" && r5();
  }
  function Gr(e) {
    if (e === "selectionchange" || e === "keyup" || e === "keydown")
      return Z2(Zl);
  }
  function Yr(e, t) {
    if (e === "click") return Z2(t);
  }
  function qr(e, t) {
    if (e === "input" || e === "change")
      return Z2(t);
  }
  function Qr(e, t) {
    return e === t && (e !== 0 || 1 / e === 1 / t) || e !== e && t !== t;
  }
  var E1 = typeof Object.is == "function" ? Object.is : Qr;
  function Kl(e, t) {
    if (E1(e, t)) return !0;
    if (typeof e != "object" || e === null || typeof t != "object" || t === null)
      return !1;
    var l = Object.keys(e), n = Object.keys(t);
    if (l.length !== n.length) return !1;
    for (n = 0; n < l.length; n++) {
      var c = l[n];
      if (!e0.call(t, c) || !E1(e[c], t[c]))
        return !1;
    }
    return !0;
  }
  function h5(e) {
    for (; e && e.firstChild; ) e = e.firstChild;
    return e;
  }
  function m5(e, t) {
    var l = h5(e);
    e = 0;
    for (var n; l; ) {
      if (l.nodeType === 3) {
        if (n = e + l.textContent.length, e <= t && n >= t)
          return { node: l, offset: t - e };
        e = n;
      }
      e: {
        for (; l; ) {
          if (l.nextSibling) {
            l = l.nextSibling;
            break e;
          }
          l = l.parentNode;
        }
        l = void 0;
      }
      l = h5(l);
    }
  }
  function v5(e, t) {
    return e && t ? e === t ? !0 : e && e.nodeType === 3 ? !1 : t && t.nodeType === 3 ? v5(e, t.parentNode) : "contains" in e ? e.contains(t) : e.compareDocumentPosition ? !!(e.compareDocumentPosition(t) & 16) : !1 : !1;
  }
  function s5(e) {
    e = e != null && e.ownerDocument != null && e.ownerDocument.defaultView != null ? e.ownerDocument.defaultView : window;
    for (var t = R2(e.document); t instanceof e.HTMLIFrameElement; ) {
      try {
        var l = typeof t.contentWindow.location.href == "string";
      } catch {
        l = !1;
      }
      if (l) e = t.contentWindow;
      else break;
      t = R2(e.document);
    }
    return t;
  }
  function y0(e) {
    var t = e && e.nodeName && e.nodeName.toLowerCase();
    return t && (t === "input" && (e.type === "text" || e.type === "search" || e.type === "tel" || e.type === "url" || e.type === "password") || t === "textarea" || e.contentEditable === "true");
  }
  var Xr = Q1 && "documentMode" in document && 11 >= document.documentMode, ul = null, F0 = null, Jl = null, D0 = !1;
  function d5(e, t, l) {
    var n = l.window === l ? l.document : l.nodeType === 9 ? l : l.ownerDocument;
    D0 || ul == null || ul !== R2(n) || (n = ul, "selectionStart" in n && y0(n) ? n = { start: n.selectionStart, end: n.selectionEnd } : (n = (n.ownerDocument && n.ownerDocument.defaultView || window).getSelection(), n = {
      anchorNode: n.anchorNode,
      anchorOffset: n.anchorOffset,
      focusNode: n.focusNode,
      focusOffset: n.focusOffset
    }), Jl && Kl(Jl, n) || (Jl = n, n = C3(F0, "onSelect"), 0 < n.length && (t = new Q2(
      "onSelect",
      "select",
      null,
      t,
      l
    ), e.push({ event: t, listeners: n }), t.target = ul)));
  }
  function Ot(e, t) {
    var l = {};
    return l[e.toLowerCase()] = t.toLowerCase(), l["Webkit" + e] = "webkit" + t, l["Moz" + e] = "moz" + t, l;
  }
  var rl = {
    animationend: Ot("Animation", "AnimationEnd"),
    animationiteration: Ot("Animation", "AnimationIteration"),
    animationstart: Ot("Animation", "AnimationStart"),
    transitionrun: Ot("Transition", "TransitionRun"),
    transitionstart: Ot("Transition", "TransitionStart"),
    transitioncancel: Ot("Transition", "TransitionCancel"),
    transitionend: Ot("Transition", "TransitionEnd")
  }, S0 = {}, o5 = {};
  Q1 && (o5 = document.createElement("div").style, "AnimationEvent" in window || (delete rl.animationend.animation, delete rl.animationiteration.animation, delete rl.animationstart.animation), "TransitionEvent" in window || delete rl.transitionend.transition);
  function Nt(e) {
    if (S0[e]) return S0[e];
    if (!rl[e]) return e;
    var t = rl[e], l;
    for (l in t)
      if (t.hasOwnProperty(l) && l in o5)
        return S0[e] = t[l];
    return e;
  }
  var E5 = Nt("animationend"), z5 = Nt("animationiteration"), p5 = Nt("animationstart"), Zr = Nt("transitionrun"), Kr = Nt("transitionstart"), Jr = Nt("transitioncancel"), g5 = Nt("transitionend"), M5 = /* @__PURE__ */ new Map(), A0 = "abort auxClick beforeToggle cancel canPlay canPlayThrough click close contextMenu copy cut drag dragEnd dragEnter dragExit dragLeave dragOver dragStart drop durationChange emptied encrypted ended error gotPointerCapture input invalid keyDown keyPress keyUp load loadedData loadedMetadata loadStart lostPointerCapture mouseDown mouseMove mouseOut mouseOver mouseUp paste pause play playing pointerCancel pointerDown pointerMove pointerOut pointerOver pointerUp progress rateChange reset resize seeked seeking stalled submit suspend timeUpdate touchCancel touchEnd touchStart volumeChange scroll toggle touchMove waiting wheel".split(
    " "
  );
  A0.push("scrollEnd");
  function C1(e, t) {
    M5.set(e, t), Vt(t, [e]);
  }
  var K2 = typeof reportError == "function" ? reportError : function(e) {
    if (typeof window == "object" && typeof window.ErrorEvent == "function") {
      var t = new window.ErrorEvent("error", {
        bubbles: !0,
        cancelable: !0,
        message: typeof e == "object" && e !== null && typeof e.message == "string" ? String(e.message) : String(e),
        error: e
      });
      if (!window.dispatchEvent(t)) return;
    } else if (typeof m == "object" && typeof m.emit == "function") {
      m.emit("uncaughtException", e);
      return;
    }
    console.log(e);
  }, D1 = [], fl = 0, B0 = 0;
  function J2() {
    for (var e = fl, t = B0 = fl = 0; t < e; ) {
      var l = D1[t];
      D1[t++] = null;
      var n = D1[t];
      D1[t++] = null;
      var c = D1[t];
      D1[t++] = null;
      var r = D1[t];
      if (D1[t++] = null, n !== null && c !== null) {
        var h = n.pending;
        h === null ? c.next = c : (c.next = h.next, h.next = c), n.pending = c;
      }
      r !== 0 && w5(l, c, r);
    }
  }
  function k2(e, t, l, n) {
    D1[fl++] = e, D1[fl++] = t, D1[fl++] = l, D1[fl++] = n, B0 |= n, e.lanes |= n, e = e.alternate, e !== null && (e.lanes |= n);
  }
  function b0(e, t, l, n) {
    return k2(e, t, l, n), W2(e);
  }
  function _t(e, t) {
    return k2(e, null, null, t), W2(e);
  }
  function w5(e, t, l) {
    e.lanes |= l;
    var n = e.alternate;
    n !== null && (n.lanes |= l);
    for (var c = !1, r = e.return; r !== null; )
      r.childLanes |= l, n = r.alternate, n !== null && (n.childLanes |= l), r.tag === 22 && (e = r.stateNode, e === null || e._visibility & 1 || (c = !0)), e = r, r = r.return;
    return e.tag === 3 ? (r = e.stateNode, c && t !== null && (c = 31 - o1(l), e = r.hiddenUpdates, n = e[c], n === null ? e[c] = [t] : n.push(t), t.lane = l | 536870912), r) : null;
  }
  function W2(e) {
    if (50 < E2)
      throw E2 = 0, Ra = null, Error(E(185));
    for (var t = e.return; t !== null; )
      e = t, t = e.return;
    return e.tag === 3 ? e.stateNode : null;
  }
  var hl = {};
  function kr(e, t, l, n) {
    this.tag = e, this.key = l, this.sibling = this.child = this.return = this.stateNode = this.type = this.elementType = null, this.index = 0, this.refCleanup = this.ref = null, this.pendingProps = t, this.dependencies = this.memoizedState = this.updateQueue = this.memoizedProps = null, this.mode = n, this.subtreeFlags = this.flags = 0, this.deletions = null, this.childLanes = this.lanes = 0, this.alternate = null;
  }
  function z1(e, t, l, n) {
    return new kr(e, t, l, n);
  }
  function T0(e) {
    return e = e.prototype, !(!e || !e.isReactComponent);
  }
  function X1(e, t) {
    var l = e.alternate;
    return l === null ? (l = z1(
      e.tag,
      t,
      e.key,
      e.mode
    ), l.elementType = e.elementType, l.type = e.type, l.stateNode = e.stateNode, l.alternate = e, e.alternate = l) : (l.pendingProps = t, l.type = e.type, l.flags = 0, l.subtreeFlags = 0, l.deletions = null), l.flags = e.flags & 65011712, l.childLanes = e.childLanes, l.lanes = e.lanes, l.child = e.child, l.memoizedProps = e.memoizedProps, l.memoizedState = e.memoizedState, l.updateQueue = e.updateQueue, t = e.dependencies, l.dependencies = t === null ? null : { lanes: t.lanes, firstContext: t.firstContext }, l.sibling = e.sibling, l.index = e.index, l.ref = e.ref, l.refCleanup = e.refCleanup, l;
  }
  function x5(e, t) {
    e.flags &= 65011714;
    var l = e.alternate;
    return l === null ? (e.childLanes = 0, e.lanes = t, e.child = null, e.subtreeFlags = 0, e.memoizedProps = null, e.memoizedState = null, e.updateQueue = null, e.dependencies = null, e.stateNode = null) : (e.childLanes = l.childLanes, e.lanes = l.lanes, e.child = l.child, e.subtreeFlags = 0, e.deletions = null, e.memoizedProps = l.memoizedProps, e.memoizedState = l.memoizedState, e.updateQueue = l.updateQueue, e.type = l.type, t = l.dependencies, e.dependencies = t === null ? null : {
      lanes: t.lanes,
      firstContext: t.firstContext
    }), e;
  }
  function $2(e, t, l, n, c, r) {
    var h = 0;
    if (n = e, typeof e == "function") T0(e) && (h = 1);
    else if (typeof e == "string")
      h = th(
        e,
        l,
        _.current
      ) ? 26 : e === "html" || e === "head" || e === "body" ? 27 : 5;
    else
      e: switch (e) {
        case Pe:
          return e = z1(31, l, t, c), e.elementType = Pe, e.lanes = r, e;
        case Fe:
          return Ut(l.children, c, r, t);
        case ne:
          h = 8, c |= 24;
          break;
        case oe:
          return e = z1(12, l, t, c | 2), e.elementType = oe, e.lanes = r, e;
        case ve:
          return e = z1(13, l, t, c), e.elementType = ve, e.lanes = r, e;
        case be:
          return e = z1(19, l, t, c), e.elementType = be, e.lanes = r, e;
        default:
          if (typeof e == "object" && e !== null)
            switch (e.$$typeof) {
              case Be:
                h = 10;
                break e;
              case f1:
                h = 9;
                break e;
              case $e:
                h = 11;
                break e;
              case ce:
                h = 14;
                break e;
              case Ue:
                h = 16, n = null;
                break e;
            }
          h = 29, l = Error(
            E(130, e === null ? "null" : typeof e, "")
          ), n = null;
      }
    return t = z1(h, l, t, c), t.elementType = e, t.type = n, t.lanes = r, t;
  }
  function Ut(e, t, l, n) {
    return e = z1(7, e, n, t), e.lanes = l, e;
  }
  function L0(e, t, l) {
    return e = z1(6, e, null, t), e.lanes = l, e;
  }
  function H5(e) {
    var t = z1(18, null, null, 0);
    return t.stateNode = e, t;
  }
  function V0(e, t, l) {
    return t = z1(
      4,
      e.children !== null ? e.children : [],
      e.key,
      t
    ), t.lanes = l, t.stateNode = {
      containerInfo: e.containerInfo,
      pendingChildren: null,
      implementation: e.implementation
    }, t;
  }
  var y5 = /* @__PURE__ */ new WeakMap();
  function S1(e, t) {
    if (typeof e == "object" && e !== null) {
      var l = y5.get(e);
      return l !== void 0 ? l : (t = {
        value: e,
        source: t,
        stack: H4(t)
      }, y5.set(e, t), t);
    }
    return {
      value: e,
      source: t,
      stack: H4(t)
    };
  }
  var ml = [], vl = 0, I2 = null, kl = 0, A1 = [], B1 = 0, ft = null, _1 = 1, U1 = "";
  function Z1(e, t) {
    ml[vl++] = kl, ml[vl++] = I2, I2 = e, kl = t;
  }
  function F5(e, t, l) {
    A1[B1++] = _1, A1[B1++] = U1, A1[B1++] = ft, ft = e;
    var n = _1;
    e = U1;
    var c = 32 - o1(n) - 1;
    n &= ~(1 << c), l += 1;
    var r = 32 - o1(t) + c;
    if (30 < r) {
      var h = c - c % 5;
      r = (n & (1 << h) - 1).toString(32), n >>= h, c -= h, _1 = 1 << 32 - o1(t) + c | l << c | n, U1 = r + e;
    } else
      _1 = 1 << r | l << c | n, U1 = e;
  }
  function C0(e) {
    e.return !== null && (Z1(e, 1), F5(e, 1, 0));
  }
  function O0(e) {
    for (; e === I2; )
      I2 = ml[--vl], ml[vl] = null, kl = ml[--vl], ml[vl] = null;
    for (; e === ft; )
      ft = A1[--B1], A1[B1] = null, U1 = A1[--B1], A1[B1] = null, _1 = A1[--B1], A1[B1] = null;
  }
  function D5(e, t) {
    A1[B1++] = _1, A1[B1++] = U1, A1[B1++] = ft, _1 = t.id, U1 = t.overflow, ft = e;
  }
  var Ke = null, we = null, ae = !1, ht = null, b1 = !1, N0 = Error(E(519));
  function mt(e) {
    var t = Error(
      E(
        418,
        1 < arguments.length && arguments[1] !== void 0 && arguments[1] ? "text" : "HTML",
        ""
      )
    );
    throw Wl(S1(t, e)), N0;
  }
  function S5(e) {
    var t = e.stateNode, l = e.type, n = e.memoizedProps;
    switch (t[Ze] = e, t[l1] = n, l) {
      case "dialog":
        ee("cancel", t), ee("close", t);
        break;
      case "iframe":
      case "object":
      case "embed":
        ee("load", t);
        break;
      case "video":
      case "audio":
        for (l = 0; l < p2.length; l++)
          ee(p2[l], t);
        break;
      case "source":
        ee("error", t);
        break;
      case "img":
      case "image":
      case "link":
        ee("error", t), ee("load", t);
        break;
      case "details":
        ee("toggle", t);
        break;
      case "input":
        ee("invalid", t), j4(
          t,
          n.value,
          n.defaultValue,
          n.checked,
          n.defaultChecked,
          n.type,
          n.name,
          !0
        );
        break;
      case "select":
        ee("invalid", t);
        break;
      case "textarea":
        ee("invalid", t), Y4(t, n.value, n.defaultValue, n.children);
    }
    l = n.children, typeof l != "string" && typeof l != "number" && typeof l != "bigint" || t.textContent === "" + l || n.suppressHydrationWarning === !0 || Xc(t.textContent, l) ? (n.popover != null && (ee("beforetoggle", t), ee("toggle", t)), n.onScroll != null && ee("scroll", t), n.onScrollEnd != null && ee("scrollend", t), n.onClick != null && (t.onclick = q1), t = !0) : t = !1, t || mt(e, !0);
  }
  function A5(e) {
    for (Ke = e.return; Ke; )
      switch (Ke.tag) {
        case 5:
        case 31:
        case 13:
          b1 = !1;
          return;
        case 27:
        case 3:
          b1 = !0;
          return;
        default:
          Ke = Ke.return;
      }
  }
  function sl(e) {
    if (e !== Ke) return !1;
    if (!ae) return A5(e), ae = !0, !1;
    var t = e.tag, l;
    if ((l = t !== 3 && t !== 27) && ((l = t === 5) && (l = e.type, l = !(l !== "form" && l !== "button") || e4(e.type, e.memoizedProps)), l = !l), l && we && mt(e), A5(e), t === 13) {
      if (e = e.memoizedState, e = e !== null ? e.dehydrated : null, !e) throw Error(E(317));
      we = e6(e);
    } else if (t === 31) {
      if (e = e.memoizedState, e = e !== null ? e.dehydrated : null, !e) throw Error(E(317));
      we = e6(e);
    } else
      t === 27 ? (t = we, Ft(e.type) ? (e = c4, c4 = null, we = e) : we = t) : we = Ke ? L1(e.stateNode.nextSibling) : null;
    return !0;
  }
  function Rt() {
    we = Ke = null, ae = !1;
  }
  function _0() {
    var e = ht;
    return e !== null && (u1 === null ? u1 = e : u1.push.apply(
      u1,
      e
    ), ht = null), e;
  }
  function Wl(e) {
    ht === null ? ht = [e] : ht.push(e);
  }
  var U0 = p(null), jt = null, K1 = null;
  function vt(e, t, l) {
    V(U0, t._currentValue), t._currentValue = l;
  }
  function J1(e) {
    e._currentValue = U0.current, B(U0);
  }
  function R0(e, t, l) {
    for (; e !== null; ) {
      var n = e.alternate;
      if ((e.childLanes & t) !== t ? (e.childLanes |= t, n !== null && (n.childLanes |= t)) : n !== null && (n.childLanes & t) !== t && (n.childLanes |= t), e === l) break;
      e = e.return;
    }
  }
  function j0(e, t, l, n) {
    var c = e.child;
    for (c !== null && (c.return = e); c !== null; ) {
      var r = c.dependencies;
      if (r !== null) {
        var h = c.child;
        r = r.firstContext;
        e: for (; r !== null; ) {
          var v = r;
          r = c;
          for (var o = 0; o < t.length; o++)
            if (v.context === t[o]) {
              r.lanes |= l, v = r.alternate, v !== null && (v.lanes |= l), R0(
                r.return,
                l,
                e
              ), n || (h = null);
              break e;
            }
          r = v.next;
        }
      } else if (c.tag === 18) {
        if (h = c.return, h === null) throw Error(E(341));
        h.lanes |= l, r = h.alternate, r !== null && (r.lanes |= l), R0(h, l, e), h = null;
      } else h = c.child;
      if (h !== null) h.return = c;
      else
        for (h = c; h !== null; ) {
          if (h === e) {
            h = null;
            break;
          }
          if (c = h.sibling, c !== null) {
            c.return = h.return, h = c;
            break;
          }
          h = h.return;
        }
      c = h;
    }
  }
  function dl(e, t, l, n) {
    e = null;
    for (var c = t, r = !1; c !== null; ) {
      if (!r) {
        if ((c.flags & 524288) !== 0) r = !0;
        else if ((c.flags & 262144) !== 0) break;
      }
      if (c.tag === 10) {
        var h = c.alternate;
        if (h === null) throw Error(E(387));
        if (h = h.memoizedProps, h !== null) {
          var v = c.type;
          E1(c.pendingProps.value, h.value) || (e !== null ? e.push(v) : e = [v]);
        }
      } else if (c === ie.current) {
        if (h = c.alternate, h === null) throw Error(E(387));
        h.memoizedState.memoizedState !== c.memoizedState.memoizedState && (e !== null ? e.push(H2) : e = [H2]);
      }
      c = c.return;
    }
    e !== null && j0(
      t,
      e,
      l,
      n
    ), t.flags |= 262144;
  }
  function P2(e) {
    for (e = e.firstContext; e !== null; ) {
      if (!E1(
        e.context._currentValue,
        e.memoizedValue
      ))
        return !0;
      e = e.next;
    }
    return !1;
  }
  function Gt(e) {
    jt = e, K1 = null, e = e.dependencies, e !== null && (e.firstContext = null);
  }
  function Je(e) {
    return B5(jt, e);
  }
  function e3(e, t) {
    return jt === null && Gt(e), B5(e, t);
  }
  function B5(e, t) {
    var l = t._currentValue;
    if (t = { context: t, memoizedValue: l, next: null }, K1 === null) {
      if (e === null) throw Error(E(308));
      K1 = t, e.dependencies = { lanes: 0, firstContext: t }, e.flags |= 524288;
    } else K1 = K1.next = t;
    return l;
  }
  var Wr = typeof AbortController < "u" ? AbortController : function() {
    var e = [], t = this.signal = {
      aborted: !1,
      addEventListener: function(l, n) {
        e.push(n);
      }
    };
    this.abort = function() {
      t.aborted = !0, e.forEach(function(l) {
        return l();
      });
    };
  }, $r = i.unstable_scheduleCallback, Ir = i.unstable_NormalPriority, Ce = {
    $$typeof: Be,
    Consumer: null,
    Provider: null,
    _currentValue: null,
    _currentValue2: null,
    _threadCount: 0
  };
  function G0() {
    return {
      controller: new Wr(),
      data: /* @__PURE__ */ new Map(),
      refCount: 0
    };
  }
  function $l(e) {
    e.refCount--, e.refCount === 0 && $r(Ir, function() {
      e.controller.abort();
    });
  }
  var Il = null, Y0 = 0, ol = 0, El = null;
  function Pr(e, t) {
    if (Il === null) {
      var l = Il = [];
      Y0 = 0, ol = Xa(), El = {
        status: "pending",
        value: void 0,
        then: function(n) {
          l.push(n);
        }
      };
    }
    return Y0++, t.then(b5, b5), t;
  }
  function b5() {
    if (--Y0 === 0 && Il !== null) {
      El !== null && (El.status = "fulfilled");
      var e = Il;
      Il = null, ol = 0, El = null;
      for (var t = 0; t < e.length; t++) (0, e[t])();
    }
  }
  function ef(e, t) {
    var l = [], n = {
      status: "pending",
      value: null,
      reason: null,
      then: function(c) {
        l.push(c);
      }
    };
    return e.then(
      function() {
        n.status = "fulfilled", n.value = t;
        for (var c = 0; c < l.length; c++) (0, l[c])(t);
      },
      function(c) {
        for (n.status = "rejected", n.reason = c, c = 0; c < l.length; c++)
          (0, l[c])(void 0);
      }
    ), n;
  }
  var T5 = b.S;
  b.S = function(e, t) {
    oc = s1(), typeof t == "object" && t !== null && typeof t.then == "function" && Pr(e, t), T5 !== null && T5(e, t);
  };
  var Yt = p(null);
  function q0() {
    var e = Yt.current;
    return e !== null ? e : Me.pooledCache;
  }
  function t3(e, t) {
    t === null ? V(Yt, Yt.current) : V(Yt, t.pool);
  }
  function L5() {
    var e = q0();
    return e === null ? null : { parent: Ce._currentValue, pool: e };
  }
  var zl = Error(E(460)), Q0 = Error(E(474)), l3 = Error(E(542)), a3 = { then: function() {
  } };
  function V5(e) {
    return e = e.status, e === "fulfilled" || e === "rejected";
  }
  function C5(e, t, l) {
    switch (l = e[l], l === void 0 ? e.push(t) : l !== t && (t.then(q1, q1), t = l), t.status) {
      case "fulfilled":
        return t.value;
      case "rejected":
        throw e = t.reason, N5(e), e;
      default:
        if (typeof t.status == "string") t.then(q1, q1);
        else {
          if (e = Me, e !== null && 100 < e.shellSuspendCounter)
            throw Error(E(482));
          e = t, e.status = "pending", e.then(
            function(n) {
              if (t.status === "pending") {
                var c = t;
                c.status = "fulfilled", c.value = n;
              }
            },
            function(n) {
              if (t.status === "pending") {
                var c = t;
                c.status = "rejected", c.reason = n;
              }
            }
          );
        }
        switch (t.status) {
          case "fulfilled":
            return t.value;
          case "rejected":
            throw e = t.reason, N5(e), e;
        }
        throw Qt = t, zl;
    }
  }
  function qt(e) {
    try {
      var t = e._init;
      return t(e._payload);
    } catch (l) {
      throw l !== null && typeof l == "object" && typeof l.then == "function" ? (Qt = l, zl) : l;
    }
  }
  var Qt = null;
  function O5() {
    if (Qt === null) throw Error(E(459));
    var e = Qt;
    return Qt = null, e;
  }
  function N5(e) {
    if (e === zl || e === l3)
      throw Error(E(483));
  }
  var pl = null, Pl = 0;
  function n3(e) {
    var t = Pl;
    return Pl += 1, pl === null && (pl = []), C5(pl, e, t);
  }
  function e2(e, t) {
    t = t.props.ref, e.ref = t !== void 0 ? t : null;
  }
  function c3(e, t) {
    throw t.$$typeof === je ? Error(E(525)) : (e = Object.prototype.toString.call(t), Error(
      E(
        31,
        e === "[object Object]" ? "object with keys {" + Object.keys(t).join(", ") + "}" : e
      )
    ));
  }
  function _5(e) {
    function t(g, z) {
      if (e) {
        var M = g.deletions;
        M === null ? (g.deletions = [z], g.flags |= 16) : M.push(z);
      }
    }
    function l(g, z) {
      if (!e) return null;
      for (; z !== null; )
        t(g, z), z = z.sibling;
      return null;
    }
    function n(g) {
      for (var z = /* @__PURE__ */ new Map(); g !== null; )
        g.key !== null ? z.set(g.key, g) : z.set(g.index, g), g = g.sibling;
      return z;
    }
    function c(g, z) {
      return g = X1(g, z), g.index = 0, g.sibling = null, g;
    }
    function r(g, z, M) {
      return g.index = M, e ? (M = g.alternate, M !== null ? (M = M.index, M < z ? (g.flags |= 67108866, z) : M) : (g.flags |= 67108866, z)) : (g.flags |= 1048576, z);
    }
    function h(g) {
      return e && g.alternate === null && (g.flags |= 67108866), g;
    }
    function v(g, z, M, S) {
      return z === null || z.tag !== 6 ? (z = L0(M, g.mode, S), z.return = g, z) : (z = c(z, M), z.return = g, z);
    }
    function o(g, z, M, S) {
      var j = M.type;
      return j === Fe ? D(
        g,
        z,
        M.props.children,
        S,
        M.key
      ) : z !== null && (z.elementType === j || typeof j == "object" && j !== null && j.$$typeof === Ue && qt(j) === z.type) ? (z = c(z, M.props), e2(z, M), z.return = g, z) : (z = $2(
        M.type,
        M.key,
        M.props,
        null,
        g.mode,
        S
      ), e2(z, M), z.return = g, z);
    }
    function w(g, z, M, S) {
      return z === null || z.tag !== 4 || z.stateNode.containerInfo !== M.containerInfo || z.stateNode.implementation !== M.implementation ? (z = V0(M, g.mode, S), z.return = g, z) : (z = c(z, M.children || []), z.return = g, z);
    }
    function D(g, z, M, S, j) {
      return z === null || z.tag !== 7 ? (z = Ut(
        M,
        g.mode,
        S,
        j
      ), z.return = g, z) : (z = c(z, M), z.return = g, z);
    }
    function A(g, z, M) {
      if (typeof z == "string" && z !== "" || typeof z == "number" || typeof z == "bigint")
        return z = L0(
          "" + z,
          g.mode,
          M
        ), z.return = g, z;
      if (typeof z == "object" && z !== null) {
        switch (z.$$typeof) {
          case Ve:
            return M = $2(
              z.type,
              z.key,
              z.props,
              null,
              g.mode,
              M
            ), e2(M, z), M.return = g, M;
          case W:
            return z = V0(
              z,
              g.mode,
              M
            ), z.return = g, z;
          case Ue:
            return z = qt(z), A(g, z, M);
        }
        if (C(z) || m1(z))
          return z = Ut(
            z,
            g.mode,
            M,
            null
          ), z.return = g, z;
        if (typeof z.then == "function")
          return A(g, n3(z), M);
        if (z.$$typeof === Be)
          return A(
            g,
            e3(g, z),
            M
          );
        c3(g, z);
      }
      return null;
    }
    function x(g, z, M, S) {
      var j = z !== null ? z.key : null;
      if (typeof M == "string" && M !== "" || typeof M == "number" || typeof M == "bigint")
        return j !== null ? null : v(g, z, "" + M, S);
      if (typeof M == "object" && M !== null) {
        switch (M.$$typeof) {
          case Ve:
            return M.key === j ? o(g, z, M, S) : null;
          case W:
            return M.key === j ? w(g, z, M, S) : null;
          case Ue:
            return M = qt(M), x(g, z, M, S);
        }
        if (C(M) || m1(M))
          return j !== null ? null : D(g, z, M, S, null);
        if (typeof M.then == "function")
          return x(
            g,
            z,
            n3(M),
            S
          );
        if (M.$$typeof === Be)
          return x(
            g,
            z,
            e3(g, M),
            S
          );
        c3(g, M);
      }
      return null;
    }
    function y(g, z, M, S, j) {
      if (typeof S == "string" && S !== "" || typeof S == "number" || typeof S == "bigint")
        return g = g.get(M) || null, v(z, g, "" + S, j);
      if (typeof S == "object" && S !== null) {
        switch (S.$$typeof) {
          case Ve:
            return g = g.get(
              S.key === null ? M : S.key
            ) || null, o(z, g, S, j);
          case W:
            return g = g.get(
              S.key === null ? M : S.key
            ) || null, w(z, g, S, j);
          case Ue:
            return S = qt(S), y(
              g,
              z,
              M,
              S,
              j
            );
        }
        if (C(S) || m1(S))
          return g = g.get(M) || null, D(z, g, S, j, null);
        if (typeof S.then == "function")
          return y(
            g,
            z,
            M,
            n3(S),
            j
          );
        if (S.$$typeof === Be)
          return y(
            g,
            z,
            M,
            e3(z, S),
            j
          );
        c3(z, S);
      }
      return null;
    }
    function N(g, z, M, S) {
      for (var j = null, re = null, U = z, K = z = 0, le = null; U !== null && K < M.length; K++) {
        U.index > K ? (le = U, U = null) : le = U.sibling;
        var fe = x(
          g,
          U,
          M[K],
          S
        );
        if (fe === null) {
          U === null && (U = le);
          break;
        }
        e && U && fe.alternate === null && t(g, U), z = r(fe, z, K), re === null ? j = fe : re.sibling = fe, re = fe, U = le;
      }
      if (K === M.length)
        return l(g, U), ae && Z1(g, K), j;
      if (U === null) {
        for (; K < M.length; K++)
          U = A(g, M[K], S), U !== null && (z = r(
            U,
            z,
            K
          ), re === null ? j = U : re.sibling = U, re = U);
        return ae && Z1(g, K), j;
      }
      for (U = n(U); K < M.length; K++)
        le = y(
          U,
          g,
          K,
          M[K],
          S
        ), le !== null && (e && le.alternate !== null && U.delete(
          le.key === null ? K : le.key
        ), z = r(
          le,
          z,
          K
        ), re === null ? j = le : re.sibling = le, re = le);
      return e && U.forEach(function(bt) {
        return t(g, bt);
      }), ae && Z1(g, K), j;
    }
    function q(g, z, M, S) {
      if (M == null) throw Error(E(151));
      for (var j = null, re = null, U = z, K = z = 0, le = null, fe = M.next(); U !== null && !fe.done; K++, fe = M.next()) {
        U.index > K ? (le = U, U = null) : le = U.sibling;
        var bt = x(g, U, fe.value, S);
        if (bt === null) {
          U === null && (U = le);
          break;
        }
        e && U && bt.alternate === null && t(g, U), z = r(bt, z, K), re === null ? j = bt : re.sibling = bt, re = bt, U = le;
      }
      if (fe.done)
        return l(g, U), ae && Z1(g, K), j;
      if (U === null) {
        for (; !fe.done; K++, fe = M.next())
          fe = A(g, fe.value, S), fe !== null && (z = r(fe, z, K), re === null ? j = fe : re.sibling = fe, re = fe);
        return ae && Z1(g, K), j;
      }
      for (U = n(U); !fe.done; K++, fe = M.next())
        fe = y(U, g, K, fe.value, S), fe !== null && (e && fe.alternate !== null && U.delete(fe.key === null ? K : fe.key), z = r(fe, z, K), re === null ? j = fe : re.sibling = fe, re = fe);
      return e && U.forEach(function(vh) {
        return t(g, vh);
      }), ae && Z1(g, K), j;
    }
    function pe(g, z, M, S) {
      if (typeof M == "object" && M !== null && M.type === Fe && M.key === null && (M = M.props.children), typeof M == "object" && M !== null) {
        switch (M.$$typeof) {
          case Ve:
            e: {
              for (var j = M.key; z !== null; ) {
                if (z.key === j) {
                  if (j = M.type, j === Fe) {
                    if (z.tag === 7) {
                      l(
                        g,
                        z.sibling
                      ), S = c(
                        z,
                        M.props.children
                      ), S.return = g, g = S;
                      break e;
                    }
                  } else if (z.elementType === j || typeof j == "object" && j !== null && j.$$typeof === Ue && qt(j) === z.type) {
                    l(
                      g,
                      z.sibling
                    ), S = c(z, M.props), e2(S, M), S.return = g, g = S;
                    break e;
                  }
                  l(g, z);
                  break;
                } else t(g, z);
                z = z.sibling;
              }
              M.type === Fe ? (S = Ut(
                M.props.children,
                g.mode,
                S,
                M.key
              ), S.return = g, g = S) : (S = $2(
                M.type,
                M.key,
                M.props,
                null,
                g.mode,
                S
              ), e2(S, M), S.return = g, g = S);
            }
            return h(g);
          case W:
            e: {
              for (j = M.key; z !== null; ) {
                if (z.key === j)
                  if (z.tag === 4 && z.stateNode.containerInfo === M.containerInfo && z.stateNode.implementation === M.implementation) {
                    l(
                      g,
                      z.sibling
                    ), S = c(z, M.children || []), S.return = g, g = S;
                    break e;
                  } else {
                    l(g, z);
                    break;
                  }
                else t(g, z);
                z = z.sibling;
              }
              S = V0(M, g.mode, S), S.return = g, g = S;
            }
            return h(g);
          case Ue:
            return M = qt(M), pe(
              g,
              z,
              M,
              S
            );
        }
        if (C(M))
          return N(
            g,
            z,
            M,
            S
          );
        if (m1(M)) {
          if (j = m1(M), typeof j != "function") throw Error(E(150));
          return M = j.call(M), q(
            g,
            z,
            M,
            S
          );
        }
        if (typeof M.then == "function")
          return pe(
            g,
            z,
            n3(M),
            S
          );
        if (M.$$typeof === Be)
          return pe(
            g,
            z,
            e3(g, M),
            S
          );
        c3(g, M);
      }
      return typeof M == "string" && M !== "" || typeof M == "number" || typeof M == "bigint" ? (M = "" + M, z !== null && z.tag === 6 ? (l(g, z.sibling), S = c(z, M), S.return = g, g = S) : (l(g, z), S = L0(M, g.mode, S), S.return = g, g = S), h(g)) : l(g, z);
    }
    return function(g, z, M, S) {
      try {
        Pl = 0;
        var j = pe(
          g,
          z,
          M,
          S
        );
        return pl = null, j;
      } catch (U) {
        if (U === zl || U === l3) throw U;
        var re = z1(29, U, null, g.mode);
        return re.lanes = S, re.return = g, re;
      }
    };
  }
  var Xt = _5(!0), U5 = _5(!1), st = !1;
  function X0(e) {
    e.updateQueue = {
      baseState: e.memoizedState,
      firstBaseUpdate: null,
      lastBaseUpdate: null,
      shared: { pending: null, lanes: 0, hiddenCallbacks: null },
      callbacks: null
    };
  }
  function Z0(e, t) {
    e = e.updateQueue, t.updateQueue === e && (t.updateQueue = {
      baseState: e.baseState,
      firstBaseUpdate: e.firstBaseUpdate,
      lastBaseUpdate: e.lastBaseUpdate,
      shared: e.shared,
      callbacks: null
    });
  }
  function dt(e) {
    return { lane: e, tag: 0, payload: null, callback: null, next: null };
  }
  function ot(e, t, l) {
    var n = e.updateQueue;
    if (n === null) return null;
    if (n = n.shared, (me & 2) !== 0) {
      var c = n.pending;
      return c === null ? t.next = t : (t.next = c.next, c.next = t), n.pending = t, t = W2(e), w5(e, null, l), t;
    }
    return k2(e, n, t, l), W2(e);
  }
  function t2(e, t, l) {
    if (t = t.updateQueue, t !== null && (t = t.shared, (l & 4194048) !== 0)) {
      var n = t.lanes;
      n &= e.pendingLanes, l |= n, t.lanes = l, B4(e, l);
    }
  }
  function K0(e, t) {
    var l = e.updateQueue, n = e.alternate;
    if (n !== null && (n = n.updateQueue, l === n)) {
      var c = null, r = null;
      if (l = l.firstBaseUpdate, l !== null) {
        do {
          var h = {
            lane: l.lane,
            tag: l.tag,
            payload: l.payload,
            callback: null,
            next: null
          };
          r === null ? c = r = h : r = r.next = h, l = l.next;
        } while (l !== null);
        r === null ? c = r = t : r = r.next = t;
      } else c = r = t;
      l = {
        baseState: n.baseState,
        firstBaseUpdate: c,
        lastBaseUpdate: r,
        shared: n.shared,
        callbacks: n.callbacks
      }, e.updateQueue = l;
      return;
    }
    e = l.lastBaseUpdate, e === null ? l.firstBaseUpdate = t : e.next = t, l.lastBaseUpdate = t;
  }
  var J0 = !1;
  function l2() {
    if (J0) {
      var e = El;
      if (e !== null) throw e;
    }
  }
  function a2(e, t, l, n) {
    J0 = !1;
    var c = e.updateQueue;
    st = !1;
    var r = c.firstBaseUpdate, h = c.lastBaseUpdate, v = c.shared.pending;
    if (v !== null) {
      c.shared.pending = null;
      var o = v, w = o.next;
      o.next = null, h === null ? r = w : h.next = w, h = o;
      var D = e.alternate;
      D !== null && (D = D.updateQueue, v = D.lastBaseUpdate, v !== h && (v === null ? D.firstBaseUpdate = w : v.next = w, D.lastBaseUpdate = o));
    }
    if (r !== null) {
      var A = c.baseState;
      h = 0, D = w = o = null, v = r;
      do {
        var x = v.lane & -536870913, y = x !== v.lane;
        if (y ? (te & x) === x : (n & x) === x) {
          x !== 0 && x === ol && (J0 = !0), D !== null && (D = D.next = {
            lane: 0,
            tag: v.tag,
            payload: v.payload,
            callback: null,
            next: null
          });
          e: {
            var N = e, q = v;
            x = t;
            var pe = l;
            switch (q.tag) {
              case 1:
                if (N = q.payload, typeof N == "function") {
                  A = N.call(pe, A, x);
                  break e;
                }
                A = N;
                break e;
              case 3:
                N.flags = N.flags & -65537 | 128;
              case 0:
                if (N = q.payload, x = typeof N == "function" ? N.call(pe, A, x) : N, x == null) break e;
                A = Y({}, A, x);
                break e;
              case 2:
                st = !0;
            }
          }
          x = v.callback, x !== null && (e.flags |= 64, y && (e.flags |= 8192), y = c.callbacks, y === null ? c.callbacks = [x] : y.push(x));
        } else
          y = {
            lane: x,
            tag: v.tag,
            payload: v.payload,
            callback: v.callback,
            next: null
          }, D === null ? (w = D = y, o = A) : D = D.next = y, h |= x;
        if (v = v.next, v === null) {
          if (v = c.shared.pending, v === null)
            break;
          y = v, v = y.next, y.next = null, c.lastBaseUpdate = y, c.shared.pending = null;
        }
      } while (!0);
      D === null && (o = A), c.baseState = o, c.firstBaseUpdate = w, c.lastBaseUpdate = D, r === null && (c.shared.lanes = 0), Mt |= h, e.lanes = h, e.memoizedState = A;
    }
  }
  function R5(e, t) {
    if (typeof e != "function")
      throw Error(E(191, e));
    e.call(t);
  }
  function j5(e, t) {
    var l = e.callbacks;
    if (l !== null)
      for (e.callbacks = null, e = 0; e < l.length; e++)
        R5(l[e], t);
  }
  var gl = p(null), i3 = p(0);
  function G5(e, t) {
    e = at, V(i3, e), V(gl, t), at = e | t.baseLanes;
  }
  function k0() {
    V(i3, at), V(gl, gl.current);
  }
  function W0() {
    at = i3.current, B(gl), B(i3);
  }
  var p1 = p(null), T1 = null;
  function Et(e) {
    var t = e.alternate;
    V(Te, Te.current & 1), V(p1, e), T1 === null && (t === null || gl.current !== null || t.memoizedState !== null) && (T1 = e);
  }
  function $0(e) {
    V(Te, Te.current), V(p1, e), T1 === null && (T1 = e);
  }
  function Y5(e) {
    e.tag === 22 ? (V(Te, Te.current), V(p1, e), T1 === null && (T1 = e)) : zt();
  }
  function zt() {
    V(Te, Te.current), V(p1, p1.current);
  }
  function g1(e) {
    B(p1), T1 === e && (T1 = null), B(Te);
  }
  var Te = p(0);
  function u3(e) {
    for (var t = e; t !== null; ) {
      if (t.tag === 13) {
        var l = t.memoizedState;
        if (l !== null && (l = l.dehydrated, l === null || a4(l) || n4(l)))
          return t;
      } else if (t.tag === 19 && (t.memoizedProps.revealOrder === "forwards" || t.memoizedProps.revealOrder === "backwards" || t.memoizedProps.revealOrder === "unstable_legacy-backwards" || t.memoizedProps.revealOrder === "together")) {
        if ((t.flags & 128) !== 0) return t;
      } else if (t.child !== null) {
        t.child.return = t, t = t.child;
        continue;
      }
      if (t === e) break;
      for (; t.sibling === null; ) {
        if (t.return === null || t.return === e) return null;
        t = t.return;
      }
      t.sibling.return = t.return, t = t.sibling;
    }
    return null;
  }
  var k1 = 0, Z = null, Ee = null, Oe = null, r3 = !1, Ml = !1, Zt = !1, f3 = 0, n2 = 0, wl = null, tf = 0;
  function Se() {
    throw Error(E(321));
  }
  function I0(e, t) {
    if (t === null) return !1;
    for (var l = 0; l < t.length && l < e.length; l++)
      if (!E1(e[l], t[l])) return !1;
    return !0;
  }
  function P0(e, t, l, n, c, r) {
    return k1 = r, Z = t, t.memoizedState = null, t.updateQueue = null, t.lanes = 0, b.H = e === null || e.memoizedState === null ? Dn : da, Zt = !1, r = l(n, c), Zt = !1, Ml && (r = Q5(
      t,
      l,
      n,
      c
    )), q5(e), r;
  }
  function q5(e) {
    b.H = u2;
    var t = Ee !== null && Ee.next !== null;
    if (k1 = 0, Oe = Ee = Z = null, r3 = !1, n2 = 0, wl = null, t) throw Error(E(300));
    e === null || Ne || (e = e.dependencies, e !== null && P2(e) && (Ne = !0));
  }
  function Q5(e, t, l, n) {
    Z = e;
    var c = 0;
    do {
      if (Ml && (wl = null), n2 = 0, Ml = !1, 25 <= c) throw Error(E(301));
      if (c += 1, Oe = Ee = null, e.updateQueue != null) {
        var r = e.updateQueue;
        r.lastEffect = null, r.events = null, r.stores = null, r.memoCache != null && (r.memoCache.index = 0);
      }
      b.H = Sn, r = t(l, n);
    } while (Ml);
    return r;
  }
  function lf() {
    var e = b.H, t = e.useState()[0];
    return t = typeof t.then == "function" ? c2(t) : t, e = e.useState()[0], (Ee !== null ? Ee.memoizedState : null) !== e && (Z.flags |= 1024), t;
  }
  function ea() {
    var e = f3 !== 0;
    return f3 = 0, e;
  }
  function ta(e, t, l) {
    t.updateQueue = e.updateQueue, t.flags &= -2053, e.lanes &= ~l;
  }
  function la(e) {
    if (r3) {
      for (e = e.memoizedState; e !== null; ) {
        var t = e.queue;
        t !== null && (t.pending = null), e = e.next;
      }
      r3 = !1;
    }
    k1 = 0, Oe = Ee = Z = null, Ml = !1, n2 = f3 = 0, wl = null;
  }
  function t1() {
    var e = {
      memoizedState: null,
      baseState: null,
      baseQueue: null,
      queue: null,
      next: null
    };
    return Oe === null ? Z.memoizedState = Oe = e : Oe = Oe.next = e, Oe;
  }
  function Le() {
    if (Ee === null) {
      var e = Z.alternate;
      e = e !== null ? e.memoizedState : null;
    } else e = Ee.next;
    var t = Oe === null ? Z.memoizedState : Oe.next;
    if (t !== null)
      Oe = t, Ee = e;
    else {
      if (e === null)
        throw Z.alternate === null ? Error(E(467)) : Error(E(310));
      Ee = e, e = {
        memoizedState: Ee.memoizedState,
        baseState: Ee.baseState,
        baseQueue: Ee.baseQueue,
        queue: Ee.queue,
        next: null
      }, Oe === null ? Z.memoizedState = Oe = e : Oe = Oe.next = e;
    }
    return Oe;
  }
  function h3() {
    return { lastEffect: null, events: null, stores: null, memoCache: null };
  }
  function c2(e) {
    var t = n2;
    return n2 += 1, wl === null && (wl = []), e = C5(wl, e, t), t = Z, (Oe === null ? t.memoizedState : Oe.next) === null && (t = t.alternate, b.H = t === null || t.memoizedState === null ? Dn : da), e;
  }
  function m3(e) {
    if (e !== null && typeof e == "object") {
      if (typeof e.then == "function") return c2(e);
      if (e.$$typeof === Be) return Je(e);
    }
    throw Error(E(438, String(e)));
  }
  function aa(e) {
    var t = null, l = Z.updateQueue;
    if (l !== null && (t = l.memoCache), t == null) {
      var n = Z.alternate;
      n !== null && (n = n.updateQueue, n !== null && (n = n.memoCache, n != null && (t = {
        data: n.data.map(function(c) {
          return c.slice();
        }),
        index: 0
      })));
    }
    if (t == null && (t = { data: [], index: 0 }), l === null && (l = h3(), Z.updateQueue = l), l.memoCache = t, l = t.data[t.index], l === void 0)
      for (l = t.data[t.index] = Array(e), n = 0; n < e; n++)
        l[n] = h1;
    return t.index++, l;
  }
  function W1(e, t) {
    return typeof t == "function" ? t(e) : t;
  }
  function v3(e) {
    var t = Le();
    return na(t, Ee, e);
  }
  function na(e, t, l) {
    var n = e.queue;
    if (n === null) throw Error(E(311));
    n.lastRenderedReducer = l;
    var c = e.baseQueue, r = n.pending;
    if (r !== null) {
      if (c !== null) {
        var h = c.next;
        c.next = r.next, r.next = h;
      }
      t.baseQueue = c = r, n.pending = null;
    }
    if (r = e.baseState, c === null) e.memoizedState = r;
    else {
      t = c.next;
      var v = h = null, o = null, w = t, D = !1;
      do {
        var A = w.lane & -536870913;
        if (A !== w.lane ? (te & A) === A : (k1 & A) === A) {
          var x = w.revertLane;
          if (x === 0)
            o !== null && (o = o.next = {
              lane: 0,
              revertLane: 0,
              gesture: null,
              action: w.action,
              hasEagerState: w.hasEagerState,
              eagerState: w.eagerState,
              next: null
            }), A === ol && (D = !0);
          else if ((k1 & x) === x) {
            w = w.next, x === ol && (D = !0);
            continue;
          } else
            A = {
              lane: 0,
              revertLane: w.revertLane,
              gesture: null,
              action: w.action,
              hasEagerState: w.hasEagerState,
              eagerState: w.eagerState,
              next: null
            }, o === null ? (v = o = A, h = r) : o = o.next = A, Z.lanes |= x, Mt |= x;
          A = w.action, Zt && l(r, A), r = w.hasEagerState ? w.eagerState : l(r, A);
        } else
          x = {
            lane: A,
            revertLane: w.revertLane,
            gesture: w.gesture,
            action: w.action,
            hasEagerState: w.hasEagerState,
            eagerState: w.eagerState,
            next: null
          }, o === null ? (v = o = x, h = r) : o = o.next = x, Z.lanes |= A, Mt |= A;
        w = w.next;
      } while (w !== null && w !== t);
      if (o === null ? h = r : o.next = v, !E1(r, e.memoizedState) && (Ne = !0, D && (l = El, l !== null)))
        throw l;
      e.memoizedState = r, e.baseState = h, e.baseQueue = o, n.lastRenderedState = r;
    }
    return c === null && (n.lanes = 0), [e.memoizedState, n.dispatch];
  }
  function ca(e) {
    var t = Le(), l = t.queue;
    if (l === null) throw Error(E(311));
    l.lastRenderedReducer = e;
    var n = l.dispatch, c = l.pending, r = t.memoizedState;
    if (c !== null) {
      l.pending = null;
      var h = c = c.next;
      do
        r = e(r, h.action), h = h.next;
      while (h !== c);
      E1(r, t.memoizedState) || (Ne = !0), t.memoizedState = r, t.baseQueue === null && (t.baseState = r), l.lastRenderedState = r;
    }
    return [r, n];
  }
  function X5(e, t, l) {
    var n = Z, c = Le(), r = ae;
    if (r) {
      if (l === void 0) throw Error(E(407));
      l = l();
    } else l = t();
    var h = !E1(
      (Ee || c).memoizedState,
      l
    );
    if (h && (c.memoizedState = l, Ne = !0), c = c.queue, ra(J5.bind(null, n, c, e), [
      e
    ]), c.getSnapshot !== t || h || Oe !== null && Oe.memoizedState.tag & 1) {
      if (n.flags |= 2048, xl(
        9,
        { destroy: void 0 },
        K5.bind(
          null,
          n,
          c,
          l,
          t
        ),
        null
      ), Me === null) throw Error(E(349));
      r || (k1 & 127) !== 0 || Z5(n, t, l);
    }
    return l;
  }
  function Z5(e, t, l) {
    e.flags |= 16384, e = { getSnapshot: t, value: l }, t = Z.updateQueue, t === null ? (t = h3(), Z.updateQueue = t, t.stores = [e]) : (l = t.stores, l === null ? t.stores = [e] : l.push(e));
  }
  function K5(e, t, l, n) {
    t.value = l, t.getSnapshot = n, k5(t) && W5(e);
  }
  function J5(e, t, l) {
    return l(function() {
      k5(t) && W5(e);
    });
  }
  function k5(e) {
    var t = e.getSnapshot;
    e = e.value;
    try {
      var l = t();
      return !E1(e, l);
    } catch {
      return !0;
    }
  }
  function W5(e) {
    var t = _t(e, 2);
    t !== null && r1(t, e, 2);
  }
  function ia(e) {
    var t = t1();
    if (typeof e == "function") {
      var l = e;
      if (e = l(), Zt) {
        it(!0);
        try {
          l();
        } finally {
          it(!1);
        }
      }
    }
    return t.memoizedState = t.baseState = e, t.queue = {
      pending: null,
      lanes: 0,
      dispatch: null,
      lastRenderedReducer: W1,
      lastRenderedState: e
    }, t;
  }
  function $5(e, t, l, n) {
    return e.baseState = l, na(
      e,
      Ee,
      typeof n == "function" ? n : W1
    );
  }
  function af(e, t, l, n, c) {
    if (o3(e)) throw Error(E(485));
    if (e = t.action, e !== null) {
      var r = {
        payload: c,
        action: e,
        next: null,
        isTransition: !0,
        status: "pending",
        value: null,
        reason: null,
        listeners: [],
        then: function(h) {
          r.listeners.push(h);
        }
      };
      b.T !== null ? l(!0) : r.isTransition = !1, n(r), l = t.pending, l === null ? (r.next = t.pending = r, I5(t, r)) : (r.next = l.next, t.pending = l.next = r);
    }
  }
  function I5(e, t) {
    var l = t.action, n = t.payload, c = e.state;
    if (t.isTransition) {
      var r = b.T, h = {};
      b.T = h;
      try {
        var v = l(c, n), o = b.S;
        o !== null && o(h, v), P5(e, t, v);
      } catch (w) {
        ua(e, t, w);
      } finally {
        r !== null && h.types !== null && (r.types = h.types), b.T = r;
      }
    } else
      try {
        r = l(c, n), P5(e, t, r);
      } catch (w) {
        ua(e, t, w);
      }
  }
  function P5(e, t, l) {
    l !== null && typeof l == "object" && typeof l.then == "function" ? l.then(
      function(n) {
        en(e, t, n);
      },
      function(n) {
        return ua(e, t, n);
      }
    ) : en(e, t, l);
  }
  function en(e, t, l) {
    t.status = "fulfilled", t.value = l, tn(t), e.state = l, t = e.pending, t !== null && (l = t.next, l === t ? e.pending = null : (l = l.next, t.next = l, I5(e, l)));
  }
  function ua(e, t, l) {
    var n = e.pending;
    if (e.pending = null, n !== null) {
      n = n.next;
      do
        t.status = "rejected", t.reason = l, tn(t), t = t.next;
      while (t !== n);
    }
    e.action = null;
  }
  function tn(e) {
    e = e.listeners;
    for (var t = 0; t < e.length; t++) (0, e[t])();
  }
  function ln(e, t) {
    return t;
  }
  function an(e, t) {
    if (ae) {
      var l = Me.formState;
      if (l !== null) {
        e: {
          var n = Z;
          if (ae) {
            if (we) {
              t: {
                for (var c = we, r = b1; c.nodeType !== 8; ) {
                  if (!r) {
                    c = null;
                    break t;
                  }
                  if (c = L1(
                    c.nextSibling
                  ), c === null) {
                    c = null;
                    break t;
                  }
                }
                r = c.data, c = r === "F!" || r === "F" ? c : null;
              }
              if (c) {
                we = L1(
                  c.nextSibling
                ), n = c.data === "F!";
                break e;
              }
            }
            mt(n);
          }
          n = !1;
        }
        n && (t = l[0]);
      }
    }
    return l = t1(), l.memoizedState = l.baseState = t, n = {
      pending: null,
      lanes: 0,
      dispatch: null,
      lastRenderedReducer: ln,
      lastRenderedState: t
    }, l.queue = n, l = Hn.bind(
      null,
      Z,
      n
    ), n.dispatch = l, n = ia(!1), r = sa.bind(
      null,
      Z,
      !1,
      n.queue
    ), n = t1(), c = {
      state: t,
      dispatch: null,
      action: e,
      pending: null
    }, n.queue = c, l = af.bind(
      null,
      Z,
      c,
      r,
      l
    ), c.dispatch = l, n.memoizedState = e, [t, l, !1];
  }
  function nn(e) {
    var t = Le();
    return cn(t, Ee, e);
  }
  function cn(e, t, l) {
    if (t = na(
      e,
      t,
      ln
    )[0], e = v3(W1)[0], typeof t == "object" && t !== null && typeof t.then == "function")
      try {
        var n = c2(t);
      } catch (h) {
        throw h === zl ? l3 : h;
      }
    else n = t;
    t = Le();
    var c = t.queue, r = c.dispatch;
    return l !== t.memoizedState && (Z.flags |= 2048, xl(
      9,
      { destroy: void 0 },
      nf.bind(null, c, l),
      null
    )), [n, r, e];
  }
  function nf(e, t) {
    e.action = t;
  }
  function un(e) {
    var t = Le(), l = Ee;
    if (l !== null)
      return cn(t, l, e);
    Le(), t = t.memoizedState, l = Le();
    var n = l.queue.dispatch;
    return l.memoizedState = e, [t, n, !1];
  }
  function xl(e, t, l, n) {
    return e = { tag: e, create: l, deps: n, inst: t, next: null }, t = Z.updateQueue, t === null && (t = h3(), Z.updateQueue = t), l = t.lastEffect, l === null ? t.lastEffect = e.next = e : (n = l.next, l.next = e, e.next = n, t.lastEffect = e), e;
  }
  function rn() {
    return Le().memoizedState;
  }
  function s3(e, t, l, n) {
    var c = t1();
    Z.flags |= e, c.memoizedState = xl(
      1 | t,
      { destroy: void 0 },
      l,
      n === void 0 ? null : n
    );
  }
  function d3(e, t, l, n) {
    var c = Le();
    n = n === void 0 ? null : n;
    var r = c.memoizedState.inst;
    Ee !== null && n !== null && I0(n, Ee.memoizedState.deps) ? c.memoizedState = xl(t, r, l, n) : (Z.flags |= e, c.memoizedState = xl(
      1 | t,
      r,
      l,
      n
    ));
  }
  function fn(e, t) {
    s3(8390656, 8, e, t);
  }
  function ra(e, t) {
    d3(2048, 8, e, t);
  }
  function cf(e) {
    Z.flags |= 4;
    var t = Z.updateQueue;
    if (t === null)
      t = h3(), Z.updateQueue = t, t.events = [e];
    else {
      var l = t.events;
      l === null ? t.events = [e] : l.push(e);
    }
  }
  function hn(e) {
    var t = Le().memoizedState;
    return cf({ ref: t, nextImpl: e }), function() {
      if ((me & 2) !== 0) throw Error(E(440));
      return t.impl.apply(void 0, arguments);
    };
  }
  function mn(e, t) {
    return d3(4, 2, e, t);
  }
  function vn(e, t) {
    return d3(4, 4, e, t);
  }
  function sn(e, t) {
    if (typeof t == "function") {
      e = e();
      var l = t(e);
      return function() {
        typeof l == "function" ? l() : t(null);
      };
    }
    if (t != null)
      return e = e(), t.current = e, function() {
        t.current = null;
      };
  }
  function dn(e, t, l) {
    l = l != null ? l.concat([e]) : null, d3(4, 4, sn.bind(null, t, e), l);
  }
  function fa() {
  }
  function on(e, t) {
    var l = Le();
    t = t === void 0 ? null : t;
    var n = l.memoizedState;
    return t !== null && I0(t, n[1]) ? n[0] : (l.memoizedState = [e, t], e);
  }
  function En(e, t) {
    var l = Le();
    t = t === void 0 ? null : t;
    var n = l.memoizedState;
    if (t !== null && I0(t, n[1]))
      return n[0];
    if (n = e(), Zt) {
      it(!0);
      try {
        e();
      } finally {
        it(!1);
      }
    }
    return l.memoizedState = [n, t], n;
  }
  function ha(e, t, l) {
    return l === void 0 || (k1 & 1073741824) !== 0 && (te & 261930) === 0 ? e.memoizedState = t : (e.memoizedState = l, e = zc(), Z.lanes |= e, Mt |= e, l);
  }
  function zn(e, t, l, n) {
    return E1(l, t) ? l : gl.current !== null ? (e = ha(e, l, n), E1(e, t) || (Ne = !0), e) : (k1 & 42) === 0 || (k1 & 1073741824) !== 0 && (te & 261930) === 0 ? (Ne = !0, e.memoizedState = l) : (e = zc(), Z.lanes |= e, Mt |= e, t);
  }
  function pn(e, t, l, n, c) {
    var r = L.p;
    L.p = r !== 0 && 8 > r ? r : 8;
    var h = b.T, v = {};
    b.T = v, sa(e, !1, t, l);
    try {
      var o = c(), w = b.S;
      if (w !== null && w(v, o), o !== null && typeof o == "object" && typeof o.then == "function") {
        var D = ef(
          o,
          n
        );
        i2(
          e,
          t,
          D,
          x1(e)
        );
      } else
        i2(
          e,
          t,
          n,
          x1(e)
        );
    } catch (A) {
      i2(
        e,
        t,
        { then: function() {
        }, status: "rejected", reason: A },
        x1()
      );
    } finally {
      L.p = r, h !== null && v.types !== null && (h.types = v.types), b.T = h;
    }
  }
  function uf() {
  }
  function ma(e, t, l, n) {
    if (e.tag !== 5) throw Error(E(476));
    var c = gn(e).queue;
    pn(
      e,
      c,
      t,
      I,
      l === null ? uf : function() {
        return Mn(e), l(n);
      }
    );
  }
  function gn(e) {
    var t = e.memoizedState;
    if (t !== null) return t;
    t = {
      memoizedState: I,
      baseState: I,
      baseQueue: null,
      queue: {
        pending: null,
        lanes: 0,
        dispatch: null,
        lastRenderedReducer: W1,
        lastRenderedState: I
      },
      next: null
    };
    var l = {};
    return t.next = {
      memoizedState: l,
      baseState: l,
      baseQueue: null,
      queue: {
        pending: null,
        lanes: 0,
        dispatch: null,
        lastRenderedReducer: W1,
        lastRenderedState: l
      },
      next: null
    }, e.memoizedState = t, e = e.alternate, e !== null && (e.memoizedState = t), t;
  }
  function Mn(e) {
    var t = gn(e);
    t.next === null && (t = e.alternate.memoizedState), i2(
      e,
      t.next.queue,
      {},
      x1()
    );
  }
  function va() {
    return Je(H2);
  }
  function wn() {
    return Le().memoizedState;
  }
  function xn() {
    return Le().memoizedState;
  }
  function rf(e) {
    for (var t = e.return; t !== null; ) {
      switch (t.tag) {
        case 24:
        case 3:
          var l = x1();
          e = dt(l);
          var n = ot(t, e, l);
          n !== null && (r1(n, t, l), t2(n, t, l)), t = { cache: G0() }, e.payload = t;
          return;
      }
      t = t.return;
    }
  }
  function ff(e, t, l) {
    var n = x1();
    l = {
      lane: n,
      revertLane: 0,
      gesture: null,
      action: l,
      hasEagerState: !1,
      eagerState: null,
      next: null
    }, o3(e) ? yn(t, l) : (l = b0(e, t, l, n), l !== null && (r1(l, e, n), Fn(l, t, n)));
  }
  function Hn(e, t, l) {
    var n = x1();
    i2(e, t, l, n);
  }
  function i2(e, t, l, n) {
    var c = {
      lane: n,
      revertLane: 0,
      gesture: null,
      action: l,
      hasEagerState: !1,
      eagerState: null,
      next: null
    };
    if (o3(e)) yn(t, c);
    else {
      var r = e.alternate;
      if (e.lanes === 0 && (r === null || r.lanes === 0) && (r = t.lastRenderedReducer, r !== null))
        try {
          var h = t.lastRenderedState, v = r(h, l);
          if (c.hasEagerState = !0, c.eagerState = v, E1(v, h))
            return k2(e, t, c, 0), Me === null && J2(), !1;
        } catch {
        }
      if (l = b0(e, t, c, n), l !== null)
        return r1(l, e, n), Fn(l, t, n), !0;
    }
    return !1;
  }
  function sa(e, t, l, n) {
    if (n = {
      lane: 2,
      revertLane: Xa(),
      gesture: null,
      action: n,
      hasEagerState: !1,
      eagerState: null,
      next: null
    }, o3(e)) {
      if (t) throw Error(E(479));
    } else
      t = b0(
        e,
        l,
        n,
        2
      ), t !== null && r1(t, e, 2);
  }
  function o3(e) {
    var t = e.alternate;
    return e === Z || t !== null && t === Z;
  }
  function yn(e, t) {
    Ml = r3 = !0;
    var l = e.pending;
    l === null ? t.next = t : (t.next = l.next, l.next = t), e.pending = t;
  }
  function Fn(e, t, l) {
    if ((l & 4194048) !== 0) {
      var n = t.lanes;
      n &= e.pendingLanes, l |= n, t.lanes = l, B4(e, l);
    }
  }
  var u2 = {
    readContext: Je,
    use: m3,
    useCallback: Se,
    useContext: Se,
    useEffect: Se,
    useImperativeHandle: Se,
    useLayoutEffect: Se,
    useInsertionEffect: Se,
    useMemo: Se,
    useReducer: Se,
    useRef: Se,
    useState: Se,
    useDebugValue: Se,
    useDeferredValue: Se,
    useTransition: Se,
    useSyncExternalStore: Se,
    useId: Se,
    useHostTransitionStatus: Se,
    useFormState: Se,
    useActionState: Se,
    useOptimistic: Se,
    useMemoCache: Se,
    useCacheRefresh: Se
  };
  u2.useEffectEvent = Se;
  var Dn = {
    readContext: Je,
    use: m3,
    useCallback: function(e, t) {
      return t1().memoizedState = [
        e,
        t === void 0 ? null : t
      ], e;
    },
    useContext: Je,
    useEffect: fn,
    useImperativeHandle: function(e, t, l) {
      l = l != null ? l.concat([e]) : null, s3(
        4194308,
        4,
        sn.bind(null, t, e),
        l
      );
    },
    useLayoutEffect: function(e, t) {
      return s3(4194308, 4, e, t);
    },
    useInsertionEffect: function(e, t) {
      s3(4, 2, e, t);
    },
    useMemo: function(e, t) {
      var l = t1();
      t = t === void 0 ? null : t;
      var n = e();
      if (Zt) {
        it(!0);
        try {
          e();
        } finally {
          it(!1);
        }
      }
      return l.memoizedState = [n, t], n;
    },
    useReducer: function(e, t, l) {
      var n = t1();
      if (l !== void 0) {
        var c = l(t);
        if (Zt) {
          it(!0);
          try {
            l(t);
          } finally {
            it(!1);
          }
        }
      } else c = t;
      return n.memoizedState = n.baseState = c, e = {
        pending: null,
        lanes: 0,
        dispatch: null,
        lastRenderedReducer: e,
        lastRenderedState: c
      }, n.queue = e, e = e.dispatch = ff.bind(
        null,
        Z,
        e
      ), [n.memoizedState, e];
    },
    useRef: function(e) {
      var t = t1();
      return e = { current: e }, t.memoizedState = e;
    },
    useState: function(e) {
      e = ia(e);
      var t = e.queue, l = Hn.bind(null, Z, t);
      return t.dispatch = l, [e.memoizedState, l];
    },
    useDebugValue: fa,
    useDeferredValue: function(e, t) {
      var l = t1();
      return ha(l, e, t);
    },
    useTransition: function() {
      var e = ia(!1);
      return e = pn.bind(
        null,
        Z,
        e.queue,
        !0,
        !1
      ), t1().memoizedState = e, [!1, e];
    },
    useSyncExternalStore: function(e, t, l) {
      var n = Z, c = t1();
      if (ae) {
        if (l === void 0)
          throw Error(E(407));
        l = l();
      } else {
        if (l = t(), Me === null)
          throw Error(E(349));
        (te & 127) !== 0 || Z5(n, t, l);
      }
      c.memoizedState = l;
      var r = { value: l, getSnapshot: t };
      return c.queue = r, fn(J5.bind(null, n, r, e), [
        e
      ]), n.flags |= 2048, xl(
        9,
        { destroy: void 0 },
        K5.bind(
          null,
          n,
          r,
          l,
          t
        ),
        null
      ), l;
    },
    useId: function() {
      var e = t1(), t = Me.identifierPrefix;
      if (ae) {
        var l = U1, n = _1;
        l = (n & ~(1 << 32 - o1(n) - 1)).toString(32) + l, t = "_" + t + "R_" + l, l = f3++, 0 < l && (t += "H" + l.toString(32)), t += "_";
      } else
        l = tf++, t = "_" + t + "r_" + l.toString(32) + "_";
      return e.memoizedState = t;
    },
    useHostTransitionStatus: va,
    useFormState: an,
    useActionState: an,
    useOptimistic: function(e) {
      var t = t1();
      t.memoizedState = t.baseState = e;
      var l = {
        pending: null,
        lanes: 0,
        dispatch: null,
        lastRenderedReducer: null,
        lastRenderedState: null
      };
      return t.queue = l, t = sa.bind(
        null,
        Z,
        !0,
        l
      ), l.dispatch = t, [e, t];
    },
    useMemoCache: aa,
    useCacheRefresh: function() {
      return t1().memoizedState = rf.bind(
        null,
        Z
      );
    },
    useEffectEvent: function(e) {
      var t = t1(), l = { impl: e };
      return t.memoizedState = l, function() {
        if ((me & 2) !== 0)
          throw Error(E(440));
        return l.impl.apply(void 0, arguments);
      };
    }
  }, da = {
    readContext: Je,
    use: m3,
    useCallback: on,
    useContext: Je,
    useEffect: ra,
    useImperativeHandle: dn,
    useInsertionEffect: mn,
    useLayoutEffect: vn,
    useMemo: En,
    useReducer: v3,
    useRef: rn,
    useState: function() {
      return v3(W1);
    },
    useDebugValue: fa,
    useDeferredValue: function(e, t) {
      var l = Le();
      return zn(
        l,
        Ee.memoizedState,
        e,
        t
      );
    },
    useTransition: function() {
      var e = v3(W1)[0], t = Le().memoizedState;
      return [
        typeof e == "boolean" ? e : c2(e),
        t
      ];
    },
    useSyncExternalStore: X5,
    useId: wn,
    useHostTransitionStatus: va,
    useFormState: nn,
    useActionState: nn,
    useOptimistic: function(e, t) {
      var l = Le();
      return $5(l, Ee, e, t);
    },
    useMemoCache: aa,
    useCacheRefresh: xn
  };
  da.useEffectEvent = hn;
  var Sn = {
    readContext: Je,
    use: m3,
    useCallback: on,
    useContext: Je,
    useEffect: ra,
    useImperativeHandle: dn,
    useInsertionEffect: mn,
    useLayoutEffect: vn,
    useMemo: En,
    useReducer: ca,
    useRef: rn,
    useState: function() {
      return ca(W1);
    },
    useDebugValue: fa,
    useDeferredValue: function(e, t) {
      var l = Le();
      return Ee === null ? ha(l, e, t) : zn(
        l,
        Ee.memoizedState,
        e,
        t
      );
    },
    useTransition: function() {
      var e = ca(W1)[0], t = Le().memoizedState;
      return [
        typeof e == "boolean" ? e : c2(e),
        t
      ];
    },
    useSyncExternalStore: X5,
    useId: wn,
    useHostTransitionStatus: va,
    useFormState: un,
    useActionState: un,
    useOptimistic: function(e, t) {
      var l = Le();
      return Ee !== null ? $5(l, Ee, e, t) : (l.baseState = e, [e, l.queue.dispatch]);
    },
    useMemoCache: aa,
    useCacheRefresh: xn
  };
  Sn.useEffectEvent = hn;
  function oa(e, t, l, n) {
    t = e.memoizedState, l = l(n, t), l = l == null ? t : Y({}, t, l), e.memoizedState = l, e.lanes === 0 && (e.updateQueue.baseState = l);
  }
  var Ea = {
    enqueueSetState: function(e, t, l) {
      e = e._reactInternals;
      var n = x1(), c = dt(n);
      c.payload = t, l != null && (c.callback = l), t = ot(e, c, n), t !== null && (r1(t, e, n), t2(t, e, n));
    },
    enqueueReplaceState: function(e, t, l) {
      e = e._reactInternals;
      var n = x1(), c = dt(n);
      c.tag = 1, c.payload = t, l != null && (c.callback = l), t = ot(e, c, n), t !== null && (r1(t, e, n), t2(t, e, n));
    },
    enqueueForceUpdate: function(e, t) {
      e = e._reactInternals;
      var l = x1(), n = dt(l);
      n.tag = 2, t != null && (n.callback = t), t = ot(e, n, l), t !== null && (r1(t, e, l), t2(t, e, l));
    }
  };
  function An(e, t, l, n, c, r, h) {
    return e = e.stateNode, typeof e.shouldComponentUpdate == "function" ? e.shouldComponentUpdate(n, r, h) : t.prototype && t.prototype.isPureReactComponent ? !Kl(l, n) || !Kl(c, r) : !0;
  }
  function Bn(e, t, l, n) {
    e = t.state, typeof t.componentWillReceiveProps == "function" && t.componentWillReceiveProps(l, n), typeof t.UNSAFE_componentWillReceiveProps == "function" && t.UNSAFE_componentWillReceiveProps(l, n), t.state !== e && Ea.enqueueReplaceState(t, t.state, null);
  }
  function Kt(e, t) {
    var l = t;
    if ("ref" in t) {
      l = {};
      for (var n in t)
        n !== "ref" && (l[n] = t[n]);
    }
    if (e = e.defaultProps) {
      l === t && (l = Y({}, l));
      for (var c in e)
        l[c] === void 0 && (l[c] = e[c]);
    }
    return l;
  }
  function bn(e) {
    K2(e);
  }
  function Tn(e) {
    console.log(e);
  }
  function Ln(e) {
    K2(e);
  }
  function E3(e, t) {
    try {
      var l = e.onUncaughtError;
      l(t.value, { componentStack: t.stack });
    } catch (n) {
      setTimeout(function() {
        throw n;
      });
    }
  }
  function Vn(e, t, l) {
    try {
      var n = e.onCaughtError;
      n(l.value, {
        componentStack: l.stack,
        errorBoundary: t.tag === 1 ? t.stateNode : null
      });
    } catch (c) {
      setTimeout(function() {
        throw c;
      });
    }
  }
  function za(e, t, l) {
    return l = dt(l), l.tag = 3, l.payload = { element: null }, l.callback = function() {
      E3(e, t);
    }, l;
  }
  function Cn(e) {
    return e = dt(e), e.tag = 3, e;
  }
  function On(e, t, l, n) {
    var c = l.type.getDerivedStateFromError;
    if (typeof c == "function") {
      var r = n.value;
      e.payload = function() {
        return c(r);
      }, e.callback = function() {
        Vn(t, l, n);
      };
    }
    var h = l.stateNode;
    h !== null && typeof h.componentDidCatch == "function" && (e.callback = function() {
      Vn(t, l, n), typeof c != "function" && (wt === null ? wt = /* @__PURE__ */ new Set([this]) : wt.add(this));
      var v = n.stack;
      this.componentDidCatch(n.value, {
        componentStack: v !== null ? v : ""
      });
    });
  }
  function hf(e, t, l, n, c) {
    if (l.flags |= 32768, n !== null && typeof n == "object" && typeof n.then == "function") {
      if (t = l.alternate, t !== null && dl(
        t,
        l,
        c,
        !0
      ), l = p1.current, l !== null) {
        switch (l.tag) {
          case 31:
          case 13:
            return T1 === null ? A3() : l.alternate === null && Ae === 0 && (Ae = 3), l.flags &= -257, l.flags |= 65536, l.lanes = c, n === a3 ? l.flags |= 16384 : (t = l.updateQueue, t === null ? l.updateQueue = /* @__PURE__ */ new Set([n]) : t.add(n), Ya(e, n, c)), !1;
          case 22:
            return l.flags |= 65536, n === a3 ? l.flags |= 16384 : (t = l.updateQueue, t === null ? (t = {
              transitions: null,
              markerInstances: null,
              retryQueue: /* @__PURE__ */ new Set([n])
            }, l.updateQueue = t) : (l = t.retryQueue, l === null ? t.retryQueue = /* @__PURE__ */ new Set([n]) : l.add(n)), Ya(e, n, c)), !1;
        }
        throw Error(E(435, l.tag));
      }
      return Ya(e, n, c), A3(), !1;
    }
    if (ae)
      return t = p1.current, t !== null ? ((t.flags & 65536) === 0 && (t.flags |= 256), t.flags |= 65536, t.lanes = c, n !== N0 && (e = Error(E(422), { cause: n }), Wl(S1(e, l)))) : (n !== N0 && (t = Error(E(423), {
        cause: n
      }), Wl(
        S1(t, l)
      )), e = e.current.alternate, e.flags |= 65536, c &= -c, e.lanes |= c, n = S1(n, l), c = za(
        e.stateNode,
        n,
        c
      ), K0(e, c), Ae !== 4 && (Ae = 2)), !1;
    var r = Error(E(520), { cause: n });
    if (r = S1(r, l), o2 === null ? o2 = [r] : o2.push(r), Ae !== 4 && (Ae = 2), t === null) return !0;
    n = S1(n, l), l = t;
    do {
      switch (l.tag) {
        case 3:
          return l.flags |= 65536, e = c & -c, l.lanes |= e, e = za(l.stateNode, n, e), K0(l, e), !1;
        case 1:
          if (t = l.type, r = l.stateNode, (l.flags & 128) === 0 && (typeof t.getDerivedStateFromError == "function" || r !== null && typeof r.componentDidCatch == "function" && (wt === null || !wt.has(r))))
            return l.flags |= 65536, c &= -c, l.lanes |= c, c = Cn(c), On(
              c,
              e,
              l,
              n
            ), K0(l, c), !1;
      }
      l = l.return;
    } while (l !== null);
    return !1;
  }
  var pa = Error(E(461)), Ne = !1;
  function ke(e, t, l, n) {
    t.child = e === null ? U5(t, null, l, n) : Xt(
      t,
      e.child,
      l,
      n
    );
  }
  function Nn(e, t, l, n, c) {
    l = l.render;
    var r = t.ref;
    if ("ref" in n) {
      var h = {};
      for (var v in n)
        v !== "ref" && (h[v] = n[v]);
    } else h = n;
    return Gt(t), n = P0(
      e,
      t,
      l,
      h,
      r,
      c
    ), v = ea(), e !== null && !Ne ? (ta(e, t, c), $1(e, t, c)) : (ae && v && C0(t), t.flags |= 1, ke(e, t, n, c), t.child);
  }
  function _n(e, t, l, n, c) {
    if (e === null) {
      var r = l.type;
      return typeof r == "function" && !T0(r) && r.defaultProps === void 0 && l.compare === null ? (t.tag = 15, t.type = r, Un(
        e,
        t,
        r,
        n,
        c
      )) : (e = $2(
        l.type,
        null,
        n,
        t,
        t.mode,
        c
      ), e.ref = t.ref, e.return = t, t.child = e);
    }
    if (r = e.child, !Da(e, c)) {
      var h = r.memoizedProps;
      if (l = l.compare, l = l !== null ? l : Kl, l(h, n) && e.ref === t.ref)
        return $1(e, t, c);
    }
    return t.flags |= 1, e = X1(r, n), e.ref = t.ref, e.return = t, t.child = e;
  }
  function Un(e, t, l, n, c) {
    if (e !== null) {
      var r = e.memoizedProps;
      if (Kl(r, n) && e.ref === t.ref)
        if (Ne = !1, t.pendingProps = n = r, Da(e, c))
          (e.flags & 131072) !== 0 && (Ne = !0);
        else
          return t.lanes = e.lanes, $1(e, t, c);
    }
    return ga(
      e,
      t,
      l,
      n,
      c
    );
  }
  function Rn(e, t, l, n) {
    var c = n.children, r = e !== null ? e.memoizedState : null;
    if (e === null && t.stateNode === null && (t.stateNode = {
      _visibility: 1,
      _pendingMarkers: null,
      _retryCache: null,
      _transitions: null
    }), n.mode === "hidden") {
      if ((t.flags & 128) !== 0) {
        if (r = r !== null ? r.baseLanes | l : l, e !== null) {
          for (n = t.child = e.child, c = 0; n !== null; )
            c = c | n.lanes | n.childLanes, n = n.sibling;
          n = c & ~r;
        } else n = 0, t.child = null;
        return jn(
          e,
          t,
          r,
          l,
          n
        );
      }
      if ((l & 536870912) !== 0)
        t.memoizedState = { baseLanes: 0, cachePool: null }, e !== null && t3(
          t,
          r !== null ? r.cachePool : null
        ), r !== null ? G5(t, r) : k0(), Y5(t);
      else
        return n = t.lanes = 536870912, jn(
          e,
          t,
          r !== null ? r.baseLanes | l : l,
          l,
          n
        );
    } else
      r !== null ? (t3(t, r.cachePool), G5(t, r), zt(), t.memoizedState = null) : (e !== null && t3(t, null), k0(), zt());
    return ke(e, t, c, l), t.child;
  }
  function r2(e, t) {
    return e !== null && e.tag === 22 || t.stateNode !== null || (t.stateNode = {
      _visibility: 1,
      _pendingMarkers: null,
      _retryCache: null,
      _transitions: null
    }), t.sibling;
  }
  function jn(e, t, l, n, c) {
    var r = q0();
    return r = r === null ? null : { parent: Ce._currentValue, pool: r }, t.memoizedState = {
      baseLanes: l,
      cachePool: r
    }, e !== null && t3(t, null), k0(), Y5(t), e !== null && dl(e, t, n, !0), t.childLanes = c, null;
  }
  function z3(e, t) {
    return t = g3(
      { mode: t.mode, children: t.children },
      e.mode
    ), t.ref = e.ref, e.child = t, t.return = e, t;
  }
  function Gn(e, t, l) {
    return Xt(t, e.child, null, l), e = z3(t, t.pendingProps), e.flags |= 2, g1(t), t.memoizedState = null, e;
  }
  function mf(e, t, l) {
    var n = t.pendingProps, c = (t.flags & 128) !== 0;
    if (t.flags &= -129, e === null) {
      if (ae) {
        if (n.mode === "hidden")
          return e = z3(t, n), t.lanes = 536870912, r2(null, e);
        if ($0(t), (e = we) ? (e = Pc(
          e,
          b1
        ), e = e !== null && e.data === "&" ? e : null, e !== null && (t.memoizedState = {
          dehydrated: e,
          treeContext: ft !== null ? { id: _1, overflow: U1 } : null,
          retryLane: 536870912,
          hydrationErrors: null
        }, l = H5(e), l.return = t, t.child = l, Ke = t, we = null)) : e = null, e === null) throw mt(t);
        return t.lanes = 536870912, null;
      }
      return z3(t, n);
    }
    var r = e.memoizedState;
    if (r !== null) {
      var h = r.dehydrated;
      if ($0(t), c)
        if (t.flags & 256)
          t.flags &= -257, t = Gn(
            e,
            t,
            l
          );
        else if (t.memoizedState !== null)
          t.child = e.child, t.flags |= 128, t = null;
        else throw Error(E(558));
      else if (Ne || dl(e, t, l, !1), c = (l & e.childLanes) !== 0, Ne || c) {
        if (n = Me, n !== null && (h = b4(n, l), h !== 0 && h !== r.retryLane))
          throw r.retryLane = h, _t(e, h), r1(n, e, h), pa;
        A3(), t = Gn(
          e,
          t,
          l
        );
      } else
        e = r.treeContext, we = L1(h.nextSibling), Ke = t, ae = !0, ht = null, b1 = !1, e !== null && D5(t, e), t = z3(t, n), t.flags |= 4096;
      return t;
    }
    return e = X1(e.child, {
      mode: n.mode,
      children: n.children
    }), e.ref = t.ref, t.child = e, e.return = t, e;
  }
  function p3(e, t) {
    var l = t.ref;
    if (l === null)
      e !== null && e.ref !== null && (t.flags |= 4194816);
    else {
      if (typeof l != "function" && typeof l != "object")
        throw Error(E(284));
      (e === null || e.ref !== l) && (t.flags |= 4194816);
    }
  }
  function ga(e, t, l, n, c) {
    return Gt(t), l = P0(
      e,
      t,
      l,
      n,
      void 0,
      c
    ), n = ea(), e !== null && !Ne ? (ta(e, t, c), $1(e, t, c)) : (ae && n && C0(t), t.flags |= 1, ke(e, t, l, c), t.child);
  }
  function Yn(e, t, l, n, c, r) {
    return Gt(t), t.updateQueue = null, l = Q5(
      t,
      n,
      l,
      c
    ), q5(e), n = ea(), e !== null && !Ne ? (ta(e, t, r), $1(e, t, r)) : (ae && n && C0(t), t.flags |= 1, ke(e, t, l, r), t.child);
  }
  function qn(e, t, l, n, c) {
    if (Gt(t), t.stateNode === null) {
      var r = hl, h = l.contextType;
      typeof h == "object" && h !== null && (r = Je(h)), r = new l(n, r), t.memoizedState = r.state !== null && r.state !== void 0 ? r.state : null, r.updater = Ea, t.stateNode = r, r._reactInternals = t, r = t.stateNode, r.props = n, r.state = t.memoizedState, r.refs = {}, X0(t), h = l.contextType, r.context = typeof h == "object" && h !== null ? Je(h) : hl, r.state = t.memoizedState, h = l.getDerivedStateFromProps, typeof h == "function" && (oa(
        t,
        l,
        h,
        n
      ), r.state = t.memoizedState), typeof l.getDerivedStateFromProps == "function" || typeof r.getSnapshotBeforeUpdate == "function" || typeof r.UNSAFE_componentWillMount != "function" && typeof r.componentWillMount != "function" || (h = r.state, typeof r.componentWillMount == "function" && r.componentWillMount(), typeof r.UNSAFE_componentWillMount == "function" && r.UNSAFE_componentWillMount(), h !== r.state && Ea.enqueueReplaceState(r, r.state, null), a2(t, n, r, c), l2(), r.state = t.memoizedState), typeof r.componentDidMount == "function" && (t.flags |= 4194308), n = !0;
    } else if (e === null) {
      r = t.stateNode;
      var v = t.memoizedProps, o = Kt(l, v);
      r.props = o;
      var w = r.context, D = l.contextType;
      h = hl, typeof D == "object" && D !== null && (h = Je(D));
      var A = l.getDerivedStateFromProps;
      D = typeof A == "function" || typeof r.getSnapshotBeforeUpdate == "function", v = t.pendingProps !== v, D || typeof r.UNSAFE_componentWillReceiveProps != "function" && typeof r.componentWillReceiveProps != "function" || (v || w !== h) && Bn(
        t,
        r,
        n,
        h
      ), st = !1;
      var x = t.memoizedState;
      r.state = x, a2(t, n, r, c), l2(), w = t.memoizedState, v || x !== w || st ? (typeof A == "function" && (oa(
        t,
        l,
        A,
        n
      ), w = t.memoizedState), (o = st || An(
        t,
        l,
        o,
        n,
        x,
        w,
        h
      )) ? (D || typeof r.UNSAFE_componentWillMount != "function" && typeof r.componentWillMount != "function" || (typeof r.componentWillMount == "function" && r.componentWillMount(), typeof r.UNSAFE_componentWillMount == "function" && r.UNSAFE_componentWillMount()), typeof r.componentDidMount == "function" && (t.flags |= 4194308)) : (typeof r.componentDidMount == "function" && (t.flags |= 4194308), t.memoizedProps = n, t.memoizedState = w), r.props = n, r.state = w, r.context = h, n = o) : (typeof r.componentDidMount == "function" && (t.flags |= 4194308), n = !1);
    } else {
      r = t.stateNode, Z0(e, t), h = t.memoizedProps, D = Kt(l, h), r.props = D, A = t.pendingProps, x = r.context, w = l.contextType, o = hl, typeof w == "object" && w !== null && (o = Je(w)), v = l.getDerivedStateFromProps, (w = typeof v == "function" || typeof r.getSnapshotBeforeUpdate == "function") || typeof r.UNSAFE_componentWillReceiveProps != "function" && typeof r.componentWillReceiveProps != "function" || (h !== A || x !== o) && Bn(
        t,
        r,
        n,
        o
      ), st = !1, x = t.memoizedState, r.state = x, a2(t, n, r, c), l2();
      var y = t.memoizedState;
      h !== A || x !== y || st || e !== null && e.dependencies !== null && P2(e.dependencies) ? (typeof v == "function" && (oa(
        t,
        l,
        v,
        n
      ), y = t.memoizedState), (D = st || An(
        t,
        l,
        D,
        n,
        x,
        y,
        o
      ) || e !== null && e.dependencies !== null && P2(e.dependencies)) ? (w || typeof r.UNSAFE_componentWillUpdate != "function" && typeof r.componentWillUpdate != "function" || (typeof r.componentWillUpdate == "function" && r.componentWillUpdate(n, y, o), typeof r.UNSAFE_componentWillUpdate == "function" && r.UNSAFE_componentWillUpdate(
        n,
        y,
        o
      )), typeof r.componentDidUpdate == "function" && (t.flags |= 4), typeof r.getSnapshotBeforeUpdate == "function" && (t.flags |= 1024)) : (typeof r.componentDidUpdate != "function" || h === e.memoizedProps && x === e.memoizedState || (t.flags |= 4), typeof r.getSnapshotBeforeUpdate != "function" || h === e.memoizedProps && x === e.memoizedState || (t.flags |= 1024), t.memoizedProps = n, t.memoizedState = y), r.props = n, r.state = y, r.context = o, n = D) : (typeof r.componentDidUpdate != "function" || h === e.memoizedProps && x === e.memoizedState || (t.flags |= 4), typeof r.getSnapshotBeforeUpdate != "function" || h === e.memoizedProps && x === e.memoizedState || (t.flags |= 1024), n = !1);
    }
    return r = n, p3(e, t), n = (t.flags & 128) !== 0, r || n ? (r = t.stateNode, l = n && typeof l.getDerivedStateFromError != "function" ? null : r.render(), t.flags |= 1, e !== null && n ? (t.child = Xt(
      t,
      e.child,
      null,
      c
    ), t.child = Xt(
      t,
      null,
      l,
      c
    )) : ke(e, t, l, c), t.memoizedState = r.state, e = t.child) : e = $1(
      e,
      t,
      c
    ), e;
  }
  function Qn(e, t, l, n) {
    return Rt(), t.flags |= 256, ke(e, t, l, n), t.child;
  }
  var Ma = {
    dehydrated: null,
    treeContext: null,
    retryLane: 0,
    hydrationErrors: null
  };
  function wa(e) {
    return { baseLanes: e, cachePool: L5() };
  }
  function xa(e, t, l) {
    return e = e !== null ? e.childLanes & ~l : 0, t && (e |= w1), e;
  }
  function Xn(e, t, l) {
    var n = t.pendingProps, c = !1, r = (t.flags & 128) !== 0, h;
    if ((h = r) || (h = e !== null && e.memoizedState === null ? !1 : (Te.current & 2) !== 0), h && (c = !0, t.flags &= -129), h = (t.flags & 32) !== 0, t.flags &= -33, e === null) {
      if (ae) {
        if (c ? Et(t) : zt(), (e = we) ? (e = Pc(
          e,
          b1
        ), e = e !== null && e.data !== "&" ? e : null, e !== null && (t.memoizedState = {
          dehydrated: e,
          treeContext: ft !== null ? { id: _1, overflow: U1 } : null,
          retryLane: 536870912,
          hydrationErrors: null
        }, l = H5(e), l.return = t, t.child = l, Ke = t, we = null)) : e = null, e === null) throw mt(t);
        return n4(e) ? t.lanes = 32 : t.lanes = 536870912, null;
      }
      var v = n.children;
      return n = n.fallback, c ? (zt(), c = t.mode, v = g3(
        { mode: "hidden", children: v },
        c
      ), n = Ut(
        n,
        c,
        l,
        null
      ), v.return = t, n.return = t, v.sibling = n, t.child = v, n = t.child, n.memoizedState = wa(l), n.childLanes = xa(
        e,
        h,
        l
      ), t.memoizedState = Ma, r2(null, n)) : (Et(t), Ha(t, v));
    }
    var o = e.memoizedState;
    if (o !== null && (v = o.dehydrated, v !== null)) {
      if (r)
        t.flags & 256 ? (Et(t), t.flags &= -257, t = ya(
          e,
          t,
          l
        )) : t.memoizedState !== null ? (zt(), t.child = e.child, t.flags |= 128, t = null) : (zt(), v = n.fallback, c = t.mode, n = g3(
          { mode: "visible", children: n.children },
          c
        ), v = Ut(
          v,
          c,
          l,
          null
        ), v.flags |= 2, n.return = t, v.return = t, n.sibling = v, t.child = n, Xt(
          t,
          e.child,
          null,
          l
        ), n = t.child, n.memoizedState = wa(l), n.childLanes = xa(
          e,
          h,
          l
        ), t.memoizedState = Ma, t = r2(null, n));
      else if (Et(t), n4(v)) {
        if (h = v.nextSibling && v.nextSibling.dataset, h) var w = h.dgst;
        h = w, n = Error(E(419)), n.stack = "", n.digest = h, Wl({ value: n, source: null, stack: null }), t = ya(
          e,
          t,
          l
        );
      } else if (Ne || dl(e, t, l, !1), h = (l & e.childLanes) !== 0, Ne || h) {
        if (h = Me, h !== null && (n = b4(h, l), n !== 0 && n !== o.retryLane))
          throw o.retryLane = n, _t(e, n), r1(h, e, n), pa;
        a4(v) || A3(), t = ya(
          e,
          t,
          l
        );
      } else
        a4(v) ? (t.flags |= 192, t.child = e.child, t = null) : (e = o.treeContext, we = L1(
          v.nextSibling
        ), Ke = t, ae = !0, ht = null, b1 = !1, e !== null && D5(t, e), t = Ha(
          t,
          n.children
        ), t.flags |= 4096);
      return t;
    }
    return c ? (zt(), v = n.fallback, c = t.mode, o = e.child, w = o.sibling, n = X1(o, {
      mode: "hidden",
      children: n.children
    }), n.subtreeFlags = o.subtreeFlags & 65011712, w !== null ? v = X1(
      w,
      v
    ) : (v = Ut(
      v,
      c,
      l,
      null
    ), v.flags |= 2), v.return = t, n.return = t, n.sibling = v, t.child = n, r2(null, n), n = t.child, v = e.child.memoizedState, v === null ? v = wa(l) : (c = v.cachePool, c !== null ? (o = Ce._currentValue, c = c.parent !== o ? { parent: o, pool: o } : c) : c = L5(), v = {
      baseLanes: v.baseLanes | l,
      cachePool: c
    }), n.memoizedState = v, n.childLanes = xa(
      e,
      h,
      l
    ), t.memoizedState = Ma, r2(e.child, n)) : (Et(t), l = e.child, e = l.sibling, l = X1(l, {
      mode: "visible",
      children: n.children
    }), l.return = t, l.sibling = null, e !== null && (h = t.deletions, h === null ? (t.deletions = [e], t.flags |= 16) : h.push(e)), t.child = l, t.memoizedState = null, l);
  }
  function Ha(e, t) {
    return t = g3(
      { mode: "visible", children: t },
      e.mode
    ), t.return = e, e.child = t;
  }
  function g3(e, t) {
    return e = z1(22, e, null, t), e.lanes = 0, e;
  }
  function ya(e, t, l) {
    return Xt(t, e.child, null, l), e = Ha(
      t,
      t.pendingProps.children
    ), e.flags |= 2, t.memoizedState = null, e;
  }
  function Zn(e, t, l) {
    e.lanes |= t;
    var n = e.alternate;
    n !== null && (n.lanes |= t), R0(e.return, t, l);
  }
  function Fa(e, t, l, n, c, r) {
    var h = e.memoizedState;
    h === null ? e.memoizedState = {
      isBackwards: t,
      rendering: null,
      renderingStartTime: 0,
      last: n,
      tail: l,
      tailMode: c,
      treeForkCount: r
    } : (h.isBackwards = t, h.rendering = null, h.renderingStartTime = 0, h.last = n, h.tail = l, h.tailMode = c, h.treeForkCount = r);
  }
  function Kn(e, t, l) {
    var n = t.pendingProps, c = n.revealOrder, r = n.tail;
    n = n.children;
    var h = Te.current, v = (h & 2) !== 0;
    if (v ? (h = h & 1 | 2, t.flags |= 128) : h &= 1, V(Te, h), ke(e, t, n, l), n = ae ? kl : 0, !v && e !== null && (e.flags & 128) !== 0)
      e: for (e = t.child; e !== null; ) {
        if (e.tag === 13)
          e.memoizedState !== null && Zn(e, l, t);
        else if (e.tag === 19)
          Zn(e, l, t);
        else if (e.child !== null) {
          e.child.return = e, e = e.child;
          continue;
        }
        if (e === t) break e;
        for (; e.sibling === null; ) {
          if (e.return === null || e.return === t)
            break e;
          e = e.return;
        }
        e.sibling.return = e.return, e = e.sibling;
      }
    switch (c) {
      case "forwards":
        for (l = t.child, c = null; l !== null; )
          e = l.alternate, e !== null && u3(e) === null && (c = l), l = l.sibling;
        l = c, l === null ? (c = t.child, t.child = null) : (c = l.sibling, l.sibling = null), Fa(
          t,
          !1,
          c,
          l,
          r,
          n
        );
        break;
      case "backwards":
      case "unstable_legacy-backwards":
        for (l = null, c = t.child, t.child = null; c !== null; ) {
          if (e = c.alternate, e !== null && u3(e) === null) {
            t.child = c;
            break;
          }
          e = c.sibling, c.sibling = l, l = c, c = e;
        }
        Fa(
          t,
          !0,
          l,
          null,
          r,
          n
        );
        break;
      case "together":
        Fa(
          t,
          !1,
          null,
          null,
          void 0,
          n
        );
        break;
      default:
        t.memoizedState = null;
    }
    return t.child;
  }
  function $1(e, t, l) {
    if (e !== null && (t.dependencies = e.dependencies), Mt |= t.lanes, (l & t.childLanes) === 0)
      if (e !== null) {
        if (dl(
          e,
          t,
          l,
          !1
        ), (l & t.childLanes) === 0)
          return null;
      } else return null;
    if (e !== null && t.child !== e.child)
      throw Error(E(153));
    if (t.child !== null) {
      for (e = t.child, l = X1(e, e.pendingProps), t.child = l, l.return = t; e.sibling !== null; )
        e = e.sibling, l = l.sibling = X1(e, e.pendingProps), l.return = t;
      l.sibling = null;
    }
    return t.child;
  }
  function Da(e, t) {
    return (e.lanes & t) !== 0 ? !0 : (e = e.dependencies, !!(e !== null && P2(e)));
  }
  function vf(e, t, l) {
    switch (t.tag) {
      case 3:
        ue(t, t.stateNode.containerInfo), vt(t, Ce, e.memoizedState.cache), Rt();
        break;
      case 27:
      case 5:
        Xe(t);
        break;
      case 4:
        ue(t, t.stateNode.containerInfo);
        break;
      case 10:
        vt(
          t,
          t.type,
          t.memoizedProps.value
        );
        break;
      case 31:
        if (t.memoizedState !== null)
          return t.flags |= 128, $0(t), null;
        break;
      case 13:
        var n = t.memoizedState;
        if (n !== null)
          return n.dehydrated !== null ? (Et(t), t.flags |= 128, null) : (l & t.child.childLanes) !== 0 ? Xn(e, t, l) : (Et(t), e = $1(
            e,
            t,
            l
          ), e !== null ? e.sibling : null);
        Et(t);
        break;
      case 19:
        var c = (e.flags & 128) !== 0;
        if (n = (l & t.childLanes) !== 0, n || (dl(
          e,
          t,
          l,
          !1
        ), n = (l & t.childLanes) !== 0), c) {
          if (n)
            return Kn(
              e,
              t,
              l
            );
          t.flags |= 128;
        }
        if (c = t.memoizedState, c !== null && (c.rendering = null, c.tail = null, c.lastEffect = null), V(Te, Te.current), n) break;
        return null;
      case 22:
        return t.lanes = 0, Rn(
          e,
          t,
          l,
          t.pendingProps
        );
      case 24:
        vt(t, Ce, e.memoizedState.cache);
    }
    return $1(e, t, l);
  }
  function Jn(e, t, l) {
    if (e !== null)
      if (e.memoizedProps !== t.pendingProps)
        Ne = !0;
      else {
        if (!Da(e, l) && (t.flags & 128) === 0)
          return Ne = !1, vf(
            e,
            t,
            l
          );
        Ne = (e.flags & 131072) !== 0;
      }
    else
      Ne = !1, ae && (t.flags & 1048576) !== 0 && F5(t, kl, t.index);
    switch (t.lanes = 0, t.tag) {
      case 16:
        e: {
          var n = t.pendingProps;
          if (e = qt(t.elementType), t.type = e, typeof e == "function")
            T0(e) ? (n = Kt(e, n), t.tag = 1, t = qn(
              null,
              t,
              e,
              n,
              l
            )) : (t.tag = 0, t = ga(
              null,
              t,
              e,
              n,
              l
            ));
          else {
            if (e != null) {
              var c = e.$$typeof;
              if (c === $e) {
                t.tag = 11, t = Nn(
                  null,
                  t,
                  e,
                  n,
                  l
                );
                break e;
              } else if (c === ce) {
                t.tag = 14, t = _n(
                  null,
                  t,
                  e,
                  n,
                  l
                );
                break e;
              }
            }
            throw t = v1(e) || e, Error(E(306, t, ""));
          }
        }
        return t;
      case 0:
        return ga(
          e,
          t,
          t.type,
          t.pendingProps,
          l
        );
      case 1:
        return n = t.type, c = Kt(
          n,
          t.pendingProps
        ), qn(
          e,
          t,
          n,
          c,
          l
        );
      case 3:
        e: {
          if (ue(
            t,
            t.stateNode.containerInfo
          ), e === null) throw Error(E(387));
          n = t.pendingProps;
          var r = t.memoizedState;
          c = r.element, Z0(e, t), a2(t, n, null, l);
          var h = t.memoizedState;
          if (n = h.cache, vt(t, Ce, n), n !== r.cache && j0(
            t,
            [Ce],
            l,
            !0
          ), l2(), n = h.element, r.isDehydrated)
            if (r = {
              element: n,
              isDehydrated: !1,
              cache: h.cache
            }, t.updateQueue.baseState = r, t.memoizedState = r, t.flags & 256) {
              t = Qn(
                e,
                t,
                n,
                l
              );
              break e;
            } else if (n !== c) {
              c = S1(
                Error(E(424)),
                t
              ), Wl(c), t = Qn(
                e,
                t,
                n,
                l
              );
              break e;
            } else
              for (e = t.stateNode.containerInfo, e.nodeType === 9 ? e = e.body : e = e.nodeName === "HTML" ? e.ownerDocument.body : e, we = L1(e.firstChild), Ke = t, ae = !0, ht = null, b1 = !0, l = U5(
                t,
                null,
                n,
                l
              ), t.child = l; l; )
                l.flags = l.flags & -3 | 4096, l = l.sibling;
          else {
            if (Rt(), n === c) {
              t = $1(
                e,
                t,
                l
              );
              break e;
            }
            ke(e, t, n, l);
          }
          t = t.child;
        }
        return t;
      case 26:
        return p3(e, t), e === null ? (l = c6(
          t.type,
          null,
          t.pendingProps,
          null
        )) ? t.memoizedState = l : ae || (l = t.type, e = t.pendingProps, n = O3(
          $.current
        ).createElement(l), n[Ze] = t, n[l1] = e, We(n, l, e), Ye(n), t.stateNode = n) : t.memoizedState = c6(
          t.type,
          e.memoizedProps,
          t.pendingProps,
          e.memoizedState
        ), null;
      case 27:
        return Xe(t), e === null && ae && (n = t.stateNode = l6(
          t.type,
          t.pendingProps,
          $.current
        ), Ke = t, b1 = !0, c = we, Ft(t.type) ? (c4 = c, we = L1(n.firstChild)) : we = c), ke(
          e,
          t,
          t.pendingProps.children,
          l
        ), p3(e, t), e === null && (t.flags |= 4194304), t.child;
      case 5:
        return e === null && ae && ((c = n = we) && (n = Yf(
          n,
          t.type,
          t.pendingProps,
          b1
        ), n !== null ? (t.stateNode = n, Ke = t, we = L1(n.firstChild), b1 = !1, c = !0) : c = !1), c || mt(t)), Xe(t), c = t.type, r = t.pendingProps, h = e !== null ? e.memoizedProps : null, n = r.children, e4(c, r) ? n = null : h !== null && e4(c, h) && (t.flags |= 32), t.memoizedState !== null && (c = P0(
          e,
          t,
          lf,
          null,
          null,
          l
        ), H2._currentValue = c), p3(e, t), ke(e, t, n, l), t.child;
      case 6:
        return e === null && ae && ((e = l = we) && (l = qf(
          l,
          t.pendingProps,
          b1
        ), l !== null ? (t.stateNode = l, Ke = t, we = null, e = !0) : e = !1), e || mt(t)), null;
      case 13:
        return Xn(e, t, l);
      case 4:
        return ue(
          t,
          t.stateNode.containerInfo
        ), n = t.pendingProps, e === null ? t.child = Xt(
          t,
          null,
          n,
          l
        ) : ke(e, t, n, l), t.child;
      case 11:
        return Nn(
          e,
          t,
          t.type,
          t.pendingProps,
          l
        );
      case 7:
        return ke(
          e,
          t,
          t.pendingProps,
          l
        ), t.child;
      case 8:
        return ke(
          e,
          t,
          t.pendingProps.children,
          l
        ), t.child;
      case 12:
        return ke(
          e,
          t,
          t.pendingProps.children,
          l
        ), t.child;
      case 10:
        return n = t.pendingProps, vt(t, t.type, n.value), ke(e, t, n.children, l), t.child;
      case 9:
        return c = t.type._context, n = t.pendingProps.children, Gt(t), c = Je(c), n = n(c), t.flags |= 1, ke(e, t, n, l), t.child;
      case 14:
        return _n(
          e,
          t,
          t.type,
          t.pendingProps,
          l
        );
      case 15:
        return Un(
          e,
          t,
          t.type,
          t.pendingProps,
          l
        );
      case 19:
        return Kn(e, t, l);
      case 31:
        return mf(e, t, l);
      case 22:
        return Rn(
          e,
          t,
          l,
          t.pendingProps
        );
      case 24:
        return Gt(t), n = Je(Ce), e === null ? (c = q0(), c === null && (c = Me, r = G0(), c.pooledCache = r, r.refCount++, r !== null && (c.pooledCacheLanes |= l), c = r), t.memoizedState = { parent: n, cache: c }, X0(t), vt(t, Ce, c)) : ((e.lanes & l) !== 0 && (Z0(e, t), a2(t, null, null, l), l2()), c = e.memoizedState, r = t.memoizedState, c.parent !== n ? (c = { parent: n, cache: n }, t.memoizedState = c, t.lanes === 0 && (t.memoizedState = t.updateQueue.baseState = c), vt(t, Ce, n)) : (n = r.cache, vt(t, Ce, n), n !== c.cache && j0(
          t,
          [Ce],
          l,
          !0
        ))), ke(
          e,
          t,
          t.pendingProps.children,
          l
        ), t.child;
      case 29:
        throw t.pendingProps;
    }
    throw Error(E(156, t.tag));
  }
  function I1(e) {
    e.flags |= 4;
  }
  function Sa(e, t, l, n, c) {
    if ((t = (e.mode & 32) !== 0) && (t = !1), t) {
      if (e.flags |= 16777216, (c & 335544128) === c)
        if (e.stateNode.complete) e.flags |= 8192;
        else if (wc()) e.flags |= 8192;
        else
          throw Qt = a3, Q0;
    } else e.flags &= -16777217;
  }
  function kn(e, t) {
    if (t.type !== "stylesheet" || (t.state.loading & 4) !== 0)
      e.flags &= -16777217;
    else if (e.flags |= 16777216, !h6(t))
      if (wc()) e.flags |= 8192;
      else
        throw Qt = a3, Q0;
  }
  function M3(e, t) {
    t !== null && (e.flags |= 4), e.flags & 16384 && (t = e.tag !== 22 ? S4() : 536870912, e.lanes |= t, Dl |= t);
  }
  function f2(e, t) {
    if (!ae)
      switch (e.tailMode) {
        case "hidden":
          t = e.tail;
          for (var l = null; t !== null; )
            t.alternate !== null && (l = t), t = t.sibling;
          l === null ? e.tail = null : l.sibling = null;
          break;
        case "collapsed":
          l = e.tail;
          for (var n = null; l !== null; )
            l.alternate !== null && (n = l), l = l.sibling;
          n === null ? t || e.tail === null ? e.tail = null : e.tail.sibling = null : n.sibling = null;
      }
  }
  function xe(e) {
    var t = e.alternate !== null && e.alternate.child === e.child, l = 0, n = 0;
    if (t)
      for (var c = e.child; c !== null; )
        l |= c.lanes | c.childLanes, n |= c.subtreeFlags & 65011712, n |= c.flags & 65011712, c.return = e, c = c.sibling;
    else
      for (c = e.child; c !== null; )
        l |= c.lanes | c.childLanes, n |= c.subtreeFlags, n |= c.flags, c.return = e, c = c.sibling;
    return e.subtreeFlags |= n, e.childLanes = l, t;
  }
  function sf(e, t, l) {
    var n = t.pendingProps;
    switch (O0(t), t.tag) {
      case 16:
      case 15:
      case 0:
      case 11:
      case 7:
      case 8:
      case 12:
      case 9:
      case 14:
        return xe(t), null;
      case 1:
        return xe(t), null;
      case 3:
        return l = t.stateNode, n = null, e !== null && (n = e.memoizedState.cache), t.memoizedState.cache !== n && (t.flags |= 2048), J1(Ce), he(), l.pendingContext && (l.context = l.pendingContext, l.pendingContext = null), (e === null || e.child === null) && (sl(t) ? I1(t) : e === null || e.memoizedState.isDehydrated && (t.flags & 256) === 0 || (t.flags |= 1024, _0())), xe(t), null;
      case 26:
        var c = t.type, r = t.memoizedState;
        return e === null ? (I1(t), r !== null ? (xe(t), kn(t, r)) : (xe(t), Sa(
          t,
          c,
          null,
          n,
          l
        ))) : r ? r !== e.memoizedState ? (I1(t), xe(t), kn(t, r)) : (xe(t), t.flags &= -16777217) : (e = e.memoizedProps, e !== n && I1(t), xe(t), Sa(
          t,
          c,
          e,
          n,
          l
        )), null;
      case 27:
        if (H1(t), l = $.current, c = t.type, e !== null && t.stateNode != null)
          e.memoizedProps !== n && I1(t);
        else {
          if (!n) {
            if (t.stateNode === null)
              throw Error(E(166));
            return xe(t), null;
          }
          e = _.current, sl(t) ? S5(t) : (e = l6(c, n, l), t.stateNode = e, I1(t));
        }
        return xe(t), null;
      case 5:
        if (H1(t), c = t.type, e !== null && t.stateNode != null)
          e.memoizedProps !== n && I1(t);
        else {
          if (!n) {
            if (t.stateNode === null)
              throw Error(E(166));
            return xe(t), null;
          }
          if (r = _.current, sl(t))
            S5(t);
          else {
            var h = O3(
              $.current
            );
            switch (r) {
              case 1:
                r = h.createElementNS(
                  "http://www.w3.org/2000/svg",
                  c
                );
                break;
              case 2:
                r = h.createElementNS(
                  "http://www.w3.org/1998/Math/MathML",
                  c
                );
                break;
              default:
                switch (c) {
                  case "svg":
                    r = h.createElementNS(
                      "http://www.w3.org/2000/svg",
                      c
                    );
                    break;
                  case "math":
                    r = h.createElementNS(
                      "http://www.w3.org/1998/Math/MathML",
                      c
                    );
                    break;
                  case "script":
                    r = h.createElement("div"), r.innerHTML = "<script><\/script>", r = r.removeChild(
                      r.firstChild
                    );
                    break;
                  case "select":
                    r = typeof n.is == "string" ? h.createElement("select", {
                      is: n.is
                    }) : h.createElement("select"), n.multiple ? r.multiple = !0 : n.size && (r.size = n.size);
                    break;
                  default:
                    r = typeof n.is == "string" ? h.createElement(c, { is: n.is }) : h.createElement(c);
                }
            }
            r[Ze] = t, r[l1] = n;
            e: for (h = t.child; h !== null; ) {
              if (h.tag === 5 || h.tag === 6)
                r.appendChild(h.stateNode);
              else if (h.tag !== 4 && h.tag !== 27 && h.child !== null) {
                h.child.return = h, h = h.child;
                continue;
              }
              if (h === t) break e;
              for (; h.sibling === null; ) {
                if (h.return === null || h.return === t)
                  break e;
                h = h.return;
              }
              h.sibling.return = h.return, h = h.sibling;
            }
            t.stateNode = r;
            e: switch (We(r, c, n), c) {
              case "button":
              case "input":
              case "select":
              case "textarea":
                n = !!n.autoFocus;
                break e;
              case "img":
                n = !0;
                break e;
              default:
                n = !1;
            }
            n && I1(t);
          }
        }
        return xe(t), Sa(
          t,
          t.type,
          e === null ? null : e.memoizedProps,
          t.pendingProps,
          l
        ), null;
      case 6:
        if (e && t.stateNode != null)
          e.memoizedProps !== n && I1(t);
        else {
          if (typeof n != "string" && t.stateNode === null)
            throw Error(E(166));
          if (e = $.current, sl(t)) {
            if (e = t.stateNode, l = t.memoizedProps, n = null, c = Ke, c !== null)
              switch (c.tag) {
                case 27:
                case 5:
                  n = c.memoizedProps;
              }
            e[Ze] = t, e = !!(e.nodeValue === l || n !== null && n.suppressHydrationWarning === !0 || Xc(e.nodeValue, l)), e || mt(t, !0);
          } else
            e = O3(e).createTextNode(
              n
            ), e[Ze] = t, t.stateNode = e;
        }
        return xe(t), null;
      case 31:
        if (l = t.memoizedState, e === null || e.memoizedState !== null) {
          if (n = sl(t), l !== null) {
            if (e === null) {
              if (!n) throw Error(E(318));
              if (e = t.memoizedState, e = e !== null ? e.dehydrated : null, !e) throw Error(E(557));
              e[Ze] = t;
            } else
              Rt(), (t.flags & 128) === 0 && (t.memoizedState = null), t.flags |= 4;
            xe(t), e = !1;
          } else
            l = _0(), e !== null && e.memoizedState !== null && (e.memoizedState.hydrationErrors = l), e = !0;
          if (!e)
            return t.flags & 256 ? (g1(t), t) : (g1(t), null);
          if ((t.flags & 128) !== 0)
            throw Error(E(558));
        }
        return xe(t), null;
      case 13:
        if (n = t.memoizedState, e === null || e.memoizedState !== null && e.memoizedState.dehydrated !== null) {
          if (c = sl(t), n !== null && n.dehydrated !== null) {
            if (e === null) {
              if (!c) throw Error(E(318));
              if (c = t.memoizedState, c = c !== null ? c.dehydrated : null, !c) throw Error(E(317));
              c[Ze] = t;
            } else
              Rt(), (t.flags & 128) === 0 && (t.memoizedState = null), t.flags |= 4;
            xe(t), c = !1;
          } else
            c = _0(), e !== null && e.memoizedState !== null && (e.memoizedState.hydrationErrors = c), c = !0;
          if (!c)
            return t.flags & 256 ? (g1(t), t) : (g1(t), null);
        }
        return g1(t), (t.flags & 128) !== 0 ? (t.lanes = l, t) : (l = n !== null, e = e !== null && e.memoizedState !== null, l && (n = t.child, c = null, n.alternate !== null && n.alternate.memoizedState !== null && n.alternate.memoizedState.cachePool !== null && (c = n.alternate.memoizedState.cachePool.pool), r = null, n.memoizedState !== null && n.memoizedState.cachePool !== null && (r = n.memoizedState.cachePool.pool), r !== c && (n.flags |= 2048)), l !== e && l && (t.child.flags |= 8192), M3(t, t.updateQueue), xe(t), null);
      case 4:
        return he(), e === null && ka(t.stateNode.containerInfo), xe(t), null;
      case 10:
        return J1(t.type), xe(t), null;
      case 19:
        if (B(Te), n = t.memoizedState, n === null) return xe(t), null;
        if (c = (t.flags & 128) !== 0, r = n.rendering, r === null)
          if (c) f2(n, !1);
          else {
            if (Ae !== 0 || e !== null && (e.flags & 128) !== 0)
              for (e = t.child; e !== null; ) {
                if (r = u3(e), r !== null) {
                  for (t.flags |= 128, f2(n, !1), e = r.updateQueue, t.updateQueue = e, M3(t, e), t.subtreeFlags = 0, e = l, l = t.child; l !== null; )
                    x5(l, e), l = l.sibling;
                  return V(
                    Te,
                    Te.current & 1 | 2
                  ), ae && Z1(t, n.treeForkCount), t.child;
                }
                e = e.sibling;
              }
            n.tail !== null && s1() > F3 && (t.flags |= 128, c = !0, f2(n, !1), t.lanes = 4194304);
          }
        else {
          if (!c)
            if (e = u3(r), e !== null) {
              if (t.flags |= 128, c = !0, e = e.updateQueue, t.updateQueue = e, M3(t, e), f2(n, !0), n.tail === null && n.tailMode === "hidden" && !r.alternate && !ae)
                return xe(t), null;
            } else
              2 * s1() - n.renderingStartTime > F3 && l !== 536870912 && (t.flags |= 128, c = !0, f2(n, !1), t.lanes = 4194304);
          n.isBackwards ? (r.sibling = t.child, t.child = r) : (e = n.last, e !== null ? e.sibling = r : t.child = r, n.last = r);
        }
        return n.tail !== null ? (e = n.tail, n.rendering = e, n.tail = e.sibling, n.renderingStartTime = s1(), e.sibling = null, l = Te.current, V(
          Te,
          c ? l & 1 | 2 : l & 1
        ), ae && Z1(t, n.treeForkCount), e) : (xe(t), null);
      case 22:
      case 23:
        return g1(t), W0(), n = t.memoizedState !== null, e !== null ? e.memoizedState !== null !== n && (t.flags |= 8192) : n && (t.flags |= 8192), n ? (l & 536870912) !== 0 && (t.flags & 128) === 0 && (xe(t), t.subtreeFlags & 6 && (t.flags |= 8192)) : xe(t), l = t.updateQueue, l !== null && M3(t, l.retryQueue), l = null, e !== null && e.memoizedState !== null && e.memoizedState.cachePool !== null && (l = e.memoizedState.cachePool.pool), n = null, t.memoizedState !== null && t.memoizedState.cachePool !== null && (n = t.memoizedState.cachePool.pool), n !== l && (t.flags |= 2048), e !== null && B(Yt), null;
      case 24:
        return l = null, e !== null && (l = e.memoizedState.cache), t.memoizedState.cache !== l && (t.flags |= 2048), J1(Ce), xe(t), null;
      case 25:
        return null;
      case 30:
        return null;
    }
    throw Error(E(156, t.tag));
  }
  function df(e, t) {
    switch (O0(t), t.tag) {
      case 1:
        return e = t.flags, e & 65536 ? (t.flags = e & -65537 | 128, t) : null;
      case 3:
        return J1(Ce), he(), e = t.flags, (e & 65536) !== 0 && (e & 128) === 0 ? (t.flags = e & -65537 | 128, t) : null;
      case 26:
      case 27:
      case 5:
        return H1(t), null;
      case 31:
        if (t.memoizedState !== null) {
          if (g1(t), t.alternate === null)
            throw Error(E(340));
          Rt();
        }
        return e = t.flags, e & 65536 ? (t.flags = e & -65537 | 128, t) : null;
      case 13:
        if (g1(t), e = t.memoizedState, e !== null && e.dehydrated !== null) {
          if (t.alternate === null)
            throw Error(E(340));
          Rt();
        }
        return e = t.flags, e & 65536 ? (t.flags = e & -65537 | 128, t) : null;
      case 19:
        return B(Te), null;
      case 4:
        return he(), null;
      case 10:
        return J1(t.type), null;
      case 22:
      case 23:
        return g1(t), W0(), e !== null && B(Yt), e = t.flags, e & 65536 ? (t.flags = e & -65537 | 128, t) : null;
      case 24:
        return J1(Ce), null;
      case 25:
        return null;
      default:
        return null;
    }
  }
  function Wn(e, t) {
    switch (O0(t), t.tag) {
      case 3:
        J1(Ce), he();
        break;
      case 26:
      case 27:
      case 5:
        H1(t);
        break;
      case 4:
        he();
        break;
      case 31:
        t.memoizedState !== null && g1(t);
        break;
      case 13:
        g1(t);
        break;
      case 19:
        B(Te);
        break;
      case 10:
        J1(t.type);
        break;
      case 22:
      case 23:
        g1(t), W0(), e !== null && B(Yt);
        break;
      case 24:
        J1(Ce);
    }
  }
  function h2(e, t) {
    try {
      var l = t.updateQueue, n = l !== null ? l.lastEffect : null;
      if (n !== null) {
        var c = n.next;
        l = c;
        do {
          if ((l.tag & e) === e) {
            n = void 0;
            var r = l.create, h = l.inst;
            n = r(), h.destroy = n;
          }
          l = l.next;
        } while (l !== c);
      }
    } catch (v) {
      de(t, t.return, v);
    }
  }
  function pt(e, t, l) {
    try {
      var n = t.updateQueue, c = n !== null ? n.lastEffect : null;
      if (c !== null) {
        var r = c.next;
        n = r;
        do {
          if ((n.tag & e) === e) {
            var h = n.inst, v = h.destroy;
            if (v !== void 0) {
              h.destroy = void 0, c = t;
              var o = l, w = v;
              try {
                w();
              } catch (D) {
                de(
                  c,
                  o,
                  D
                );
              }
            }
          }
          n = n.next;
        } while (n !== r);
      }
    } catch (D) {
      de(t, t.return, D);
    }
  }
  function $n(e) {
    var t = e.updateQueue;
    if (t !== null) {
      var l = e.stateNode;
      try {
        j5(t, l);
      } catch (n) {
        de(e, e.return, n);
      }
    }
  }
  function In(e, t, l) {
    l.props = Kt(
      e.type,
      e.memoizedProps
    ), l.state = e.memoizedState;
    try {
      l.componentWillUnmount();
    } catch (n) {
      de(e, t, n);
    }
  }
  function m2(e, t) {
    try {
      var l = e.ref;
      if (l !== null) {
        switch (e.tag) {
          case 26:
          case 27:
          case 5:
            var n = e.stateNode;
            break;
          case 30:
            n = e.stateNode;
            break;
          default:
            n = e.stateNode;
        }
        typeof l == "function" ? e.refCleanup = l(n) : l.current = n;
      }
    } catch (c) {
      de(e, t, c);
    }
  }
  function R1(e, t) {
    var l = e.ref, n = e.refCleanup;
    if (l !== null)
      if (typeof n == "function")
        try {
          n();
        } catch (c) {
          de(e, t, c);
        } finally {
          e.refCleanup = null, e = e.alternate, e != null && (e.refCleanup = null);
        }
      else if (typeof l == "function")
        try {
          l(null);
        } catch (c) {
          de(e, t, c);
        }
      else l.current = null;
  }
  function Pn(e) {
    var t = e.type, l = e.memoizedProps, n = e.stateNode;
    try {
      e: switch (t) {
        case "button":
        case "input":
        case "select":
        case "textarea":
          l.autoFocus && n.focus();
          break e;
        case "img":
          l.src ? n.src = l.src : l.srcSet && (n.srcset = l.srcSet);
      }
    } catch (c) {
      de(e, e.return, c);
    }
  }
  function Aa(e, t, l) {
    try {
      var n = e.stateNode;
      Nf(n, e.type, l, t), n[l1] = t;
    } catch (c) {
      de(e, e.return, c);
    }
  }
  function ec(e) {
    return e.tag === 5 || e.tag === 3 || e.tag === 26 || e.tag === 27 && Ft(e.type) || e.tag === 4;
  }
  function Ba(e) {
    e: for (; ; ) {
      for (; e.sibling === null; ) {
        if (e.return === null || ec(e.return)) return null;
        e = e.return;
      }
      for (e.sibling.return = e.return, e = e.sibling; e.tag !== 5 && e.tag !== 6 && e.tag !== 18; ) {
        if (e.tag === 27 && Ft(e.type) || e.flags & 2 || e.child === null || e.tag === 4) continue e;
        e.child.return = e, e = e.child;
      }
      if (!(e.flags & 2)) return e.stateNode;
    }
  }
  function ba(e, t, l) {
    var n = e.tag;
    if (n === 5 || n === 6)
      e = e.stateNode, t ? (l.nodeType === 9 ? l.body : l.nodeName === "HTML" ? l.ownerDocument.body : l).insertBefore(e, t) : (t = l.nodeType === 9 ? l.body : l.nodeName === "HTML" ? l.ownerDocument.body : l, t.appendChild(e), l = l._reactRootContainer, l != null || t.onclick !== null || (t.onclick = q1));
    else if (n !== 4 && (n === 27 && Ft(e.type) && (l = e.stateNode, t = null), e = e.child, e !== null))
      for (ba(e, t, l), e = e.sibling; e !== null; )
        ba(e, t, l), e = e.sibling;
  }
  function w3(e, t, l) {
    var n = e.tag;
    if (n === 5 || n === 6)
      e = e.stateNode, t ? l.insertBefore(e, t) : l.appendChild(e);
    else if (n !== 4 && (n === 27 && Ft(e.type) && (l = e.stateNode), e = e.child, e !== null))
      for (w3(e, t, l), e = e.sibling; e !== null; )
        w3(e, t, l), e = e.sibling;
  }
  function tc(e) {
    var t = e.stateNode, l = e.memoizedProps;
    try {
      for (var n = e.type, c = t.attributes; c.length; )
        t.removeAttributeNode(c[0]);
      We(t, n, l), t[Ze] = e, t[l1] = l;
    } catch (r) {
      de(e, e.return, r);
    }
  }
  var P1 = !1, _e = !1, Ta = !1, lc = typeof WeakSet == "function" ? WeakSet : Set, qe = null;
  function of(e, t) {
    if (e = e.containerInfo, Ia = Y3, e = s5(e), y0(e)) {
      if ("selectionStart" in e)
        var l = {
          start: e.selectionStart,
          end: e.selectionEnd
        };
      else
        e: {
          l = (l = e.ownerDocument) && l.defaultView || window;
          var n = l.getSelection && l.getSelection();
          if (n && n.rangeCount !== 0) {
            l = n.anchorNode;
            var c = n.anchorOffset, r = n.focusNode;
            n = n.focusOffset;
            try {
              l.nodeType, r.nodeType;
            } catch {
              l = null;
              break e;
            }
            var h = 0, v = -1, o = -1, w = 0, D = 0, A = e, x = null;
            t: for (; ; ) {
              for (var y; A !== l || c !== 0 && A.nodeType !== 3 || (v = h + c), A !== r || n !== 0 && A.nodeType !== 3 || (o = h + n), A.nodeType === 3 && (h += A.nodeValue.length), (y = A.firstChild) !== null; )
                x = A, A = y;
              for (; ; ) {
                if (A === e) break t;
                if (x === l && ++w === c && (v = h), x === r && ++D === n && (o = h), (y = A.nextSibling) !== null) break;
                A = x, x = A.parentNode;
              }
              A = y;
            }
            l = v === -1 || o === -1 ? null : { start: v, end: o };
          } else l = null;
        }
      l = l || { start: 0, end: 0 };
    } else l = null;
    for (Pa = { focusedElem: e, selectionRange: l }, Y3 = !1, qe = t; qe !== null; )
      if (t = qe, e = t.child, (t.subtreeFlags & 1028) !== 0 && e !== null)
        e.return = t, qe = e;
      else
        for (; qe !== null; ) {
          switch (t = qe, r = t.alternate, e = t.flags, t.tag) {
            case 0:
              if ((e & 4) !== 0 && (e = t.updateQueue, e = e !== null ? e.events : null, e !== null))
                for (l = 0; l < e.length; l++)
                  c = e[l], c.ref.impl = c.nextImpl;
              break;
            case 11:
            case 15:
              break;
            case 1:
              if ((e & 1024) !== 0 && r !== null) {
                e = void 0, l = t, c = r.memoizedProps, r = r.memoizedState, n = l.stateNode;
                try {
                  var N = Kt(
                    l.type,
                    c
                  );
                  e = n.getSnapshotBeforeUpdate(
                    N,
                    r
                  ), n.__reactInternalSnapshotBeforeUpdate = e;
                } catch (q) {
                  de(
                    l,
                    l.return,
                    q
                  );
                }
              }
              break;
            case 3:
              if ((e & 1024) !== 0) {
                if (e = t.stateNode.containerInfo, l = e.nodeType, l === 9)
                  l4(e);
                else if (l === 1)
                  switch (e.nodeName) {
                    case "HEAD":
                    case "HTML":
                    case "BODY":
                      l4(e);
                      break;
                    default:
                      e.textContent = "";
                  }
              }
              break;
            case 5:
            case 26:
            case 27:
            case 6:
            case 4:
            case 17:
              break;
            default:
              if ((e & 1024) !== 0) throw Error(E(163));
          }
          if (e = t.sibling, e !== null) {
            e.return = t.return, qe = e;
            break;
          }
          qe = t.return;
        }
  }
  function ac(e, t, l) {
    var n = l.flags;
    switch (l.tag) {
      case 0:
      case 11:
      case 15:
        tt(e, l), n & 4 && h2(5, l);
        break;
      case 1:
        if (tt(e, l), n & 4)
          if (e = l.stateNode, t === null)
            try {
              e.componentDidMount();
            } catch (h) {
              de(l, l.return, h);
            }
          else {
            var c = Kt(
              l.type,
              t.memoizedProps
            );
            t = t.memoizedState;
            try {
              e.componentDidUpdate(
                c,
                t,
                e.__reactInternalSnapshotBeforeUpdate
              );
            } catch (h) {
              de(
                l,
                l.return,
                h
              );
            }
          }
        n & 64 && $n(l), n & 512 && m2(l, l.return);
        break;
      case 3:
        if (tt(e, l), n & 64 && (e = l.updateQueue, e !== null)) {
          if (t = null, l.child !== null)
            switch (l.child.tag) {
              case 27:
              case 5:
                t = l.child.stateNode;
                break;
              case 1:
                t = l.child.stateNode;
            }
          try {
            j5(e, t);
          } catch (h) {
            de(l, l.return, h);
          }
        }
        break;
      case 27:
        t === null && n & 4 && tc(l);
      case 26:
      case 5:
        tt(e, l), t === null && n & 4 && Pn(l), n & 512 && m2(l, l.return);
        break;
      case 12:
        tt(e, l);
        break;
      case 31:
        tt(e, l), n & 4 && ic(e, l);
        break;
      case 13:
        tt(e, l), n & 4 && uc(e, l), n & 64 && (e = l.memoizedState, e !== null && (e = e.dehydrated, e !== null && (l = yf.bind(
          null,
          l
        ), Qf(e, l))));
        break;
      case 22:
        if (n = l.memoizedState !== null || P1, !n) {
          t = t !== null && t.memoizedState !== null || _e, c = P1;
          var r = _e;
          P1 = n, (_e = t) && !r ? lt(
            e,
            l,
            (l.subtreeFlags & 8772) !== 0
          ) : tt(e, l), P1 = c, _e = r;
        }
        break;
      case 30:
        break;
      default:
        tt(e, l);
    }
  }
  function nc(e) {
    var t = e.alternate;
    t !== null && (e.alternate = null, nc(t)), e.child = null, e.deletions = null, e.sibling = null, e.tag === 5 && (t = e.stateNode, t !== null && u0(t)), e.stateNode = null, e.return = null, e.dependencies = null, e.memoizedProps = null, e.memoizedState = null, e.pendingProps = null, e.stateNode = null, e.updateQueue = null;
  }
  var De = null, n1 = !1;
  function et(e, t, l) {
    for (l = l.child; l !== null; )
      cc(e, t, l), l = l.sibling;
  }
  function cc(e, t, l) {
    if (d1 && typeof d1.onCommitFiberUnmount == "function")
      try {
        d1.onCommitFiberUnmount(Ol, l);
      } catch {
      }
    switch (l.tag) {
      case 26:
        _e || R1(l, t), et(
          e,
          t,
          l
        ), l.memoizedState ? l.memoizedState.count-- : l.stateNode && (l = l.stateNode, l.parentNode.removeChild(l));
        break;
      case 27:
        _e || R1(l, t);
        var n = De, c = n1;
        Ft(l.type) && (De = l.stateNode, n1 = !1), et(
          e,
          t,
          l
        ), M2(l.stateNode), De = n, n1 = c;
        break;
      case 5:
        _e || R1(l, t);
      case 6:
        if (n = De, c = n1, De = null, et(
          e,
          t,
          l
        ), De = n, n1 = c, De !== null)
          if (n1)
            try {
              (De.nodeType === 9 ? De.body : De.nodeName === "HTML" ? De.ownerDocument.body : De).removeChild(l.stateNode);
            } catch (r) {
              de(
                l,
                t,
                r
              );
            }
          else
            try {
              De.removeChild(l.stateNode);
            } catch (r) {
              de(
                l,
                t,
                r
              );
            }
        break;
      case 18:
        De !== null && (n1 ? (e = De, $c(
          e.nodeType === 9 ? e.body : e.nodeName === "HTML" ? e.ownerDocument.body : e,
          l.stateNode
        ), Cl(e)) : $c(De, l.stateNode));
        break;
      case 4:
        n = De, c = n1, De = l.stateNode.containerInfo, n1 = !0, et(
          e,
          t,
          l
        ), De = n, n1 = c;
        break;
      case 0:
      case 11:
      case 14:
      case 15:
        pt(2, l, t), _e || pt(4, l, t), et(
          e,
          t,
          l
        );
        break;
      case 1:
        _e || (R1(l, t), n = l.stateNode, typeof n.componentWillUnmount == "function" && In(
          l,
          t,
          n
        )), et(
          e,
          t,
          l
        );
        break;
      case 21:
        et(
          e,
          t,
          l
        );
        break;
      case 22:
        _e = (n = _e) || l.memoizedState !== null, et(
          e,
          t,
          l
        ), _e = n;
        break;
      default:
        et(
          e,
          t,
          l
        );
    }
  }
  function ic(e, t) {
    if (t.memoizedState === null && (e = t.alternate, e !== null && (e = e.memoizedState, e !== null))) {
      e = e.dehydrated;
      try {
        Cl(e);
      } catch (l) {
        de(t, t.return, l);
      }
    }
  }
  function uc(e, t) {
    if (t.memoizedState === null && (e = t.alternate, e !== null && (e = e.memoizedState, e !== null && (e = e.dehydrated, e !== null))))
      try {
        Cl(e);
      } catch (l) {
        de(t, t.return, l);
      }
  }
  function Ef(e) {
    switch (e.tag) {
      case 31:
      case 13:
      case 19:
        var t = e.stateNode;
        return t === null && (t = e.stateNode = new lc()), t;
      case 22:
        return e = e.stateNode, t = e._retryCache, t === null && (t = e._retryCache = new lc()), t;
      default:
        throw Error(E(435, e.tag));
    }
  }
  function x3(e, t) {
    var l = Ef(e);
    t.forEach(function(n) {
      if (!l.has(n)) {
        l.add(n);
        var c = Ff.bind(null, e, n);
        n.then(c, c);
      }
    });
  }
  function c1(e, t) {
    var l = t.deletions;
    if (l !== null)
      for (var n = 0; n < l.length; n++) {
        var c = l[n], r = e, h = t, v = h;
        e: for (; v !== null; ) {
          switch (v.tag) {
            case 27:
              if (Ft(v.type)) {
                De = v.stateNode, n1 = !1;
                break e;
              }
              break;
            case 5:
              De = v.stateNode, n1 = !1;
              break e;
            case 3:
            case 4:
              De = v.stateNode.containerInfo, n1 = !0;
              break e;
          }
          v = v.return;
        }
        if (De === null) throw Error(E(160));
        cc(r, h, c), De = null, n1 = !1, r = c.alternate, r !== null && (r.return = null), c.return = null;
      }
    if (t.subtreeFlags & 13886)
      for (t = t.child; t !== null; )
        rc(t, e), t = t.sibling;
  }
  var O1 = null;
  function rc(e, t) {
    var l = e.alternate, n = e.flags;
    switch (e.tag) {
      case 0:
      case 11:
      case 14:
      case 15:
        c1(t, e), i1(e), n & 4 && (pt(3, e, e.return), h2(3, e), pt(5, e, e.return));
        break;
      case 1:
        c1(t, e), i1(e), n & 512 && (_e || l === null || R1(l, l.return)), n & 64 && P1 && (e = e.updateQueue, e !== null && (n = e.callbacks, n !== null && (l = e.shared.hiddenCallbacks, e.shared.hiddenCallbacks = l === null ? n : l.concat(n))));
        break;
      case 26:
        var c = O1;
        if (c1(t, e), i1(e), n & 512 && (_e || l === null || R1(l, l.return)), n & 4) {
          var r = l !== null ? l.memoizedState : null;
          if (n = e.memoizedState, l === null)
            if (n === null)
              if (e.stateNode === null) {
                e: {
                  n = e.type, l = e.memoizedProps, c = c.ownerDocument || c;
                  t: switch (n) {
                    case "title":
                      r = c.getElementsByTagName("title")[0], (!r || r[Ul] || r[Ze] || r.namespaceURI === "http://www.w3.org/2000/svg" || r.hasAttribute("itemprop")) && (r = c.createElement(n), c.head.insertBefore(
                        r,
                        c.querySelector("head > title")
                      )), We(r, n, l), r[Ze] = e, Ye(r), n = r;
                      break e;
                    case "link":
                      var h = r6(
                        "link",
                        "href",
                        c
                      ).get(n + (l.href || ""));
                      if (h) {
                        for (var v = 0; v < h.length; v++)
                          if (r = h[v], r.getAttribute("href") === (l.href == null || l.href === "" ? null : l.href) && r.getAttribute("rel") === (l.rel == null ? null : l.rel) && r.getAttribute("title") === (l.title == null ? null : l.title) && r.getAttribute("crossorigin") === (l.crossOrigin == null ? null : l.crossOrigin)) {
                            h.splice(v, 1);
                            break t;
                          }
                      }
                      r = c.createElement(n), We(r, n, l), c.head.appendChild(r);
                      break;
                    case "meta":
                      if (h = r6(
                        "meta",
                        "content",
                        c
                      ).get(n + (l.content || ""))) {
                        for (v = 0; v < h.length; v++)
                          if (r = h[v], r.getAttribute("content") === (l.content == null ? null : "" + l.content) && r.getAttribute("name") === (l.name == null ? null : l.name) && r.getAttribute("property") === (l.property == null ? null : l.property) && r.getAttribute("http-equiv") === (l.httpEquiv == null ? null : l.httpEquiv) && r.getAttribute("charset") === (l.charSet == null ? null : l.charSet)) {
                            h.splice(v, 1);
                            break t;
                          }
                      }
                      r = c.createElement(n), We(r, n, l), c.head.appendChild(r);
                      break;
                    default:
                      throw Error(E(468, n));
                  }
                  r[Ze] = e, Ye(r), n = r;
                }
                e.stateNode = n;
              } else
                f6(
                  c,
                  e.type,
                  e.stateNode
                );
            else
              e.stateNode = u6(
                c,
                n,
                e.memoizedProps
              );
          else
            r !== n ? (r === null ? l.stateNode !== null && (l = l.stateNode, l.parentNode.removeChild(l)) : r.count--, n === null ? f6(
              c,
              e.type,
              e.stateNode
            ) : u6(
              c,
              n,
              e.memoizedProps
            )) : n === null && e.stateNode !== null && Aa(
              e,
              e.memoizedProps,
              l.memoizedProps
            );
        }
        break;
      case 27:
        c1(t, e), i1(e), n & 512 && (_e || l === null || R1(l, l.return)), l !== null && n & 4 && Aa(
          e,
          e.memoizedProps,
          l.memoizedProps
        );
        break;
      case 5:
        if (c1(t, e), i1(e), n & 512 && (_e || l === null || R1(l, l.return)), e.flags & 32) {
          c = e.stateNode;
          try {
            al(c, "");
          } catch (N) {
            de(e, e.return, N);
          }
        }
        n & 4 && e.stateNode != null && (c = e.memoizedProps, Aa(
          e,
          c,
          l !== null ? l.memoizedProps : c
        )), n & 1024 && (Ta = !0);
        break;
      case 6:
        if (c1(t, e), i1(e), n & 4) {
          if (e.stateNode === null)
            throw Error(E(162));
          n = e.memoizedProps, l = e.stateNode;
          try {
            l.nodeValue = n;
          } catch (N) {
            de(e, e.return, N);
          }
        }
        break;
      case 3:
        if (U3 = null, c = O1, O1 = N3(t.containerInfo), c1(t, e), O1 = c, i1(e), n & 4 && l !== null && l.memoizedState.isDehydrated)
          try {
            Cl(t.containerInfo);
          } catch (N) {
            de(e, e.return, N);
          }
        Ta && (Ta = !1, fc(e));
        break;
      case 4:
        n = O1, O1 = N3(
          e.stateNode.containerInfo
        ), c1(t, e), i1(e), O1 = n;
        break;
      case 12:
        c1(t, e), i1(e);
        break;
      case 31:
        c1(t, e), i1(e), n & 4 && (n = e.updateQueue, n !== null && (e.updateQueue = null, x3(e, n)));
        break;
      case 13:
        c1(t, e), i1(e), e.child.flags & 8192 && e.memoizedState !== null != (l !== null && l.memoizedState !== null) && (y3 = s1()), n & 4 && (n = e.updateQueue, n !== null && (e.updateQueue = null, x3(e, n)));
        break;
      case 22:
        c = e.memoizedState !== null;
        var o = l !== null && l.memoizedState !== null, w = P1, D = _e;
        if (P1 = w || c, _e = D || o, c1(t, e), _e = D, P1 = w, i1(e), n & 8192)
          e: for (t = e.stateNode, t._visibility = c ? t._visibility & -2 : t._visibility | 1, c && (l === null || o || P1 || _e || Jt(e)), l = null, t = e; ; ) {
            if (t.tag === 5 || t.tag === 26) {
              if (l === null) {
                o = l = t;
                try {
                  if (r = o.stateNode, c)
                    h = r.style, typeof h.setProperty == "function" ? h.setProperty("display", "none", "important") : h.display = "none";
                  else {
                    v = o.stateNode;
                    var A = o.memoizedProps.style, x = A != null && A.hasOwnProperty("display") ? A.display : null;
                    v.style.display = x == null || typeof x == "boolean" ? "" : ("" + x).trim();
                  }
                } catch (N) {
                  de(o, o.return, N);
                }
              }
            } else if (t.tag === 6) {
              if (l === null) {
                o = t;
                try {
                  o.stateNode.nodeValue = c ? "" : o.memoizedProps;
                } catch (N) {
                  de(o, o.return, N);
                }
              }
            } else if (t.tag === 18) {
              if (l === null) {
                o = t;
                try {
                  var y = o.stateNode;
                  c ? Ic(y, !0) : Ic(o.stateNode, !1);
                } catch (N) {
                  de(o, o.return, N);
                }
              }
            } else if ((t.tag !== 22 && t.tag !== 23 || t.memoizedState === null || t === e) && t.child !== null) {
              t.child.return = t, t = t.child;
              continue;
            }
            if (t === e) break e;
            for (; t.sibling === null; ) {
              if (t.return === null || t.return === e) break e;
              l === t && (l = null), t = t.return;
            }
            l === t && (l = null), t.sibling.return = t.return, t = t.sibling;
          }
        n & 4 && (n = e.updateQueue, n !== null && (l = n.retryQueue, l !== null && (n.retryQueue = null, x3(e, l))));
        break;
      case 19:
        c1(t, e), i1(e), n & 4 && (n = e.updateQueue, n !== null && (e.updateQueue = null, x3(e, n)));
        break;
      case 30:
        break;
      case 21:
        break;
      default:
        c1(t, e), i1(e);
    }
  }
  function i1(e) {
    var t = e.flags;
    if (t & 2) {
      try {
        for (var l, n = e.return; n !== null; ) {
          if (ec(n)) {
            l = n;
            break;
          }
          n = n.return;
        }
        if (l == null) throw Error(E(160));
        switch (l.tag) {
          case 27:
            var c = l.stateNode, r = Ba(e);
            w3(e, r, c);
            break;
          case 5:
            var h = l.stateNode;
            l.flags & 32 && (al(h, ""), l.flags &= -33);
            var v = Ba(e);
            w3(e, v, h);
            break;
          case 3:
          case 4:
            var o = l.stateNode.containerInfo, w = Ba(e);
            ba(
              e,
              w,
              o
            );
            break;
          default:
            throw Error(E(161));
        }
      } catch (D) {
        de(e, e.return, D);
      }
      e.flags &= -3;
    }
    t & 4096 && (e.flags &= -4097);
  }
  function fc(e) {
    if (e.subtreeFlags & 1024)
      for (e = e.child; e !== null; ) {
        var t = e;
        fc(t), t.tag === 5 && t.flags & 1024 && t.stateNode.reset(), e = e.sibling;
      }
  }
  function tt(e, t) {
    if (t.subtreeFlags & 8772)
      for (t = t.child; t !== null; )
        ac(e, t.alternate, t), t = t.sibling;
  }
  function Jt(e) {
    for (e = e.child; e !== null; ) {
      var t = e;
      switch (t.tag) {
        case 0:
        case 11:
        case 14:
        case 15:
          pt(4, t, t.return), Jt(t);
          break;
        case 1:
          R1(t, t.return);
          var l = t.stateNode;
          typeof l.componentWillUnmount == "function" && In(
            t,
            t.return,
            l
          ), Jt(t);
          break;
        case 27:
          M2(t.stateNode);
        case 26:
        case 5:
          R1(t, t.return), Jt(t);
          break;
        case 22:
          t.memoizedState === null && Jt(t);
          break;
        case 30:
          Jt(t);
          break;
        default:
          Jt(t);
      }
      e = e.sibling;
    }
  }
  function lt(e, t, l) {
    for (l = l && (t.subtreeFlags & 8772) !== 0, t = t.child; t !== null; ) {
      var n = t.alternate, c = e, r = t, h = r.flags;
      switch (r.tag) {
        case 0:
        case 11:
        case 15:
          lt(
            c,
            r,
            l
          ), h2(4, r);
          break;
        case 1:
          if (lt(
            c,
            r,
            l
          ), n = r, c = n.stateNode, typeof c.componentDidMount == "function")
            try {
              c.componentDidMount();
            } catch (w) {
              de(n, n.return, w);
            }
          if (n = r, c = n.updateQueue, c !== null) {
            var v = n.stateNode;
            try {
              var o = c.shared.hiddenCallbacks;
              if (o !== null)
                for (c.shared.hiddenCallbacks = null, c = 0; c < o.length; c++)
                  R5(o[c], v);
            } catch (w) {
              de(n, n.return, w);
            }
          }
          l && h & 64 && $n(r), m2(r, r.return);
          break;
        case 27:
          tc(r);
        case 26:
        case 5:
          lt(
            c,
            r,
            l
          ), l && n === null && h & 4 && Pn(r), m2(r, r.return);
          break;
        case 12:
          lt(
            c,
            r,
            l
          );
          break;
        case 31:
          lt(
            c,
            r,
            l
          ), l && h & 4 && ic(c, r);
          break;
        case 13:
          lt(
            c,
            r,
            l
          ), l && h & 4 && uc(c, r);
          break;
        case 22:
          r.memoizedState === null && lt(
            c,
            r,
            l
          ), m2(r, r.return);
          break;
        case 30:
          break;
        default:
          lt(
            c,
            r,
            l
          );
      }
      t = t.sibling;
    }
  }
  function La(e, t) {
    var l = null;
    e !== null && e.memoizedState !== null && e.memoizedState.cachePool !== null && (l = e.memoizedState.cachePool.pool), e = null, t.memoizedState !== null && t.memoizedState.cachePool !== null && (e = t.memoizedState.cachePool.pool), e !== l && (e != null && e.refCount++, l != null && $l(l));
  }
  function Va(e, t) {
    e = null, t.alternate !== null && (e = t.alternate.memoizedState.cache), t = t.memoizedState.cache, t !== e && (t.refCount++, e != null && $l(e));
  }
  function N1(e, t, l, n) {
    if (t.subtreeFlags & 10256)
      for (t = t.child; t !== null; )
        hc(
          e,
          t,
          l,
          n
        ), t = t.sibling;
  }
  function hc(e, t, l, n) {
    var c = t.flags;
    switch (t.tag) {
      case 0:
      case 11:
      case 15:
        N1(
          e,
          t,
          l,
          n
        ), c & 2048 && h2(9, t);
        break;
      case 1:
        N1(
          e,
          t,
          l,
          n
        );
        break;
      case 3:
        N1(
          e,
          t,
          l,
          n
        ), c & 2048 && (e = null, t.alternate !== null && (e = t.alternate.memoizedState.cache), t = t.memoizedState.cache, t !== e && (t.refCount++, e != null && $l(e)));
        break;
      case 12:
        if (c & 2048) {
          N1(
            e,
            t,
            l,
            n
          ), e = t.stateNode;
          try {
            var r = t.memoizedProps, h = r.id, v = r.onPostCommit;
            typeof v == "function" && v(
              h,
              t.alternate === null ? "mount" : "update",
              e.passiveEffectDuration,
              -0
            );
          } catch (o) {
            de(t, t.return, o);
          }
        } else
          N1(
            e,
            t,
            l,
            n
          );
        break;
      case 31:
        N1(
          e,
          t,
          l,
          n
        );
        break;
      case 13:
        N1(
          e,
          t,
          l,
          n
        );
        break;
      case 23:
        break;
      case 22:
        r = t.stateNode, h = t.alternate, t.memoizedState !== null ? r._visibility & 2 ? N1(
          e,
          t,
          l,
          n
        ) : v2(e, t) : r._visibility & 2 ? N1(
          e,
          t,
          l,
          n
        ) : (r._visibility |= 2, Hl(
          e,
          t,
          l,
          n,
          (t.subtreeFlags & 10256) !== 0 || !1
        )), c & 2048 && La(h, t);
        break;
      case 24:
        N1(
          e,
          t,
          l,
          n
        ), c & 2048 && Va(t.alternate, t);
        break;
      default:
        N1(
          e,
          t,
          l,
          n
        );
    }
  }
  function Hl(e, t, l, n, c) {
    for (c = c && ((t.subtreeFlags & 10256) !== 0 || !1), t = t.child; t !== null; ) {
      var r = e, h = t, v = l, o = n, w = h.flags;
      switch (h.tag) {
        case 0:
        case 11:
        case 15:
          Hl(
            r,
            h,
            v,
            o,
            c
          ), h2(8, h);
          break;
        case 23:
          break;
        case 22:
          var D = h.stateNode;
          h.memoizedState !== null ? D._visibility & 2 ? Hl(
            r,
            h,
            v,
            o,
            c
          ) : v2(
            r,
            h
          ) : (D._visibility |= 2, Hl(
            r,
            h,
            v,
            o,
            c
          )), c && w & 2048 && La(
            h.alternate,
            h
          );
          break;
        case 24:
          Hl(
            r,
            h,
            v,
            o,
            c
          ), c && w & 2048 && Va(h.alternate, h);
          break;
        default:
          Hl(
            r,
            h,
            v,
            o,
            c
          );
      }
      t = t.sibling;
    }
  }
  function v2(e, t) {
    if (t.subtreeFlags & 10256)
      for (t = t.child; t !== null; ) {
        var l = e, n = t, c = n.flags;
        switch (n.tag) {
          case 22:
            v2(l, n), c & 2048 && La(
              n.alternate,
              n
            );
            break;
          case 24:
            v2(l, n), c & 2048 && Va(n.alternate, n);
            break;
          default:
            v2(l, n);
        }
        t = t.sibling;
      }
  }
  var s2 = 8192;
  function yl(e, t, l) {
    if (e.subtreeFlags & s2)
      for (e = e.child; e !== null; )
        mc(
          e,
          t,
          l
        ), e = e.sibling;
  }
  function mc(e, t, l) {
    switch (e.tag) {
      case 26:
        yl(
          e,
          t,
          l
        ), e.flags & s2 && e.memoizedState !== null && lh(
          l,
          O1,
          e.memoizedState,
          e.memoizedProps
        );
        break;
      case 5:
        yl(
          e,
          t,
          l
        );
        break;
      case 3:
      case 4:
        var n = O1;
        O1 = N3(e.stateNode.containerInfo), yl(
          e,
          t,
          l
        ), O1 = n;
        break;
      case 22:
        e.memoizedState === null && (n = e.alternate, n !== null && n.memoizedState !== null ? (n = s2, s2 = 16777216, yl(
          e,
          t,
          l
        ), s2 = n) : yl(
          e,
          t,
          l
        ));
        break;
      default:
        yl(
          e,
          t,
          l
        );
    }
  }
  function vc(e) {
    var t = e.alternate;
    if (t !== null && (e = t.child, e !== null)) {
      t.child = null;
      do
        t = e.sibling, e.sibling = null, e = t;
      while (e !== null);
    }
  }
  function d2(e) {
    var t = e.deletions;
    if ((e.flags & 16) !== 0) {
      if (t !== null)
        for (var l = 0; l < t.length; l++) {
          var n = t[l];
          qe = n, dc(
            n,
            e
          );
        }
      vc(e);
    }
    if (e.subtreeFlags & 10256)
      for (e = e.child; e !== null; )
        sc(e), e = e.sibling;
  }
  function sc(e) {
    switch (e.tag) {
      case 0:
      case 11:
      case 15:
        d2(e), e.flags & 2048 && pt(9, e, e.return);
        break;
      case 3:
        d2(e);
        break;
      case 12:
        d2(e);
        break;
      case 22:
        var t = e.stateNode;
        e.memoizedState !== null && t._visibility & 2 && (e.return === null || e.return.tag !== 13) ? (t._visibility &= -3, H3(e)) : d2(e);
        break;
      default:
        d2(e);
    }
  }
  function H3(e) {
    var t = e.deletions;
    if ((e.flags & 16) !== 0) {
      if (t !== null)
        for (var l = 0; l < t.length; l++) {
          var n = t[l];
          qe = n, dc(
            n,
            e
          );
        }
      vc(e);
    }
    for (e = e.child; e !== null; ) {
      switch (t = e, t.tag) {
        case 0:
        case 11:
        case 15:
          pt(8, t, t.return), H3(t);
          break;
        case 22:
          l = t.stateNode, l._visibility & 2 && (l._visibility &= -3, H3(t));
          break;
        default:
          H3(t);
      }
      e = e.sibling;
    }
  }
  function dc(e, t) {
    for (; qe !== null; ) {
      var l = qe;
      switch (l.tag) {
        case 0:
        case 11:
        case 15:
          pt(8, l, t);
          break;
        case 23:
        case 22:
          if (l.memoizedState !== null && l.memoizedState.cachePool !== null) {
            var n = l.memoizedState.cachePool.pool;
            n != null && n.refCount++;
          }
          break;
        case 24:
          $l(l.memoizedState.cache);
      }
      if (n = l.child, n !== null) n.return = l, qe = n;
      else
        e: for (l = e; qe !== null; ) {
          n = qe;
          var c = n.sibling, r = n.return;
          if (nc(n), n === l) {
            qe = null;
            break e;
          }
          if (c !== null) {
            c.return = r, qe = c;
            break e;
          }
          qe = r;
        }
    }
  }
  var zf = {
    getCacheForType: function(e) {
      var t = Je(Ce), l = t.data.get(e);
      return l === void 0 && (l = e(), t.data.set(e, l)), l;
    },
    cacheSignal: function() {
      return Je(Ce).controller.signal;
    }
  }, pf = typeof WeakMap == "function" ? WeakMap : Map, me = 0, Me = null, P = null, te = 0, se = 0, M1 = null, gt = !1, Fl = !1, Ca = !1, at = 0, Ae = 0, Mt = 0, kt = 0, Oa = 0, w1 = 0, Dl = 0, o2 = null, u1 = null, Na = !1, y3 = 0, oc = 0, F3 = 1 / 0, D3 = null, wt = null, Re = 0, xt = null, Sl = null, nt = 0, _a = 0, Ua = null, Ec = null, E2 = 0, Ra = null;
  function x1() {
    return (me & 2) !== 0 && te !== 0 ? te & -te : b.T !== null ? Xa() : T4();
  }
  function zc() {
    if (w1 === 0)
      if ((te & 536870912) === 0 || ae) {
        var e = C2;
        C2 <<= 1, (C2 & 3932160) === 0 && (C2 = 262144), w1 = e;
      } else w1 = 536870912;
    return e = p1.current, e !== null && (e.flags |= 32), w1;
  }
  function r1(e, t, l) {
    (e === Me && (se === 2 || se === 9) || e.cancelPendingCommit !== null) && (Al(e, 0), Ht(
      e,
      te,
      w1,
      !1
    )), _l(e, l), ((me & 2) === 0 || e !== Me) && (e === Me && ((me & 2) === 0 && (kt |= l), Ae === 4 && Ht(
      e,
      te,
      w1,
      !1
    )), j1(e));
  }
  function pc(e, t, l) {
    if ((me & 6) !== 0) throw Error(E(327));
    var n = !l && (t & 127) === 0 && (t & e.expiredLanes) === 0 || Nl(e, t), c = n ? wf(e, t) : Ga(e, t, !0), r = n;
    do {
      if (c === 0) {
        Fl && !n && Ht(e, t, 0, !1);
        break;
      } else {
        if (l = e.current.alternate, r && !gf(l)) {
          c = Ga(e, t, !1), r = !1;
          continue;
        }
        if (c === 2) {
          if (r = t, e.errorRecoveryDisabledLanes & r)
            var h = 0;
          else
            h = e.pendingLanes & -536870913, h = h !== 0 ? h : h & 536870912 ? 536870912 : 0;
          if (h !== 0) {
            t = h;
            e: {
              var v = e;
              c = o2;
              var o = v.current.memoizedState.isDehydrated;
              if (o && (Al(v, h).flags |= 256), h = Ga(
                v,
                h,
                !1
              ), h !== 2) {
                if (Ca && !o) {
                  v.errorRecoveryDisabledLanes |= r, kt |= r, c = 4;
                  break e;
                }
                r = u1, u1 = c, r !== null && (u1 === null ? u1 = r : u1.push.apply(
                  u1,
                  r
                ));
              }
              c = h;
            }
            if (r = !1, c !== 2) continue;
          }
        }
        if (c === 1) {
          Al(e, 0), Ht(e, t, 0, !0);
          break;
        }
        e: {
          switch (n = e, r = c, r) {
            case 0:
            case 1:
              throw Error(E(345));
            case 4:
              if ((t & 4194048) !== t) break;
            case 6:
              Ht(
                n,
                t,
                w1,
                !gt
              );
              break e;
            case 2:
              u1 = null;
              break;
            case 3:
            case 5:
              break;
            default:
              throw Error(E(329));
          }
          if ((t & 62914560) === t && (c = y3 + 300 - s1(), 10 < c)) {
            if (Ht(
              n,
              t,
              w1,
              !gt
            ), N2(n, 0, !0) !== 0) break e;
            nt = t, n.timeoutHandle = kc(
              gc.bind(
                null,
                n,
                l,
                u1,
                D3,
                Na,
                t,
                w1,
                kt,
                Dl,
                gt,
                r,
                "Throttled",
                -0,
                0
              ),
              c
            );
            break e;
          }
          gc(
            n,
            l,
            u1,
            D3,
            Na,
            t,
            w1,
            kt,
            Dl,
            gt,
            r,
            null,
            -0,
            0
          );
        }
      }
      break;
    } while (!0);
    j1(e);
  }
  function gc(e, t, l, n, c, r, h, v, o, w, D, A, x, y) {
    if (e.timeoutHandle = -1, A = t.subtreeFlags, A & 8192 || (A & 16785408) === 16785408) {
      A = {
        stylesheets: null,
        count: 0,
        imgCount: 0,
        imgBytes: 0,
        suspenseyImages: [],
        waitingForImages: !0,
        waitingForViewTransition: !1,
        unsuspend: q1
      }, mc(
        t,
        r,
        A
      );
      var N = (r & 62914560) === r ? y3 - s1() : (r & 4194048) === r ? oc - s1() : 0;
      if (N = ah(
        A,
        N
      ), N !== null) {
        nt = r, e.cancelPendingCommit = N(
          Sc.bind(
            null,
            e,
            t,
            r,
            l,
            n,
            c,
            h,
            v,
            o,
            D,
            A,
            null,
            x,
            y
          )
        ), Ht(e, r, h, !w);
        return;
      }
    }
    Sc(
      e,
      t,
      r,
      l,
      n,
      c,
      h,
      v,
      o
    );
  }
  function gf(e) {
    for (var t = e; ; ) {
      var l = t.tag;
      if ((l === 0 || l === 11 || l === 15) && t.flags & 16384 && (l = t.updateQueue, l !== null && (l = l.stores, l !== null)))
        for (var n = 0; n < l.length; n++) {
          var c = l[n], r = c.getSnapshot;
          c = c.value;
          try {
            if (!E1(r(), c)) return !1;
          } catch {
            return !1;
          }
        }
      if (l = t.child, t.subtreeFlags & 16384 && l !== null)
        l.return = t, t = l;
      else {
        if (t === e) break;
        for (; t.sibling === null; ) {
          if (t.return === null || t.return === e) return !0;
          t = t.return;
        }
        t.sibling.return = t.return, t = t.sibling;
      }
    }
    return !0;
  }
  function Ht(e, t, l, n) {
    t &= ~Oa, t &= ~kt, e.suspendedLanes |= t, e.pingedLanes &= ~t, n && (e.warmLanes |= t), n = e.expirationTimes;
    for (var c = t; 0 < c; ) {
      var r = 31 - o1(c), h = 1 << r;
      n[r] = -1, c &= ~h;
    }
    l !== 0 && A4(e, l, t);
  }
  function S3() {
    return (me & 6) === 0 ? (z2(0), !1) : !0;
  }
  function ja() {
    if (P !== null) {
      if (se === 0)
        var e = P.return;
      else
        e = P, K1 = jt = null, la(e), pl = null, Pl = 0, e = P;
      for (; e !== null; )
        Wn(e.alternate, e), e = e.return;
      P = null;
    }
  }
  function Al(e, t) {
    var l = e.timeoutHandle;
    l !== -1 && (e.timeoutHandle = -1, Rf(l)), l = e.cancelPendingCommit, l !== null && (e.cancelPendingCommit = null, l()), nt = 0, ja(), Me = e, P = l = X1(e.current, null), te = t, se = 0, M1 = null, gt = !1, Fl = Nl(e, t), Ca = !1, Dl = w1 = Oa = kt = Mt = Ae = 0, u1 = o2 = null, Na = !1, (t & 8) !== 0 && (t |= t & 32);
    var n = e.entangledLanes;
    if (n !== 0)
      for (e = e.entanglements, n &= t; 0 < n; ) {
        var c = 31 - o1(n), r = 1 << c;
        t |= e[c], n &= ~r;
      }
    return at = t, J2(), l;
  }
  function Mc(e, t) {
    Z = null, b.H = u2, t === zl || t === l3 ? (t = O5(), se = 3) : t === Q0 ? (t = O5(), se = 4) : se = t === pa ? 8 : t !== null && typeof t == "object" && typeof t.then == "function" ? 6 : 1, M1 = t, P === null && (Ae = 1, E3(
      e,
      S1(t, e.current)
    ));
  }
  function wc() {
    var e = p1.current;
    return e === null ? !0 : (te & 4194048) === te ? T1 === null : (te & 62914560) === te || (te & 536870912) !== 0 ? e === T1 : !1;
  }
  function xc() {
    var e = b.H;
    return b.H = u2, e === null ? u2 : e;
  }
  function Hc() {
    var e = b.A;
    return b.A = zf, e;
  }
  function A3() {
    Ae = 4, gt || (te & 4194048) !== te && p1.current !== null || (Fl = !0), (Mt & 134217727) === 0 && (kt & 134217727) === 0 || Me === null || Ht(
      Me,
      te,
      w1,
      !1
    );
  }
  function Ga(e, t, l) {
    var n = me;
    me |= 2;
    var c = xc(), r = Hc();
    (Me !== e || te !== t) && (D3 = null, Al(e, t)), t = !1;
    var h = Ae;
    e: do
      try {
        if (se !== 0 && P !== null) {
          var v = P, o = M1;
          switch (se) {
            case 8:
              ja(), h = 6;
              break e;
            case 3:
            case 2:
            case 9:
            case 6:
              p1.current === null && (t = !0);
              var w = se;
              if (se = 0, M1 = null, Bl(e, v, o, w), l && Fl) {
                h = 0;
                break e;
              }
              break;
            default:
              w = se, se = 0, M1 = null, Bl(e, v, o, w);
          }
        }
        Mf(), h = Ae;
        break;
      } catch (D) {
        Mc(e, D);
      }
    while (!0);
    return t && e.shellSuspendCounter++, K1 = jt = null, me = n, b.H = c, b.A = r, P === null && (Me = null, te = 0, J2()), h;
  }
  function Mf() {
    for (; P !== null; ) yc(P);
  }
  function wf(e, t) {
    var l = me;
    me |= 2;
    var n = xc(), c = Hc();
    Me !== e || te !== t ? (D3 = null, F3 = s1() + 500, Al(e, t)) : Fl = Nl(
      e,
      t
    );
    e: do
      try {
        if (se !== 0 && P !== null) {
          t = P;
          var r = M1;
          t: switch (se) {
            case 1:
              se = 0, M1 = null, Bl(e, t, r, 1);
              break;
            case 2:
            case 9:
              if (V5(r)) {
                se = 0, M1 = null, Fc(t);
                break;
              }
              t = function() {
                se !== 2 && se !== 9 || Me !== e || (se = 7), j1(e);
              }, r.then(t, t);
              break e;
            case 3:
              se = 7;
              break e;
            case 4:
              se = 5;
              break e;
            case 7:
              V5(r) ? (se = 0, M1 = null, Fc(t)) : (se = 0, M1 = null, Bl(e, t, r, 7));
              break;
            case 5:
              var h = null;
              switch (P.tag) {
                case 26:
                  h = P.memoizedState;
                case 5:
                case 27:
                  var v = P;
                  if (h ? h6(h) : v.stateNode.complete) {
                    se = 0, M1 = null;
                    var o = v.sibling;
                    if (o !== null) P = o;
                    else {
                      var w = v.return;
                      w !== null ? (P = w, B3(w)) : P = null;
                    }
                    break t;
                  }
              }
              se = 0, M1 = null, Bl(e, t, r, 5);
              break;
            case 6:
              se = 0, M1 = null, Bl(e, t, r, 6);
              break;
            case 8:
              ja(), Ae = 6;
              break e;
            default:
              throw Error(E(462));
          }
        }
        xf();
        break;
      } catch (D) {
        Mc(e, D);
      }
    while (!0);
    return K1 = jt = null, b.H = n, b.A = c, me = l, P !== null ? 0 : (Me = null, te = 0, J2(), Ae);
  }
  function xf() {
    for (; P !== null && !Q9(); )
      yc(P);
  }
  function yc(e) {
    var t = Jn(e.alternate, e, at);
    e.memoizedProps = e.pendingProps, t === null ? B3(e) : P = t;
  }
  function Fc(e) {
    var t = e, l = t.alternate;
    switch (t.tag) {
      case 15:
      case 0:
        t = Yn(
          l,
          t,
          t.pendingProps,
          t.type,
          void 0,
          te
        );
        break;
      case 11:
        t = Yn(
          l,
          t,
          t.pendingProps,
          t.type.render,
          t.ref,
          te
        );
        break;
      case 5:
        la(t);
      default:
        Wn(l, t), t = P = x5(t, at), t = Jn(l, t, at);
    }
    e.memoizedProps = e.pendingProps, t === null ? B3(e) : P = t;
  }
  function Bl(e, t, l, n) {
    K1 = jt = null, la(t), pl = null, Pl = 0;
    var c = t.return;
    try {
      if (hf(
        e,
        c,
        t,
        l,
        te
      )) {
        Ae = 1, E3(
          e,
          S1(l, e.current)
        ), P = null;
        return;
      }
    } catch (r) {
      if (c !== null) throw P = c, r;
      Ae = 1, E3(
        e,
        S1(l, e.current)
      ), P = null;
      return;
    }
    t.flags & 32768 ? (ae || n === 1 ? e = !0 : Fl || (te & 536870912) !== 0 ? e = !1 : (gt = e = !0, (n === 2 || n === 9 || n === 3 || n === 6) && (n = p1.current, n !== null && n.tag === 13 && (n.flags |= 16384))), Dc(t, e)) : B3(t);
  }
  function B3(e) {
    var t = e;
    do {
      if ((t.flags & 32768) !== 0) {
        Dc(
          t,
          gt
        );
        return;
      }
      e = t.return;
      var l = sf(
        t.alternate,
        t,
        at
      );
      if (l !== null) {
        P = l;
        return;
      }
      if (t = t.sibling, t !== null) {
        P = t;
        return;
      }
      P = t = e;
    } while (t !== null);
    Ae === 0 && (Ae = 5);
  }
  function Dc(e, t) {
    do {
      var l = df(e.alternate, e);
      if (l !== null) {
        l.flags &= 32767, P = l;
        return;
      }
      if (l = e.return, l !== null && (l.flags |= 32768, l.subtreeFlags = 0, l.deletions = null), !t && (e = e.sibling, e !== null)) {
        P = e;
        return;
      }
      P = e = l;
    } while (e !== null);
    Ae = 6, P = null;
  }
  function Sc(e, t, l, n, c, r, h, v, o) {
    e.cancelPendingCommit = null;
    do
      b3();
    while (Re !== 0);
    if ((me & 6) !== 0) throw Error(E(327));
    if (t !== null) {
      if (t === e.current) throw Error(E(177));
      if (r = t.lanes | t.childLanes, r |= B0, er(
        e,
        l,
        r,
        h,
        v,
        o
      ), e === Me && (P = Me = null, te = 0), Sl = t, xt = e, nt = l, _a = r, Ua = c, Ec = n, (t.subtreeFlags & 10256) !== 0 || (t.flags & 10256) !== 0 ? (e.callbackNode = null, e.callbackPriority = 0, Df(L2, function() {
        return Lc(), null;
      })) : (e.callbackNode = null, e.callbackPriority = 0), n = (t.flags & 13878) !== 0, (t.subtreeFlags & 13878) !== 0 || n) {
        n = b.T, b.T = null, c = L.p, L.p = 2, h = me, me |= 4;
        try {
          of(e, t, l);
        } finally {
          me = h, L.p = c, b.T = n;
        }
      }
      Re = 1, Ac(), Bc(), bc();
    }
  }
  function Ac() {
    if (Re === 1) {
      Re = 0;
      var e = xt, t = Sl, l = (t.flags & 13878) !== 0;
      if ((t.subtreeFlags & 13878) !== 0 || l) {
        l = b.T, b.T = null;
        var n = L.p;
        L.p = 2;
        var c = me;
        me |= 4;
        try {
          rc(t, e);
          var r = Pa, h = s5(e.containerInfo), v = r.focusedElem, o = r.selectionRange;
          if (h !== v && v && v.ownerDocument && v5(
            v.ownerDocument.documentElement,
            v
          )) {
            if (o !== null && y0(v)) {
              var w = o.start, D = o.end;
              if (D === void 0 && (D = w), "selectionStart" in v)
                v.selectionStart = w, v.selectionEnd = Math.min(
                  D,
                  v.value.length
                );
              else {
                var A = v.ownerDocument || document, x = A && A.defaultView || window;
                if (x.getSelection) {
                  var y = x.getSelection(), N = v.textContent.length, q = Math.min(o.start, N), pe = o.end === void 0 ? q : Math.min(o.end, N);
                  !y.extend && q > pe && (h = pe, pe = q, q = h);
                  var g = m5(
                    v,
                    q
                  ), z = m5(
                    v,
                    pe
                  );
                  if (g && z && (y.rangeCount !== 1 || y.anchorNode !== g.node || y.anchorOffset !== g.offset || y.focusNode !== z.node || y.focusOffset !== z.offset)) {
                    var M = A.createRange();
                    M.setStart(g.node, g.offset), y.removeAllRanges(), q > pe ? (y.addRange(M), y.extend(z.node, z.offset)) : (M.setEnd(z.node, z.offset), y.addRange(M));
                  }
                }
              }
            }
            for (A = [], y = v; y = y.parentNode; )
              y.nodeType === 1 && A.push({
                element: y,
                left: y.scrollLeft,
                top: y.scrollTop
              });
            for (typeof v.focus == "function" && v.focus(), v = 0; v < A.length; v++) {
              var S = A[v];
              S.element.scrollLeft = S.left, S.element.scrollTop = S.top;
            }
          }
          Y3 = !!Ia, Pa = Ia = null;
        } finally {
          me = c, L.p = n, b.T = l;
        }
      }
      e.current = t, Re = 2;
    }
  }
  function Bc() {
    if (Re === 2) {
      Re = 0;
      var e = xt, t = Sl, l = (t.flags & 8772) !== 0;
      if ((t.subtreeFlags & 8772) !== 0 || l) {
        l = b.T, b.T = null;
        var n = L.p;
        L.p = 2;
        var c = me;
        me |= 4;
        try {
          ac(e, t.alternate, t);
        } finally {
          me = c, L.p = n, b.T = l;
        }
      }
      Re = 3;
    }
  }
  function bc() {
    if (Re === 4 || Re === 3) {
      Re = 0, X9();
      var e = xt, t = Sl, l = nt, n = Ec;
      (t.subtreeFlags & 10256) !== 0 || (t.flags & 10256) !== 0 ? Re = 5 : (Re = 0, Sl = xt = null, Tc(e, e.pendingLanes));
      var c = e.pendingLanes;
      if (c === 0 && (wt = null), c0(l), t = t.stateNode, d1 && typeof d1.onCommitFiberRoot == "function")
        try {
          d1.onCommitFiberRoot(
            Ol,
            t,
            void 0,
            (t.current.flags & 128) === 128
          );
        } catch {
        }
      if (n !== null) {
        t = b.T, c = L.p, L.p = 2, b.T = null;
        try {
          for (var r = e.onRecoverableError, h = 0; h < n.length; h++) {
            var v = n[h];
            r(v.value, {
              componentStack: v.stack
            });
          }
        } finally {
          b.T = t, L.p = c;
        }
      }
      (nt & 3) !== 0 && b3(), j1(e), c = e.pendingLanes, (l & 261930) !== 0 && (c & 42) !== 0 ? e === Ra ? E2++ : (E2 = 0, Ra = e) : E2 = 0, z2(0);
    }
  }
  function Tc(e, t) {
    (e.pooledCacheLanes &= t) === 0 && (t = e.pooledCache, t != null && (e.pooledCache = null, $l(t)));
  }
  function b3() {
    return Ac(), Bc(), bc(), Lc();
  }
  function Lc() {
    if (Re !== 5) return !1;
    var e = xt, t = _a;
    _a = 0;
    var l = c0(nt), n = b.T, c = L.p;
    try {
      L.p = 32 > l ? 32 : l, b.T = null, l = Ua, Ua = null;
      var r = xt, h = nt;
      if (Re = 0, Sl = xt = null, nt = 0, (me & 6) !== 0) throw Error(E(331));
      var v = me;
      if (me |= 4, sc(r.current), hc(
        r,
        r.current,
        h,
        l
      ), me = v, z2(0, !1), d1 && typeof d1.onPostCommitFiberRoot == "function")
        try {
          d1.onPostCommitFiberRoot(Ol, r);
        } catch {
        }
      return !0;
    } finally {
      L.p = c, b.T = n, Tc(e, t);
    }
  }
  function Vc(e, t, l) {
    t = S1(l, t), t = za(e.stateNode, t, 2), e = ot(e, t, 2), e !== null && (_l(e, 2), j1(e));
  }
  function de(e, t, l) {
    if (e.tag === 3)
      Vc(e, e, l);
    else
      for (; t !== null; ) {
        if (t.tag === 3) {
          Vc(
            t,
            e,
            l
          );
          break;
        } else if (t.tag === 1) {
          var n = t.stateNode;
          if (typeof t.type.getDerivedStateFromError == "function" || typeof n.componentDidCatch == "function" && (wt === null || !wt.has(n))) {
            e = S1(l, e), l = Cn(2), n = ot(t, l, 2), n !== null && (On(
              l,
              n,
              t,
              e
            ), _l(n, 2), j1(n));
            break;
          }
        }
        t = t.return;
      }
  }
  function Ya(e, t, l) {
    var n = e.pingCache;
    if (n === null) {
      n = e.pingCache = new pf();
      var c = /* @__PURE__ */ new Set();
      n.set(t, c);
    } else
      c = n.get(t), c === void 0 && (c = /* @__PURE__ */ new Set(), n.set(t, c));
    c.has(l) || (Ca = !0, c.add(l), e = Hf.bind(null, e, t, l), t.then(e, e));
  }
  function Hf(e, t, l) {
    var n = e.pingCache;
    n !== null && n.delete(t), e.pingedLanes |= e.suspendedLanes & l, e.warmLanes &= ~l, Me === e && (te & l) === l && (Ae === 4 || Ae === 3 && (te & 62914560) === te && 300 > s1() - y3 ? (me & 2) === 0 && Al(e, 0) : Oa |= l, Dl === te && (Dl = 0)), j1(e);
  }
  function Cc(e, t) {
    t === 0 && (t = S4()), e = _t(e, t), e !== null && (_l(e, t), j1(e));
  }
  function yf(e) {
    var t = e.memoizedState, l = 0;
    t !== null && (l = t.retryLane), Cc(e, l);
  }
  function Ff(e, t) {
    var l = 0;
    switch (e.tag) {
      case 31:
      case 13:
        var n = e.stateNode, c = e.memoizedState;
        c !== null && (l = c.retryLane);
        break;
      case 19:
        n = e.stateNode;
        break;
      case 22:
        n = e.stateNode._retryCache;
        break;
      default:
        throw Error(E(314));
    }
    n !== null && n.delete(t), Cc(e, l);
  }
  function Df(e, t) {
    return t0(e, t);
  }
  var T3 = null, bl = null, qa = !1, L3 = !1, Qa = !1, yt = 0;
  function j1(e) {
    e !== bl && e.next === null && (bl === null ? T3 = bl = e : bl = bl.next = e), L3 = !0, qa || (qa = !0, Af());
  }
  function z2(e, t) {
    if (!Qa && L3) {
      Qa = !0;
      do
        for (var l = !1, n = T3; n !== null; ) {
          if (e !== 0) {
            var c = n.pendingLanes;
            if (c === 0) var r = 0;
            else {
              var h = n.suspendedLanes, v = n.pingedLanes;
              r = (1 << 31 - o1(42 | e) + 1) - 1, r &= c & ~(h & ~v), r = r & 201326741 ? r & 201326741 | 1 : r ? r | 2 : 0;
            }
            r !== 0 && (l = !0, Uc(n, r));
          } else
            r = te, r = N2(
              n,
              n === Me ? r : 0,
              n.cancelPendingCommit !== null || n.timeoutHandle !== -1
            ), (r & 3) === 0 || Nl(n, r) || (l = !0, Uc(n, r));
          n = n.next;
        }
      while (l);
      Qa = !1;
    }
  }
  function Sf() {
    Oc();
  }
  function Oc() {
    L3 = qa = !1;
    var e = 0;
    yt !== 0 && Uf() && (e = yt);
    for (var t = s1(), l = null, n = T3; n !== null; ) {
      var c = n.next, r = Nc(n, t);
      r === 0 ? (n.next = null, l === null ? T3 = c : l.next = c, c === null && (bl = l)) : (l = n, (e !== 0 || (r & 3) !== 0) && (L3 = !0)), n = c;
    }
    Re !== 0 && Re !== 5 || z2(e), yt !== 0 && (yt = 0);
  }
  function Nc(e, t) {
    for (var l = e.suspendedLanes, n = e.pingedLanes, c = e.expirationTimes, r = e.pendingLanes & -62914561; 0 < r; ) {
      var h = 31 - o1(r), v = 1 << h, o = c[h];
      o === -1 ? ((v & l) === 0 || (v & n) !== 0) && (c[h] = P9(v, t)) : o <= t && (e.expiredLanes |= v), r &= ~v;
    }
    if (t = Me, l = te, l = N2(
      e,
      e === t ? l : 0,
      e.cancelPendingCommit !== null || e.timeoutHandle !== -1
    ), n = e.callbackNode, l === 0 || e === t && (se === 2 || se === 9) || e.cancelPendingCommit !== null)
      return n !== null && n !== null && l0(n), e.callbackNode = null, e.callbackPriority = 0;
    if ((l & 3) === 0 || Nl(e, l)) {
      if (t = l & -l, t === e.callbackPriority) return t;
      switch (n !== null && l0(n), c0(l)) {
        case 2:
        case 8:
          l = F4;
          break;
        case 32:
          l = L2;
          break;
        case 268435456:
          l = D4;
          break;
        default:
          l = L2;
      }
      return n = _c.bind(null, e), l = t0(l, n), e.callbackPriority = t, e.callbackNode = l, t;
    }
    return n !== null && n !== null && l0(n), e.callbackPriority = 2, e.callbackNode = null, 2;
  }
  function _c(e, t) {
    if (Re !== 0 && Re !== 5)
      return e.callbackNode = null, e.callbackPriority = 0, null;
    var l = e.callbackNode;
    if (b3() && e.callbackNode !== l)
      return null;
    var n = te;
    return n = N2(
      e,
      e === Me ? n : 0,
      e.cancelPendingCommit !== null || e.timeoutHandle !== -1
    ), n === 0 ? null : (pc(e, n, t), Nc(e, s1()), e.callbackNode != null && e.callbackNode === l ? _c.bind(null, e) : null);
  }
  function Uc(e, t) {
    if (b3()) return null;
    pc(e, t, !0);
  }
  function Af() {
    jf(function() {
      (me & 6) !== 0 ? t0(
        y4,
        Sf
      ) : Oc();
    });
  }
  function Xa() {
    if (yt === 0) {
      var e = ol;
      e === 0 && (e = V2, V2 <<= 1, (V2 & 261888) === 0 && (V2 = 256)), yt = e;
    }
    return yt;
  }
  function Rc(e) {
    return e == null || typeof e == "symbol" || typeof e == "boolean" ? null : typeof e == "function" ? e : j2("" + e);
  }
  function jc(e, t) {
    var l = t.ownerDocument.createElement("input");
    return l.name = t.name, l.value = t.value, e.id && l.setAttribute("form", e.id), t.parentNode.insertBefore(l, t), e = new FormData(e), l.parentNode.removeChild(l), e;
  }
  function Bf(e, t, l, n, c) {
    if (t === "submit" && l && l.stateNode === c) {
      var r = Rc(
        (c[l1] || null).action
      ), h = n.submitter;
      h && (t = (t = h[l1] || null) ? Rc(t.formAction) : h.getAttribute("formAction"), t !== null && (r = t, h = null));
      var v = new Q2(
        "action",
        "action",
        null,
        n,
        c
      );
      e.push({
        event: v,
        listeners: [
          {
            instance: null,
            listener: function() {
              if (n.defaultPrevented) {
                if (yt !== 0) {
                  var o = h ? jc(c, h) : new FormData(c);
                  ma(
                    l,
                    {
                      pending: !0,
                      data: o,
                      method: c.method,
                      action: r
                    },
                    null,
                    o
                  );
                }
              } else
                typeof r == "function" && (v.preventDefault(), o = h ? jc(c, h) : new FormData(c), ma(
                  l,
                  {
                    pending: !0,
                    data: o,
                    method: c.method,
                    action: r
                  },
                  r,
                  o
                ));
            },
            currentTarget: c
          }
        ]
      });
    }
  }
  for (var Za = 0; Za < A0.length; Za++) {
    var Ka = A0[Za], bf = Ka.toLowerCase(), Tf = Ka[0].toUpperCase() + Ka.slice(1);
    C1(
      bf,
      "on" + Tf
    );
  }
  C1(E5, "onAnimationEnd"), C1(z5, "onAnimationIteration"), C1(p5, "onAnimationStart"), C1("dblclick", "onDoubleClick"), C1("focusin", "onFocus"), C1("focusout", "onBlur"), C1(Zr, "onTransitionRun"), C1(Kr, "onTransitionStart"), C1(Jr, "onTransitionCancel"), C1(g5, "onTransitionEnd"), tl("onMouseEnter", ["mouseout", "mouseover"]), tl("onMouseLeave", ["mouseout", "mouseover"]), tl("onPointerEnter", ["pointerout", "pointerover"]), tl("onPointerLeave", ["pointerout", "pointerover"]), Vt(
    "onChange",
    "change click focusin focusout input keydown keyup selectionchange".split(" ")
  ), Vt(
    "onSelect",
    "focusout contextmenu dragend focusin keydown keyup mousedown mouseup selectionchange".split(
      " "
    )
  ), Vt("onBeforeInput", [
    "compositionend",
    "keypress",
    "textInput",
    "paste"
  ]), Vt(
    "onCompositionEnd",
    "compositionend focusout keydown keypress keyup mousedown".split(" ")
  ), Vt(
    "onCompositionStart",
    "compositionstart focusout keydown keypress keyup mousedown".split(" ")
  ), Vt(
    "onCompositionUpdate",
    "compositionupdate focusout keydown keypress keyup mousedown".split(" ")
  );
  var p2 = "abort canplay canplaythrough durationchange emptied encrypted ended error loadeddata loadedmetadata loadstart pause play playing progress ratechange resize seeked seeking stalled suspend timeupdate volumechange waiting".split(
    " "
  ), Lf = new Set(
    "beforetoggle cancel close invalid load scroll scrollend toggle".split(" ").concat(p2)
  );
  function Gc(e, t) {
    t = (t & 4) !== 0;
    for (var l = 0; l < e.length; l++) {
      var n = e[l], c = n.event;
      n = n.listeners;
      e: {
        var r = void 0;
        if (t)
          for (var h = n.length - 1; 0 <= h; h--) {
            var v = n[h], o = v.instance, w = v.currentTarget;
            if (v = v.listener, o !== r && c.isPropagationStopped())
              break e;
            r = v, c.currentTarget = w;
            try {
              r(c);
            } catch (D) {
              K2(D);
            }
            c.currentTarget = null, r = o;
          }
        else
          for (h = 0; h < n.length; h++) {
            if (v = n[h], o = v.instance, w = v.currentTarget, v = v.listener, o !== r && c.isPropagationStopped())
              break e;
            r = v, c.currentTarget = w;
            try {
              r(c);
            } catch (D) {
              K2(D);
            }
            c.currentTarget = null, r = o;
          }
      }
    }
  }
  function ee(e, t) {
    var l = t[i0];
    l === void 0 && (l = t[i0] = /* @__PURE__ */ new Set());
    var n = e + "__bubble";
    l.has(n) || (Yc(t, e, 2, !1), l.add(n));
  }
  function Ja(e, t, l) {
    var n = 0;
    t && (n |= 4), Yc(
      l,
      e,
      n,
      t
    );
  }
  var V3 = "_reactListening" + Math.random().toString(36).slice(2);
  function ka(e) {
    if (!e[V3]) {
      e[V3] = !0, C4.forEach(function(l) {
        l !== "selectionchange" && (Lf.has(l) || Ja(l, !1, e), Ja(l, !0, e));
      });
      var t = e.nodeType === 9 ? e : e.ownerDocument;
      t === null || t[V3] || (t[V3] = !0, Ja("selectionchange", !1, t));
    }
  }
  function Yc(e, t, l, n) {
    switch (z6(t)) {
      case 2:
        var c = ih;
        break;
      case 8:
        c = uh;
        break;
      default:
        c = h4;
    }
    l = c.bind(
      null,
      t,
      l,
      e
    ), c = void 0, !o0 || t !== "touchstart" && t !== "touchmove" && t !== "wheel" || (c = !0), n ? c !== void 0 ? e.addEventListener(t, l, {
      capture: !0,
      passive: c
    }) : e.addEventListener(t, l, !0) : c !== void 0 ? e.addEventListener(t, l, {
      passive: c
    }) : e.addEventListener(t, l, !1);
  }
  function Wa(e, t, l, n, c) {
    var r = n;
    if ((t & 1) === 0 && (t & 2) === 0 && n !== null)
      e: for (; ; ) {
        if (n === null) return;
        var h = n.tag;
        if (h === 3 || h === 4) {
          var v = n.stateNode.containerInfo;
          if (v === c) break;
          if (h === 4)
            for (h = n.return; h !== null; ) {
              var o = h.tag;
              if ((o === 3 || o === 4) && h.stateNode.containerInfo === c)
                return;
              h = h.return;
            }
          for (; v !== null; ) {
            if (h = It(v), h === null) return;
            if (o = h.tag, o === 5 || o === 6 || o === 26 || o === 27) {
              n = r = h;
              continue e;
            }
            v = v.parentNode;
          }
        }
        n = n.return;
      }
    Z4(function() {
      var w = r, D = s0(l), A = [];
      e: {
        var x = M5.get(e);
        if (x !== void 0) {
          var y = Q2, N = e;
          switch (e) {
            case "keypress":
              if (Y2(l) === 0) break e;
            case "keydown":
            case "keyup":
              y = yr;
              break;
            case "focusin":
              N = "focus", y = g0;
              break;
            case "focusout":
              N = "blur", y = g0;
              break;
            case "beforeblur":
            case "afterblur":
              y = g0;
              break;
            case "click":
              if (l.button === 2) break e;
            case "auxclick":
            case "dblclick":
            case "mousedown":
            case "mousemove":
            case "mouseup":
            case "mouseout":
            case "mouseover":
            case "contextmenu":
              y = k4;
              break;
            case "drag":
            case "dragend":
            case "dragenter":
            case "dragexit":
            case "dragleave":
            case "dragover":
            case "dragstart":
            case "drop":
              y = vr;
              break;
            case "touchcancel":
            case "touchend":
            case "touchmove":
            case "touchstart":
              y = Sr;
              break;
            case E5:
            case z5:
            case p5:
              y = or;
              break;
            case g5:
              y = Br;
              break;
            case "scroll":
            case "scrollend":
              y = hr;
              break;
            case "wheel":
              y = Tr;
              break;
            case "copy":
            case "cut":
            case "paste":
              y = zr;
              break;
            case "gotpointercapture":
            case "lostpointercapture":
            case "pointercancel":
            case "pointerdown":
            case "pointermove":
            case "pointerout":
            case "pointerover":
            case "pointerup":
              y = $4;
              break;
            case "toggle":
            case "beforetoggle":
              y = Vr;
          }
          var q = (t & 4) !== 0, pe = !q && (e === "scroll" || e === "scrollend"), g = q ? x !== null ? x + "Capture" : null : x;
          q = [];
          for (var z = w, M; z !== null; ) {
            var S = z;
            if (M = S.stateNode, S = S.tag, S !== 5 && S !== 26 && S !== 27 || M === null || g === null || (S = jl(z, g), S != null && q.push(
              g2(z, S, M)
            )), pe) break;
            z = z.return;
          }
          0 < q.length && (x = new y(
            x,
            N,
            null,
            l,
            D
          ), A.push({ event: x, listeners: q }));
        }
      }
      if ((t & 7) === 0) {
        e: {
          if (x = e === "mouseover" || e === "pointerover", y = e === "mouseout" || e === "pointerout", x && l !== v0 && (N = l.relatedTarget || l.fromElement) && (It(N) || N[$t]))
            break e;
          if ((y || x) && (x = D.window === D ? D : (x = D.ownerDocument) ? x.defaultView || x.parentWindow : window, y ? (N = l.relatedTarget || l.toElement, y = w, N = N ? It(N) : null, N !== null && (pe = G(N), q = N.tag, N !== pe || q !== 5 && q !== 27 && q !== 6) && (N = null)) : (y = null, N = w), y !== N)) {
            if (q = k4, S = "onMouseLeave", g = "onMouseEnter", z = "mouse", (e === "pointerout" || e === "pointerover") && (q = $4, S = "onPointerLeave", g = "onPointerEnter", z = "pointer"), pe = y == null ? x : Rl(y), M = N == null ? x : Rl(N), x = new q(
              S,
              z + "leave",
              y,
              l,
              D
            ), x.target = pe, x.relatedTarget = M, S = null, It(D) === w && (q = new q(
              g,
              z + "enter",
              N,
              l,
              D
            ), q.target = M, q.relatedTarget = pe, S = q), pe = S, y && N)
              t: {
                for (q = Vf, g = y, z = N, M = 0, S = g; S; S = q(S))
                  M++;
                S = 0;
                for (var j = z; j; j = q(j))
                  S++;
                for (; 0 < M - S; )
                  g = q(g), M--;
                for (; 0 < S - M; )
                  z = q(z), S--;
                for (; M--; ) {
                  if (g === z || z !== null && g === z.alternate) {
                    q = g;
                    break t;
                  }
                  g = q(g), z = q(z);
                }
                q = null;
              }
            else q = null;
            y !== null && qc(
              A,
              x,
              y,
              q,
              !1
            ), N !== null && pe !== null && qc(
              A,
              pe,
              N,
              q,
              !0
            );
          }
        }
        e: {
          if (x = w ? Rl(w) : window, y = x.nodeName && x.nodeName.toLowerCase(), y === "select" || y === "input" && x.type === "file")
            var re = c5;
          else if (a5(x))
            if (i5)
              re = qr;
            else {
              re = Gr;
              var U = jr;
            }
          else
            y = x.nodeName, !y || y.toLowerCase() !== "input" || x.type !== "checkbox" && x.type !== "radio" ? w && m0(w.elementType) && (re = c5) : re = Yr;
          if (re && (re = re(e, w))) {
            n5(
              A,
              re,
              l,
              D
            );
            break e;
          }
          U && U(e, x, w), e === "focusout" && w && x.type === "number" && w.memoizedProps.value != null && h0(x, "number", x.value);
        }
        switch (U = w ? Rl(w) : window, e) {
          case "focusin":
            (a5(U) || U.contentEditable === "true") && (ul = U, F0 = w, Jl = null);
            break;
          case "focusout":
            Jl = F0 = ul = null;
            break;
          case "mousedown":
            D0 = !0;
            break;
          case "contextmenu":
          case "mouseup":
          case "dragend":
            D0 = !1, d5(A, l, D);
            break;
          case "selectionchange":
            if (Xr) break;
          case "keydown":
          case "keyup":
            d5(A, l, D);
        }
        var K;
        if (w0)
          e: {
            switch (e) {
              case "compositionstart":
                var le = "onCompositionStart";
                break e;
              case "compositionend":
                le = "onCompositionEnd";
                break e;
              case "compositionupdate":
                le = "onCompositionUpdate";
                break e;
            }
            le = void 0;
          }
        else
          il ? t5(e, l) && (le = "onCompositionEnd") : e === "keydown" && l.keyCode === 229 && (le = "onCompositionStart");
        le && (I4 && l.locale !== "ko" && (il || le !== "onCompositionStart" ? le === "onCompositionEnd" && il && (K = K4()) : (rt = D, E0 = "value" in rt ? rt.value : rt.textContent, il = !0)), U = C3(w, le), 0 < U.length && (le = new W4(
          le,
          e,
          null,
          l,
          D
        ), A.push({ event: le, listeners: U }), K ? le.data = K : (K = l5(l), K !== null && (le.data = K)))), (K = Or ? Nr(e, l) : _r(e, l)) && (le = C3(w, "onBeforeInput"), 0 < le.length && (U = new W4(
          "onBeforeInput",
          "beforeinput",
          null,
          l,
          D
        ), A.push({
          event: U,
          listeners: le
        }), U.data = K)), Bf(
          A,
          e,
          w,
          l,
          D
        );
      }
      Gc(A, t);
    });
  }
  function g2(e, t, l) {
    return {
      instance: e,
      listener: t,
      currentTarget: l
    };
  }
  function C3(e, t) {
    for (var l = t + "Capture", n = []; e !== null; ) {
      var c = e, r = c.stateNode;
      if (c = c.tag, c !== 5 && c !== 26 && c !== 27 || r === null || (c = jl(e, l), c != null && n.unshift(
        g2(e, c, r)
      ), c = jl(e, t), c != null && n.push(
        g2(e, c, r)
      )), e.tag === 3) return n;
      e = e.return;
    }
    return [];
  }
  function Vf(e) {
    if (e === null) return null;
    do
      e = e.return;
    while (e && e.tag !== 5 && e.tag !== 27);
    return e || null;
  }
  function qc(e, t, l, n, c) {
    for (var r = t._reactName, h = []; l !== null && l !== n; ) {
      var v = l, o = v.alternate, w = v.stateNode;
      if (v = v.tag, o !== null && o === n) break;
      v !== 5 && v !== 26 && v !== 27 || w === null || (o = w, c ? (w = jl(l, r), w != null && h.unshift(
        g2(l, w, o)
      )) : c || (w = jl(l, r), w != null && h.push(
        g2(l, w, o)
      ))), l = l.return;
    }
    h.length !== 0 && e.push({ event: t, listeners: h });
  }
  var Cf = /\r\n?/g, Of = /\u0000|\uFFFD/g;
  function Qc(e) {
    return (typeof e == "string" ? e : "" + e).replace(Cf, `
`).replace(Of, "");
  }
  function Xc(e, t) {
    return t = Qc(t), Qc(e) === t;
  }
  function ze(e, t, l, n, c, r) {
    switch (l) {
      case "children":
        typeof n == "string" ? t === "body" || t === "textarea" && n === "" || al(e, n) : (typeof n == "number" || typeof n == "bigint") && t !== "body" && al(e, "" + n);
        break;
      case "className":
        U2(e, "class", n);
        break;
      case "tabIndex":
        U2(e, "tabindex", n);
        break;
      case "dir":
      case "role":
      case "viewBox":
      case "width":
      case "height":
        U2(e, l, n);
        break;
      case "style":
        Q4(e, n, r);
        break;
      case "data":
        if (t !== "object") {
          U2(e, "data", n);
          break;
        }
      case "src":
      case "href":
        if (n === "" && (t !== "a" || l !== "href")) {
          e.removeAttribute(l);
          break;
        }
        if (n == null || typeof n == "function" || typeof n == "symbol" || typeof n == "boolean") {
          e.removeAttribute(l);
          break;
        }
        n = j2("" + n), e.setAttribute(l, n);
        break;
      case "action":
      case "formAction":
        if (typeof n == "function") {
          e.setAttribute(
            l,
            "javascript:throw new Error('A React form was unexpectedly submitted. If you called form.submit() manually, consider using form.requestSubmit() instead. If you\\'re trying to use event.stopPropagation() in a submit event handler, consider also calling event.preventDefault().')"
          );
          break;
        } else
          typeof r == "function" && (l === "formAction" ? (t !== "input" && ze(e, t, "name", c.name, c, null), ze(
            e,
            t,
            "formEncType",
            c.formEncType,
            c,
            null
          ), ze(
            e,
            t,
            "formMethod",
            c.formMethod,
            c,
            null
          ), ze(
            e,
            t,
            "formTarget",
            c.formTarget,
            c,
            null
          )) : (ze(e, t, "encType", c.encType, c, null), ze(e, t, "method", c.method, c, null), ze(e, t, "target", c.target, c, null)));
        if (n == null || typeof n == "symbol" || typeof n == "boolean") {
          e.removeAttribute(l);
          break;
        }
        n = j2("" + n), e.setAttribute(l, n);
        break;
      case "onClick":
        n != null && (e.onclick = q1);
        break;
      case "onScroll":
        n != null && ee("scroll", e);
        break;
      case "onScrollEnd":
        n != null && ee("scrollend", e);
        break;
      case "dangerouslySetInnerHTML":
        if (n != null) {
          if (typeof n != "object" || !("__html" in n))
            throw Error(E(61));
          if (l = n.__html, l != null) {
            if (c.children != null) throw Error(E(60));
            e.innerHTML = l;
          }
        }
        break;
      case "multiple":
        e.multiple = n && typeof n != "function" && typeof n != "symbol";
        break;
      case "muted":
        e.muted = n && typeof n != "function" && typeof n != "symbol";
        break;
      case "suppressContentEditableWarning":
      case "suppressHydrationWarning":
      case "defaultValue":
      case "defaultChecked":
      case "innerHTML":
      case "ref":
        break;
      case "autoFocus":
        break;
      case "xlinkHref":
        if (n == null || typeof n == "function" || typeof n == "boolean" || typeof n == "symbol") {
          e.removeAttribute("xlink:href");
          break;
        }
        l = j2("" + n), e.setAttributeNS(
          "http://www.w3.org/1999/xlink",
          "xlink:href",
          l
        );
        break;
      case "contentEditable":
      case "spellCheck":
      case "draggable":
      case "value":
      case "autoReverse":
      case "externalResourcesRequired":
      case "focusable":
      case "preserveAlpha":
        n != null && typeof n != "function" && typeof n != "symbol" ? e.setAttribute(l, "" + n) : e.removeAttribute(l);
        break;
      case "inert":
      case "allowFullScreen":
      case "async":
      case "autoPlay":
      case "controls":
      case "default":
      case "defer":
      case "disabled":
      case "disablePictureInPicture":
      case "disableRemotePlayback":
      case "formNoValidate":
      case "hidden":
      case "loop":
      case "noModule":
      case "noValidate":
      case "open":
      case "playsInline":
      case "readOnly":
      case "required":
      case "reversed":
      case "scoped":
      case "seamless":
      case "itemScope":
        n && typeof n != "function" && typeof n != "symbol" ? e.setAttribute(l, "") : e.removeAttribute(l);
        break;
      case "capture":
      case "download":
        n === !0 ? e.setAttribute(l, "") : n !== !1 && n != null && typeof n != "function" && typeof n != "symbol" ? e.setAttribute(l, n) : e.removeAttribute(l);
        break;
      case "cols":
      case "rows":
      case "size":
      case "span":
        n != null && typeof n != "function" && typeof n != "symbol" && !isNaN(n) && 1 <= n ? e.setAttribute(l, n) : e.removeAttribute(l);
        break;
      case "rowSpan":
      case "start":
        n == null || typeof n == "function" || typeof n == "symbol" || isNaN(n) ? e.removeAttribute(l) : e.setAttribute(l, n);
        break;
      case "popover":
        ee("beforetoggle", e), ee("toggle", e), _2(e, "popover", n);
        break;
      case "xlinkActuate":
        Y1(
          e,
          "http://www.w3.org/1999/xlink",
          "xlink:actuate",
          n
        );
        break;
      case "xlinkArcrole":
        Y1(
          e,
          "http://www.w3.org/1999/xlink",
          "xlink:arcrole",
          n
        );
        break;
      case "xlinkRole":
        Y1(
          e,
          "http://www.w3.org/1999/xlink",
          "xlink:role",
          n
        );
        break;
      case "xlinkShow":
        Y1(
          e,
          "http://www.w3.org/1999/xlink",
          "xlink:show",
          n
        );
        break;
      case "xlinkTitle":
        Y1(
          e,
          "http://www.w3.org/1999/xlink",
          "xlink:title",
          n
        );
        break;
      case "xlinkType":
        Y1(
          e,
          "http://www.w3.org/1999/xlink",
          "xlink:type",
          n
        );
        break;
      case "xmlBase":
        Y1(
          e,
          "http://www.w3.org/XML/1998/namespace",
          "xml:base",
          n
        );
        break;
      case "xmlLang":
        Y1(
          e,
          "http://www.w3.org/XML/1998/namespace",
          "xml:lang",
          n
        );
        break;
      case "xmlSpace":
        Y1(
          e,
          "http://www.w3.org/XML/1998/namespace",
          "xml:space",
          n
        );
        break;
      case "is":
        _2(e, "is", n);
        break;
      case "innerText":
      case "textContent":
        break;
      default:
        (!(2 < l.length) || l[0] !== "o" && l[0] !== "O" || l[1] !== "n" && l[1] !== "N") && (l = rr.get(l) || l, _2(e, l, n));
    }
  }
  function $a(e, t, l, n, c, r) {
    switch (l) {
      case "style":
        Q4(e, n, r);
        break;
      case "dangerouslySetInnerHTML":
        if (n != null) {
          if (typeof n != "object" || !("__html" in n))
            throw Error(E(61));
          if (l = n.__html, l != null) {
            if (c.children != null) throw Error(E(60));
            e.innerHTML = l;
          }
        }
        break;
      case "children":
        typeof n == "string" ? al(e, n) : (typeof n == "number" || typeof n == "bigint") && al(e, "" + n);
        break;
      case "onScroll":
        n != null && ee("scroll", e);
        break;
      case "onScrollEnd":
        n != null && ee("scrollend", e);
        break;
      case "onClick":
        n != null && (e.onclick = q1);
        break;
      case "suppressContentEditableWarning":
      case "suppressHydrationWarning":
      case "innerHTML":
      case "ref":
        break;
      case "innerText":
      case "textContent":
        break;
      default:
        if (!O4.hasOwnProperty(l))
          e: {
            if (l[0] === "o" && l[1] === "n" && (c = l.endsWith("Capture"), t = l.slice(2, c ? l.length - 7 : void 0), r = e[l1] || null, r = r != null ? r[l] : null, typeof r == "function" && e.removeEventListener(t, r, c), typeof n == "function")) {
              typeof r != "function" && r !== null && (l in e ? e[l] = null : e.hasAttribute(l) && e.removeAttribute(l)), e.addEventListener(t, n, c);
              break e;
            }
            l in e ? e[l] = n : n === !0 ? e.setAttribute(l, "") : _2(e, l, n);
          }
    }
  }
  function We(e, t, l) {
    switch (t) {
      case "div":
      case "span":
      case "svg":
      case "path":
      case "a":
      case "g":
      case "p":
      case "li":
        break;
      case "img":
        ee("error", e), ee("load", e);
        var n = !1, c = !1, r;
        for (r in l)
          if (l.hasOwnProperty(r)) {
            var h = l[r];
            if (h != null)
              switch (r) {
                case "src":
                  n = !0;
                  break;
                case "srcSet":
                  c = !0;
                  break;
                case "children":
                case "dangerouslySetInnerHTML":
                  throw Error(E(137, t));
                default:
                  ze(e, t, r, h, l, null);
              }
          }
        c && ze(e, t, "srcSet", l.srcSet, l, null), n && ze(e, t, "src", l.src, l, null);
        return;
      case "input":
        ee("invalid", e);
        var v = r = h = c = null, o = null, w = null;
        for (n in l)
          if (l.hasOwnProperty(n)) {
            var D = l[n];
            if (D != null)
              switch (n) {
                case "name":
                  c = D;
                  break;
                case "type":
                  h = D;
                  break;
                case "checked":
                  o = D;
                  break;
                case "defaultChecked":
                  w = D;
                  break;
                case "value":
                  r = D;
                  break;
                case "defaultValue":
                  v = D;
                  break;
                case "children":
                case "dangerouslySetInnerHTML":
                  if (D != null)
                    throw Error(E(137, t));
                  break;
                default:
                  ze(e, t, n, D, l, null);
              }
          }
        j4(
          e,
          r,
          v,
          o,
          w,
          h,
          c,
          !1
        );
        return;
      case "select":
        ee("invalid", e), n = h = r = null;
        for (c in l)
          if (l.hasOwnProperty(c) && (v = l[c], v != null))
            switch (c) {
              case "value":
                r = v;
                break;
              case "defaultValue":
                h = v;
                break;
              case "multiple":
                n = v;
              default:
                ze(e, t, c, v, l, null);
            }
        t = r, l = h, e.multiple = !!n, t != null ? ll(e, !!n, t, !1) : l != null && ll(e, !!n, l, !0);
        return;
      case "textarea":
        ee("invalid", e), r = c = n = null;
        for (h in l)
          if (l.hasOwnProperty(h) && (v = l[h], v != null))
            switch (h) {
              case "value":
                n = v;
                break;
              case "defaultValue":
                c = v;
                break;
              case "children":
                r = v;
                break;
              case "dangerouslySetInnerHTML":
                if (v != null) throw Error(E(91));
                break;
              default:
                ze(e, t, h, v, l, null);
            }
        Y4(e, n, c, r);
        return;
      case "option":
        for (o in l)
          l.hasOwnProperty(o) && (n = l[o], n != null) && (o === "selected" ? e.selected = n && typeof n != "function" && typeof n != "symbol" : ze(e, t, o, n, l, null));
        return;
      case "dialog":
        ee("beforetoggle", e), ee("toggle", e), ee("cancel", e), ee("close", e);
        break;
      case "iframe":
      case "object":
        ee("load", e);
        break;
      case "video":
      case "audio":
        for (n = 0; n < p2.length; n++)
          ee(p2[n], e);
        break;
      case "image":
        ee("error", e), ee("load", e);
        break;
      case "details":
        ee("toggle", e);
        break;
      case "embed":
      case "source":
      case "link":
        ee("error", e), ee("load", e);
      case "area":
      case "base":
      case "br":
      case "col":
      case "hr":
      case "keygen":
      case "meta":
      case "param":
      case "track":
      case "wbr":
      case "menuitem":
        for (w in l)
          if (l.hasOwnProperty(w) && (n = l[w], n != null))
            switch (w) {
              case "children":
              case "dangerouslySetInnerHTML":
                throw Error(E(137, t));
              default:
                ze(e, t, w, n, l, null);
            }
        return;
      default:
        if (m0(t)) {
          for (D in l)
            l.hasOwnProperty(D) && (n = l[D], n !== void 0 && $a(
              e,
              t,
              D,
              n,
              l,
              void 0
            ));
          return;
        }
    }
    for (v in l)
      l.hasOwnProperty(v) && (n = l[v], n != null && ze(e, t, v, n, l, null));
  }
  function Nf(e, t, l, n) {
    switch (t) {
      case "div":
      case "span":
      case "svg":
      case "path":
      case "a":
      case "g":
      case "p":
      case "li":
        break;
      case "input":
        var c = null, r = null, h = null, v = null, o = null, w = null, D = null;
        for (y in l) {
          var A = l[y];
          if (l.hasOwnProperty(y) && A != null)
            switch (y) {
              case "checked":
                break;
              case "value":
                break;
              case "defaultValue":
                o = A;
              default:
                n.hasOwnProperty(y) || ze(e, t, y, null, n, A);
            }
        }
        for (var x in n) {
          var y = n[x];
          if (A = l[x], n.hasOwnProperty(x) && (y != null || A != null))
            switch (x) {
              case "type":
                r = y;
                break;
              case "name":
                c = y;
                break;
              case "checked":
                w = y;
                break;
              case "defaultChecked":
                D = y;
                break;
              case "value":
                h = y;
                break;
              case "defaultValue":
                v = y;
                break;
              case "children":
              case "dangerouslySetInnerHTML":
                if (y != null)
                  throw Error(E(137, t));
                break;
              default:
                y !== A && ze(
                  e,
                  t,
                  x,
                  y,
                  n,
                  A
                );
            }
        }
        f0(
          e,
          h,
          v,
          o,
          w,
          D,
          r,
          c
        );
        return;
      case "select":
        y = h = v = x = null;
        for (r in l)
          if (o = l[r], l.hasOwnProperty(r) && o != null)
            switch (r) {
              case "value":
                break;
              case "multiple":
                y = o;
              default:
                n.hasOwnProperty(r) || ze(
                  e,
                  t,
                  r,
                  null,
                  n,
                  o
                );
            }
        for (c in n)
          if (r = n[c], o = l[c], n.hasOwnProperty(c) && (r != null || o != null))
            switch (c) {
              case "value":
                x = r;
                break;
              case "defaultValue":
                v = r;
                break;
              case "multiple":
                h = r;
              default:
                r !== o && ze(
                  e,
                  t,
                  c,
                  r,
                  n,
                  o
                );
            }
        t = v, l = h, n = y, x != null ? ll(e, !!l, x, !1) : !!n != !!l && (t != null ? ll(e, !!l, t, !0) : ll(e, !!l, l ? [] : "", !1));
        return;
      case "textarea":
        y = x = null;
        for (v in l)
          if (c = l[v], l.hasOwnProperty(v) && c != null && !n.hasOwnProperty(v))
            switch (v) {
              case "value":
                break;
              case "children":
                break;
              default:
                ze(e, t, v, null, n, c);
            }
        for (h in n)
          if (c = n[h], r = l[h], n.hasOwnProperty(h) && (c != null || r != null))
            switch (h) {
              case "value":
                x = c;
                break;
              case "defaultValue":
                y = c;
                break;
              case "children":
                break;
              case "dangerouslySetInnerHTML":
                if (c != null) throw Error(E(91));
                break;
              default:
                c !== r && ze(e, t, h, c, n, r);
            }
        G4(e, x, y);
        return;
      case "option":
        for (var N in l)
          x = l[N], l.hasOwnProperty(N) && x != null && !n.hasOwnProperty(N) && (N === "selected" ? e.selected = !1 : ze(
            e,
            t,
            N,
            null,
            n,
            x
          ));
        for (o in n)
          x = n[o], y = l[o], n.hasOwnProperty(o) && x !== y && (x != null || y != null) && (o === "selected" ? e.selected = x && typeof x != "function" && typeof x != "symbol" : ze(
            e,
            t,
            o,
            x,
            n,
            y
          ));
        return;
      case "img":
      case "link":
      case "area":
      case "base":
      case "br":
      case "col":
      case "embed":
      case "hr":
      case "keygen":
      case "meta":
      case "param":
      case "source":
      case "track":
      case "wbr":
      case "menuitem":
        for (var q in l)
          x = l[q], l.hasOwnProperty(q) && x != null && !n.hasOwnProperty(q) && ze(e, t, q, null, n, x);
        for (w in n)
          if (x = n[w], y = l[w], n.hasOwnProperty(w) && x !== y && (x != null || y != null))
            switch (w) {
              case "children":
              case "dangerouslySetInnerHTML":
                if (x != null)
                  throw Error(E(137, t));
                break;
              default:
                ze(
                  e,
                  t,
                  w,
                  x,
                  n,
                  y
                );
            }
        return;
      default:
        if (m0(t)) {
          for (var pe in l)
            x = l[pe], l.hasOwnProperty(pe) && x !== void 0 && !n.hasOwnProperty(pe) && $a(
              e,
              t,
              pe,
              void 0,
              n,
              x
            );
          for (D in n)
            x = n[D], y = l[D], !n.hasOwnProperty(D) || x === y || x === void 0 && y === void 0 || $a(
              e,
              t,
              D,
              x,
              n,
              y
            );
          return;
        }
    }
    for (var g in l)
      x = l[g], l.hasOwnProperty(g) && x != null && !n.hasOwnProperty(g) && ze(e, t, g, null, n, x);
    for (A in n)
      x = n[A], y = l[A], !n.hasOwnProperty(A) || x === y || x == null && y == null || ze(e, t, A, x, n, y);
  }
  function Zc(e) {
    switch (e) {
      case "css":
      case "script":
      case "font":
      case "img":
      case "image":
      case "input":
      case "link":
        return !0;
      default:
        return !1;
    }
  }
  function _f() {
    if (typeof performance.getEntriesByType == "function") {
      for (var e = 0, t = 0, l = performance.getEntriesByType("resource"), n = 0; n < l.length; n++) {
        var c = l[n], r = c.transferSize, h = c.initiatorType, v = c.duration;
        if (r && v && Zc(h)) {
          for (h = 0, v = c.responseEnd, n += 1; n < l.length; n++) {
            var o = l[n], w = o.startTime;
            if (w > v) break;
            var D = o.transferSize, A = o.initiatorType;
            D && Zc(A) && (o = o.responseEnd, h += D * (o < v ? 1 : (v - w) / (o - w)));
          }
          if (--n, t += 8 * (r + h) / (c.duration / 1e3), e++, 10 < e) break;
        }
      }
      if (0 < e) return t / e / 1e6;
    }
    return navigator.connection && (e = navigator.connection.downlink, typeof e == "number") ? e : 5;
  }
  var Ia = null, Pa = null;
  function O3(e) {
    return e.nodeType === 9 ? e : e.ownerDocument;
  }
  function Kc(e) {
    switch (e) {
      case "http://www.w3.org/2000/svg":
        return 1;
      case "http://www.w3.org/1998/Math/MathML":
        return 2;
      default:
        return 0;
    }
  }
  function Jc(e, t) {
    if (e === 0)
      switch (t) {
        case "svg":
          return 1;
        case "math":
          return 2;
        default:
          return 0;
      }
    return e === 1 && t === "foreignObject" ? 0 : e;
  }
  function e4(e, t) {
    return e === "textarea" || e === "noscript" || typeof t.children == "string" || typeof t.children == "number" || typeof t.children == "bigint" || typeof t.dangerouslySetInnerHTML == "object" && t.dangerouslySetInnerHTML !== null && t.dangerouslySetInnerHTML.__html != null;
  }
  var t4 = null;
  function Uf() {
    var e = window.event;
    return e && e.type === "popstate" ? e === t4 ? !1 : (t4 = e, !0) : (t4 = null, !1);
  }
  var kc = typeof setTimeout == "function" ? setTimeout : void 0, Rf = typeof clearTimeout == "function" ? clearTimeout : void 0, Wc = typeof Promise == "function" ? Promise : void 0, jf = typeof queueMicrotask == "function" ? queueMicrotask : typeof Wc < "u" ? function(e) {
    return Wc.resolve(null).then(e).catch(Gf);
  } : kc;
  function Gf(e) {
    setTimeout(function() {
      throw e;
    });
  }
  function Ft(e) {
    return e === "head";
  }
  function $c(e, t) {
    var l = t, n = 0;
    do {
      var c = l.nextSibling;
      if (e.removeChild(l), c && c.nodeType === 8)
        if (l = c.data, l === "/$" || l === "/&") {
          if (n === 0) {
            e.removeChild(c), Cl(t);
            return;
          }
          n--;
        } else if (l === "$" || l === "$?" || l === "$~" || l === "$!" || l === "&")
          n++;
        else if (l === "html")
          M2(e.ownerDocument.documentElement);
        else if (l === "head") {
          l = e.ownerDocument.head, M2(l);
          for (var r = l.firstChild; r; ) {
            var h = r.nextSibling, v = r.nodeName;
            r[Ul] || v === "SCRIPT" || v === "STYLE" || v === "LINK" && r.rel.toLowerCase() === "stylesheet" || l.removeChild(r), r = h;
          }
        } else
          l === "body" && M2(e.ownerDocument.body);
      l = c;
    } while (l);
    Cl(t);
  }
  function Ic(e, t) {
    var l = e;
    e = 0;
    do {
      var n = l.nextSibling;
      if (l.nodeType === 1 ? t ? (l._stashedDisplay = l.style.display, l.style.display = "none") : (l.style.display = l._stashedDisplay || "", l.getAttribute("style") === "" && l.removeAttribute("style")) : l.nodeType === 3 && (t ? (l._stashedText = l.nodeValue, l.nodeValue = "") : l.nodeValue = l._stashedText || ""), n && n.nodeType === 8)
        if (l = n.data, l === "/$") {
          if (e === 0) break;
          e--;
        } else
          l !== "$" && l !== "$?" && l !== "$~" && l !== "$!" || e++;
      l = n;
    } while (l);
  }
  function l4(e) {
    var t = e.firstChild;
    for (t && t.nodeType === 10 && (t = t.nextSibling); t; ) {
      var l = t;
      switch (t = t.nextSibling, l.nodeName) {
        case "HTML":
        case "HEAD":
        case "BODY":
          l4(l), u0(l);
          continue;
        case "SCRIPT":
        case "STYLE":
          continue;
        case "LINK":
          if (l.rel.toLowerCase() === "stylesheet") continue;
      }
      e.removeChild(l);
    }
  }
  function Yf(e, t, l, n) {
    for (; e.nodeType === 1; ) {
      var c = l;
      if (e.nodeName.toLowerCase() !== t.toLowerCase()) {
        if (!n && (e.nodeName !== "INPUT" || e.type !== "hidden"))
          break;
      } else if (n) {
        if (!e[Ul])
          switch (t) {
            case "meta":
              if (!e.hasAttribute("itemprop")) break;
              return e;
            case "link":
              if (r = e.getAttribute("rel"), r === "stylesheet" && e.hasAttribute("data-precedence"))
                break;
              if (r !== c.rel || e.getAttribute("href") !== (c.href == null || c.href === "" ? null : c.href) || e.getAttribute("crossorigin") !== (c.crossOrigin == null ? null : c.crossOrigin) || e.getAttribute("title") !== (c.title == null ? null : c.title))
                break;
              return e;
            case "style":
              if (e.hasAttribute("data-precedence")) break;
              return e;
            case "script":
              if (r = e.getAttribute("src"), (r !== (c.src == null ? null : c.src) || e.getAttribute("type") !== (c.type == null ? null : c.type) || e.getAttribute("crossorigin") !== (c.crossOrigin == null ? null : c.crossOrigin)) && r && e.hasAttribute("async") && !e.hasAttribute("itemprop"))
                break;
              return e;
            default:
              return e;
          }
      } else if (t === "input" && e.type === "hidden") {
        var r = c.name == null ? null : "" + c.name;
        if (c.type === "hidden" && e.getAttribute("name") === r)
          return e;
      } else return e;
      if (e = L1(e.nextSibling), e === null) break;
    }
    return null;
  }
  function qf(e, t, l) {
    if (t === "") return null;
    for (; e.nodeType !== 3; )
      if ((e.nodeType !== 1 || e.nodeName !== "INPUT" || e.type !== "hidden") && !l || (e = L1(e.nextSibling), e === null)) return null;
    return e;
  }
  function Pc(e, t) {
    for (; e.nodeType !== 8; )
      if ((e.nodeType !== 1 || e.nodeName !== "INPUT" || e.type !== "hidden") && !t || (e = L1(e.nextSibling), e === null)) return null;
    return e;
  }
  function a4(e) {
    return e.data === "$?" || e.data === "$~";
  }
  function n4(e) {
    return e.data === "$!" || e.data === "$?" && e.ownerDocument.readyState !== "loading";
  }
  function Qf(e, t) {
    var l = e.ownerDocument;
    if (e.data === "$~") e._reactRetry = t;
    else if (e.data !== "$?" || l.readyState !== "loading")
      t();
    else {
      var n = function() {
        t(), l.removeEventListener("DOMContentLoaded", n);
      };
      l.addEventListener("DOMContentLoaded", n), e._reactRetry = n;
    }
  }
  function L1(e) {
    for (; e != null; e = e.nextSibling) {
      var t = e.nodeType;
      if (t === 1 || t === 3) break;
      if (t === 8) {
        if (t = e.data, t === "$" || t === "$!" || t === "$?" || t === "$~" || t === "&" || t === "F!" || t === "F")
          break;
        if (t === "/$" || t === "/&") return null;
      }
    }
    return e;
  }
  var c4 = null;
  function e6(e) {
    e = e.nextSibling;
    for (var t = 0; e; ) {
      if (e.nodeType === 8) {
        var l = e.data;
        if (l === "/$" || l === "/&") {
          if (t === 0)
            return L1(e.nextSibling);
          t--;
        } else
          l !== "$" && l !== "$!" && l !== "$?" && l !== "$~" && l !== "&" || t++;
      }
      e = e.nextSibling;
    }
    return null;
  }
  function t6(e) {
    e = e.previousSibling;
    for (var t = 0; e; ) {
      if (e.nodeType === 8) {
        var l = e.data;
        if (l === "$" || l === "$!" || l === "$?" || l === "$~" || l === "&") {
          if (t === 0) return e;
          t--;
        } else l !== "/$" && l !== "/&" || t++;
      }
      e = e.previousSibling;
    }
    return null;
  }
  function l6(e, t, l) {
    switch (t = O3(l), e) {
      case "html":
        if (e = t.documentElement, !e) throw Error(E(452));
        return e;
      case "head":
        if (e = t.head, !e) throw Error(E(453));
        return e;
      case "body":
        if (e = t.body, !e) throw Error(E(454));
        return e;
      default:
        throw Error(E(451));
    }
  }
  function M2(e) {
    for (var t = e.attributes; t.length; )
      e.removeAttributeNode(t[0]);
    u0(e);
  }
  var V1 = /* @__PURE__ */ new Map(), a6 = /* @__PURE__ */ new Set();
  function N3(e) {
    return typeof e.getRootNode == "function" ? e.getRootNode() : e.nodeType === 9 ? e : e.ownerDocument;
  }
  var ct = L.d;
  L.d = {
    f: Xf,
    r: Zf,
    D: Kf,
    C: Jf,
    L: kf,
    m: Wf,
    X: If,
    S: $f,
    M: Pf
  };
  function Xf() {
    var e = ct.f(), t = S3();
    return e || t;
  }
  function Zf(e) {
    var t = Pt(e);
    t !== null && t.tag === 5 && t.type === "form" ? Mn(t) : ct.r(e);
  }
  var Tl = typeof document > "u" ? null : document;
  function n6(e, t, l) {
    var n = Tl;
    if (n && typeof t == "string" && t) {
      var c = F1(t);
      c = 'link[rel="' + e + '"][href="' + c + '"]', typeof l == "string" && (c += '[crossorigin="' + l + '"]'), a6.has(c) || (a6.add(c), e = { rel: e, crossOrigin: l, href: t }, n.querySelector(c) === null && (t = n.createElement("link"), We(t, "link", e), Ye(t), n.head.appendChild(t)));
    }
  }
  function Kf(e) {
    ct.D(e), n6("dns-prefetch", e, null);
  }
  function Jf(e, t) {
    ct.C(e, t), n6("preconnect", e, t);
  }
  function kf(e, t, l) {
    ct.L(e, t, l);
    var n = Tl;
    if (n && e && t) {
      var c = 'link[rel="preload"][as="' + F1(t) + '"]';
      t === "image" && l && l.imageSrcSet ? (c += '[imagesrcset="' + F1(
        l.imageSrcSet
      ) + '"]', typeof l.imageSizes == "string" && (c += '[imagesizes="' + F1(
        l.imageSizes
      ) + '"]')) : c += '[href="' + F1(e) + '"]';
      var r = c;
      switch (t) {
        case "style":
          r = Ll(e);
          break;
        case "script":
          r = Vl(e);
      }
      V1.has(r) || (e = Y(
        {
          rel: "preload",
          href: t === "image" && l && l.imageSrcSet ? void 0 : e,
          as: t
        },
        l
      ), V1.set(r, e), n.querySelector(c) !== null || t === "style" && n.querySelector(w2(r)) || t === "script" && n.querySelector(x2(r)) || (t = n.createElement("link"), We(t, "link", e), Ye(t), n.head.appendChild(t)));
    }
  }
  function Wf(e, t) {
    ct.m(e, t);
    var l = Tl;
    if (l && e) {
      var n = t && typeof t.as == "string" ? t.as : "script", c = 'link[rel="modulepreload"][as="' + F1(n) + '"][href="' + F1(e) + '"]', r = c;
      switch (n) {
        case "audioworklet":
        case "paintworklet":
        case "serviceworker":
        case "sharedworker":
        case "worker":
        case "script":
          r = Vl(e);
      }
      if (!V1.has(r) && (e = Y({ rel: "modulepreload", href: e }, t), V1.set(r, e), l.querySelector(c) === null)) {
        switch (n) {
          case "audioworklet":
          case "paintworklet":
          case "serviceworker":
          case "sharedworker":
          case "worker":
          case "script":
            if (l.querySelector(x2(r)))
              return;
        }
        n = l.createElement("link"), We(n, "link", e), Ye(n), l.head.appendChild(n);
      }
    }
  }
  function $f(e, t, l) {
    ct.S(e, t, l);
    var n = Tl;
    if (n && e) {
      var c = el(n).hoistableStyles, r = Ll(e);
      t = t || "default";
      var h = c.get(r);
      if (!h) {
        var v = { loading: 0, preload: null };
        if (h = n.querySelector(
          w2(r)
        ))
          v.loading = 5;
        else {
          e = Y(
            { rel: "stylesheet", href: e, "data-precedence": t },
            l
          ), (l = V1.get(r)) && i4(e, l);
          var o = h = n.createElement("link");
          Ye(o), We(o, "link", e), o._p = new Promise(function(w, D) {
            o.onload = w, o.onerror = D;
          }), o.addEventListener("load", function() {
            v.loading |= 1;
          }), o.addEventListener("error", function() {
            v.loading |= 2;
          }), v.loading |= 4, _3(h, t, n);
        }
        h = {
          type: "stylesheet",
          instance: h,
          count: 1,
          state: v
        }, c.set(r, h);
      }
    }
  }
  function If(e, t) {
    ct.X(e, t);
    var l = Tl;
    if (l && e) {
      var n = el(l).hoistableScripts, c = Vl(e), r = n.get(c);
      r || (r = l.querySelector(x2(c)), r || (e = Y({ src: e, async: !0 }, t), (t = V1.get(c)) && u4(e, t), r = l.createElement("script"), Ye(r), We(r, "link", e), l.head.appendChild(r)), r = {
        type: "script",
        instance: r,
        count: 1,
        state: null
      }, n.set(c, r));
    }
  }
  function Pf(e, t) {
    ct.M(e, t);
    var l = Tl;
    if (l && e) {
      var n = el(l).hoistableScripts, c = Vl(e), r = n.get(c);
      r || (r = l.querySelector(x2(c)), r || (e = Y({ src: e, async: !0, type: "module" }, t), (t = V1.get(c)) && u4(e, t), r = l.createElement("script"), Ye(r), We(r, "link", e), l.head.appendChild(r)), r = {
        type: "script",
        instance: r,
        count: 1,
        state: null
      }, n.set(c, r));
    }
  }
  function c6(e, t, l, n) {
    var c = (c = $.current) ? N3(c) : null;
    if (!c) throw Error(E(446));
    switch (e) {
      case "meta":
      case "title":
        return null;
      case "style":
        return typeof l.precedence == "string" && typeof l.href == "string" ? (t = Ll(l.href), l = el(
          c
        ).hoistableStyles, n = l.get(t), n || (n = {
          type: "style",
          instance: null,
          count: 0,
          state: null
        }, l.set(t, n)), n) : { type: "void", instance: null, count: 0, state: null };
      case "link":
        if (l.rel === "stylesheet" && typeof l.href == "string" && typeof l.precedence == "string") {
          e = Ll(l.href);
          var r = el(
            c
          ).hoistableStyles, h = r.get(e);
          if (h || (c = c.ownerDocument || c, h = {
            type: "stylesheet",
            instance: null,
            count: 0,
            state: { loading: 0, preload: null }
          }, r.set(e, h), (r = c.querySelector(
            w2(e)
          )) && !r._p && (h.instance = r, h.state.loading = 5), V1.has(e) || (l = {
            rel: "preload",
            as: "style",
            href: l.href,
            crossOrigin: l.crossOrigin,
            integrity: l.integrity,
            media: l.media,
            hrefLang: l.hrefLang,
            referrerPolicy: l.referrerPolicy
          }, V1.set(e, l), r || eh(
            c,
            e,
            l,
            h.state
          ))), t && n === null)
            throw Error(E(528, ""));
          return h;
        }
        if (t && n !== null)
          throw Error(E(529, ""));
        return null;
      case "script":
        return t = l.async, l = l.src, typeof l == "string" && t && typeof t != "function" && typeof t != "symbol" ? (t = Vl(l), l = el(
          c
        ).hoistableScripts, n = l.get(t), n || (n = {
          type: "script",
          instance: null,
          count: 0,
          state: null
        }, l.set(t, n)), n) : { type: "void", instance: null, count: 0, state: null };
      default:
        throw Error(E(444, e));
    }
  }
  function Ll(e) {
    return 'href="' + F1(e) + '"';
  }
  function w2(e) {
    return 'link[rel="stylesheet"][' + e + "]";
  }
  function i6(e) {
    return Y({}, e, {
      "data-precedence": e.precedence,
      precedence: null
    });
  }
  function eh(e, t, l, n) {
    e.querySelector('link[rel="preload"][as="style"][' + t + "]") ? n.loading = 1 : (t = e.createElement("link"), n.preload = t, t.addEventListener("load", function() {
      return n.loading |= 1;
    }), t.addEventListener("error", function() {
      return n.loading |= 2;
    }), We(t, "link", l), Ye(t), e.head.appendChild(t));
  }
  function Vl(e) {
    return '[src="' + F1(e) + '"]';
  }
  function x2(e) {
    return "script[async]" + e;
  }
  function u6(e, t, l) {
    if (t.count++, t.instance === null)
      switch (t.type) {
        case "style":
          var n = e.querySelector(
            'style[data-href~="' + F1(l.href) + '"]'
          );
          if (n)
            return t.instance = n, Ye(n), n;
          var c = Y({}, l, {
            "data-href": l.href,
            "data-precedence": l.precedence,
            href: null,
            precedence: null
          });
          return n = (e.ownerDocument || e).createElement(
            "style"
          ), Ye(n), We(n, "style", c), _3(n, l.precedence, e), t.instance = n;
        case "stylesheet":
          c = Ll(l.href);
          var r = e.querySelector(
            w2(c)
          );
          if (r)
            return t.state.loading |= 4, t.instance = r, Ye(r), r;
          n = i6(l), (c = V1.get(c)) && i4(n, c), r = (e.ownerDocument || e).createElement("link"), Ye(r);
          var h = r;
          return h._p = new Promise(function(v, o) {
            h.onload = v, h.onerror = o;
          }), We(r, "link", n), t.state.loading |= 4, _3(r, l.precedence, e), t.instance = r;
        case "script":
          return r = Vl(l.src), (c = e.querySelector(
            x2(r)
          )) ? (t.instance = c, Ye(c), c) : (n = l, (c = V1.get(r)) && (n = Y({}, l), u4(n, c)), e = e.ownerDocument || e, c = e.createElement("script"), Ye(c), We(c, "link", n), e.head.appendChild(c), t.instance = c);
        case "void":
          return null;
        default:
          throw Error(E(443, t.type));
      }
    else
      t.type === "stylesheet" && (t.state.loading & 4) === 0 && (n = t.instance, t.state.loading |= 4, _3(n, l.precedence, e));
    return t.instance;
  }
  function _3(e, t, l) {
    for (var n = l.querySelectorAll(
      'link[rel="stylesheet"][data-precedence],style[data-precedence]'
    ), c = n.length ? n[n.length - 1] : null, r = c, h = 0; h < n.length; h++) {
      var v = n[h];
      if (v.dataset.precedence === t) r = v;
      else if (r !== c) break;
    }
    r ? r.parentNode.insertBefore(e, r.nextSibling) : (t = l.nodeType === 9 ? l.head : l, t.insertBefore(e, t.firstChild));
  }
  function i4(e, t) {
    e.crossOrigin == null && (e.crossOrigin = t.crossOrigin), e.referrerPolicy == null && (e.referrerPolicy = t.referrerPolicy), e.title == null && (e.title = t.title);
  }
  function u4(e, t) {
    e.crossOrigin == null && (e.crossOrigin = t.crossOrigin), e.referrerPolicy == null && (e.referrerPolicy = t.referrerPolicy), e.integrity == null && (e.integrity = t.integrity);
  }
  var U3 = null;
  function r6(e, t, l) {
    if (U3 === null) {
      var n = /* @__PURE__ */ new Map(), c = U3 = /* @__PURE__ */ new Map();
      c.set(l, n);
    } else
      c = U3, n = c.get(l), n || (n = /* @__PURE__ */ new Map(), c.set(l, n));
    if (n.has(e)) return n;
    for (n.set(e, null), l = l.getElementsByTagName(e), c = 0; c < l.length; c++) {
      var r = l[c];
      if (!(r[Ul] || r[Ze] || e === "link" && r.getAttribute("rel") === "stylesheet") && r.namespaceURI !== "http://www.w3.org/2000/svg") {
        var h = r.getAttribute(t) || "";
        h = e + h;
        var v = n.get(h);
        v ? v.push(r) : n.set(h, [r]);
      }
    }
    return n;
  }
  function f6(e, t, l) {
    e = e.ownerDocument || e, e.head.insertBefore(
      l,
      t === "title" ? e.querySelector("head > title") : null
    );
  }
  function th(e, t, l) {
    if (l === 1 || t.itemProp != null) return !1;
    switch (e) {
      case "meta":
      case "title":
        return !0;
      case "style":
        if (typeof t.precedence != "string" || typeof t.href != "string" || t.href === "")
          break;
        return !0;
      case "link":
        if (typeof t.rel != "string" || typeof t.href != "string" || t.href === "" || t.onLoad || t.onError)
          break;
        return t.rel === "stylesheet" ? (e = t.disabled, typeof t.precedence == "string" && e == null) : !0;
      case "script":
        if (t.async && typeof t.async != "function" && typeof t.async != "symbol" && !t.onLoad && !t.onError && t.src && typeof t.src == "string")
          return !0;
    }
    return !1;
  }
  function h6(e) {
    return !(e.type === "stylesheet" && (e.state.loading & 3) === 0);
  }
  function lh(e, t, l, n) {
    if (l.type === "stylesheet" && (typeof n.media != "string" || matchMedia(n.media).matches !== !1) && (l.state.loading & 4) === 0) {
      if (l.instance === null) {
        var c = Ll(n.href), r = t.querySelector(
          w2(c)
        );
        if (r) {
          t = r._p, t !== null && typeof t == "object" && typeof t.then == "function" && (e.count++, e = R3.bind(e), t.then(e, e)), l.state.loading |= 4, l.instance = r, Ye(r);
          return;
        }
        r = t.ownerDocument || t, n = i6(n), (c = V1.get(c)) && i4(n, c), r = r.createElement("link"), Ye(r);
        var h = r;
        h._p = new Promise(function(v, o) {
          h.onload = v, h.onerror = o;
        }), We(r, "link", n), l.instance = r;
      }
      e.stylesheets === null && (e.stylesheets = /* @__PURE__ */ new Map()), e.stylesheets.set(l, t), (t = l.state.preload) && (l.state.loading & 3) === 0 && (e.count++, l = R3.bind(e), t.addEventListener("load", l), t.addEventListener("error", l));
    }
  }
  var r4 = 0;
  function ah(e, t) {
    return e.stylesheets && e.count === 0 && G3(e, e.stylesheets), 0 < e.count || 0 < e.imgCount ? function(l) {
      var n = setTimeout(function() {
        if (e.stylesheets && G3(e, e.stylesheets), e.unsuspend) {
          var r = e.unsuspend;
          e.unsuspend = null, r();
        }
      }, 6e4 + t);
      0 < e.imgBytes && r4 === 0 && (r4 = 62500 * _f());
      var c = setTimeout(
        function() {
          if (e.waitingForImages = !1, e.count === 0 && (e.stylesheets && G3(e, e.stylesheets), e.unsuspend)) {
            var r = e.unsuspend;
            e.unsuspend = null, r();
          }
        },
        (e.imgBytes > r4 ? 50 : 800) + t
      );
      return e.unsuspend = l, function() {
        e.unsuspend = null, clearTimeout(n), clearTimeout(c);
      };
    } : null;
  }
  function R3() {
    if (this.count--, this.count === 0 && (this.imgCount === 0 || !this.waitingForImages)) {
      if (this.stylesheets) G3(this, this.stylesheets);
      else if (this.unsuspend) {
        var e = this.unsuspend;
        this.unsuspend = null, e();
      }
    }
  }
  var j3 = null;
  function G3(e, t) {
    e.stylesheets = null, e.unsuspend !== null && (e.count++, j3 = /* @__PURE__ */ new Map(), t.forEach(nh, e), j3 = null, R3.call(e));
  }
  function nh(e, t) {
    if (!(t.state.loading & 4)) {
      var l = j3.get(e);
      if (l) var n = l.get(null);
      else {
        l = /* @__PURE__ */ new Map(), j3.set(e, l);
        for (var c = e.querySelectorAll(
          "link[data-precedence],style[data-precedence]"
        ), r = 0; r < c.length; r++) {
          var h = c[r];
          (h.nodeName === "LINK" || h.getAttribute("media") !== "not all") && (l.set(h.dataset.precedence, h), n = h);
        }
        n && l.set(null, n);
      }
      c = t.instance, h = c.getAttribute("data-precedence"), r = l.get(h) || n, r === n && l.set(null, c), l.set(h, c), this.count++, n = R3.bind(this), c.addEventListener("load", n), c.addEventListener("error", n), r ? r.parentNode.insertBefore(c, r.nextSibling) : (e = e.nodeType === 9 ? e.head : e, e.insertBefore(c, e.firstChild)), t.state.loading |= 4;
    }
  }
  var H2 = {
    $$typeof: Be,
    Provider: null,
    Consumer: null,
    _currentValue: I,
    _currentValue2: I,
    _threadCount: 0
  };
  function ch(e, t, l, n, c, r, h, v, o) {
    this.tag = 1, this.containerInfo = e, this.pingCache = this.current = this.pendingChildren = null, this.timeoutHandle = -1, this.callbackNode = this.next = this.pendingContext = this.context = this.cancelPendingCommit = null, this.callbackPriority = 0, this.expirationTimes = a0(-1), this.entangledLanes = this.shellSuspendCounter = this.errorRecoveryDisabledLanes = this.expiredLanes = this.warmLanes = this.pingedLanes = this.suspendedLanes = this.pendingLanes = 0, this.entanglements = a0(0), this.hiddenUpdates = a0(null), this.identifierPrefix = n, this.onUncaughtError = c, this.onCaughtError = r, this.onRecoverableError = h, this.pooledCache = null, this.pooledCacheLanes = 0, this.formState = o, this.incompleteTransitions = /* @__PURE__ */ new Map();
  }
  function m6(e, t, l, n, c, r, h, v, o, w, D, A) {
    return e = new ch(
      e,
      t,
      l,
      h,
      o,
      w,
      D,
      A,
      v
    ), t = 1, r === !0 && (t |= 24), r = z1(3, null, null, t), e.current = r, r.stateNode = e, t = G0(), t.refCount++, e.pooledCache = t, t.refCount++, r.memoizedState = {
      element: n,
      isDehydrated: l,
      cache: t
    }, X0(r), e;
  }
  function v6(e) {
    return e ? (e = hl, e) : hl;
  }
  function s6(e, t, l, n, c, r) {
    c = v6(c), n.context === null ? n.context = c : n.pendingContext = c, n = dt(t), n.payload = { element: l }, r = r === void 0 ? null : r, r !== null && (n.callback = r), l = ot(e, n, t), l !== null && (r1(l, e, t), t2(l, e, t));
  }
  function d6(e, t) {
    if (e = e.memoizedState, e !== null && e.dehydrated !== null) {
      var l = e.retryLane;
      e.retryLane = l !== 0 && l < t ? l : t;
    }
  }
  function f4(e, t) {
    d6(e, t), (e = e.alternate) && d6(e, t);
  }
  function o6(e) {
    if (e.tag === 13 || e.tag === 31) {
      var t = _t(e, 67108864);
      t !== null && r1(t, e, 67108864), f4(e, 67108864);
    }
  }
  function E6(e) {
    if (e.tag === 13 || e.tag === 31) {
      var t = x1();
      t = n0(t);
      var l = _t(e, t);
      l !== null && r1(l, e, t), f4(e, t);
    }
  }
  var Y3 = !0;
  function ih(e, t, l, n) {
    var c = b.T;
    b.T = null;
    var r = L.p;
    try {
      L.p = 2, h4(e, t, l, n);
    } finally {
      L.p = r, b.T = c;
    }
  }
  function uh(e, t, l, n) {
    var c = b.T;
    b.T = null;
    var r = L.p;
    try {
      L.p = 8, h4(e, t, l, n);
    } finally {
      L.p = r, b.T = c;
    }
  }
  function h4(e, t, l, n) {
    if (Y3) {
      var c = m4(n);
      if (c === null)
        Wa(
          e,
          t,
          n,
          q3,
          l
        ), p6(e, n);
      else if (fh(
        c,
        e,
        t,
        l,
        n
      ))
        n.stopPropagation();
      else if (p6(e, n), t & 4 && -1 < rh.indexOf(e)) {
        for (; c !== null; ) {
          var r = Pt(c);
          if (r !== null)
            switch (r.tag) {
              case 3:
                if (r = r.stateNode, r.current.memoizedState.isDehydrated) {
                  var h = Lt(r.pendingLanes);
                  if (h !== 0) {
                    var v = r;
                    for (v.pendingLanes |= 2, v.entangledLanes |= 2; h; ) {
                      var o = 1 << 31 - o1(h);
                      v.entanglements[1] |= o, h &= ~o;
                    }
                    j1(r), (me & 6) === 0 && (F3 = s1() + 500, z2(0));
                  }
                }
                break;
              case 31:
              case 13:
                v = _t(r, 2), v !== null && r1(v, r, 2), S3(), f4(r, 2);
            }
          if (r = m4(n), r === null && Wa(
            e,
            t,
            n,
            q3,
            l
          ), r === c) break;
          c = r;
        }
        c !== null && n.stopPropagation();
      } else
        Wa(
          e,
          t,
          n,
          null,
          l
        );
    }
  }
  function m4(e) {
    return e = s0(e), v4(e);
  }
  var q3 = null;
  function v4(e) {
    if (q3 = null, e = It(e), e !== null) {
      var t = G(e);
      if (t === null) e = null;
      else {
        var l = t.tag;
        if (l === 13) {
          if (e = Q(t), e !== null) return e;
          e = null;
        } else if (l === 31) {
          if (e = T(t), e !== null) return e;
          e = null;
        } else if (l === 3) {
          if (t.stateNode.current.memoizedState.isDehydrated)
            return t.tag === 3 ? t.stateNode.containerInfo : null;
          e = null;
        } else t !== e && (e = null);
      }
    }
    return q3 = e, null;
  }
  function z6(e) {
    switch (e) {
      case "beforetoggle":
      case "cancel":
      case "click":
      case "close":
      case "contextmenu":
      case "copy":
      case "cut":
      case "auxclick":
      case "dblclick":
      case "dragend":
      case "dragstart":
      case "drop":
      case "focusin":
      case "focusout":
      case "input":
      case "invalid":
      case "keydown":
      case "keypress":
      case "keyup":
      case "mousedown":
      case "mouseup":
      case "paste":
      case "pause":
      case "play":
      case "pointercancel":
      case "pointerdown":
      case "pointerup":
      case "ratechange":
      case "reset":
      case "resize":
      case "seeked":
      case "submit":
      case "toggle":
      case "touchcancel":
      case "touchend":
      case "touchstart":
      case "volumechange":
      case "change":
      case "selectionchange":
      case "textInput":
      case "compositionstart":
      case "compositionend":
      case "compositionupdate":
      case "beforeblur":
      case "afterblur":
      case "beforeinput":
      case "blur":
      case "fullscreenchange":
      case "focus":
      case "hashchange":
      case "popstate":
      case "select":
      case "selectstart":
        return 2;
      case "drag":
      case "dragenter":
      case "dragexit":
      case "dragleave":
      case "dragover":
      case "mousemove":
      case "mouseout":
      case "mouseover":
      case "pointermove":
      case "pointerout":
      case "pointerover":
      case "scroll":
      case "touchmove":
      case "wheel":
      case "mouseenter":
      case "mouseleave":
      case "pointerenter":
      case "pointerleave":
        return 8;
      case "message":
        switch (Z9()) {
          case y4:
            return 2;
          case F4:
            return 8;
          case L2:
          case K9:
            return 32;
          case D4:
            return 268435456;
          default:
            return 32;
        }
      default:
        return 32;
    }
  }
  var s4 = !1, Dt = null, St = null, At = null, y2 = /* @__PURE__ */ new Map(), F2 = /* @__PURE__ */ new Map(), Bt = [], rh = "mousedown mouseup touchcancel touchend touchstart auxclick dblclick pointercancel pointerdown pointerup dragend dragstart drop compositionend compositionstart keydown keypress keyup input textInput copy cut paste click change contextmenu reset".split(
    " "
  );
  function p6(e, t) {
    switch (e) {
      case "focusin":
      case "focusout":
        Dt = null;
        break;
      case "dragenter":
      case "dragleave":
        St = null;
        break;
      case "mouseover":
      case "mouseout":
        At = null;
        break;
      case "pointerover":
      case "pointerout":
        y2.delete(t.pointerId);
        break;
      case "gotpointercapture":
      case "lostpointercapture":
        F2.delete(t.pointerId);
    }
  }
  function D2(e, t, l, n, c, r) {
    return e === null || e.nativeEvent !== r ? (e = {
      blockedOn: t,
      domEventName: l,
      eventSystemFlags: n,
      nativeEvent: r,
      targetContainers: [c]
    }, t !== null && (t = Pt(t), t !== null && o6(t)), e) : (e.eventSystemFlags |= n, t = e.targetContainers, c !== null && t.indexOf(c) === -1 && t.push(c), e);
  }
  function fh(e, t, l, n, c) {
    switch (t) {
      case "focusin":
        return Dt = D2(
          Dt,
          e,
          t,
          l,
          n,
          c
        ), !0;
      case "dragenter":
        return St = D2(
          St,
          e,
          t,
          l,
          n,
          c
        ), !0;
      case "mouseover":
        return At = D2(
          At,
          e,
          t,
          l,
          n,
          c
        ), !0;
      case "pointerover":
        var r = c.pointerId;
        return y2.set(
          r,
          D2(
            y2.get(r) || null,
            e,
            t,
            l,
            n,
            c
          )
        ), !0;
      case "gotpointercapture":
        return r = c.pointerId, F2.set(
          r,
          D2(
            F2.get(r) || null,
            e,
            t,
            l,
            n,
            c
          )
        ), !0;
    }
    return !1;
  }
  function g6(e) {
    var t = It(e.target);
    if (t !== null) {
      var l = G(t);
      if (l !== null) {
        if (t = l.tag, t === 13) {
          if (t = Q(l), t !== null) {
            e.blockedOn = t, L4(e.priority, function() {
              E6(l);
            });
            return;
          }
        } else if (t === 31) {
          if (t = T(l), t !== null) {
            e.blockedOn = t, L4(e.priority, function() {
              E6(l);
            });
            return;
          }
        } else if (t === 3 && l.stateNode.current.memoizedState.isDehydrated) {
          e.blockedOn = l.tag === 3 ? l.stateNode.containerInfo : null;
          return;
        }
      }
    }
    e.blockedOn = null;
  }
  function Q3(e) {
    if (e.blockedOn !== null) return !1;
    for (var t = e.targetContainers; 0 < t.length; ) {
      var l = m4(e.nativeEvent);
      if (l === null) {
        l = e.nativeEvent;
        var n = new l.constructor(
          l.type,
          l
        );
        v0 = n, l.target.dispatchEvent(n), v0 = null;
      } else
        return t = Pt(l), t !== null && o6(t), e.blockedOn = l, !1;
      t.shift();
    }
    return !0;
  }
  function M6(e, t, l) {
    Q3(e) && l.delete(t);
  }
  function hh() {
    s4 = !1, Dt !== null && Q3(Dt) && (Dt = null), St !== null && Q3(St) && (St = null), At !== null && Q3(At) && (At = null), y2.forEach(M6), F2.forEach(M6);
  }
  function X3(e, t) {
    e.blockedOn === t && (e.blockedOn = null, s4 || (s4 = !0, i.unstable_scheduleCallback(
      i.unstable_NormalPriority,
      hh
    )));
  }
  var Z3 = null;
  function w6(e) {
    Z3 !== e && (Z3 = e, i.unstable_scheduleCallback(
      i.unstable_NormalPriority,
      function() {
        Z3 === e && (Z3 = null);
        for (var t = 0; t < e.length; t += 3) {
          var l = e[t], n = e[t + 1], c = e[t + 2];
          if (typeof n != "function") {
            if (v4(n || l) === null)
              continue;
            break;
          }
          var r = Pt(l);
          r !== null && (e.splice(t, 3), t -= 3, ma(
            r,
            {
              pending: !0,
              data: c,
              method: l.method,
              action: n
            },
            n,
            c
          ));
        }
      }
    ));
  }
  function Cl(e) {
    function t(o) {
      return X3(o, e);
    }
    Dt !== null && X3(Dt, e), St !== null && X3(St, e), At !== null && X3(At, e), y2.forEach(t), F2.forEach(t);
    for (var l = 0; l < Bt.length; l++) {
      var n = Bt[l];
      n.blockedOn === e && (n.blockedOn = null);
    }
    for (; 0 < Bt.length && (l = Bt[0], l.blockedOn === null); )
      g6(l), l.blockedOn === null && Bt.shift();
    if (l = (e.ownerDocument || e).$$reactFormReplay, l != null)
      for (n = 0; n < l.length; n += 3) {
        var c = l[n], r = l[n + 1], h = c[l1] || null;
        if (typeof r == "function")
          h || w6(l);
        else if (h) {
          var v = null;
          if (r && r.hasAttribute("formAction")) {
            if (c = r, h = r[l1] || null)
              v = h.formAction;
            else if (v4(c) !== null) continue;
          } else v = h.action;
          typeof v == "function" ? l[n + 1] = v : (l.splice(n, 3), n -= 3), w6(l);
        }
      }
  }
  function x6() {
    function e(r) {
      r.canIntercept && r.info === "react-transition" && r.intercept({
        handler: function() {
          return new Promise(function(h) {
            return c = h;
          });
        },
        focusReset: "manual",
        scroll: "manual"
      });
    }
    function t() {
      c !== null && (c(), c = null), n || setTimeout(l, 20);
    }
    function l() {
      if (!n && !navigation.transition) {
        var r = navigation.currentEntry;
        r && r.url != null && navigation.navigate(r.url, {
          state: r.getState(),
          info: "react-transition",
          history: "replace"
        });
      }
    }
    if (typeof navigation == "object") {
      var n = !1, c = null;
      return navigation.addEventListener("navigate", e), navigation.addEventListener("navigatesuccess", t), navigation.addEventListener("navigateerror", t), setTimeout(l, 100), function() {
        n = !0, navigation.removeEventListener("navigate", e), navigation.removeEventListener("navigatesuccess", t), navigation.removeEventListener("navigateerror", t), c !== null && (c(), c = null);
      };
    }
  }
  function d4(e) {
    this._internalRoot = e;
  }
  K3.prototype.render = d4.prototype.render = function(e) {
    var t = this._internalRoot;
    if (t === null) throw Error(E(409));
    var l = t.current, n = x1();
    s6(l, n, e, t, null, null);
  }, K3.prototype.unmount = d4.prototype.unmount = function() {
    var e = this._internalRoot;
    if (e !== null) {
      this._internalRoot = null;
      var t = e.containerInfo;
      s6(e.current, 2, null, e, null, null), S3(), t[$t] = null;
    }
  };
  function K3(e) {
    this._internalRoot = e;
  }
  K3.prototype.unstable_scheduleHydration = function(e) {
    if (e) {
      var t = T4();
      e = { blockedOn: null, target: e, priority: t };
      for (var l = 0; l < Bt.length && t !== 0 && t < Bt[l].priority; l++) ;
      Bt.splice(l, 0, e), l === 0 && g6(e);
    }
  };
  var H6 = u.version;
  if (H6 !== "19.2.3")
    throw Error(
      E(
        527,
        H6,
        "19.2.3"
      )
    );
  L.findDOMNode = function(e) {
    var t = e._reactInternals;
    if (t === void 0)
      throw typeof e.render == "function" ? Error(E(188)) : (e = Object.keys(e).join(","), Error(E(268, e)));
    return e = R(t), e = e !== null ? k(e) : null, e = e === null ? null : e.stateNode, e;
  };
  var mh = {
    bundleType: 0,
    version: "19.2.3",
    rendererPackageName: "react-dom",
    currentDispatcherRef: b,
    reconcilerVersion: "19.2.3"
  };
  if (typeof __REACT_DEVTOOLS_GLOBAL_HOOK__ < "u") {
    var J3 = __REACT_DEVTOOLS_GLOBAL_HOOK__;
    if (!J3.isDisabled && J3.supportsFiber)
      try {
        Ol = J3.inject(
          mh
        ), d1 = J3;
      } catch {
      }
  }
  return B2.createRoot = function(e, t) {
    if (!O(e)) throw Error(E(299));
    var l = !1, n = "", c = bn, r = Tn, h = Ln;
    return t != null && (t.unstable_strictMode === !0 && (l = !0), t.identifierPrefix !== void 0 && (n = t.identifierPrefix), t.onUncaughtError !== void 0 && (c = t.onUncaughtError), t.onCaughtError !== void 0 && (r = t.onCaughtError), t.onRecoverableError !== void 0 && (h = t.onRecoverableError)), t = m6(
      e,
      1,
      !1,
      null,
      null,
      l,
      n,
      null,
      c,
      r,
      h,
      x6
    ), e[$t] = t.current, ka(e), new d4(t);
  }, B2.hydrateRoot = function(e, t, l) {
    if (!O(e)) throw Error(E(299));
    var n = !1, c = "", r = bn, h = Tn, v = Ln, o = null;
    return l != null && (l.unstable_strictMode === !0 && (n = !0), l.identifierPrefix !== void 0 && (c = l.identifierPrefix), l.onUncaughtError !== void 0 && (r = l.onUncaughtError), l.onCaughtError !== void 0 && (h = l.onCaughtError), l.onRecoverableError !== void 0 && (v = l.onRecoverableError), l.formState !== void 0 && (o = l.formState)), t = m6(
      e,
      1,
      !0,
      t,
      l ?? null,
      n,
      c,
      o,
      r,
      h,
      v,
      x6
    ), t.context = v6(null), l = t.current, n = x1(), n = n0(n), c = dt(n), c.callback = null, ot(l, c, n), l = n, t.current.lanes = l, _l(t, l), j1(t), e[$t] = t.current, ka(e), new K3(t);
  }, B2.version = "19.2.3", B2;
}
var O6;
function _o() {
  if (O6) return p4.exports;
  O6 = 1;
  function m() {
    if (!(typeof __REACT_DEVTOOLS_GLOBAL_HOOK__ > "u" || typeof __REACT_DEVTOOLS_GLOBAL_HOOK__.checkDCE != "function"))
      try {
        __REACT_DEVTOOLS_GLOBAL_HOOK__.checkDCE(m);
      } catch (i) {
        console.log(i);
      }
  }
  return m(), p4.exports = No(), p4.exports;
}
var Uo = _o();
const Qe = {
  sm: 640,
  md: 768,
  lg: 1024,
  xl: 1280,
  "2xl": 1536
}, Ro = () => {
  const [m, i] = ye.useState(() => {
    if (typeof window > "u")
      return {
        isMobile: !1,
        isTablet: !1,
        isDesktop: !0,
        breakpoint: "lg",
        width: 1024,
        height: 768
      };
    const u = window.innerWidth, f = window.innerHeight;
    let E = "xs";
    return u >= Qe["2xl"] ? E = "2xl" : u >= Qe.xl ? E = "xl" : u >= Qe.lg ? E = "lg" : u >= Qe.md ? E = "md" : u >= Qe.sm ? E = "sm" : E = "xs", {
      isMobile: u < Qe.md,
      isTablet: u >= Qe.md && u < Qe.lg,
      isDesktop: u >= Qe.lg,
      breakpoint: E,
      width: u,
      height: f
    };
  });
  return ye.useEffect(() => {
    const u = () => {
      const f = window.innerWidth, E = window.innerHeight;
      let O = "xs";
      f >= Qe["2xl"] ? O = "2xl" : f >= Qe.xl ? O = "xl" : f >= Qe.lg ? O = "lg" : f >= Qe.md ? O = "md" : f >= Qe.sm ? O = "sm" : O = "xs", i({
        isMobile: f < Qe.md,
        isTablet: f >= Qe.md && f < Qe.lg,
        isDesktop: f >= Qe.lg,
        breakpoint: O,
        width: f,
        height: E
      });
    };
    return window.addEventListener("resize", u), u(), () => {
      window.removeEventListener("resize", u);
    };
  }, []), m;
}, jo = ye.createContext(void 0), Go = ({ children: m }) => {
  const i = Ro();
  return /* @__PURE__ */ F.jsx(jo.Provider, { value: i, children: m });
};
function Yo(m, i) {
  const u = document.getElementById(m);
  if (!u) {
    console.log(`Elemento com ID "${m}" não encontrado`);
    return;
  }
  u.classList.contains("infomoney-currency-converter-wrapper") || u.classList.add("infomoney-currency-converter-wrapper"), Uo.createRoot(u).render(
    /* @__PURE__ */ F.jsx(ye.StrictMode, { children: /* @__PURE__ */ F.jsx(Ho, { children: /* @__PURE__ */ F.jsx(Go, { children: /* @__PURE__ */ F.jsx(wo, { children: /* @__PURE__ */ F.jsx(G9, { ...i }) }) }) }) })
  );
}
const qo = /* @__PURE__ */ Object.freeze(/* @__PURE__ */ Object.defineProperty({
  __proto__: null,
  initCurrencyConverter: Yo
}, Symbol.toStringTag, { value: "Module" }));
class Ko {
  /**
   * Mapeia uma resposta da API para o objeto Currency
   * Usa os dados reais da API (symbol da resposta)
   */
  static mapApiResponseToCurrency(i, u) {
    const f = u.symbol?.toUpperCase() || i.toUpperCase(), E = this.getCurrencySymbol(f), O = this.getCurrencyName(f);
    return {
      code: f,
      // Dados da API (apiResponse.symbol)
      symbol: E,
      // Fallback (API não retorna)
      name: O
      // Fallback (API não retorna)
    };
  }
  /**
   * Obtém o símbolo monetário baseado no código (fallback mínimo)
   */
  static getCurrencySymbol(i) {
    return {
      BRL: "R$",
      USD: "$",
      EUR: "€",
      GBP: "£",
      JPY: "¥",
      CNY: "¥"
    }[i.toUpperCase()] || i;
  }
  /**
   * Obtém o nome da moeda baseado no código (fallback mínimo)
   */
  static getCurrencyName(i) {
    return {
      BRL: "Real Brasileiro",
      USD: "Dólar Americano",
      EUR: "Euro",
      GBP: "Libra Esterlina",
      JPY: "Iene Japonês",
      CNY: "Yuan Chinês"
    }[i.toUpperCase()] || i;
  }
  /**
   * Mapeia múltiplas respostas da API para uma lista de Currency
   */
  static mapApiResponsesToCurrencies(i) {
    return i.map(
      ({ code: u, apiData: f }) => this.mapApiResponseToCurrency(u, f)
    );
  }
  /**
   * Converte o contrato AllCurrenciesContract para lista de strings (códigos)
   */
  static contractToCurrencyCodeList(i) {
    return i.currencies.map((u) => u.code.toUpperCase());
  }
  /**
   * Converte o contrato AllCurrenciesContract para lista de Currency
   */
  static contractToCurrencyList(i) {
    return this.mapApiResponsesToCurrencies(i.currencies);
  }
  /**
   * MÉTODO PRINCIPAL: Monta o objeto completo para usar no CurrencyConverter
   * Converte os dados da API em tudo que o componente precisa
   */
  static mapContractToConverterData(i, u = "USD") {
    const f = this.mapApiResponsesToCurrencies(i.currencies), E = {}, O = i.currencies.find(
      (H) => H.apiData.symbol?.toUpperCase() === u.toUpperCase()
    ), G = O ? (O.apiData.bid + O.apiData.ask) / 2 : 1;
    i.currencies.forEach(({ apiData: H }) => {
      const R = H.symbol?.toUpperCase() || "", k = (H.bid + H.ask) / 2;
      E[R] = R === u.toUpperCase() ? 1 : k / G;
    });
    const Q = i.currencies.map((H) => new Date(H.apiData.tradeDate)).filter((H) => !isNaN(H.getTime())).sort((H, R) => R.getTime() - H.getTime())[0], T = Q ? Q.toLocaleString("pt-BR", {
      day: "2-digit",
      month: "2-digit",
      year: "numeric",
      hour: "2-digit",
      minute: "2-digit"
    }) : (/* @__PURE__ */ new Date()).toLocaleString("pt-BR");
    return {
      currencies: f,
      exchangeRates: E,
      lastUpdated: T
    };
  }
}
const Qo = 300 * 1e3, T2 = /* @__PURE__ */ new Map();
async function N6(m) {
  const i = T2.get(m);
  if (!i)
    return;
  if (Date.now() - i.timestamp > Qo) {
    T2.delete(m);
    return;
  }
  return i.data;
}
async function _6(m) {
  const i = T2.get(m);
  if (i)
    return i.data;
}
async function U6(m, i) {
  T2.set(m, {
    data: i,
    timestamp: Date.now()
  });
}
async function Jo() {
  T2.clear();
}
function R6(m) {
  const i = {
    Accept: "application/json",
    "Content-Type": "application/json",
    "Accept-Language": "pt-BR,pt;q=0.9,en;q=0.8,en-GB;q=0.7,en-US;q=0.6",
    "Accept-Encoding": "gzip, deflate, br, zstd"
  };
  return m.subscriptionKey && (i["Ocp-Apim-Subscription-Key"] = m.subscriptionKey), m.origin && (i.Origin = m.origin), m.referer && (i.Referer = m.referer), i;
}
async function Y9(m, i) {
  const u = await N6(m);
  if (u !== void 0)
    return u;
  try {
    const f = m.toLowerCase(), E = `${i.baseUrl}${i.endpoint || ""}/${f}`, O = R6(i), G = await fetch(E, {
      method: "GET",
      headers: O
    });
    if (G.status === 200) {
      const T = await G.json();
      return await U6(m, T), T;
    }
    const Q = await N6(m);
    if (Q !== void 0)
      return Q;
    try {
      const T = R6(i), H = await fetch(E, {
        method: "GET",
        headers: T
      });
      if (H.status === 200) {
        const R = await H.json();
        return await U6(m, R), R;
      } else {
        const R = await _6(m);
        if (R !== void 0)
          return R;
      }
    } catch {
      const H = await _6(m);
      if (H !== void 0)
        return H;
    }
    return null;
  } catch {
    return null;
  }
}
async function ko(m, i) {
  const u = m.map(async (O) => {
    const G = await Y9(O, i);
    return { currencyCode: O, data: G };
  }), f = await Promise.all(u), E = {};
  return f.forEach(({ currencyCode: O, data: G }) => {
    G && (E[O] = G);
  }), E;
}
async function Wo(m, i) {
  const u = [], f = m.map(async (E) => {
    const O = await Y9(E, i);
    O && u.push({
      code: E,
      apiData: O
    });
  });
  return await Promise.all(f), {
    currencies: u
  };
}
typeof window < "u" && Promise.all([
  Promise.resolve().then(() => bo),
  Promise.resolve().then(() => qo)
]).then(([{ CurrencyConverter: m }, { initCurrencyConverter: i }]) => {
  window.InfomoneyCurrencyConverter = {
    CurrencyConverter: m,
    initCurrencyConverter: i
  };
});
export {
  _9 as ChevronIcon,
  G9 as CurrencyConverter,
  Xo as CurrencyInput,
  Ko as CurrencyMapper,
  Zo as CurrencySelector,
  k3 as FlagIcon,
  Jo as clearCache,
  Y9 as fetchCurrencyRate,
  ko as fetchMultipleCurrencyRates,
  Wo as fetchMultipleCurrencyRatesWithQueue,
  N6 as getCachedRate,
  _6 as getCachedRateEvenExpired,
  Yo as initCurrencyConverter,
  U6 as setCachedRate
};
