...
const MOEDAS_PATH = '/moedas';
const MOEDAS_QUERY = '?type=json';
...
/**
 * URL completa para /moedas?type=json (compatível com CurrencyApiService legado)
 */
export function getMoedasUrl(): string {
  const config = getCurrencyApiConfig();
  const base = config.baseUrl.replace(/\/+$/, '');
  return `${base}${MOEDAS_PATH}${MOEDAS_QUERY}`;
}
