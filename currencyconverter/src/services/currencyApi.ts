...
export async function fetchMultipleCurrencyRatesWithQueue(
  ...
  // Tenta usar o adapter do WordPress primeiro
  try {
    const { createCurrencyApiAdapter } = await import('./currencyApiAdapter');
    const adapter = createCurrencyApiAdapter();
    const result = await adapter.fetchMultipleCurrencyRatesWithQueue(currencyCodes, config);
    
    // Se o adapter retornou dados, usa eles
    if (result && result.currencies && result.currencies.length > 0) {
      return result;
    }
  } catch (error) {
    // Se o adapter não estiver disponível ou falhar, continua com a implementação padrão
  }

  // Fallback: implementação padrão com fetch
