...
export const useDeviceService = (defaultDevice?: Device) => {
  const [device, setDeviceState] = useState<Device>(() => {
    ...
    if (typeof window !== 'undefined') {
      const detected = window.innerWidth < 768 ? 'mobile' : 'web';
      return detected;
    }
    return 'web';
  });
  ...
  useEffect(() => {
    const handleResize = () => {
      ...
      if (!defaultDevice) {
        ...
        // Só atualiza se o device detectado for diferente do atual
        setDeviceState((prevDevice) => {
          if (prevDevice !== detectedDevice) {
  ...
  }, [defaultDevice]); // Removido 'device' das dependências para evitar loops
...
export const currencyToCountryMap: Record<string, string> = {
  "BRL": "BR",
  "USD": "US",
  "EUR": "DE", // Euro - usa Alemanha como representante (ou pode usar "EU" se disponível)
  "GBP": "GB",
  "JPY": "JP",
  "CNY": "CN",
